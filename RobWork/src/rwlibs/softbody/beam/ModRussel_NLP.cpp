...
bool ModRussel_NLP::get_bounds_info(Ipopt::Index n, Ipopt::Number* x_l, Ipopt::Number* x_u,
    ...
    for(Index i = 0; i < n; i++) { x_l[i] = -rw::math::Pi; }

    // upper bounds of variables
    for(Index i = 0; i < n; i++) { x_u[i] = rw::math::Pi; }
...
bool ModRussel_NLP::get_starting_point(Ipopt::Index n, bool init_x, Ipopt::Number* x, bool init_z,
    ...
    for(int i = 0; i < n; i++) x[i] = _xinit[i];
...
// g[gBase]
void ModRussel_NLP::eval_g_point(int pIdx, int gBase, Index n, const Number* x, bool new_x, Index m,
                                 Number* g) {
    const rw::math::Transform3D<> planeTbeam = get_planeTbeam();    //
    ...
    double sumU = 0.0;
    for(int i = 0; i < (int) pIdx; i++) { sumU += cos(x[i]); }
...
void ModRussel_NLP::setStartingGuess(const Eigen::VectorXd& xinituser) {
    ...
    for(int i = 0; i < (int) _xinit.size(); i++) _xinit[i] = xinituser[i + 1];
}
