...
void pqpCollide(PQP_Model& ma, const Transform3D<>& wTa, PQP_Model& mb, const Transform3D<>& wTb,
    ...
    if(!firstContact) cflag = PQP_ALL_CONTACTS;
...
void ProximityStrategyPQP::destroyModel(rw::proximity::ProximityModel* model) {
    ...
    for(std::size_t i = 0; i < geoms.size(); i++) { removeGeometry(model, geoms[i]); }
}
...
bool ProximityStrategyPQP::addGeometry(rw::proximity::ProximityModel* model,
    ...
    if(_modelCache.has(key)) { pqpmodel = _modelCache.get(key); }
    else {
        TriMesh::Ptr mesh = gdata->getTriMesh(false);
        if(mesh->getSize() == 0) return false;
...
bool ProximityStrategyPQP::doIsWithinDistance(ProximityModel::Ptr aModel, const Transform3D<>& wTa,
    ...
    for(const RWPQPModel& ma : qdata.a->models) {
        for(const RWPQPModel& mb : qdata.b->models) {
            ...
            if(qdata.cache->_toleranceResult.CloserThanTolerance() != 0) { return true; }
        }
    }
    ...
    for(const RWPQPModel& ma : qdata.a->models) {
        ...
        for(const RWPQPModel& mb : qdata.b->models) {
            ...
            for(size_t i = 0; i < result.id1s.size(); i++) {
                ...
                if(result.distances[(*res).second] > dist) { (*res).second = (int) i; }
            }
            ...
            for(size_t j = 0; j < result.id2s.size(); j++) {
                ...
                if(result.distances[(*res).second] > dist) { (*res).second = (int) j; }
            }
...
ProximityStrategyPQP::getGeometryIDs(rw::proximity::ProximityModel* model) {
    ...
    PQPProximityModel* pmodel = (PQPProximityModel*) model;
    for(RWPQPModel& m : pmodel->models) { res.push_back(m.geo->getId()); }
    return res;
}
...
ProximityStrategyPQP::getGeometries(rw::proximity::ProximityModel* model) {
    ...
    PQPProximityModel* pmodel = (PQPProximityModel*) model;
    for(RWPQPModel& m : pmodel->models) { res.push_back(m.geo); }
    return res;
}
...
void ProximityStrategyPQP::getCollisionContacts(std::vector<CollisionStrategy::Contact>& contacts,
