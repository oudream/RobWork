...
void toTransform(const Transform3D<>& tr, yaobi::Real T[3][4]) {
    for(int i = 0; i < 3; i++)
        for(int j = 0; j < 4; j++) T[i][j] = static_cast<yaobi::Real>(tr(i, j));
}
...
bool ProximityStrategyYaobi::addGeometry(rw::proximity::ProximityModel* model,
    ...
    if(_modelCache.has(geom->getId())) { yaobimodel = _modelCache.get(geom->getId()); }
    else {
...
bool ProximityStrategyYaobi::doInCollision(ProximityModel::Ptr aModel, const Transform3D<>& wTa,
    ...
    if(!firstContact) qtype = yaobi::ALL_CONTACTS;
    ...
    for(const RWYaobiModel& ma : a->models) {
        for(const RWYaobiModel& mb : b->models) {
            ...
            if(firstContact && result.IsColliding()) return true;
            if(result.IsColliding()) isColliding = true;
...
ProximityStrategyYaobi::getGeometryIDs(rw::proximity::ProximityModel* model) {
    ...
    YaobiProximityModel* pmodel = (YaobiProximityModel*) model;
    for(RWYaobiModel& m : pmodel->models) { res.push_back(m.geo->getId()); }
    return res;
}
...
ProximityStrategyYaobi::getGeometries(rw::proximity::ProximityModel* model) {
    ...
    YaobiProximityModel* pmodel = (YaobiProximityModel*) model;
    for(RWYaobiModel& m : pmodel->models) { res.push_back(m.geo); }
    return res;
}
