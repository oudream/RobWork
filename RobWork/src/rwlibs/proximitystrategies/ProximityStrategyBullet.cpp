...
btCollisionShape* getColShapes(const std::vector<Frame*>& frames,
    ...
    for(const Frame* frame : frames) {
        ...
        if(frame == NULL || CollisionModelInfo::get(frame).size() == 0) continue;
...
btTriangleMesh* createTriMesh(const std::vector<Frame*>& frames,
                              rw::core::Ptr<rw::kinematics::Frame> parent,
    ...
    for(Frame *frame, frames) {
        ...
        // check if frame has collision descriptor
        if(CollisionModelInfo::get(frame).size() == 0) continue;
        ...
        if(frame != parent) { rw_pTf = Kinematics::frameTframe(parent, frame, state) * colT3d; }
        btTransform pTf = makeBtTransform(rw_pTf);
            ...
            /*
                                    btGImpactConvexDecompositionShape *colShape  = new
                                        btGImpactConvexDecompositionShape(
                                               trimesh, btVector3(1.f,1.f,1.f), btScalar(margin) );
            */
            rw::core::Ptr<btGImpactMeshShape> tmpShape = ownedPtr(new btGImpactMeshShape(trimesh));
            tmpShape->setMargin(0.0005);
            tmpShape->postUpdate();
...
bool ProximityStrategyBullet::inCollision(ProximityModel::Ptr aModel, const Transform3D<>& wTa,
    ...
    for(const BulletModel& ma : qdata.a->models) {
        for(const BulletModel& mb : qdata.b->models) {
            ...
            if(cback.contacts.size() > 0) return true;
        }
    }
...
DistanceResult& ProximityStrategyBullet::distance(ProximityModel::Ptr aModel,
    ...
    for(const BulletModel& ma : qdata.a->models) {
        for(const BulletModel& mb : qdata.b->models) {
            ...
            if(result.distance < rwresult.distance) rwresult.distance = result.distance;
        }
    }
    return rwresult;
}
...
ProximityStrategyBullet::getGeometryIDs(rw::proximity::ProximityModel* model) {
    ...
    ProximityModelBullet* pmodel = (ProximityModelBullet*) model;
    for(BulletModel& m : pmodel->models) { res.push_back(m.geoid); }
    return res;
}
