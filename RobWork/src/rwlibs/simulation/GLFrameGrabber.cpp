...
void GLFrameGrabber::grab(rw::core::Ptr<rw::kinematics::Frame> frame,
                          const rw::kinematics::State& state) {
    if(_view.isNull()) RW_THROW("GLFrameGrabber must be initialized before grab is called!");
...
void GLFrameGrabber::resize(int width, int height) {
    if(_view.isNull()) RW_THROW("Resizing GLFrameGrabber must be done after initialization!");
