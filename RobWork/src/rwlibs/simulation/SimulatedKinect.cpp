...
rw::core::Ptr<CameraModel> makeDefaultCameraModel(const std::string& name,
                                                  ...
                                                  double fov, int w, int h, double nearVal,
                                                  double farVal) {
...
SimulatedKinect::SimulatedKinect(const std::string& name,
    ...
    _near(0.01), _far(6), _fieldOfView(43), _grabSingleFrame(false), _width(640), _height(480),
    _img(
        new rw::sensor::Image(_width, _height, rw::sensor::Image::RGB, rw::sensor::Image::Depth8U)),
...
void SimulatedKinect::update(const Simulator::UpdateInfo& info, rw::kinematics::State& state) {
    ...
    if(_frameRate < 0.00001) {
        if(_grabSingleFrame) {
            ...
            if(_noiseEnabled) applyNoise(_scan);
