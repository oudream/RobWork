...
int BFGS::optimizer(vector& startguess, BFGS_function_struct function, double tolerance,
    ...
    for(vector::Index i = 0; i < dim; i++) {
        for(vector::Index j = 0; j < dim; j++) { Hk(i, j) = 0; }
        Hk(i, i) = initialStepsize;
    }

    // Allocate memory for gradient
    vector gfk(dim);
    for(vector::Index i = 0; i < dim; i++) gfk(i) = 0;

    // Allocate memory for next gradient
    vector gfk1(dim);
    for(vector::Index i = 0; i < dim; i++) gfk1(i) = 0;

    // Allocate memory for gradient differences
    vector yk(dim);
    for(vector::Index i = 0; i < dim; i++) yk(i) = 0;

    // Allocate memory for search direction
    vector pk(dim);
    for(vector::Index i = 0; i < dim; i++) pk(i) = 0;
    ...
    vector xk1(dim);
    for(vector::Index i = 0; i < dim; i++) xk1(i) = 0;
    ...
    rw::core::Log::debugLog() << "Solution x: "
                              << "\n";
    for(unsigned int i = 0; i < xk.size(); i++) rw::core::Log::debugLog() << "	" << xk(i) << "\n";
    startguess = xk;
    return STATUS;
}
...
void BFGS::colDotRow(vector& colvec, vector& rowvec, matrix& result) {
    ...
    for(vector::Index i = 0; i < dimRow; i++)
        for(vector::Index j = 0; j < dimCol; j++) result(i, j) = colvec(i) * rowvec(j);
}

double BFGS::lineSearch(BFGS_function_struct function, vector& xk, vector& pk, double c1, double c2,
                        double alphamax) {
    double eps = 1e-6;
    ...
    while(i < 20) {
        ...
        if(std::abs(dphi_alpha[2]) <= -c2 * dphi_alpha[0]) { return alpha[2]; }
        if(dphi_alpha[2] >= 0) {
            return zoom(alpha[2],
                        alpha[1],
                        phi_alpha[2],
...
double BFGS::zoom(double& alphalow, double& alphahigh, double& phi_alphalow, double& dphi_alphalow,
    ...
    while((ite < 100) && (std::abs(alphahigh - alphalow) > 1e-6)) {
        alphaj =
            quadraticInterpolation(phi_alphalow, dphi_alphalow, alphalow, phi_alphahigh, alphahigh);
        ...
        if((phi_alphaj > phi_alpha_zero + c1 * alphaj * dphi_alpha_zero) ||
           (phi_alphaj >= phi_alphalow)) {
            ...
            if(std::abs(dphi_alphaj) <= -c2 * dphi_alpha_zero) return alphaj;
