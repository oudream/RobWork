...
double StablePose1DModel::fitError(const rw::math::Rotation3D<>& sample) const {
    ...
    double dx = 0.0;
    if(!Math::isNaN(_dx)) { dx = dot(sample.getRow(0), _normal) - _dx; }

    double dy = 0.0;
    if(!Math::isNaN(_dy)) { dy = dot(sample.getRow(1), _normal) - _dy; }

    double dz = 0.0;
    if(!Math::isNaN(_dz)) { dz = dot(sample.getRow(2), _normal) - _dz; }
...
double StablePose1DModel::refit(const std::vector<rw::math::Rotation3D<>>& samples) {
    ...
    if(best_plane != &x_plane) {
        Vector3D<> c;
        for(const Vector3D<>& p : x_points) { c += p; }
    ...
    if(best_plane != &y_plane) {
        Vector3D<> c;
        for(const Vector3D<>& p : y_points) { c += p; }
    ...
    if(best_plane != &z_plane) {
        Vector3D<> c;
        for(const Vector3D<>& p : z_points) { c += p; }
