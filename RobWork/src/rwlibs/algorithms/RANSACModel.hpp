    ...
    template<class MODEL, class DATA> class RANSACModel
    {
      ...
      public:
        ...
        static std::vector<MODEL> findModels(const std::vector<DATA>& data, int maxIterations,
            ...
            if(models.size() == 1) {
                ...
                try {
                    ...
                    if(models[0].first.invalid() ||
                       models[0].first.getNumberOfInliers() < static_cast<size_t>(dataRequired)) {
                        return std::vector<MODEL>();
                    }
                }
                catch(...) {
            ...
            for(size_t i = 0; i < modelsPtr.size() - 1; i++) {
                if(modelsPtr[i].first != NULL) { newModels.push_back(*modelsPtr[i].first); }
            }
        ...
        static MODEL bestModel(const std::vector<MODEL>& models) {
            if(models.size() == 0) { return MODEL(); }

            size_t idx = 0;

            for(size_t i = 0; i < models.size(); ++i) {
                if(models[idx] < models[i]) { idx = i; }
            }
        ...
        bool operator<(const MODEL& model) const {
            ...
            if(a < b) { return true; }
            else if(a == b) { return getQuality() > model.getQuality(); }
            else { return false; }
        }
