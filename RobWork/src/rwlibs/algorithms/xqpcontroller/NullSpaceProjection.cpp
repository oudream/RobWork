...
NullSpaceProjection::NullSpaceProjection(Device* device, rw::core::Ptr<Frame> controlFrame,
...
Q NullSpaceProjection::solve(const Q& q, const Q& dqcurrent, const Q& dq1) {
    ...
    if(status == QPSolver::SUBOPTIMAL) std::cout << "Returns something suboptimal" << std::endl;
    if(status == QPSolver::FAILURE) std::cout << "Returns Error" << std::endl;
