...
XQPController::XQPController(Device::Ptr device, rw::core::Ptr<Frame> controlFrame,
                             const State& state, double dt) :
    _device(device),
    _controlFrame(controlFrame), _state(state), _dt(dt), _dof(device->getDOF()), _space(BaseFrame),
...
Q XQPController::inequalitySolve(const Eigen::MatrixXd& G, const Eigen::VectorXd& b,
    ...
    for(std::list<Constraint>::const_iterator it = constraints.begin(); it != constraints.end();
        ++it) {
        const Constraint& c = *it;
        for(size_t i = 0; i < c._a.size(); i++) { cmat(index, i) = c._a(i); }
        limits(index) = c._b;
    }
    ...
    for(int i = 0; i < qstart.size(); i++) {
        if(qstart(i) < lower(i)) qstart(i) = lower(i);
        if(qstart(i) > upper(i)) qstart(i) = upper(i);
    }
    for(std::list<Constraint>::const_iterator it = constraints.begin(); it != constraints.end();
        ++it) {
        const Constraint& c = *it;
        ...
        for(int i = 0; i < qstart.size(); i++) {
            if(c._a(i) < 0) qstart(i) = lower(i);
            else qstart(i) = upper(i);
        }
    ...
    if(status == QPSolver::FAILURE) std::cout << "Solution appears to be invalid" << std::endl;
...
Q XQPController::solve(const rw::math::Q& q, const rw::math::Q& dq,
                       ...
                       const std::list<Constraint>& constraints) {
    if(q.size() != _dof) RW_THROW("Length of input configuration does not match the Device");
