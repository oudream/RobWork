...
namespace rwlibs { namespace algorithms {
    ...
    template<class VALUE_TYPE> class KDTreeQ : public rw::common::Serializable
    {
      ...
      private:
        //! default constructor
        KDTreeQ() : _dim(0), _nrOfNodes(0), _root(NULL), _nodes(new std::vector<TreeNode*>()) {}

        //! constructor
        KDTreeQ(TreeNode* root, std::vector<TreeNode*>* nodes) :
            _dim(root->_kdnode->key.size()), _nrOfNodes(nodes->size()), _root(root), _nodes(nodes) {
        }
    ...
    template<class T> typename KDTreeQ<T>::KDNode* KDTreeQ<T>::search(const rw::math::Q& nnkey) {
    ...
    template<class T> typename KDTreeQ<T>::KDNode& KDTreeQ<T>::nnSearch(const rw::math::Q& nnkey) {
        // std::cout << "nnSearch " << _dim << std::endl;

        RW_ASSERT(nnkey.size() == _dim);
        if(_root == NULL) RW_THROW("KDTreeQ has no data!");
        ...
        if(result.n == NULL) RW_THROW("KDTreeQ has no data!");
        return *result.n;
    }
    ...
    void KDTreeQ<T>::nnSearchElipseRect(const rw::math::Q& nnkey, const rw::math::Q& radi,
        ...
        while(!unhandled.empty()) {
            ...
            if(j == _dim) {    // this is in range if
                ...
                if(dist < distSqr) nodes.push_back(n->_kdnode);
            }
