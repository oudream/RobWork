...
void QPController::calculateVelocityLimits(VectorBase& lower, VectorBase& upper,
    ...
    for(size_t i = 0; i < _n; i++) {
        ...
        if(upper(i) == accmax) _upperLimitType[i] = 'a';
        else if(upper(i) == velmax) _upperLimitType[i] = 'v';
        else _upperLimitType[i] = 'q';

        if(lower(i) == accmin) _lowerLimitType[i] = 'a';
        else if(lower(i) == velmin) _lowerLimitType[i] = 'v';
        else _lowerLimitType[i] = 'q';
...
QPController::VectorBase QPController::inequalitySolve(const Eigen::MatrixXd& G,
                                                       const VectorBase& b, const VectorBase& lower,
