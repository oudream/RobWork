...
Eigen::VectorXd QPSolver::getInitialConfig(Eigen::MatrixXd& A, const Eigen::VectorXd& b) {
    ...
    for(size_t i = 0; i < n; i++) { x(i) = 0.5 * (b(2 * i) - b(2 * i + 1)); }
...
Eigen::VectorXd QPSolver::inequalitySolve(const Eigen::MatrixXd& G, const Eigen::VectorXd& d,
    ...
    for(int i = 0; i < b.size(); i++)
        if(bcompare(i) + ERROR_LIMIT < b(i)) {
            status = FAILURE;
            std::cout << "Warning: Invalid start configuration" << i << "   " << bcompare(i) - b(i)
                      << std::endl;
            //    return xstart;
        }
    ...
    for(size_t i = 0; i < m; i++)
        if(fabs(b[i] - bcompare[i]) < EPS) Wk.push_back((int) i);
        else notWk.push_back((int) i);
    ...
    for(size_t i = 0; i < n * (m + 1); i++) {
        ...
        for(int i = 0; i < G.rows(); i++)
            for(int j = 0; j < G.rows(); j++) M(i, j) = G(i, j);
        size_t next = n;
        ...
        Eigen::VectorXd g_k;
        g_k = G * x + d;
        for(int i = 0; i < g_k.size(); i++) rhs(i) = -g_k(i);
        ...
        for(size_t i = 0; i < n; i++) p_k(i) = pl(i);
        ...
        if(p_k.lpNorm<Eigen::Infinity>() < 1e-8) {
            ...
            for(IntList::iterator it = notWk.begin(); it != notWk.end(); it++) {
                ...
                for(int i = 0; i < A.cols(); i++) a_i(i) = A(*it, i);
