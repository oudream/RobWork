...
double SyncVelocityRamp::ds(double t) {
    ...
    if(t < _tau_s) return ds1;

    // double tau2 = std::min(t, _duration-_tau_e);
    double ds2 = ds1;
    if(t <= _duration - _tau_e) return ds2;
...
double SyncVelocityRamp::dds(double t) {
    if(t < _tau_s) return _dwmax;
    if(t < _duration - _tau_e) return 0;
    if(t < _duration) return -_dwmax;
    return 0;
}
