...
PiHStrategy::update(AssemblyParameterization::Ptr parameters, AssemblyState::Ptr real,
                    AssemblyState::Ptr assumed, ControlState::Ptr controlState,
                    rw::kinematics::State& state, rw::sensor::FTSensor* ftSensor,
                    double time) const {
...
Transform3D<> PiHStrategy::getApproach(AssemblyParameterization::Ptr parameters) {
    ...
    if(std::fabs(p->theta) >= Pi / 2.) RW_THROW("The theta angle must be less than 90 degrees.");
    if(p->pegRadius > p->holeRadius) RW_THROW("The peg must not be greater than the hole.");
    ...
    const Transform3D<> femaletcpTmaletcp(
        offset + Vector3D<>(0, p->distTContact * sin(p->phi), p->distTContact * cos(p->phi)), rot);
...
Transform3DTrajectory::Ptr PiHStrategy::generateTrajectory(const PiHParameterization::Ptr p,
    ...
    // Linear move
    {
        ...
        do {
            ...
            if(TubeTtargetp.norm2() <= 0.002) { HoleTdevice.P() = T1.P(); }
            else { HoleTdevice.P() += TubeTtargetp * (0.002 / TubeTtargetp.norm2()); }
    ...
    // Move linear
    {
        ...
        do {
            ...
            if(TubeTtargetp.norm2() <= 0.002) { HoleTdevice.P() = T2.P(); }
            else { HoleTdevice.P() += TubeTtargetp * (0.002 / TubeTtargetp.norm2()); }
            const Transform3D<> femaleTmaleTarget =
                inverse(inverse(_femaleTfemaleTcp) * assumed->femaleOffset) * HoleTdevice *
                inverse(_maleTmaleTcp);
