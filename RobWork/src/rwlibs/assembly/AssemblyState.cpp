...
AssemblyState::AssemblyState(CartesianTarget::Ptr target) {
    ...
    maleflexT      = target->getPropertyMap().get<Path<Transform3D<>>>("MaleFlexT",
                                                                  std::vector<Transform3D<>>());
    femaleflexT    = target->getPropertyMap().get<Path<Transform3D<>>>("FemaleFlexT",
                                                                    std::vector<Transform3D<>>());
    contacts =
        target->getPropertyMap().get<Path<Transform3D<>>>("Contacts", std::vector<Transform3D<>>());
...
CartesianTarget::Ptr AssemblyState::toCartesianTarget(const AssemblyState& state) {
    ...
    if(state.phase != "") target->getPropertyMap().set<std::string>("Phase", state.phase);
