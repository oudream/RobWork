...
AssemblyTask::AssemblyTask(CartesianTask::Ptr task, AssemblyRegistry::Ptr registry) {
    if(registry == NULL) registry = ownedPtr(new AssemblyRegistry());
    maleID   = task->getPropertyMap().get<std::string>("MaleID", "");
    femaleID = task->getPropertyMap().get<std::string>("FemaleID", "");
    femaleTmaleTarget =
        task->getPropertyMap().get<Transform3D<>>("femaleTmaleTarget", Transform3D<>::identity());

    std::string stratID = task->getPropertyMap().get<std::string>("Strategy", "");
    if(stratID == "") strategy = NULL;
    else strategy = registry->getStrategy(stratID);
    if(strategy != NULL) {
...
CartesianTask::Ptr AssemblyTask::toCartesianTask() {
    ...
    if(strategy != NULL) root->getPropertyMap().set<std::string>("Strategy", strategy->getID());
    else root->getPropertyMap().set<std::string>("Strategy", "");
    if(parameters != NULL) {
    ...
    if(maleTCP != "") root->getPropertyMap().set<std::string>("MaleTCP", maleTCP);
    if(femaleTCP != "") root->getPropertyMap().set<std::string>("FemaleTCP", femaleTCP);

    if(taskID != "") root->getPropertyMap().set<std::string>("TaskID", taskID);
    if(workcellName != "") root->getPropertyMap().set<std::string>("WorkcellName", workcellName);
    if(generator != "") root->getPropertyMap().set<std::string>("Generator", generator);
    if(date != "") root->getPropertyMap().set<std::string>("Date", date);
    if(author != "") root->getPropertyMap().set<std::string>("Author", author);
    ...
    if(maleFTSensor != "") root->getPropertyMap().set<std::string>("MaleFTSensor", maleFTSensor);
    if(femaleFTSensor != "")
...
std::vector<AssemblyTask::Ptr> AssemblyTask::load(const std::string& filename,
    ...
    else { RW_THROW("Could not load AssemblyTaks (must be CartesianTask)"); }
...
AssemblyTask::Ptr AssemblyTask::clone() const {
    ...
    if(parameters == NULL) res->parameters = NULL;
    else res->parameters = parameters->clone();
