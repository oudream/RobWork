...
SpiralStrategy::update(AssemblyParameterization::Ptr parameters, AssemblyState::Ptr real,
    ...
    if(spiralState->_trajectory.isNull()) spiralState->_trajectory = generateTrajectory(sp_param);
...
SpiralStrategy::generateTrajectory(SpiralParameterization::Ptr param) const {
    ...
    else if(param->n <= 0.0) { RW_THROW("For SpiralStrategy the n parameter must be positive!"); }
    ...
    for(double t = 0; t < 2 * Pi * param->n; t += param->d_path) {
        const Vector3D<> peg_tip_pose(
            -a * t * cos(t), -a * t * sin(t), sqrt(d * d - a * a * t * t) - param->length_push - d);
        ...
        const Rotation3D<> peg_angle(cos(Y),
                                     ...
                                     sin(X),
                                     cos(X) * cos(Y));
        if(t == 0 && (peg_tip_pose - initial_pose.P()).norm2() == 0) continue;
        T3Dpath.push_back(Transform3D<>(peg_tip_pose, peg_angle));
    }
    if(T3Dpath.size() <= 1) { RW_THROW("A path of size zero was generated!"); }
    ...
    for(size_t i = 2; i < T3Dpath.size(); i++) { times.push_back(param->d_path); }
