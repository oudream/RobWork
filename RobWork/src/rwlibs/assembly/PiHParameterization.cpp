...
PropertyMap::Ptr PiHParameterization::toPropertyMap() const {
    ...
    if(!map->has("HoleRadius")) map->add("HoleRadius", "Radius of hole (in millimeters).", 0.);
    if(!map->has("PegRadius")) map->add("PegRadius", "Radius of peg (in millimeters).", 0.);
    if(!map->has("Theta")) map->add("Theta", "Angle (in radians).", 0.);
    if(!map->has("Phi")) map->add("Phi", "Angle (in radians).", 0.);
    if(!map->has("DistX")) map->add("DistX", "Distance in x (in millimeters).", 0.);
    if(!map->has("DistY")) map->add("DistY", "Distance in y (in millimeters).", 0.);
    if(!map->has("DistTContact"))
        map->add("DistTContact", "Distance to contact (in millimeters).", 0.);
    if(!map->has("x0")) map->add("x0", "Distance (in millimeters).", 0.);
...
AssemblyParameterization::Ptr PiHParameterization::clone() const {
    return ownedPtr(
        new PiHParameterization(holeRadius, pegRadius, theta, phi, distX, distY, distTContact, x0));
}
