...
SpiralParameterization::SpiralParameterization(double r, double n, double length_peg,
    ...
    r(r),
    n(n), length_peg(length_peg), length_push(length_push), length_start_traj(length_start_traj),
    depth_in_hole_for_success(depth_in_hole_for_success), speed(speed), d_path(d_path),
...
PropertyMap::Ptr SpiralParameterization::toPropertyMap() const {
    ...
    if(!map->has("r")) map->add("r", "Radius (in millimeters).", 0.);
    if(!map->has("n")) map->add("n", "Number of turns. Must be a positive value.", 0.);
