...
void renderSquare(Vector3D<> pos1, Vector3D<> pos2, Vector3D<> pos3, Vector3D<> pos4,
    ...
    if(boxColor.size() == 4u) { glColor4f(boxColor[0], boxColor[1], boxColor[2], boxColor[3]); }
    else if(boxColor.size() == 3u) { glColor3f(boxColor[0], boxColor[1], boxColor[2]); }
    ...
    else if(boxColor.size() == 3u) { glColor3f(borderColor[0], borderColor[1], borderColor[2]); }
...
void moveCloserToCam(Transform3D<> fTc, Vector3D<>& pos, double distance, SceneCamera::Ptr cam) {
    ...
    if(movePercent > max_movePercent) { movePercent = 0.90 * max_movePercent; }

    // make sure the label isn't to close in general
    if(movePercent > 0.90) { movePercent = 0.9; }
...
std::vector<rw::math::Vector3D<>> RenderText::getLabelCorners(Transform3D<> fTc, Vector3D<> pos,
    ...
    if(Vector3D<>(fTc.P() - pos).norm2() < 0.01) { offset = 0; }
