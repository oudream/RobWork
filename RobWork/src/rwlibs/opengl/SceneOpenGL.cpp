...
class SimpleCameraGroup : public CameraGroup
{
  public:
    SimpleCameraGroup(const std::string& name) :
        ...
        _useMultiSample(false), _initialized(false), _renderToImage(false), _renderToDepth(false),
        _samples(0), _fbId(-1), _fbTmpId(-1), _renderColorTmpId(-1), _renderId(-1),
        _renderDepthId(-1), textureId(-1), _aMultisampleTexture(-1) {}
    ...
    void setMultiSample(int samples) {
        ...
        else { _useMultiSample = true; }
    }

    void init() {
        {
            const std::string error = SceneOpenGL::detectGLerror();
            if(!error.empty()) RW_WARN("OpenGL error detected:" << error);
        }
        ...
        if((_offscreenRender == false)) {
            ...
            if(_fbId != maxGLuintSize) RWGLFrameBuffer::glDeleteFramebuffersEXT(1, &_fbId);
            if(_renderId != maxGLuintSize) RWGLFrameBuffer::glDeleteRenderbuffersEXT(1, &_renderId);
            ...
            if(useMultisample) { glEnable(GL_MULTISAMPLE); }
    ...
    SceneCamera::Ptr getMainCamera() {
        if(_mainCam == NULL)
            if(_cameras.size() > 0) return _cameras.front();
        return _mainCam;
    }

    bool setOffscreenRenderEnabled(bool enable) {
        // if both are same then do nothing
        if(enable == _offscreenRender) return _offscreenRender;
        RWGLFrameBuffer::initialize();
        if(RWGLFrameBuffer::hasFrameBuffers()) _offscreenRender = enable;
        else _offscreenRender = false;
        _initialized = false;
    ...
    void setCopyToImage(rw::sensor::Image::Ptr img) {
        _img = img;
        if(img != NULL) { _renderToImage = true; }
        else { _renderToImage = false; }
    };

    void setCopyToScan25D(rw::geometry::PointCloud::Ptr scan) {
        _scan25 = scan;
        if(scan != NULL) { _renderToDepth = true; }
        else { _renderToDepth = false; }
        _initialized = false;
    };
...
struct RenderPreVisitor
{
    ...
    bool operator()(SceneNode::Ptr& child, SceneNode::Ptr& parent) {
        if(GroupNode* gnode = child->asGroupNode()) {
            ...
            if((!dnode->isTransparent() && !_drawAlpha) || dnode->getName() == "BackgroundRender" ||
               dnode->getName() == "FloorGrid") {
                glPushMatrix();
...
void drawScene(SceneGraph* graph, CameraGroup::Ptr camGroup, SceneGraph::RenderInfo& info,
    ...
    RW_ASSERT(scam);
    if(scam != NULL) {
        if(!scam->isInitialized()) scam->init();
    ...
    for(SceneCamera::Ptr cam : camGroup->getCameras()) {
        ...
        if(subRootNode == NULL) continue;

        ViewPort vp = getViewPort(cam);
        glViewport(vp.x, vp.y, vp.w, vp.h);

        // optionally clear buffers
        if(cam->isClearBufferEnabled()) { glClear(cam->getClearBufferMask()); }
        ...
        else { glLoadMatrixf(matrix); }
        // setup model view
        glMatrixMode(GL_MODELVIEW);
        if(usePickMatrix) { glPushName(0); }
        ...
        if(usePickMatrix) {
            if(!cam->isDepthTestEnabled()) { glPopName(); }
        }
    }
    if(scam != NULL) {
...
#define GL_SELECT_BUFSIZE 512
    ...
    for(int i = 0; i < hits; i++) {
        ...
        for(int j = 0; j < stack; j++) { object.push_back({depth, _selectBuf[index++]}); }
    }
    ...
    do {
        for(DrawableNode::Ptr& d : drawables) {
            ...
            if(*((GLuint*) &dnode) == object.back().pointer) { ret = d; }
        }
        object.pop_back();
...
DrawableNode::Ptr SceneOpenGL::makeDrawable(const std::string& name, rw::core::Ptr<Model3D> model,
                                            int dmask) {
...
DrawableNode::Ptr SceneOpenGL::makeDrawable(const std::string& name, rw::core::Ptr<Render> render,
                                            int dmask) {
    Drawable::Ptr drawable = ownedPtr(new Drawable(render, name, dmask));
    return drawable;
}
