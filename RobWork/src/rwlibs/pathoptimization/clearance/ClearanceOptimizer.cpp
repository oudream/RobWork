...
QPath ClearanceOptimizer::optimize(const QPath& inputPath, double stepsize, size_t maxcount,
                                   double maxtime) {
    if(maxcount == 0 && maxtime <= 0) { RW_THROW("No stopping criteria was set."); }
    _stepsize = stepsize;
    ...
    if(path.size() <= 2) return inputPath;
    size_t cnt = 0;
...
#pragma omp parallel for schedule(dynamic, 1)
        ...
        for(AugmentedPath::iterator it = ++(newPath.begin()); it != --(newPath.end()); ++it) {
            ...
            if(it->second < _minClearance && !_qConstraint->inCollision(qnew)) {
                ...
                if(_qConstraintUser != nullptr && _qConstraintUser->inCollision(qnew)) { continue; }
...
Q ClearanceOptimizer::randomDirection() const {
    Q q(_dof);
    do {
        for(size_t i = 0; i < _dof; ++i) q(i) = Random::ran(-_stepsize, _stepsize);
...
void ClearanceOptimizer::setMinimumClearance(const double dist) {
    if(dist < 0) { RW_THROW("Minimum clearance must be greater or equal to zero."); }
    _minClearance = dist;
}
