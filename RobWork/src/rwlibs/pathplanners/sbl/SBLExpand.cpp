...
Q expandUniform(const Q& q, const SBLExpand::QBox& outer, const SBLExpand::QBox& inner,
    ...
    for(int i = 0; i < len; i++) {
        ...
        if(lower <= upper) { result[i] = Random::ran(lower, upper); }
        else return Q();
    }
...
double estimateDiameter(const Device& device, const State& state, int maxCnt) {
    ...
    std::vector<Q> qs;
    for(int cnt = 0; cnt < maxCnt; ++cnt) { qs.push_back(sampler->sample()); }
    ...
    for(const Q& q1 : qs) {
        for(const Q& q2 : qs) {
            ...
            const double dist = MetricUtil::dist2(Kinematics::worldTframe(device.getEnd(), s1).P(),
  ...
  private:
    rw::math::Q doExpand(const rw::math::Q& q) {
        for(double denom = 1;; ++denom) {
            ...
            if(qn.empty() || !_constraint->inCollision(qn)) return qn;
        }
        return Q();
    }
...
class JacobianShrinkingUniformBox : public SBLExpand
{
  public:
    JacobianShrinkingUniformBox(QConstraint::Ptr constraint, const QBox& outer,
        ...
        _outer(outer), _jacobian(jacobian), _device(device), _state(state), _angle_max(angle_max),
        _disp_max(disp_max) {}
...
SBLExpand::Ptr SBLExpand::makeShrinkingUniformJacobianBox(QConstraint::Ptr constraint,
    ...
    if(!jacobian) jacobian = device->baseJCend(state);
