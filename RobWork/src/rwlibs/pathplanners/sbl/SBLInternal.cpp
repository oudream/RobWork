...
class SpatialIndex
{
  ...
  public:
    void reset(const NodeVector& nodes, const IndexPair& pair) {
        ...
        nodeCount = 0;
        for(NI p = nodes.begin(); p != nodes.end(); ++p) addNode(*p);
    }
  ...
  private:
    Node* randomNodeSkewed() const {
        ...
        if(cellCount == 1) return randomNodeFromCell(*cellsInUse.at(0));

        double sum = 0;
        for(int i = 0; i < cellCount; i++) {
            const Cell& cell = *cellsInUse[i];
            sum += (double) (nodeCount - cell.size()) / (double) (nodeCount * (cellCount - 1));
            if(sum > pos) return randomNodeFromCell(cell);
        }
  ...
  private:
    Node* randomNodeUniform() const {
        ...
        for(int i = 0; i < (int) cellsInUse.size(); i++) {
            ...
            if(acc > choice) { return cellsInUse[i]->at(acc - choice - 1); }
        }
        RW_ASSERT(0);
        return 0;
    }
  ...
  public:
    Node* randomNode() const {
        ...
        if(cellsInUse.size() < 50) { return randomNodeSkewed(); }
        else { return randomNodeStandard(); }
    }
    ...
    Node* nodeNearTo(Node* node) {
        switch(options.nearNodeSelection) {
            ...
            case SBLOptions::UniformFromCell: {
                Cell& cell = cellOfNode(node);
                if(cell.empty()) return randomNodeUniform();
                else return randomNodeFromCell(cell);
            }
            case SBLOptions::NearestFromCell: {
                Cell& cell = cellOfNode(node);
                if(cell.empty()) return randomNodeUniform();
                else return nearestNodeFromCell(cell, node);
            }
...
class SBL
{
    ...
    TreeChoice selectTree() {
        switch(options.treeSelection) {
            ...
            case SBLOptions::WeightedTree: {
                ...
                if(Random::ran(0, total) < sizeStart) return Goal;
                else return Start;
            }
    ...
    Motion connect(Node* node, const TreeChoice& choice) {
        if(options.connectAt == SBLOptions::ConnectAlways ||
           (_isReset && options.connectAt == SBLOptions::ConnectAtReset)) {
            ...
            if(dist < options.connectRadius) { return connectTrees(node, near2); }
        }
        return Motion();
    }
  ...
  private:
    void addNodeAndParents(Node* node, NodeVector& start_index_nodes,
  ...
  private:
    void insertRootPath(Node* node, PriorityQueue& node_queue) {
        for(; node != 0; node = node->parent) {
            if(!node->edge->isFullyChecked()) node_queue.push(node);
        }
    }
    ...
    Motion connectTrees(Node* a, Node* b) {
        ...
        while(!node_queue.empty()) {
            ...
            if(node->edge->inCollisionPartialCheck()) {
                if(node != &bridge) { deleteEdge(a, b, node, bridge.edge); }
                return Motion();
            }
  ...
  private:
    Node* pushPathFromRoot(Node* node, Motion& path) {
        Node* root = node;
        if(node->parent != 0) root = pushPathFromRoot(node->parent, path);
  ...
  public:
    ~SBL() {
        for(NI p = nodes.begin(); p != nodes.end(); ++p) delete *p;
    }
...
void addAsRootIfCfree(const Q& q, const QConstraint& constraint, SBL& sbl, const TreeChoice& tree) {
    if(!constraint.inCollision(q)) sbl.addRootNode(q, tree);
}

void addAsRootsIfCfree(const Motion& qs, const QConstraint& constraint, SBL& sbl,
                       const TreeChoice& tree) {
    for(const Q& q : qs) { addAsRootIfCfree(q, constraint, sbl, tree); }
}
