...
double PRMPlanner::estimateRneighbor(size_t roadmapsize) {
    ...
    for(size_t i = 0; i < std::min(roadmapsize, _Nneighbor); i++) { queue.pop(); }
    if(queue.empty()) RW_THROW("Unable to estimate distance to " << roadmapsize << " neighbors");

    return queue.top();
}

bool PRMPlanner::inCollision(const Q& a, const Q& b) const {
    return _constraint->inCollision(a) || _constraint->inCollision(b) || _edge->inCollision(a, b);
}
...
void PRMPlanner::addEdges(Node node) {
    ...
    if(_neighborSearchStrategy == BRUTE_FORCE) {
        ...
        for(VALUE nnode : _kdnodesSearchResult) {
            ...
            if(dist < _Rneighbor) { addEdge(nnode->value, node, dist); }
            neighborTimer.resume();
        }
    }
...
PRMPlanner::Node PRMPlanner::addNode(const Q& q, bool checked) {
    ...
    if(_neighborSearchStrategy == PARTIAL_INDEX_TABLE) { _partialIndexTable->addNode(newNode, q); }
    else if(_neighborSearchStrategy == KDTREE) { _kdtree->addNode(q, newNode); }
    return newNode;
}
...
void PRMPlanner::buildRoadmap(size_t nodecount) {
    ...
    if(_neighborSearchStrategy == PARTIAL_INDEX_TABLE) {
        _partialIndexTable =
            std::shared_ptr<prm::PartialIndexTable<Node>>(new prm::PartialIndexTable<Node>(
    ...
    while(cnt < nodecount) {
        ...
        if(_collisionCheckingStrategy != LAZY) {
            if(_constraint->inCollision(q)) continue;
        }
...
void PRMPlanner::enhanceAround(const Q& q) {
    ...
    for(size_t cnt = 0; cnt < _enhanceAroundSeedCount; cnt++) {
        ...
        if(_collisionCheckingStrategy != LAZY) {
            if(_constraint->inCollision(ran)) continue;
        }
...
void PRMPlanner::enhanceRoadmap() {
    ...
    for(size_t cnt = 0; cnt < _enhanceRandomCnt; cnt++) enhanceAround(_sampler->sample());
...
bool PRMPlanner::doQuery(const rw::math::Q& qInit, const rw::math::Q& qGoal, QPath& path,
    ...
    while(!stop.stop()) {
        ...
        switch(_shortestPathSearchStrategy) {
            ...
            case DIJKSTRA: pathFound = searchForShortestPathDijkstra(nInit, nGoal, nodepath); break;
        }
...
bool PRMPlanner::enhanceEdgeCheck(Edge& e) {
    ...
    while(p < length) {
        Q q = q1 + p * vn;
        if(_constraint->inCollision(q)) return false;
        p += 2 * resolution;
    }
...
bool PRMPlanner::inCollision(std::list<Node>& path) {
    if(_collisionCheckingStrategy == FULL) return false;
    ...
    if(_collisionCheckingStrategy == LAZY) {
        for(int i = 0; i < (int) path.size(); i++) {
            ...
            if(!_graph[n].checked) {
                ...
                else { _graph[n].checked = true; }
            }
        }
    }

    // std::cout<<"All Nodes checked"<<std::endl;
    std::priority_queue<Edge, std::vector<Edge>, EdgeCompare> edgeQueue(EdgeCompare(&_graph),
                                                                        std::vector<Edge>(0));
    ...
    for(size_t i = 0; i < path.size() - 1; i++) {
        ...
        if(_graph[e].resolution > _resolution) { edgeQueue.push(e); }
    }
    ...
    while(!edgeQueue.empty()) {
        ...
        if(_graph[edge].resolution > _resolution) edgeQueue.push(edge);
    }
    return false;
}
...
bool PRMPlanner::searchForShortestPathDijkstra(const Node& nInit, const Node& nGoal,
    ...
    for(boost::tie(vi, vend) = boost::vertices(_graph); vi != vend; ++vi) put(indexMap, *vi, cnt++);
...
void PRMPlanner::removeCollidingNode(Node node) {
    ...
    else if(_neighborSearchStrategy == KDTREE) { _kdtree->removeNode(_graph[node].q); }
