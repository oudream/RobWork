...
rw::core::Ptr< csgjs_model > toCSGJS (TriMeshData::Ptr m1)
{
    ...
    for (Eigen::Index i = 0; i < m1->_triangles.rows (); i++) {
        ...
        Triangle< double > t (
            Vector3D< double > (
                m1->_vertecies (v1, 0), m1->_vertecies (v1, 1), m1->_vertecies (v1, 2)),
            Vector3D< double > (
                m1->_vertecies (v2, 0), m1->_vertecies (v2, 1), m1->_vertecies (v2, 2)),
            Vector3D< double > (
                m1->_vertecies (v3, 0), m1->_vertecies (v3, 1), m1->_vertecies (v3, 2)));
