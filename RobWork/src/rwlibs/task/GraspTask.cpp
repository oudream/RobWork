...
void writeOutcome(std::ostream& out, GraspTarget& target) {
    ...
    Q quality =
        res->qualityAfterLifting;    // target->getPropertyMap().get<Q>("QualityAfterLifting",
                                     // Q::zero(1));
    ...
    if(status == GraspResult::Success || status == GraspResult::ObjectSlipped) {
        ...
        else { out << "     <failure cause=\"OBJECTSLIPPED\" />\n"; }
    }
    else {
        out << "     <failure cause=\"";
        switch(status) {
...
void writeUIBK(GraspTask::Ptr gtask, const std::string& outfile) {
    ...
    for(GraspSubTask& task : gtask->getSubTasks()) {
        ...
        fstr << "   <notes> GraspController:" << graspcontrollerID << " TCP:" << tcpID
             << " CloseQ:" << closeQ << "</notes>\n";
        fstr << "   <params>";
        for(size_t i = 0; i < openQ.size(); i++) fstr << openQ[i] << " ";
        fstr << "   </params>\n"
             << "  </gripper>\n";
        fstr << "  <grasps>\n";
...
rwlibs::task::CartesianTask::Ptr GraspTask::toCartesianTask() {
    ...
    for(GraspSubTask& stask : _subtasks) {
        ...
        for(GraspTarget& gtarget : stask.targets) {
            ...
            else { ctarget->getPropertyMap().set<int>("LiftStatus", GraspResult::ObjectDropped); }

            ctarget->getPropertyMap().set<Transform3DPath>("InterferenceTs",
                                                           Transform3DPath(result->interferenceTs));
...
void GraspTask::saveText(GraspTask::Ptr gtask, const std::string& name) {
    std::ofstream outfile(name.c_str());
    if(!outfile.is_open()) RW_THROW("Could not open file: " << name);
    ...
    for(GraspSubTask& task : gtask->getSubTasks()) {
        ...
        for(GraspTarget& target : task.targets) {
            ...
            int status =
                target.result->testStatus;    // target->getPropertyMap().get<int>("TestStatus",
                                              // GraspTask::UnInitialized);

            outfile << pos(0) << sep << pos(1) << sep << pos(2) << sep << quat.getQx() << sep
                    << quat.getQy() << sep << quat.getQz() << sep << quat.getQw() << sep << status
                    << sep;
            ...
            outfile << t3d.P()[0] << sep << t3d.P()[1] << sep << t3d.P()[2] << sep << quat.getQx()
                    << sep << quat.getQy() << sep << quat.getQz() << sep << quat.getQw() << sep;
            ...
            t3d = target.result
                      ->objectTtcpLift;    // target->getPropertyMap().get<Transform3D<>
            ...
            Q distance =
                target.result
                    ->gripperConfigurationLift;    //->getPropertyMap().get<Q>("GripperConfigurationPost",
                                                   // Q::zero(gripperDim));
            for(size_t i = 0; i < distance.size(); i++) outfile << distance[i] << sep;
...
struct compareElemStrings : public std::binary_function<std::string, std::string, bool>
{
    ...
    bool operator()(const std::string& s1, const std::string& s2) const {
        ...
        if(found != std::string::npos) { s1_tmp = s1.substr(found + 1); }

        found = s2.find_last_of(':');
        if(found != std::string::npos) { s2_tmp = s2.substr(found + 1); }
...
std::pair<bool, double> toDouble(const std::string& str) {
    ...
    buf >> rest;
    if(buf) return nothing;
    else return std::make_pair(true, x);
}
...
rwlibs::task::CartesianTarget::Ptr readGrasp(PTree& tree, ParserState& state) {
    ...
    for(CI p = tree.begin(); p != tree.end(); ++p) {
        ...
        if(isName(p->first, "pose")) {
            ...
            for(CI p1 = rot_tree.begin(); p1 != rot_tree.end(); ++p1) {
                ...
                if(isName(p1->first, "quaternion")) {
                    ...
                    if(vals.size() != 4) RW_THROW("quaternion is wrongly dimensioned!");
                    ...
                    if(!qrotDone) {
                        std::vector<double> vals = readArray(p1->second);
                        if(vals.size() != 9) RW_THROW("rotmatrix is wrongly dimensioned!");
            ...
            if(has_child(p->second, "success")) {
                ...
                else { qualities.push_back(0.0); }
            }
            else if(has_child(p->second, "failure")) {
                status = GraspResult::ObjectDropped;
                std::string cause =
                    p->second.get_child("failure").get_child("<xmlattr>").get<std::string>("cause");
                qualities.push_back(0.0);
                if(cause == "UNINITIALIZED") { status = GraspResult::UnInitialized; }
                else if(cause == "COLLISIONINITIALLY") { status = GraspResult::CollisionInitially; }
                else if(cause == "TIMEOUT") { status = GraspResult::TimeOut; }
                else if(cause == "OBJECTMISSED") { status = GraspResult::ObjectMissed; }
                else if(cause == "OBJECTDROPPED") { status = GraspResult::ObjectDropped; }
                else if(cause == "OBJECTSLIPPED") { status = GraspResult::ObjectSlipped; }
                else if(cause == "SIMULATIONFAILURE") { status = GraspResult::SimulationFailure; }
                else if(cause == "POSEESTIMATEFAILURE") {
                    status = GraspResult::PoseEstimateFailure;
                }
                else if(cause == "INVKINFAILURE") { status = GraspResult::InvKinFailure; }
                else if(cause == "COLLISIONEXE") { status = GraspResult::CollisionDuringExecution; }
            }
            else { qualities.push_back(0.0); }
        ...
        else { target->getPropertyMap().set<Q>("QualityAfterLifting", Q(1, -1)); }
    }
    return target;
}
...
rwlibs::task::CartesianTask::Ptr readExperiment(PTree& tree, ParserState& state) {
    ...
    for(CI p = tree.begin(); p != tree.end(); ++p) {
        ...
        else { RW_THROW("Unknown element!" << p->first); }
...
GraspTask::GraspTask(rwlibs::task::CartesianTask::Ptr task) {
    ...
    for(size_t i = 0; i < task->getTasks().size(); i++) {
        ...
        for(size_t j = 0; j < stask->getTargets().size(); j++) {
            ...
            std::vector<double> contactlist = ctarget->getPropertyMap().get<std::vector<double>>(
                "ContactsGrasp", std::vector<double>());
            if(contactlist.size() > 0) {
            ...
            contactlist = ctarget->getPropertyMap().get<std::vector<double>>("ContactsLift",
                                                                             std::vector<double>());
            if(contactlist.size() > 0) {
                for(size_t m = 0; m < contactlist.size(); m += 9) {
                    rw::sensor::Contact3D contact;
            ...
            result->interferenceTs =
                ctarget->getPropertyMap().get<Transform3DPath>("InterferenceTs", Transform3DPath());
            result->interferenceDistances = ctarget->getPropertyMap().get<std::vector<double>>(
                "InterferenceDistances", std::vector<double>());
...
void GraspTask::filterTasks(std::vector<GraspResult::TestStatus>& includeMask) {
    ...
    for(int i = 0; i < GraspResult::SizeOfStatusArray; i++) { includeMap[i] = false; }

    for(GraspResult::TestStatus includeRule : includeMask) { includeMap[(int) includeRule] = true; }
    ...
    for(GraspSubTask& stask : getSubTasks()) {
        ...
        for(GraspTarget& target : stask.targets) {
            ...
            if(includeMap[status]) { stargets.push_back(target); }
        }
