...
TaskLoader::Ptr TaskLoader::Factory::getTaskLoader(const std::string& format,
    ...
    for(Extension::Ptr ext : exts) {
        ...
        else { return ext->getObject().cast<const TaskLoader>()->clone(); }
    }
    if(StringUtil::toLower(format) == "xml") {
        if(id.empty()) return rw::core::ownedPtr(new DOMTaskLoader());
        else if(StringUtil::toUpper(id) == "DOM") return rw::core::ownedPtr(new DOMTaskLoader());
#ifdef RW_HAVE_XERCES
        else if(StringUtil::toUpper(id) == "XERCES") return rw::core::ownedPtr(new XMLTaskLoader());
#endif
    }
    return NULL;
}
