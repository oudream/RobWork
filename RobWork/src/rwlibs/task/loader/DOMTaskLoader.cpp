...
int readIntAttribute(DOMElem::Ptr element, const std::string& id) {
    if(!element->hasAttribute(id)) { RW_THROW("Unable to find attribute: \"" << id << "\""); }
...
MotionType readMotionTypeAttribute(DOMElem::Ptr element, const std::string& id) {
    ...
    if(type == DOMTaskFormat::idLinearMotion()) return MotionType::Linear;
    if(type == DOMTaskFormat::idP2PMotion()) return MotionType::P2P;
    if(type == DOMTaskFormat::idCircularMotion()) return MotionType::Circular;
...
template<class T> typename Task<T>::Ptr DOMTaskLoader::readTemplateTask(DOMElem::Ptr element) {
    ...
    if(element->hasChildren()) {
        for(DOMElem::Ptr child : element->getChildren()) {
            if(!child.isNull()) {
                if(child->isName(DOMTaskFormat::idTargets())) { readTargets<T>(child, task); }
