...
void XMLTaskSaver::writeMotion(typename Motion<T>::Ptr motion, xercesc::DOMElement* element,
    ...
    if(motion->motionType() == MotionType::Circular) {
        ...
        targetElement->appendChild(doc->createTextNode(
            XMLStr(_targetMap[motion.template cast<CircularMotion<T>>()->midTarget()]).uni()));
    }
...
void XMLTaskSaver::writeEntities(typename Task<T>::Ptr task, xercesc::DOMElement* element,
    ...
    for(Entity::Ptr entity : entities) {
        switch(entity->entityType()) {
            case EntityType::Task: writeTask(entity.cast<Task<T>>(), entriesElement, doc); break;
            case EntityType::Motion:
                writeMotion<T>(entity.cast<Motion<T>>(), entriesElement, doc);
                break;
            case EntityType::Action: writeAction(entity.cast<Action>(), entriesElement, doc); break;
...
template<class T> bool XMLTaskSaver::saveImpl(typename Task<T>::Ptr task, XMLFormatTarget* target) {
    ...
    else { RW_THROW("XMLTaskSaver: Unable to find a suitable DOM Implementation"); }
    return true;
}
