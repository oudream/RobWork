...
MotionType readMotionTypeAttribute(xercesc::DOMElement* element, const XMLCh* id) {
    ...
    if(type == XMLStr(XMLTaskFormat::idLinearMotion()).str()) return MotionType::Linear;
    if(type == XMLStr(XMLTaskFormat::idP2PMotion()).str()) return MotionType::P2P;
    if(type == XMLStr(XMLTaskFormat::idCircularMotion()).str()) return MotionType::Circular;
...
template<class T> typename Target<T>::Ptr XMLTaskLoader::readTarget(xercesc::DOMElement* element) {
    T value;
    DOMNodeList* children     = element->getChildNodes();
    const XMLSize_t nodeCount = children->getLength();
...
template<class T> typename Motion<T>::Ptr XMLTaskLoader::readMotion(xercesc::DOMElement* element) {
    DOMNodeList* children     = element->getChildNodes();
    const XMLSize_t nodeCount = children->getLength();
    ...
    switch(type) {
        ...
        case MotionType::Circular:
            ...
            break;
        case MotionType::P2P: motion = ownedPtr(new P2PMotion<T>(startTarget, endTarget)); break;
    }
...
TaskBase::Ptr XMLTaskLoader::readTask(xercesc::DOMElement* element) {
    if(XMLString::equals(XMLTaskFormat::idQTask(), element->getNodeName())) {
        ...
        RW_THROW("Element name does not match " + XMLStr(XMLTaskFormat::idQTask()).str() + " or " +
                 XMLStr(XMLTaskFormat::idCartesianTask()).str() + " as expected");
    }
}
...
QTask::Ptr XMLTaskLoader::getQTask() {
    if(_qTask == NULL) RW_THROW("No QTask Loaded");
    return _qTask;
}

CartesianTask::Ptr XMLTaskLoader::getCartesianTask() {
    if(_cartTask == NULL) RW_THROW("No CartesianTask Loaded");
    return _cartTask;
}

TaskBase::Ptr XMLTaskLoader::getTask() {
    if(_task == NULL) RW_THROW("No Task Loaded");
    return _task;
}
