...
Rule::Rule(const PropertyBase& property) :
    ...
    if(property.getType().getId() == PropertyType::String) {
        ...
        if(p.getValue()) _value = ownedPtr(new Mathematica::Symbol("True"));
        else _value = ownedPtr(new Mathematica::Symbol("False"));
    }
    ...
    else { RW_THROW("Option \"" << property.getIdentifier() << "\" is not supported!"); }
}
...
Rule::toPropertyMap(const std::list<rw::core::Ptr<const Mathematica::Expression>>& rules) {
    ...
    for(rw::core::Ptr<const Mathematica::Expression> rule : rules) { map->add(toProperty(*rule)); }
    return map;
}

PropertyBase::Ptr Rule::toProperty(const Mathematica::Expression& rule) {
    if(rule.getType() != Mathematica::Expression::Function) RW_THROW("Expected function.");
