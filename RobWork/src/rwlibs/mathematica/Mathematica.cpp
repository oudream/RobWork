...
bool Mathematica::Link::isOpen() const {
    if(impl.isNull()) return false;
    else return impl->open();
}

bool Mathematica::Link::ready() const {
    if(!isOpen()) RW_THROW("Mathematica link is not open!");
...
void Mathematica::Link::operator>>(rw::core::Ptr<Packet>& result) const {
    ...
    if(pkt) {
        ...
        if(WSError(lp)) { error(impl); }
    }
    if(result == NULL) RW_THROW("Did not get any packet.");
}
...
Mathematica::Link::Ptr Mathematica::createLink(const std::string& name, LinkProtocol protocol) {
    ...
    args << "-linkcreate";
    if(name != "") args << " -linkname \"" + name + "\"";
    args << " -linkprotocol ";
    if(protocol == SharedMemory) { args << "SharedMemory"; }
    else if(protocol == TCP_IP) { args << "TCPIP"; }
...
void Mathematica::finalize() {
    while(_links.size() > 0) { closeLink(_links.front()); }
    if(_env != NULL) {
        delete _env;
        _env = NULL;
    }
...
void Mathematica::FunctionBase::out(std::ostream& stream, std::size_t indent) const {
    for(std::size_t i = 0; i < indent; i++) stream << " ";
    indent += 3;
    stream << getName() << "[";
...
void Mathematica::Function::addArguments(const std::list<Expression::Ptr>& args) {
    for(const Expression::Ptr arg : args) { addArgument(*arg); }
}
...
void Mathematica::Function::setArgument(std::size_t i, Expression::Ptr arg) {
    ...
    if(it == _arguments.end()) RW_THROW("This index does not exist.");
...
Mathematica::Function::getArguments() const {
    ...
    for(const rw::core::Ptr<const Mathematica::Expression> arg : _arguments) { res.push_back(arg); }
    return res;
}
...
void Mathematica::put(LinkImpl::Ptr link, const Expression& expression) {
    ...
    switch(expression.getType()) {
        case Expression::Function: {
            ...
            for(const rw::core::Ptr<const Expression> arg : args) { put(link, *arg); }
        } break;
...
std::string Mathematica::readString(LinkImpl::Ptr link, bool symbol) {
    ...
    if(WSGetNext(link->lp) == type) {
        ...
        if(symbol) RW_THROW("Expected symbol");
        else RW_THROW("Expected string");
    }
    return res;
}
...
void Mathematica::addExpression(Function::Ptr exp, LinkImpl::Ptr link) {
    ...
    switch(WSGetNext(link->lp)) {
        ...
        case WSTKREAL:
            ...
            break;
        case WSTKFUNC:
            if(WSGetArgCount(link->lp, &len) == 0) { error(link); }
            else {
                ...
                if(fctName == "RawArray") {
                    ...
                    else { RW_THROW("RawArray of type " << type << " can not yet be handled!"); }
                }
                else {
                    Function::Ptr const fct = ownedPtr(new Function(fctName));
                    for(i = 1; i <= len; ++i) { addExpression(fct, link); }
                    exp->addArgument(fct);
                }
            }
            break;
        case WSTKERROR:
        default:
            if(WSError(link->lp)) { RW_THROW(WSErrorMessage(link->lp)); }
            else { RW_THROW("Error detected by this program."); }
            break;
    }
}
...
int Mathematica::expectFunction(LinkImpl::Ptr link, const std::string& name) {
    ...
    else { RW_THROW("Expected function!"); }
    return len;
}
...
Mathematica::AutoExpression::AutoExpression(const std::list<AutoExpression>& args) :
    ...
    for(const AutoExpression& val : args) { _exp.cast<List>()->add(val); }
}
