...
Image::Image(rw::core::Ptr<const rw::sensor::Image> image) : Mathematica::FunctionBase("Image") {
    ...
    else { RW_THROW("Color encoding not supported."); }
    if(image->getPixelDepth() == rw::sensor::Image::Depth8U) type = "Byte";
    else RW_THROW("Depth not supported.");
...
std::list<rw::core::Ptr<const Mathematica::Expression>> Image::getArguments() const {
    ...
    res.push_back(_data);
    if(!_type.isNull()) res.push_back(_type);
    for(const Rule::Ptr rule : _rules) { res.push_back(rule); }
    return res;
}

Mathematica::Expression::Ptr Image::clone() const {
    const Image::Ptr img = ownedPtr(new Image(*_data));
    if(!_type.isNull()) img->_type = _type->clone().cast<Mathematica::String>();
    for(const Rule::Ptr rule : _rules) { img->_rules.push_back(rule->clone().cast<Rule>()); }
    return img;
}
...
rw::sensor::Image::Ptr Image::toRobWorkImage(const Mathematica::Expression& expression) {
    ...
    if(args.size() == 0) RW_THROW("Expected 1 or more arguments in Image expression.");
    ...
    for(it++; it != args.end(); it++) { rules.push_back(*it); }
    ...
    if(type == "Byte") {
        if(colorSpace == "RGB") {
            ...
            if(!dataFct.isNull()) array = RawArray<unsigned char, 3>::fromExpression(*dataFct);
            else array = dataArg.cast<const Mathematica::Array<unsigned char>>();
            if(array.isNull()) RW_THROW("The data array was not of expected type.");
        ...
        else { RW_THROW("Image with ColorSpace " << colorSpace << " not yet supported."); }
    }
    else { RW_THROW("Image type " << type << " not yet supported."); }
