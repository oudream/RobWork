...
{
  public:
    ExpandedBinary(rw::core::Ptr<QConstraint> constraint, QMetric::CPtr metric, double resolution) :
        ...
        */
        ...
        while(level <= maxLevel) {
            ...
            while(pos <= maxPos) {
                ...
                if(_constraint->inCollision(q)) { return true; }
                else { pos += step; }
            }
            level += 1;
        }
  ...
  private:
    bool doInCollision(const Q& qstart, const Q& qend) const {
        for(QEdgeConstraint::Ptr constraint : _constraints) {
            if(constraint->inCollision(qstart, qend)) return true;
        }
        return false;
    }
