...
Image::Image(char* imageData, unsigned int width, unsigned int height, ColorCode colorCode,
             PixelDepth depth) :
    _width(width),
    _height(height), _colorCode(colorCode), _depth(depth), _nrChannels(toNrOfChannels(colorCode)),
...
Pixel4f Image::getPixelf(size_t x, size_t y) const {
    ...
    if(_depth == Image::Depth32F) {
        ...
        for(size_t i = 1; i < _nrChannels; i++) p.ch[i] = _imageData[cidx + i * _stride];
        return p;
    }
...
void Image::getPixel(size_t x, size_t y, Pixel4f& dst) const {
    ...
    if(_depth == Image::Depth32F) {
        ...
        for(size_t i = 1; i < _nrChannels; i++) dst.ch[i] = _imageData[cidx + i * _stride];
    }
    // is an int so we need to convert it to float
...
void Image::setPixel(size_t x, size_t y, const Pixel4f& value) {
    ...
    if(_depth == Image::Depth32F || _depth == Image::Depth32S) {
        ...
        for(size_t i = 1; i < _nrChannels; i++) ((float*) &_imageData[cidx])[i] = value.ch[i];
    }
...
bool Image::saveAsPGM(const std::string& fileName) const {
    if(_colorCode != GRAY) { RW_THROW("Image must be of type GRAY!"); }
...
Image::Ptr Image::copyFlip(bool horizontal, bool vertical) const {
    ...
    else { memcpy(dstData, _imageData, _arrSize); }
    return dstImg;
}
