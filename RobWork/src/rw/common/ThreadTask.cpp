...
bool ThreadTask::setThreadPool(ThreadPool::Ptr pool) {
    ...
    if(_state->getVariable() != INITIALIZATION) return false;
...
bool ThreadTask::execute() {
    ...
    for(ThreadTask::Ptr subtask : children) { subtask->execute(); }
    return true;
}
...
void ThreadTask::waitUntilDone() {
    ...
    while(state != DONE) { state = wait(state); }
}
...
bool ThreadTask::addSubTask(ThreadTask::Ptr subtask) {
    ...
    if(state == POSTWORK || state == DONE) return false;
    {
        // Lock mutex to get exclusive access to change private member variables.
        // This ensures:
    ...
    if(state != INITIALIZATION) subtask->execute();
    return true;
}
...
void ThreadTask::setKeepAlive(bool keepAlive) {
    ...
    if(_keepAlive->getVariable() == keepAlive) return;
    bool doFinish = false;
    {
        // Lock mutex to get exclusive access to change private member variables.
...
void ThreadTask::finish() {
    ...
    if(!parentCallback.empty()) parentCallback(this);
}
