 ...
 */
void validate(boost::any& v, const std::vector<std::string>& values, StringOptionList* target_type,
              int);
 ...
 */
void validate(boost::any& v, const std::vector<std::string>& values, DoubleOptionList* target_type,
              int);
...
void handleResult(bool success, boost::any& v, const std::string& name, const TYPE& value) {
    if(success) {
        try {
            std::vector<Option<TYPE>>& values = boost::any_cast<std::vector<Option<TYPE>>&>(v);
    ...
    else { RW_THROW("invalid property value, use syntax: <name>=<value>"); }
}
...
void ProgramOptions::addStringOption(const std::string& name, const std::string& defval,
    ...
    else { _optionDesc.add_options()(name.c_str(), po::value<std::string>(), desc.c_str()); }
...
void ProgramOptions::addBoolOption(const std::string& name, bool defval, const std::string& desc) {
    _optionDesc.add_options()(name.c_str(), po::bool_switch()->default_value(defval), desc.c_str());
...
int ProgramOptions::checkVariablesMap(po::variables_map& vm) {
    ...
    for(std::string& o : _additionalBoolOptions) {
        if(_pmap.has(o)) { _pmap.set(o, vm[o.c_str()].as<bool>()); }
        else { _pmap.add(o, "", vm[o.c_str()].as<bool>()); }
    }
    return 0;
}
