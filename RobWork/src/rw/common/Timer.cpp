...
long Timer::getTimeMs() const {
    if(_isPaused) return _totalTime;    // convert to sec
    else return _totalTime + (long) TimerUtil::currentTimeMs() - _relativeTime;
}

std::string Timer::toString(const std::string& format) {
    char line[64];
    long timems = _totalTime;
    if(!_isPaused) timems += (long) TimerUtil::currentTimeMs() - _relativeTime;
    ...
    while(!sstr.eof()) {
        ...
        if(line[0] == 'h') {
            if(line[1] == 'h' && hh < 10) output << "0";
            output << hh << ":";
        }
        else if(line[0] == 'm') {
            if(line[1] == 'm' && mm < 10) output << "0";
            output << mm << ":";
        }
        else if(line[0] == 's') {
            if(line[1] == 's' && ss < 10) output << "0";
            output << ss << ":";
        }
        else if(line[0] == 'z') {
