    ...
    class InputArchive : public virtual Archive
    {
      public:
        ...
        void readEnterScope(const std::string& id, const std::string& idDefault = "") {
            if(id.empty()) { doReadEnterScope(idDefault); }
            else { doReadEnterScope(id); }
        }
        ...
        void readLeaveScope(const std::string& id, const std::string& idDefault = "") {
            if(id.empty()) { doReadLeaveScope(idDefault); }
            else { doReadLeaveScope(id); }
        }
        ...
        void read(T& object, const std::string& id, const std::string& idDefault) {
            if(!id.empty()) doRead(object, id);
            else doRead(object, idDefault);
        }
      ...
      protected:
        //! @copydoc read
        template<class T> void doRead(T& object, const std::string& id) { readImpl(object, id); }
      ...
      private:
        ...
        void readImpl(
            ...
            if(std::is_const<T>::value) { RW_THROW("type T cannot be of type const!"); }
            ...
            else { serialization::read(object, *this, id); }
        }
    };
}}    // namespace rw::common
#endif
