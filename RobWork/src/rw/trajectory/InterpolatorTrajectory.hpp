    ...
    template<class T> class InterpolatorTrajectory : public rw::trajectory::Trajectory<T>
    {
      ...
      public:
        ...
        double duration() const {
            if(_segments.empty()) return -1;
            else return _segments.back().t2;
        }
        ...
        void add(const B<T>& blend, const I<T>& interpolator) {
            add(rw::core::ownedPtr(new B<T>(blend)), rw::core::ownedPtr(new I<T>(interpolator)));
        }
        ...
        getSegment(size_t index) const {
            ...
            return std::pair<rw::core::Ptr<Blend<T>>, rw::core::Ptr<Interpolator<T>>>(blend,
                                                                                      interpolator);
        }
      ...
      private:
        ...
        void addSegment(typename Blend<T>::Ptr blend1, typename Blend<T>::Ptr blend2,
            ...
            if(!_segments.empty()) { _segments.back().blend2 = segment.blend1; }
        ...
        const Segment& segmentSearch(double t, double index, double delta) const {
            ...
            if(segment.t1 > t) return segmentSearch(t, index - delta / 2.0, delta / 2.0);
            else if(segment.t2 < t) return segmentSearch(t, index + delta / 2.0, delta / 2.0);
            else return segment;
        }
        ...
        const Segment& getSegment(double t) const {
            ...
            if(n < 1) { RW_THROW("Cannot request values from an empty Trajectory"); }

            if(t < 0) { t = 0.0; }
            else if(_segments.back().t2 < t) { t = _segments.back().t2; }
            /*
