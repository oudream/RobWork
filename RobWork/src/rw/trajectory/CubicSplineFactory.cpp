...
InterpolatorTrajectory<Q>::Ptr CubicSplineFactory::makeNaturalSpline(QPath::Ptr qpath,
    ...
    for(size_t i = 0; i < qpath->size(); i++) { times.push_back(i * timeStep); }
...
CubicSplineFactory::makeNaturalSpline(const QPath& qpath, const std::vector<double>& times) {
    ...
    if(qpath.size() < 2) RW_THROW("Path must be longer than 1!");
    if(qpath.size() != times.size()) RW_THROW("Length of path and times need to be equal");
    ...
    for(size_t j = 0; j < (size_t) dim; j++) {
        for(size_t i = 0; i < (size_t) N + 1; i++) { Y[i] = (T) (qpath[i])[j]; }
        ...
        for(size_t i = 0; i < (size_t) N + 1; i++) { a[i * dim + j] = Y[i]; }
...
CubicSplineFactory::makeNaturalSpline(const Path<T>& path, double timeStep) {
    std::vector<double> times;
    for(size_t i = 0; i < path.size(); i++) { times.push_back(i * timeStep); }
    return makeNaturalSpline(path, times);
}
...
template InterpolatorTrajectory<Q>::Ptr CubicSplineFactory::makeNaturalSpline(const Path<Q>& path,
                                                                              double timeStep);
...
CubicSplineFactory::makeNaturalSpline(const Path<T>& path, const std::vector<double>& times) {
    if(path.size() < 2) { RW_THROW("Path must be longer than 1!"); }
    if(path.size() != times.size()) { RW_THROW("Length of path and times need to be equal"); }
    ...
    for(size_t i = 0; i < N; i++) { H[i] = (double) (times[i + 1] - times[i]); }
#if EIGEN_VERSION_AT_LEAST(3, 1, 0)
...
#endif
    for(size_t j = 0; j < (size_t) dim; j++) {
        for(size_t i = 0; i < (size_t) N + 1; i++) { Y[i] = (double) (path[i])[j]; }
...
template InterpolatorTrajectory<Q>::Ptr CubicSplineFactory::makeClampedSpline(const Path<Q>& path,
                                                                              const Q& dStart,
                                                                              const Q& dEnd,
                                                                              double timeStep);
...
template<typename T>
rw::core::Ptr<rw::trajectory::InterpolatorTrajectory<T>>
CubicSplineFactory::makeClampedSpline(const Path<Timed<T>>& tpath, const T& dStart, const T& dEnd) {
    Path<T> path;
    std::vector<double> times;
...
template InterpolatorTrajectory<Q>::Ptr
CubicSplineFactory::makeClampedSpline(const Path<Timed<Q>>& path, const Q& dStart, const Q& dEnd);
...
CubicSplineFactory::makeClampedSpline(const Path<T>& path, const std::vector<double>& times,
    ...
    if(path.size() != times.size()) RW_THROW("Length of path and times need to match");
    ...
    for(size_t i = 0; i < N; i++) { H[i] = (float) (times[i + 1] - times[i]); }
#if EIGEN_VERSION_AT_LEAST(3, 1, 0)
    Eigen::SparseMatrix<double> A((int) N + 1, (int) N + 1);
    ...
    for(size_t j = 0; j < (size_t) dim; j++) {
        ...
        for(size_t i = 0; i < N + 1; i++) { a[i * dim + j] = Y[i]; }
...
template InterpolatorTrajectory<Q>::Ptr
CubicSplineFactory::makeClampedSpline(const Path<Q>& path, const std::vector<double>& times,
                                      const Q& dStart, const Q& dEnd);
...
CubicSplineFactory::makeClampedSpline(const Path<Quaternion<>>& path,
                                      const std::vector<double>& times, const Quaternion<>& dStart,
                                      const Quaternion<>& dEnd);
...
CubicSplineFactory::makeNaturalSpline(const Path<rw::math::Transform3D<>>& path,
    ...
    for(const Transform3D<>& trans : path) { NPath.push_back(Transform3DVector<>(trans)); }
