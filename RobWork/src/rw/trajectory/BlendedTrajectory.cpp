...
BlendedTrajectory<T>::BlendedTrajectory(rw::models::Device::Ptr deviceIn,
                                        ...
                                        const std::vector<double>& betaIn, const double vscaleIn,
                                        const double ascaleIn, const bool verbose) {
    ...
    if(init()) {
        if(verbose) std::cout << "Creating blends..." << std::endl;
        for(unsigned int i = 1; i < Npath - 1; i++) {
            if(verbose) std::cout << "Blend: " << i << std::endl;
            beta = betas[i - 1];
                ...
                if(dqI[k] * dqF[k] < 0) {    // TURNING JOINT
                    ...
                    if(TB * fabs(dqav[k]) -
                       ...
                       -epseps) {    // The very small negative number is to make sure that the if
                        ...
                        Tmid[k]      = TI + deltaTmid[k];
                        // Try first solution
                        vmin[k] = (0.5 * aBMax[k] * dqF[k] - 0.5 * aBMin[k] * dqI[k] -
...
template<class T> bool BlendedTrajectory<T>::init() {
    ...
    for(unsigned int i = 0; i < Npath - 1; i++) { sList[i].resize(K); }
...
template<class T> bool BlendedTrajectory<T>::checkPath() {
    ...
    if(Npath < 2) RW_THROW("Invalid path size < 2!");
...
template<class T> void BlendedTrajectory<T>::updateLimits() {
    for(unsigned int k = 0; k < K; k++) {
        ...
        if(fabs(deltaqnext[k]) < eps && fabs(deltaqnextnext[k]) < eps) { aBMaxNext[k] = epseps; }
...
template<class T> void BlendedTrajectory<T>::updateWI() {
    ...
    for(unsigned int k = 0; k < K; k++) {
        if(fabs(deltaq[k]) > deltaqmax) deltaqmax = fabs(deltaq[k]);
    }
...
template<class T> void BlendedTrajectory<T>::updateWF() {
    ...
    for(unsigned int k = 0; k < K; k++) {
        if(fabs(deltaqnext[k]) > deltaqnextmax) deltaqnextmax = fabs(deltaqnext[k]);
    }
...
template<class T> T BlendedTrajectory<T>::x(double t) const {
    if(t > t_total) RW_THROW("Time " << t << " is outside of interface [0;duration]");
    if(t < 0.0 || t > t_total) return T::zero(K);

    T x(K);
    for(unsigned int confNumber = 0; confNumber < Npath - 2; confNumber++) {
        if(t < TIList[confNumber]) {    // We are in a linear segment
            for(unsigned int k = 0; k < K; k++) { x[k] = sList[confNumber][k].x(t); }
            return x;
        }
    ...
    for(unsigned int k = 0; k < K; k++) { x[k] = sList[Npath - 2][k].x(t); }
    return x;
}
...
template<class T> T BlendedTrajectory<T>::dx(double t) const {
    ...
    if(t < 0.0 || t > t_total) return rw::math::Q::zero(K);

    rw::math::Q dx(K);
    for(unsigned int confNumber = 0; confNumber < Npath - 2; confNumber++) {
        if(t < TIList[confNumber]) {    // We are in a linear segment
            for(unsigned int k = 0; k < K; k++) { dx[k] = sList[confNumber][k].dx(t); }
            return dx;
        }
        else if(t < TFList[confNumber]) {    // We are in a blend
            for(unsigned int k = 0; k < K; k++) {
    ...
    // We are in the final linear segment
    for(unsigned int k = 0; k < K; k++) { dx[k] = sList[Npath - 2][k].dx(t); }
    return dx;
}

template<class T> T BlendedTrajectory<T>::ddx(double t) const {
    if(t < 0.0 || t > t_total) RW_THROW("Time " << t << " is outside of interface [0;duration]");

    rw::math::Q ddx(K);
    for(unsigned int confNumber = 0; confNumber < Npath - 2; confNumber++) {
        if(t < TIList[confNumber]) {    // We are in a linear segment
            for(unsigned int k = 0; k < K; k++) { ddx[k] = sList[confNumber][k].ddx(t); }
            return ddx;
        }
        else if(t < TFList[confNumber]) {    // We are in a blend
            for(unsigned int k = 0; k < K; k++) {
    ...
    // We are in the final linear segment
    for(unsigned int k = 0; k < K; k++) { ddx[k] = sList[Npath - 2][k].ddx(t); }
    return ddx;
}
...
template<class T> T BlendedTrajectory<T>::findQext(T& qI, T& q, T& qF) {
    T qext(K);
    for(unsigned int k = 0; k < K; k++) {
        if(fabs(qI[k] - q[k]) < eps && fabs(qF[k] - q[k]) < eps) { qext[k] = q[k]; }
        else {
...
double BlendedTrajectory<T>::findDWF(T& deltaq, T& qI, T& qext, T& qextnext, T& qF, T& qInext,
                                     T& deltaqnext, T& deltaqnextnext, T& dqI, T& dqMax, T& aBMin,
                                     T& aBMax, T& aBMaxNext) {
    T dqIBMaxNext(K);
    T dqFMax(K);
    double dwF = inf;
    ...
    for(unsigned int k = 0; k < K; k++) {
        ...
        if(fabs(deltaq[k]) < eps && fabs(deltaqnext[k]) < eps) { dqFMax[k] = dqMax[k]; }
        else {
            if(fabs(deltaq[k]) < eps) {
                ...
                if(Math::sign(deltaq[k]) != Math::sign(deltaqnext[k])) {    // Turning joint
                    ...
                    dqFMax[k] = std::min(dqMax[k],
                                         sqrt(2.0 * fabs((qF[k] - (qI[k] - (fastPow(dqI[k], 2.0)) /
                                                                               (2.0 * aBMax[k]))) *
                                                         aBMin[k])));
                }
            }
...
template<class T> double BlendedTrajectory<T>::tau(double x, double uI, double uF, double a) {
    return (uF - uI) / a +
           fabs(x - ((fastPow(uF, 2) - fastPow(uI, 2)) / (2.0 * a))) / std::max(fabs(uI), fabs(uF));
}
...
double BlendedTrajectory<T>::findTS(double wI, double dwI, double wFprev, double dwFprev,
                                    ...
                                    double TSdecc) {
    return TSdecc +
           tau(1.0 - wFprev - wI - (fastPow(dwI, 2.0) - fastPow(dwSreached, 2.0)) / (2.0 * ddwSMin),
               dwFprev,
               dwSreached,
               ddwSMax);
}
...
template<class T> double BlendedTrajectory<T>::phimin(double v, double a, double tau, double t) {
    if(fabs(v) < eps && fabs(a) < eps) { return 0.0; }
    else {
        if(t < 0.0) return 0.0;
        else if(t >= 0.0 && t <= v / a) return 0.5 * a * fastPow(t, 2.0);
        else if(t > v / a && t <= tau) return 0.5 * fastPow(v, 2.0) / a + v * (t - v / a);
        else    // t > tau
            return 0.0;
    }
}

template<class T> double BlendedTrajectory<T>::dphimin(double v, double a, double tau, double t) {
    if(fabs(v) < eps && fabs(a) < eps) { return 0.0; }
    else {
        if(t < 0.0) return 0.0;
        else if(t >= 0.0 && t <= v / a) return a * t;
        else if(t > v / a && t <= tau) return v;
        else    // t > tau
            return 0.0;
    }
}

template<class T> double BlendedTrajectory<T>::ddphimin(double v, double a, double tau, double t) {
    if(fabs(v) < eps && fabs(a) < eps) { return 0.0; }
    else {
        if(t < 0.0) return 0.0;
        else if(t >= 0.0 && t <= v / a) return a;
        else if(t > v / a && t <= tau) return 0.0;
        else    // t > tau
            return 0.0;
    }
}
...
template<class T> double BlendedTrajectory<T>::phimax(double v, double a, double tau, double t) {
    if(fabs(v) < eps && fabs(a) < eps) { return 0.0; }
        ...
        if(t <= tau - v / a) return 0.0;
        else if(t > tau - v / a && t <= tau) return 0.5 * a * fastPow(t - tau + v / a, 2.0);
        else    // t > tau
            return 0.0;
    }
}

template<class T> double BlendedTrajectory<T>::dphimax(double v, double a, double tau, double t) {
    if(fabs(v) < eps && fabs(a) < eps) { return 0.0; }
    else {
        if(t <= tau - v / a) return 0.0;
        else if(t > tau - v / a && t <= tau) return a * t + v - a * tau;
        else    // t > tau
            return 0.0;
    }
}

template<class T> double BlendedTrajectory<T>::ddphimax(double v, double a, double tau, double t) {
    if(fabs(v) < eps && fabs(a) < eps) { return 0.0; }
    else {
        if(t <= tau - v / a) return 0.0;
        else if(t > tau - v / a && t <= tau) return a;
        else    // t > tau
            return 0.0;
    }
}
...
double BlendedTrajectory<T>::phistarmin(double v0, double v, double a, double tau, double t) {
    if(fabs(v) < eps && fabs(a) < eps) { return 0.0; }
        ...
        if(t < 0.0) return 0.0;
        else if(t >= 0.0 && t <= tau) return v0 * t + (phimin(v - v0, a, tau, t));
        else    // t > tau
            return 0.0;
    }
}
...
double BlendedTrajectory<T>::dphistarmin(double v0, double v, double a, double tau, double t) {
    if(fabs(v) < eps && fabs(a) < eps) { return 0.0; }
        ...
        if(t < 0.0) return 0.0;
        else if(t >= 0.0 && t <= tau) return v0 + dphimin(v - v0, a, tau, t);
        else    // t > tau
            return 0.0;
    }
}
...
double BlendedTrajectory<T>::ddphistarmin(double v0, double v, double a, double tau, double t) {
    if(fabs(v) < eps && fabs(a) < eps) { return 0.0; }
        ...
        if(t < 0.0) return 0.0;
        else if(t >= 0.0 && t <= tau) return ddphimin(v - v0, a, tau, t);
        else    // t > tau
            return 0.0;
    }
}
...
double BlendedTrajectory<T>::phistarmax(double v0, double v, double a, double tau, double t) {
    if(fabs(v) < eps && fabs(a) < eps) { return 0.0; }
        ...
        if(t < 0.0) return 0.0;
        else if(t >= 0.0 && t <= tau) return v0 * t + phimax(v - v0, a, tau, t);
        else    // t > tau
            return 0.0;
    }
}
...
double BlendedTrajectory<T>::dphistarmax(double v0, double v, double a, double tau, double t) {
    if(fabs(v) < eps && fabs(a) < eps) { return 0.0; }
        ...
        if(t < 0.0) return 0.0;
        else if(t >= 0.0 && t <= tau) return v0 + dphimax(v - v0, a, tau, t);
        else    // t > tau
            return 0.0;
    }
}
...
double BlendedTrajectory<T>::ddphistarmax(double v0, double v, double a, double tau, double t) {
    if(fabs(v) < eps && fabs(a) < eps) { return 0.0; }
        ...
        if(t < 0.0) return 0.0;
        else if(t >= 0.0 && t <= tau) return ddphimax(v - v0, a, tau, t);
        else    // t > tau
            return 0.0;
    }
}
