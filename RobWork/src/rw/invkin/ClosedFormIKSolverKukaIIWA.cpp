...
ClosedFormIKSolverKukaIIWA::ClosedFormIKSolverKukaIIWA(
    ...
    _device(device),
    _checkJointLimits(true), _qLow(device->getBounds().first), _qHigh(device->getBounds().second) {
    _frames.push_back(_device->getBase());
    for(const Joint* const joint : _device->getJoints()) { _frames.push_back(joint); }
...
std::pair<double, double> ClosedFormIKSolverKukaIIWA::findBaseAngles(const Vector2D<>& pos,
    ...
    if(pos[0] != 0 || pos[1] != 0) { res.first = std::atan2(pos[1], pos[0]); }
    else {
...
void ClosedFormIKSolverKukaIIWA::addBaseAngleSolutions(const Rotation3D<>& baseRend,
                                                       ...
                                                       const Vector3D<>& basePtarget4, State& state,
    ...
    if(_checkJointLimits) {    // Abort if outside limits
        if(tmpQ[1] < _qLow[1] || tmpQ[1] > _qHigh[1]) return;
    }
...
void ClosedFormIKSolverKukaIIWA::addOuterSolutions(const Rotation3D<>& baseRend,
    ...
    if(_checkJointLimits) {    // Abort if outside limits
        if(tmpQ[3] < _qLow[3] || tmpQ[3] > _qHigh[3]) return;
    }
...
void ClosedFormIKSolverKukaIIWA::addRotationSolutions(const Rotation3D<>& baseRend, State& state,
    ...
    if(_checkJointLimits) {    // Abort if outside limits
        if(tmpQ[5] < _qLow[5] || tmpQ[5] > _qHigh[5]) return;
    }
    ...
    if(_checkJointLimits) {    // Abort if outside limits
        if(tmpQ[6] < _qLow[6] || tmpQ[6] > _qHigh[6]) return;
    }
...
Vector3D<> ClosedFormIKSolverKukaIIWA::randomPerpendicularVector(const Vector3D<>& v) {
    Vector3D<> perp;
    if(v[0] < v[1]) {
        if(v[0] < v[2]) perp = normalize(cross(v, Vector3D<>::x()));
        else perp = normalize(cross(v, Vector3D<>::z()));
    }
    else {
        if(v[1] < v[2]) perp = normalize(cross(v, Vector3D<>::y()));
        else perp = normalize(cross(v, Vector3D<>::z()));
    }
