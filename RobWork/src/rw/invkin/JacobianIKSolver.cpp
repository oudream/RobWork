...
JacobianIKSolver::JacobianIKSolver(Device::CPtr device, rw::core::Ptr<const Frame> foi,
    ...
    _devJac(device->baseJCframe(foi, state)), _useJointClamping(false), _useInterpolation(false),
    _checkJointLimits(false), _solverType(SVD), _w(Eigen::VectorXd::Ones(_device->getDOF())),
...
bool JacobianIKSolver::solveLocal(const Transform3D<>& bTed, double maxError, State& state,
    ...
    for(int cnt = 0; cnt < maxIterations; ++cnt) {
        ...
        if(normInf(b_eXed) <= maxError) { return true; }
