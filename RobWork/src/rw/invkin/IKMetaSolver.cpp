...
bool IKMetaSolver::betweenLimits(const Q& q) const {
    for(size_t i = 0; i < q.size(); i++) {
        if(q(i) < _bounds.first(i) || _bounds.second(i) < q(i)) return false;
    }
    return true;
}
...
Q IKMetaSolver::getRandomConfig() const {
    Q q(_dof);
    for(int i = 0; i < (int) _dof; i++) { q(i) = Math::ran(_bounds.first(i), _bounds.second(i)); }
    return q;
}
    ...
    while(cnt-- > 0) {
        ...
        for(std::vector<Q>::iterator it = solutions.begin(); it != solutions.end(); ++it) {
            if(!_checkForLimits || betweenLimits(*it)) {
                if(_constraint != NULL) {
                    if(_constraint->inCollision(*it)) { continue; }
                }
