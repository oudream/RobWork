...
JointType getType(const Joint& joint) {
    if(dynamic_cast<const RevoluteJoint*>(&joint)) return RevoluteType;
    else if(dynamic_cast<const PrismaticJoint*>(&joint)) return PrismaticType;
    ...
    do {
        ...
        for(int j = (int) joints.size() - 1; j >= 0; --j) {
            ...
            if(type == RevoluteType) {
                ...
                double k1 = wp * (dot(Pid, Bai) * dot(Pic, Bai)) +
                ...
                else if(secdev1 < 0) { qi = phi1; }
                else if(secdev2 < 0) { qi = phi2; }
                else { std::cout << "No maximizing value found" << std::endl; }
            }
            else if(type == PrismaticType) { qi = dot((Pid - Pic), Bai); }
            else {
