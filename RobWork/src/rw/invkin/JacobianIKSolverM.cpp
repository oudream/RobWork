...
std::vector<std::shared_ptr<FKRange>> createFKRanges(rw::core::Ptr<const Frame> base,
...
bool JacobianIKSolverM::solveLocal(const std::vector<Transform3D<>>& bTed,
    ...
    for(int cnt = 0; cnt < maxIterations; ++cnt) {
        ...
        if(!untilSmallChange) {
            ...
            if(std::fabs(error - lastError) < maxError[0] /*|| error-lastError>0*/) return true;
        }
        lastError = error;
        ...
        switch(_solverType) {
            ...
            case(DLS): {
                ...
                double maxChange = dTheta.lpNorm<Eigen::Infinity>();
                if(maxChange > 45.0 * Deg2Rad) { dTheta *= (45.0 * Deg2Rad) / maxChange; }
                Q dq(dTheta);
                q += dq;
            } break;
        }

        if(_useJointClamping) q = Math::clampQ(q, bounds.first, bounds.second);
...

{
    ...
    if(_useInterpolation) {
        ...
        for(double t = _interpolationStep; t < interpolators[0].duration();
            ...
            // calculate the bTes
            for(int i = 0; i < N; i++) bTeds_via[i] = interpolators[i].x(t);
