...
PieperSolver::PieperSolver(SerialDevice& dev, const Transform3D<>& joint6Tend, const State& state) :
    _dhparams(DHParameterSet::getDHParameters(&dev)), _endTjoint6(inverse(joint6Tend)) {
    _baseTdhRef =
...
void PieperSolver::setCheckJointLimits(bool check) {
    if(check) RW_THROW("Limit Check Not Supported in rw::invkin::PieperSolver");
}

void PieperSolver::init() {
    if(_dhparams.size() != 6) RW_THROW("Need 6 DH-parameters");
...
std::vector<Q> PieperSolver::solve(const Transform3D<>& baseTend,
    ...
    for(std::vector<Q>::iterator it = result.begin(); it != result.end(); ++it) {
        for(size_t i = 0; i < (*it).size(); i++) (*it)(i) -= _dhparams[i].theta();
    }
...
void PieperSolver::solveTheta456(double theta1, double theta2, double theta3, Transform3D<>& T06,
    ...
    double alt4, alt6;
    if(theta4 > 0) alt4 = theta4 - Pi;
    else alt4 = theta4 + Pi;

    if(theta6 > 0) alt6 = theta6 - Pi;
    else alt6 = theta6 + Pi;
    ...
    double c1 =
        ...
        (Power(a1, 2) + Power(a3, 2) * Power(c2, 2) * Power(c3, 2) +
         ...
         2 * d4 *
             ...
             salpha3 +
         Power(d4, 2) *
             (Power(c2, 2) * (Power(c3, 2) * Power(calpha1, 2) * Power(calpha2, 2) + Power(s3, 2)) +
...
std::vector<double> PieperSolver::solveTheta2Case2(double r, double theta3) const {
    ...
    if(fabs(a) < 1e-12) { result.push_back(-c / b); }
    else if(fabs(d) < 1e-12) result.push_back(-b / (2 * a));
    else if(d > 0) {
        ...
        -(Power(a1, 2) * salpha1 * (a2 + a3 * c3 + d4 * s3 * salpha3) +
          salpha1 * (a2 + a3 * c3 + d4 * s3 * salpha3) *
              (Power(a2, 2) + Power(a3, 2) + Power(d2, 2) + 2 * calpha2 * d2 * d3 + Power(d3, 2) +
               2 * calpha3 * (calpha2 * d2 + d3) * d4 + Power(d4, 2) - r +
...
std::vector<double> PieperSolver::solveTheta3Case2(double z) const {
    ...
    double d = b * b - 4 * a * c;
    std::vector<double> result;
    if(fabs(d) < 1e-12) result.push_back(-b / (2 * a));
    else if(d > 0) {
        result.push_back((-b - sqrt(d)) / (2 * a));
        result.push_back((-b + sqrt(d)) / (2 * a));
    }
...
std::vector<double> PieperSolver::fSolve() const {
    ...
    for(size_t i = 0; i < (std::size_t) eigen.second.size(); i++) {
        ...
        if(eigen.second(i).imag() == 0) result.push_back(eigen.second(i).real());
    }
    return result;
}
...
std::vector<double> PieperSolver::fSolve(double s1, double s2, double s3) const {
    ...
    std::vector<double> result;
    for(int i = 0; i < 4; i++) {
        if(has_solution[i]) { result.push_back(solution[i]); }
    }
    return result;
}
...
std::vector<double> PieperSolver::ddfSolve() const {
    ...
    if(dc < 0) { return std::vector<double>(); }
    else {
        std::vector<double> result;
...
void PieperSolver::setupCoefficients(double r, double z) const {
    a = (Power(a1, 2) +
         ...
         (2 *
          (2 * Power(calpha1, 2) *
           ...
           Power(salpha1, 2) *
               (-Power(a2 - a3, 2) + Power(d2, 2) - r + Power(d3, 2) * (1 - 2 * Power(salpha2, 2)) +
                2 * d3 * d4 *
    ...
    c = Power(a1, 2) / 2. - Power(a2, 2) + Power(a3, 2) * (3 - 4 * Power(calpha2, 2)) +
        ...
        (Power(a2, 4) + Power(a3, 4) +
         ...
         2 * Power(a2, 2) *
             (-Power(a3, 2) + Power(d2, 2) + Power(d3, 2) + 2 * calpha3 * d3 * d4 + Power(d4, 2) +
