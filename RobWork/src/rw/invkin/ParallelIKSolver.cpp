...
void ParallelIKSolver::updateDeltaX(const std::vector<Target>& targets,
    ...
    for(std::size_t ti = 0; ti < targets.size(); ti++) {
        ...
        rw::core::Ptr<const Frame> targetRefFrame =
            (targets[ti].refFrame == NULL) ? rw::core::Ptr<const Frame>(_device->getBase())
                                           : targets[ti].refFrame.cptr();
...
void ParallelIKSolver::updateJacobian(const std::vector<Target>& targets,
    ...
    std::size_t conDOF = 0;
    for(std::size_t i = 0; i < targets.size(); i++) { conDOF += targets[i].dof(); }
        ...
        for(std::size_t frameI = 0; frameI < frames.size(); frameI++) {
            ...
            for(std::size_t i = 0; i < 6; i++) {
                if(target.enabled[i]) {
                    ...
                    if(depJoint != NULL) { index = _jointIndex[*depJoint]; }
...
std::vector<Q> ParallelIKSolver::solve(const std::vector<Target>& targets,
    ...
    for(std::size_t i = 0; i < targets.size(); i++) {
        ...
        rw::core::Ptr<const Frame> targetRefFrame =
            (targets[i].refFrame == NULL) ? rw::core::Ptr<const Frame>(_device->getBase())
                                          : targets[i].refFrame.cptr();
        for(std::size_t i = 0; i < _junctions.size(); i++) {
            ...
            for(std::size_t legI = 0; legI < legs.size(); legI++) {
                ...
                while(searchFrame != NULL && refFrame == NULL) {
                    if(searchFrame == targetRefFrame) { refFrame = searchFrame; }
        ...
        while(frame != NULL && tcpFrame == NULL) {
            for(std::size_t i = 0; i < allJoints.size(); i++) {
                if(allJoints[i] == frame) tcpFrame = allJoints[i];
            }
            if(tcpFrame == NULL) frame = frame->getParent(state);
        }
        if(tcpFrame == NULL)
    ...
    std::vector<Q> sol;
    if(!_checkJointLimits) { sol.push_back(q); }
    else if(Models::inBounds(_device->getFullQ(state), bounds)) { sol.push_back(q); }
