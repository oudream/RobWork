...
PrismaticJoint::PrismaticJoint(const std::string& name, const Transform3D<>& transform) :
    ...
    if(rot == Rotation3D<>::identity()) _impl = new PrismaticJointZeroRotationImpl(transform.P());
    else if(transform.P() == Vector3D<>(0, 0, 0))
        _impl = new PrismaticJointZeroOffsetImpl(transform.R());
    else _impl = new PrismaticJointImplBasic(transform);
}
...
void PrismaticJoint::setFixedTransform(const rw::math::Transform3D<>& t3d) {
    ...
    if(rot == Rotation3D<>::identity()) _impl = new PrismaticJointZeroRotationImpl(t3d.P());
    else if(t3d.P() == Vector3D<>(0, 0, 0)) _impl = new PrismaticJointZeroOffsetImpl(t3d.R());
    else _impl = new PrismaticJointImplBasic(t3d);
    delete tmp;
}
...
rw::math::Transform3D<>
PrismaticJoint::getJointTransform(const rw::kinematics::State& state) const {
