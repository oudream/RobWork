...
RigidObject::RigidObject(rw::core::Ptr<rw::kinematics::Frame> baseframe, Geometry::Ptr geom) :
    ...
    _Ibody(rw::math::InertiaMatrix<>::makeSolidSphereInertia(1.0, 0.1)) {
    if(geom->getFrame() == NULL) geom->setFrame(baseframe);
}
...
RigidObject::RigidObject(std::vector<rw::kinematics::Frame*> frames, Geometry::Ptr geom) :
    ...
    _Ibody(rw::math::InertiaMatrix<>::makeSolidSphereInertia(1.0, 0.1)) {
    if(geom->getFrame() == NULL) geom->setFrame(getBase());
}
...
void RigidObject::removeGeometry(Geometry::Ptr geom) {
    ...
    if(iter != _geometry.end()) _geometry.erase(iter);
}
...
void RigidObject::removeModel(Model3D::Ptr model) {
    ...
    if(iter != _models.end()) _models.erase(iter);
}
