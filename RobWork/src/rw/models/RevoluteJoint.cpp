...
RevoluteJoint::RevoluteJoint(const std::string& name, const Transform3D<>& transform) :
    Joint(name, 1) {
    if(transform.P() == Vector3D<>(0, 0, 0)) _impl = new RevoluteJointZeroOffsetImpl(transform.R());
    else _impl = new RevoluteJointBasic(transform);
}
...
void RevoluteJoint::setFixedTransform(const rw::math::Transform3D<>& t3d) {
    ...
    if(t3d.P() == Vector3D<>(0, 0, 0)) _impl = new RevoluteJointZeroOffsetImpl(t3d.R());
    else _impl = new RevoluteJointBasic(t3d);
    delete tmp;
}
