...
ParallelLeg::ParallelLeg(std::vector<Frame*> frames) : _kinematicChain(frames) {
    ...
    for(; iter != _kinematicChain.end(); ++iter) {
        ...
        if(joint != NULL) {
            if(joint->isActive()) { _actuatedJoints.push_back(joint); }
            else { _unactuatedJoints.push_back(joint); }
        }
    }
    _jacobian = new Jacobian(Jacobian::zero(6, getJointDOFs()).e());
}

ParallelLeg::ParallelLeg(std::vector<Frame::Ptr> frames) :
    _kinematicChain(frames.size()), _kinematicChainPtr(frames) {
    int i                                  = 0;
    std::vector<Frame::Ptr>::iterator iter = frames.begin();
    for(; iter != frames.end(); ++iter) {
        _kinematicChain[i++] = iter->get();
        Joint* joint         = (*iter).cast<Joint>().get();
        if(joint != NULL) {
            if(joint->isActive()) { _actuatedJoints.push_back(joint); }
            else { _unactuatedJoints.push_back(joint); }
        }
    ...
    for(iter++; iter != _kinematicChain.end(); ++iter) {
        ...
        if(const Joint* const joint = dynamic_cast<const Joint*>(*iter)) {
            ...
            for(std::size_t i = 0; i < 6; i++) {
                for(int j = 0; j < DOFs; j++) { (*_jacobian)(i, index + j) = 0; }
            }
...
Jacobian ParallelLeg::baseJframe(rw::core::Ptr<const Frame> frame, const State& state) const {
    ...
    for(iter++; iter != _kinematicChain.end(); ++iter) {
        ...
        if(const Joint* const joint = dynamic_cast<const Joint*>(*iter)) {
            ...
            for(std::size_t i = 0; i < 6; i++) {
                for(int j = 0; j < DOFs; j++) { jac(i, index + j) = 0; }
            }
...
Transform3D<double> ParallelLeg::baseTframe(rw::core::Ptr<const Frame> frame,
...
std::size_t ParallelLeg::getJointDOFs() const {
    ...
    for(std::size_t i = 0; i < _actuatedJoints.size(); i++) {
        ...
        else { actuatedDOF += _actuatedJoints[i]->getDOF(); }
    }
    for(std::size_t i = 0; i < _unactuatedJoints.size(); i++) {
        ...
        else { unactuatedDOF += _unactuatedJoints[i]->getDOF(); }
    }
