...
void Log::setLog(Log::Ptr log) {
    if(rw::core::RobWork::getInstance()->getLogPtr() == log) return;
...
void Log::setWriterForMask(int mask, LogWriter::Ptr writer) {
    ...
    size_t index = 0;
    do {
        if(mask & pattern) _writers[index] = writer;
        pattern = pattern << 1;
        index++;
...
rw::core::LogWriter& Log::get(LogIndex id) {
    if(!isLogEnabled(id)) return *_defaultWriter;
    if(isValidLogIndex(id)) return *_writers[id];
...
void Log::flushAll() {
    for(size_t i = 0; i < _writers.size(); i++) {
        if(_writers[i] != NULL) _writers[i]->flush();
    }
}
...
void Log::increaseTabLevel() {
    _tabLevel++;
    for(size_t i = 0; i < _writers.size(); i++) {
        if(_writers[i] != NULL) _writers[i]->setTabLevel(_tabLevel);
    }
}
...
bool Log::isValidLogIndex(LogIndex id) {
    if(id < 0 || _writers.size() < (size_t) id) return false;
    if(_writers[id] == NULL) return false;
    return true;
}
