...
std::vector<std::string> Plugin::getExtensionPointIDs() {
    ...
    for(std::size_t i = 0; i < desc.size(); i++) ids.push_back(desc[i].id);
    return ids;
}

rw::core::Ptr<Plugin> Plugin::load(const std::string& filename) {
    boost::filesystem::path file(filename);
    if(!exists(file)) { RW_THROW("The file does not exist: " << filename); }
...
rw::core::Ptr<Plugin> Plugin::loadDirect(const std::string& filename) {
    ...
    if(h == NULL) {
        ...
        else if(GetLastError() == 126) { RW_THROW("Error 126: Error loading dependent dll"); }
        else if(boost::filesystem::exists(filename)) {
            RW_THROW("Unknown Error: Library exists but, could not opened. Library: " << filename);
        }
        else { RW_THROW(" Could not open library: " << filename); }
    }
    ...
    if(*tmp == NULL) {
        ...
        const DWORD res =
            FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM |
                          ...
                          0,                  // Min/Max Buffer size
                          NULL);              // Arguments
        if(!res) { RW_WARN("FormatMessage returned error: " << GetLastError() << "!"); }
        else { RW_WARN("Error " << errId << ": " << buffer); }
        return NULL;
    }
...
void Plugin::close(const OSHandle* handle) {
    if(handle != NULL) {
        ...
        if(close != 0) {
            char* err = dlerror();
            if(err != NULL) RW_THROW("Error: Could not close library: " << err);
...
rw::core::Ptr<Plugin> Plugin::loadDirect(const std::string& filename) {
    ...
    if(handle == NULL || err != NULL) RW_THROW("Unknown Error: Could not open library: " << err);
...
class LazyPlugin : public Plugin
{
  public:
    ...
    rw::core::Ptr<Extension> makeExtension(const std::string& id) {
        ...
        if(desc == NULL) RW_THROW("Not a valid id!");
        // now comes the loading part
        if(_srcPlugin == NULL) _srcPlugin = Plugin::load(_libfile);
