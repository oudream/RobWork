...
PropertyBase::Ptr DOMCorePropertyMapLoader::readProperty(DOMElem::Ptr element, bool checkHeader) {
	...
	for( DOMElem::Ptr child: value->getChildren() ){
		if (child->isName(DOMPropertyMapFormat::idPropertyMap())) {
			return ownedPtr(new Property<PropertyMap>(name, description, DOMCorePropertyMapLoader::readProperties(child, true)));
		} else if (child->isName(DOMCoreBasisTypes::idString())) {
			return ownedPtr(new Property<std::string>(name, description, DOMCoreBasisTypes::readString(child)));
		} else if (child->isName(DOMCoreBasisTypes::idStringList())) {
			return ownedPtr(new Property<std::vector<std::string> >(name, description, DOMCoreBasisTypes::readStringList(child)));
		} else if (child->isName(DOMCoreBasisTypes::idIntList())) {
			return ownedPtr(new Property<std::vector<int> >(name, description, DOMCoreBasisTypes::readIntList(child)));
		} else if (child->isName(DOMCoreBasisTypes::idDoubleList())) {
			return ownedPtr(new Property<std::vector<double> >(name, description, DOMCoreBasisTypes::readDoubleList(child)));
		} else if (child->isName(DOMCoreBasisTypes::idDouble())) {
			return ownedPtr(new Property<double>(name, description, DOMCoreBasisTypes::readDouble(child)));
		} else if (child->isName(DOMCoreBasisTypes::idFloat())) {
			return ownedPtr(new Property<float>(name, description, DOMCoreBasisTypes::readFloat(child)));
		} else if (child->isName(DOMCoreBasisTypes::idInteger())) {
			return ownedPtr(new Property<int>(name, description, DOMCoreBasisTypes::readInt(child)));
		} else if (child->isName(DOMCoreBasisTypes::idBoolean())) {
			return ownedPtr(new Property<bool>(name, description, DOMCoreBasisTypes::readBool(child)));
		} else if (child->isName(DOMCoreBasisTypes::idVector3D())){
			RW_THROW("Vector3D is not implemented in DOMCorePropertyMapLoader, Use DOMPropertyMapLoader instead");
			return NULL;
		} else if (child->isName(DOMCoreBasisTypes::idVector2D())) {
			RW_THROW("Vector2D is not implemented in DOMCorePropertyMapLoader, Use DOMPropertyMapLoader instead");
			return NULL;
		} else if (child->isName(DOMCoreBasisTypes::idQ())) {
			RW_THROW("Q is not implemented in DOMCorePropertyMapLoader, Use DOMPropertyMapLoader instead");
			return NULL;
		} else if (child->isName(DOMCoreBasisTypes::idTransform3D())) {
			RW_THROW("TransForm3D is not implemented in DOMCorePropertyMapLoader, Use DOMPropertyMapLoader instead");
			return NULL;
		} else if (child->isName(DOMCoreBasisTypes::idRotation3D())) {
			RW_THROW("Rotation3D is not implemented in DOMCorePropertyMapLoader, Use DOMPropertyMapLoader instead");
			return NULL;
		} else if (child->isName(DOMCoreBasisTypes::idEAA())) {
			RW_THROW("EAA is not implemented in DOMCorePropertyMapLoader, Use DOMPropertyMapLoader instead");
			return NULL;
		} else if (child->isName(DOMCoreBasisTypes::idRPY())) {
			RW_THROW("RPY is not implemented in DOMCorePropertyMapLoader, Use DOMPropertyMapLoader instead");
			return NULL;
		} else if (child->isName(DOMCoreBasisTypes::idQuaternion())) {
			RW_THROW("Quaternion is not implemented in DOMCorePropertyMapLoader, Use DOMPropertyMapLoader instead");
			return NULL;
		} else if (child->isName(DOMCoreBasisTypes::idRotation2D())) {
			RW_THROW("Rotation2D is not implemented in DOMCorePropertyMapLoader, Use DOMPropertyMapLoader instead");
			return NULL;
		} else if (child->isName(DOMCoreBasisTypes::idVelocityScrew6D())) {
			RW_THROW("VelocityScrew6D is not implemented in DOMCorePropertyMapLoader, Use DOMPropertyMapLoader instead");
			return NULL;
		} else if (child->isName("QPath")){
			RW_THROW("QPath is not implemented in DOMCorePropertyMapLoader, Use DOMPropertyMapLoader instead");
			return NULL;
		} else if (child->isName("T3DPath")){
			RW_THROW("T3DPath is not implemented in DOMCorePropertyMapLoader, Use DOMPropertyMapLoader instead");
			return NULL;
		} else {
...
PropertyMap DOMCorePropertyMapLoader::load(std::istream& instream, const std::string& schemaFileName) {
...
PropertyMap DOMCorePropertyMapLoader::load(const std::string& filename, const std::string& schemaFileName) {
    DOMParser::Ptr parser = DOMParser::make();
    // todo: add schema load to interface
