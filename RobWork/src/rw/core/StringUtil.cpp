...
std::string StringUtil::getRelativeDirectoryName(const std::string& path, std::string dir_name) {
    while(!dir_name.empty()) {
        if(dir_name.back() == '/' || dir_name.back() == '\\') dir_name.pop_back();
        else break;
    }
    std::string new_dir              = std::string();
    const std::string::size_type pos = dir_name.find_last_of("/\\");
    if(pos != std::string::npos) new_dir = dir_name.substr(pos + 1, std::string::npos);
    ...
    if(pos_path != std::string::npos) {
        ...
        while(!new_path.empty()) {
            if(new_path.front() == '/' || new_path.front() == '\\') new_path = new_path.substr(1);
            else break;
        }
        return new_path;
    }
...
std::string StringUtil::getFileName(const std::string& pathAndFilename) {
    ...
    if(pos != std::string::npos) return pathAndFilename.substr(pos + 1, std::string::npos);
    else return pathAndFilename;
}
...
std::string StringUtil::getFileExtension(const std::string& path) {
    ...
    if(pos == std::string::npos) return "";
        ...
        if(x == '/' || x == '\\') return "";
        else return path.substr(pos, path.size());
    }
}

std::string StringUtil::toUpper(const std::string& str) {
    std::string result(str);
    for(size_t i = 0; i < str.length(); i++) result[i] = toupper(str[i]);
...
std::string StringUtil::toLower(const std::string& str) {
    std::string result(str);
    for(size_t i = 0; i < str.length(); i++) result[i] = tolower(str[i]);
...
std::pair<bool, std::vector<double>> StringUtil::toDoubles(const std::vector<std::string>& words) {
