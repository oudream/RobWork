...
void IOUtil::readFile(const std::string& file_name, std::vector<char>& result) {
    ...
    // Check input.
    if(!inp.is_open()) RW_THROW("Can't read file " << StringUtil::quote(file_name));
    ...
    if(inp.bad()) RW_THROW("Reading of file " << StringUtil::quote(file_name) << " failed.");
...
std::string IOUtil::resolveFileName(const std::string& raw_filename,
    ...
    if(in.is_open()) { return raw_filename; }
...
std::string IOUtil::getAbsoluteFileName(const std::string& file) {
    if(StringUtil::isAbsoluteFileName(file)) { return file; }
...
#endif
    if(res == NULL) RW_THROW("Absolute filename could not be derived, for file: " << file);
...
#endif
            if(!std::regex_match(filename.c_str(), match, regex)) continue;

            if(addPath) result.push_back(it->path().string());
            else {
#if(BOOST_FILESYSTEM_VERSION == 2)
...
std::string IOUtil::getFirstXMLElement(const std::string& filename) {
    ...
    if(!info.hit) RW_THROW("file \"" << filename << "\" is not a wellformed xml document!");
    return result;
    ...
    if(!res) RW_THROW("file \"" << filename << "\" is not a wellformed xml document! " << result);
    return result;
#endif
}
