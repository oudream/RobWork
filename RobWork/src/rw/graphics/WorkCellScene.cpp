    ...
    bool operator()(SceneNode::Ptr& child, SceneNode::Ptr& parent) {
        if(child->asDrawableNode()) {
            ...
            if(_drawable) {
                ...
                if(_drawable == child) found = true;
            }
            else if(child->getName() == _name) { found = true; }
...
void WorkCellScene::setWorkCell(rw::models::WorkCell::Ptr wc) {
    ...
    if(wc != NULL) {
        ...
        for(StateStringMapData data : fnameToStateMap) {
            ...
            if(frame != NULL) {
                if(data.second.visible.first) setVisible(data.second.visible.second, frame);
                ...
                if(data.second.alpha.first) setTransparency(data.second.alpha.second, frame);
                if(data.second.dtype.first) setDrawMask(data.second.dmask.second, frame);
                if(data.second.dtype.first) setDrawType(data.second.dtype.second, frame);
...
void WorkCellScene::setState(const State& state) {
    ...
    for(DeformObjectMap::value_type data : _deformableObjectsMap) {
        for(Model3D::Ptr model : data.second) { data.first->update(model, state); }
    }
}
        ...
        for(Object::Ptr obj : objs) {
            ...
            if(DeformableObject::Ptr dobj = obj.cast<DeformableObject>()) {
                ...
                for(Geometry::Ptr geom : geoms) {
                    if(geom->getFrame() != frame) continue;
...
void WorkCellScene::setVisible(bool visible, const rw::core::Ptr<Frame> f) {
    if(_frameNodeMap.find(f.get()) == _frameNodeMap.end()) return;
...
bool WorkCellScene::isFrameAxisVisible(const rw::core::Ptr<Frame> f) const {
    if(_frameNodeMap.find(f.get()) == _frameNodeMap.end()) RW_THROW("Frame is not in the scene!");
    const std::map<const Frame*, FrameVisualState>::const_iterator it =
        _frameStateMap.find(f.get());
    if(it != _frameStateMap.end()) return it->second.frameAxisVisible.second;
    else return false;
}
...
bool WorkCellScene::isFrameLabelVisible(const Frame::Ptr f) const {
    ...
    const std::map<const Frame*, FrameVisualState>::const_iterator it =
        _frameStateMap.find(f.get());
    if(it != _frameStateMap.end()) { return it->second.frameLabelVisible.second; }
...
void WorkCellScene::setDrawType(DrawableNode::DrawType type, const rw::core::Ptr<Frame> f) {
    ...
    for(DrawableNode::Ptr& d : _frameDrawableMap[f.get()]) { d->setDrawType(type); }
}
...
DrawableNode::DrawType WorkCellScene::getDrawType(const rw::core::Ptr<Frame> f) const {
    ...
    const std::map<const Frame*, FrameVisualState>::const_iterator it =
        _frameStateMap.find(f.get());
    if(it != _frameStateMap.end()) return it->second.dtype.second;
    else return DrawableNode::SOLID;
}
...
void WorkCellScene::setDrawMask(unsigned int mask, const rw::core::Ptr<Frame> f) {
    ...
    for(DrawableNode::Ptr& d : _frameDrawableMap[f.get()]) { d->setMask(mask); }
}
...
unsigned int WorkCellScene::getDrawMask(const rw::core::Ptr<Frame> f) const {
    ...
    const std::map<const Frame*, FrameVisualState>::const_iterator it =
        _frameStateMap.find(f.get());
    if(it != _frameStateMap.end()) return it->second.dmask.second;
    else return DrawableNode::SOLID;
}
...
void WorkCellScene::setTransparency(double alpha, const rw::core::Ptr<Frame> f) {
    ...
    for(DrawableNode::Ptr& d : _frameDrawableMap[f.get()]) { d->setTransparency((float) alpha); }
}
...
DrawableNode::Ptr WorkCellScene::addDrawable(const std::string& filename,
...
DrawableNode::Ptr WorkCellScene::addFrameAxis(const std::string& name, double size,
                                              rw::core::Ptr<Frame> frame, int dmask) {
...
DrawableNode::Ptr WorkCellScene::addScan(const std::string& name,
                                         const class rw::geometry::PointCloud& scan,
                                         rw::core::Ptr<Frame> frame, int dmask) {
...
std::vector<DrawableNode::Ptr> WorkCellScene::getDrawables(const rw::core::Ptr<Frame> f) const {
    ...
    if(it == _frameDrawableMap.end()) return std::vector<DrawableNode::Ptr>();
    else return it->second;
}

std::vector<DrawableNode::Ptr> WorkCellScene::getDrawablesRec(rw::core::Ptr<Frame> f, State&) {
    if(_frameNodeMap.find(f.get()) == _frameNodeMap.end()) return std::vector<DrawableNode::Ptr>();
...
DrawableNode::Ptr WorkCellScene::findDrawable(const std::string& name,
                                              const rw::core::Ptr<Frame> f) {
    const FrameNodeMap::const_iterator it = _frameNodeMap.find(f.get());
    if(it == _frameNodeMap.end()) return NULL;
    else return _scene->findDrawable(name, it->second);
}
...
bool WorkCellScene::removeDrawables(const rw::core::Ptr<Frame> f) {
    ...
    const FrameNodeMap::const_iterator it = _frameNodeMap.find(f.get());
    if(it == _frameNodeMap.end()) return false;
    else return _scene->removeDrawables(it->second);
}
...
bool WorkCellScene::removeDrawable(const std::string& name) {
    ...
    if(visitor._dnode.isNull()) return true;
...
bool WorkCellScene::removeDrawable(DrawableNode::Ptr drawable, const rw::core::Ptr<Frame> f) {
    ...
    if(itD != _frameDrawableMap.end()) {
        std::vector<DrawableNode::Ptr>& drawables = itD->second;
        for(std::vector<DrawableNode::Ptr>::iterator it = drawables.begin(); it != drawables.end();
            it++) {
            if((*it) == drawable) {
                drawables.erase(it);
                break;
    ...
    const FrameNodeMap::const_iterator it = _frameNodeMap.find(f.get());
    if(it == _frameNodeMap.end()) return false;
    else return _scene->removeDrawable(drawable, it->second);
}
...
bool WorkCellScene::removeDrawable(const std::string& name, const rw::core::Ptr<Frame> f) {
    ...
    const FrameNodeMap::const_iterator it = _frameNodeMap.find(f.get());
    if(it == _frameNodeMap.end()) return false;
    else return _scene->removeChild(name, it->second);
}
...
Frame* WorkCellScene::getFrame(DrawableNode::Ptr d) const {
    ...
    if(it != _nodeFrameMap.end()) return it->second;
    else return NULL;
}
...
GroupNode::Ptr WorkCellScene::getNode(const rw::core::Ptr<Frame> frame) const {
    ...
    if(it != _frameNodeMap.end()) return it->second;
    else return NULL;
}
