...
DynamicLibraryLoaderBase::DynamicLibraryLoaderBase(const std::string& fname) {
    ...
    if(h == NULL) {
        ...
        else { RW_THROW("Unknown Error: Could not open library"); }
    }
    else { _err = NULL; }
}
...
DynamicLibraryLoaderBase::DynamicLibraryLoaderBase(const std::string& filename) {
    ...
    _err    = dlerror();
    if(_handle == NULL || _err != NULL) RW_THROW("Unknown Error: Could not open library: " << _err);
}

DynamicLibraryLoaderBase::~DynamicLibraryLoaderBase() {
    if(_handle != 0) dlclose(_handle);
}
...
void* DynamicLibraryLoaderBase::getObject(const std::string& funcname) {
    if(_err != 0) { return NULL; }

    const char* func;
    if(funcname.size() == 0) func = "factory0";
    else func = funcname.c_str();

    void* (*factory_func)(void) = NULL;
    if(getSymbol((void**) &factory_func, func)) { return factory_func(); }
    else std::cout << "Unable to identify factory function in dynamic library" << std::endl;
    return 0;
}
