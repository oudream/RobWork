...
xercesc::DOMDocument* XMLPropertySaver::createDOMDocument(const PropertyMap& map) {
    ...
    else { RW_THROW("XMLPropertySaver: Unable to find a suitable DOM Implementation"); }
    return doc;
}
...
void XMLPropertySaver::save(const PropertyValueBase& property, xercesc::DOMElement* parent,
                            xercesc::DOMDocument* doc) {
    ...
    switch(property.getType().getId()) {
        ...
        case PropertyType::PropertyValueBasePtrList: {
            ...
            for(const PropertyValueBase::Ptr& p : prop->getValue()) { save(*p, elem, doc); }
            break;
        }
        ...
        case PropertyType::Float: {
            const PropertyValue<float>* prop = dynamic_cast<const PropertyValue<float>*>(&property);
        ...
        case PropertyType::Bool: {
            const PropertyValue<bool>* prop = dynamic_cast<const PropertyValue<bool>*>(&property);
        ...
        case PropertyType::EAA: {
            const PropertyValue<EAA<>>* prop = dynamic_cast<const PropertyValue<EAA<>>*>(&property);
            elem                             = XMLBasisTypes::createEAA(prop->getValue(), doc);
            break;
        }
        case PropertyType::RPY: {
            const PropertyValue<RPY<>>* prop = dynamic_cast<const PropertyValue<RPY<>>*>(&property);
        ...
        case PropertyType::QPath: {
            const PropertyValue<QPath>* prop = dynamic_cast<const PropertyValue<QPath>*>(&property);
    ...
    if(elem != NULL) parent->appendChild(elem);
}
