...
double XMLBasisTypes::getUnit(const XMLCh* key) {
    ...
    if(it == _Units._map.end()) RW_THROW("Invalid Unit Attribute " << tmp.str());
    return (*it).second;
}
...
std::vector<double> parseNArray(const std::string& data) {
    ...
    if(!result.first) RW_THROW("Failed to parse \"" << data << " as a set of doubles");
...
inline T readVectorStructure(DOMElement* element, bool doCheckHeader, const XMLCh* id) {
    ...
    double scale = 1;
    if(element->hasAttributes()) scale = readUnit(element);
...
Transform3D<> XMLBasisTypes::readTransform3D(DOMElement* element, bool doCheckHeader) {
    ...
    for(XMLSize_t i = 0; i < nodeCount; i++) {
        ...
        if(child != NULL) {
            ...
            else { rotation = readRotation3DStructure(child); }
        }
    }
    rotation.normalize();
 ...
 */
rw::kinematics::State XMLBasisTypes::readState(xercesc::DOMElement* element, WorkCell::Ptr workcell,
                                               bool doCheckHeader) {
...
const XMLCh* XMLBasisTypes::readElementTextXMLCh(xercesc::DOMElement* element,
    ...
    for(size_t i = 0; i < children->getLength(); i++) {
        DOMNode* child = children->item(i);
        // std::cout<<"readElementText Child = "<<XMLStr(child->getNodeName()).str()<<std::endl;
        if(dynamic_cast<DOMText*>(children->item(0)) != NULL) return child->getNodeValue();
        // return XMLStr(child->getNodeValue()).str();
    }
    if(exceptionOnEmpty)
        RW_THROW("Unable to find value in Node " + XMLStr(element->getNodeName()).str());
...
xercesc::DOMElement* XMLBasisTypes::createString(const std::string& str,
                                                 xercesc::DOMDocument* doc) {
...
xercesc::DOMElement* XMLBasisTypes::createStringList(const std::vector<std::string>& strings,
    ...
    for(const std::string& str : strings) { element->appendChild(createString(str, doc)); }
    return element;
}
