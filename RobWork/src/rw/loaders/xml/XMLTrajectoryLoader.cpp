...
XMLTrajectoryLoader::XMLTrajectoryLoader(std::istream& instream,
                                         const std::string& schemaFileName) {
    XercesDOMParser parser;
    xercesc::DOMDocument* doc =
...
template<class T> class LinearInterpolatorParser
{
  public:
    static LinearInterpolator<T>* read(xercesc::DOMElement* element) {
        ...
        if(index != 2) RW_THROW("Expected 3 via points for CircularInterpolator. Found: " << index);
...
LloydHaywardBlendParser<Rotation3D<>>::read(xercesc::DOMElement*, Interpolator<Rotation3D<>>* int1,
