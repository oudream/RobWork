...
struct AppendToOutputFromFile
{
    ...
    void append(std::string const& filename) const {
        ...
        if(StringUtil::isAbsoluteFileName(filename)) { absfilename = filename; }
        else {
...
bool XMLRWPreParser::parse(const std::string& absfilename, std::vector<char>& output,
    ...
    if(!in) { RW_THROW("Could not open input file: " << absfilename); }
    ...
    std::getline(in, line);
    std::string t("<?");
    if(line.compare(0, t.length(), t) != 0) in.seekg(oldpos);    // get back to the position

    std::vector<char> vec;
    std::copy(
        std::istream_iterator<char>(in), std::istream_iterator<char>(), std::back_inserter(vec));
...
bool XMLRWPreParser::parse(const std::string& absfilename, std::vector<char>& output,
                           std::vector<std::pair<size_t, file_position>>&
                               filemap) { /*
                                             if( !StringUtil::IsAbsoluteFileName( absfilename ) ){
                                                 RW_THROW( "Error the absfilename is not an absolute
