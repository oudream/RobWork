...
void addPropertyToMap(const DummyProperty& dprop, core::PropertyMap& map) {
    if(dprop._type == "string") { map.add(dprop._name, dprop._desc, dprop._val); }
...
std::string createScopedName(std::string str, std::vector<std::string> scope) {
    std::string tmpstr;
    for(size_t i = 0; i < scope.size(); i++) { tmpstr += scope[i] + "."; }
    tmpstr += str;
    return tmpstr;
}
...
Frame* addModelToFrame(DummyModel& model, rw::core::Ptr<Frame> parent, StateStructure* tree,
    ...
    for(size_t i = 0; i < model._geo.size(); i++) {
        ...
        if(model._colmodel && model._isDrawable) {
            ...
            Model3D::Ptr model3d = Model3DFactory::getModel(
                val.str(),
                model._name,
                !model._customMaterial,
                Model3D::Material(
                    "stlmat", float(model._r), float(model._g), float(model._b), float(model._a)));
            ...
            // For Polytype CAD models, we want to save the filepath for potential serialization
            // later.
            if(model._geo[i]._type == PolyType) { geom->setFilePath(filePath.str()); }

            geom->setScale(model._geo[i]._scale);
            if(object != NULL) { object->addGeometry(geom); }
        }
            ...
            // For Polytype CAD models, we want to save the filepath for potential serialization
            // later.
            if(model._geo[i]._type == PolyType) { model3d->setFilePath(filePath.str()); }
            else { model3d->setFilePath(val.str()); }

            model3d->scale(model._geo[i]._scale);
            if(object != NULL) { object->addModel(model3d); }
        }
        //} catch (const std::exception& e){
        //}
    }
...
void addLimits(std::vector<DummyLimit>& limits, Joint::Ptr j) {
    ...
    if(j.cast<RevoluteJoint>() != NULL) { convFactor[0] = Deg2Rad; }
    ...
    if(posLimits.size() > 0) j->setBounds(std::pair<Q, Q>(minPos, maxPos));
    if(velLimits.size() > 0) j->setMaxVelocity(maxVel);
    if(accLimits.size() > 0) j->setMaxAcceleration(maxAcc);
    return;
}
...
Frame* createFrame(DummyFrame& dframe, DummySetup& setup) {
    ...
    if(dframe._isDepend) {
        ...
        if(res == setup.frameMap.end()) {
            ...
            if(res == setup.frameMap.end()) {
                RW_WARN("The frame: " << dframe.getName()
                                      << " Depends on an unknown frame: " << dframe.getDependsOn());
                return NULL;
            }
        }
        ...
        if(dframe._type == "Revolute") {
            ...
            if(dframe._state != ActiveState || !owner->isActive()) joint->setActive(false);
            frame = joint;
        }
        ...
        if(dframe._state != ActiveState) j->setActive(false);
    ...
    else { RW_THROW("FRAME is of illegal type!! " << dframe._type); }
...
void addDevicePropsToFrame(DummyDevice& dev, const std::string& name, DummySetup& setup) {
    ...
    for(size_t j = 0; j < proplist.size(); j++) {
        frame->getPropertyMap().add(
            proplist[j]->getIdentifier(), proplist[j]->getDescription(), proplist[j]->getValue());
    }
...
Device::Ptr createDevice(DummyDevice& dev, DummySetup& setup) {
    ...
    if(dev._type == SerialType) {
        ...
        for(DummyFrame& dframe : dev._frames) { chain.push_back(createFrame(dframe, setup)); }
        ...
        for(size_t i = 1; i < dev._frames.size(); i++) {
            ...
            if(dev._frames[i].getRefFrame() == dev._frames[i - 1].getName()) {
                ...
                if(!chainName.empty()) {
                    for(std::size_t j = i + 1; j < dev._frames.size() && !newLeg; j++) {
                        if(dev._frames[j].getRefFrame() != dev._frames[j - 1].getName()) break;
        ...
        for(size_t i = 0; i < chains.size(); i++) {
            if(dev._junctions.size() == 0) legs.push_back(new ParallelLeg(chains[i]));
            for(std::size_t j = 0; j < chains[i].size(); j++) {
                if(Joint* const joint = dynamic_cast<Joint*>(chains[i][j])) joints.push_back(joint);
            }
        }
            ...
            for(std::size_t j = 0; j < dev._junctions[i].chains.size(); j++) {
                ...
                for(std::size_t k = 0; k < split.size(); k++) {
                    ...
                    if(totalChain.size() > 0) {
                        if(totalChain.back() == *firstFrameIt) { firstFrameIt++; }
                    }
                    totalChain.insert(
        ...
        for(size_t i = 1; i < dev._frames.size(); i++) {
            ...
            if(res == frameMap.end()) {
                RW_THROW("Error: the frame \""
                         << frame.getName() << "\" references to a non existing or illegal frame!!"
                         << dev._frames[i].getRefFrame());
            }
            child                      = createFrame(frame, setup);
            frameMap[child->getName()] = child.get();

            if(setup.toChildMap[parentname].empty()) { endEffectors.push_back(parent.get()); }

            if(dev._frames[i]._type == "EndEffector") { endEffectors.push_back(child.get()); }
            parent     = child;
            parentname = frame.getRefFrame();
        }
        if(endEffectors.size() == 0) endEffectors.push_back(child.get());
        ...
        for(DummyFrame& dframe : dev._frames) {
            ...
            if(res == setup.frameMap.end()) { RW_THROW("Error: parent " << pname << " not found"); }
    ...
    else if(dev._type == CompositeType) { RW_THROW("CompositeDevice not supported yet"); }
    else { RW_THROW("Error: Unknown device type!!"); }
    ...
    for(; colsetup != dev._colsetups.end(); ++colsetup) { setup.colsetups.push_back(*colsetup); }
    ...
    for(QConfig& config : dev._qconfig) {
        if(config.name == "Home") {
            ...
            for(size_t i = 0; i < q.size(); i++) q[i] = config.q[i];
...
CollisionSetup defaultCollisionSetup(const WorkCell& workcell) {
    ...
    for(rit = frameList.rbegin(); rit != frameList.rend(); rit++) {
        if((*rit)->isDAF() || rit->cast<MovableFrame>()) { continue; }
        for(it = frameList.begin(); (*it) != (*rit); it++) {
            ...
            if((*it)->isDAF() || it->cast<MovableFrame>()) { continue; }
...
rw::models::WorkCell::Ptr XMLRWLoader::loadWorkCell(const std::string& fname) {
    try {
        ...
        for(DummyFrame& df : setup.dwc->_framelist) { strToFrame[df.getName()] = &df; }
        for(DummyDevice& dd : setup.dwc->_devlist) {
            for(DummyFrame& df : dd._frames) { strToFrame[df.getName()] = &df; }
        }
        ...
        // add devices to workcell
        {
            ...
            for(; first != setup.devMap.end(); ++first) { wc->addDevice((*first).second); }
        }

        // add all objects to scene
        {
            std::map<Frame*, RigidObject::Ptr>::iterator first = setup.objectMap.begin();
            for(; first != setup.objectMap.end(); ++first) { wc->add((*first).second); }
        }
