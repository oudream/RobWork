...
Frame* addModelToFrame(DummyModel& model, Frame *parent, StateStructure *tree, DummySetup &setup) {
	...
	for (size_t i = 0; i < model._geo.size(); i++) {
		...
		if (model._colmodel && model._isDrawable) {
			...
			Model3D::Ptr model3d = Model3DFactory::getModel(val.str(), model._name, !model._customMaterial, Model3D::Material("stlmat", float(model._r), float(model._g), float(model._b), float(model._a)));
			...
			Model3D::Ptr model3d = Model3DFactory::getModel(val.str(), val.str(), !model._customMaterial, Model3D::Material("stlmat", float(model._r), float(model._g), float(model._b), float(model._a)));
