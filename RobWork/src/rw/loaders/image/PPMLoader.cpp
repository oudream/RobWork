...
class Reader
{
  public:
    Reader(const std::string& filename) : _filename(filename) {
        ...
        if(!(in().is_open())) { RW_THROW("Can't open file " << StringUtil::quote(_filename)); }
        okOrThrow();
    }
    ...
    template<class X> X getNextAsciiString() {
        ...
        while(std::isspace(tmp, std::locale()) != 0) { tmp = in().get(); }
        ...
        if(uLongPair.first) { output = (X) uLongPair.second; }
        else { RW_THROW("Can't convert ascii color value \"" << input << "\" to ungisned long"); }
...
void getHeader(Reader& streamIn, fileType& type, unsigned int& width, unsigned int& heigth,
    ...
    while(step < 4) {
        ...
        // Space
        if(std::isspace(tmp, std::locale())) { continue; }
        ...
        switch(step) {
            case 0:
                // Magic number
                if(input.compare("P6") == 0) { type = binary; }
                else if(input.compare("P3") == 0) { type = ascii; }
                else {
                    type = none;
            ...
            case 1:
                ...
                if(uIntPair.first) { width = uIntPair.second; }
                else {
                    width = 0;
                    RW_THROW("Can't open file " << StringUtil::quote(streamIn.fileName())
            ...
            case 2:
                // Heigth
                uIntPair = rw::core::StringUtil::toUInt(input);
                if(uIntPair.first) { heigth = uIntPair.second; }
                else {
                    heigth = 0;
                    RW_THROW("Can't open file " << StringUtil::quote(streamIn.fileName())
            ...
            case 3:
                ...
                if(uLongPair.first) { pixelMax = uLongPair.second; }
                else {
                    pixelMax = 0;
                    RW_THROW("Can't open file " << StringUtil::quote(streamIn.fileName())
...
rw::sensor::Image::Ptr PPMLoader::load(const std::string& filename) {
    ...
    if(pixelMax < 256) { depth = Image::Depth8U; }
    else if(pixelMax < 65536) { depth = Image::Depth16U; }
    else if(pixelMax <= 4294967295) { depth = Image::Depth32S; }
    else {
    ...
    if(type == binary) { getBinaryImageData(readerObj, img); }
    else if(type == ascii) {
        if(depth == Image::Depth8U) getAsciiImageData<boost::uint8_t>(readerObj, img);
        else if(depth == Image::Depth16U) getAsciiImageData<boost::uint16_t>(readerObj, img);
        else if(depth == Image::Depth32S) getAsciiImageData<boost::int32_t>(readerObj, img);
    }
    else {
        RW_THROW("Can't open file " << StringUtil::quote(readerObj.fileName())
                                    << " - Unknown type");
