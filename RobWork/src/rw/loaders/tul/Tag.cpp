    ...
    template< typename ScannerT > struct definition
    {
      ...
      public:
        definition (TULGrammar const& /*self*/)
        {
            ...
            header_line = (str_p ("NameOfData") >> quoted_string) |
                          (str_p ("DateOfData") >> quoted_string) | (str_p ("Scale") >> number) |
                          (str_p ("TypeOfData") >> quoted_string) | (str_p ("NrOfFrames") >> int_p);
            ...
            // An attribute.
            attribute = attribute_key >> *attribute_value >> discard_node_d[required_newlines];

            // The name of the attribute.
            attribute_key = lexeme_d[
                // Group into a single node.
                token_node_d[(alpha_p | ch_p ('.') | ch_p ('_') | ch_p ('#')) >>
                             *(alnum_p | ch_p ('.') | ch_p ('_'))]];

            // One of the values for an attribute. So far we support only
            // numbers, vectors and strings. I can't think of other types
            // right now.
            attribute_value = configuration | quoted_string | number;

            // A vector of values (x1, x2, ...). At least one xi is
            // required, but we should support vectors of length zero at
            // some point also.
            configuration = root_node_d[ch_p ('(')] >> (number % discard_node_d[ch_p (',')]) >>
                            discard_node_d[ch_p (')')];

            // A string in quotes. Beware the use of lexeme_d.
            quoted_string = lexeme_d[
                // Group into a single node.
                token_node_d[ch_p ('"') >> *(anychar_p - '"') >> ch_p ('"')]];
...
void parseFile (const std::string& filename, tree_parse_info< iterator_t >& info)
{
    TULGrammar parser;
    ...
    if (!info.full)
        RW_THROW ("Parse of file " << StringUtil::quote (filename)
                                   << " failed: " << std::string (first, info.stop));
}
...
std::string getString (const iter_t& node)
{
    ...
    RW_ASSERT (s.size () >= 2 && s[0] == '"' && s[s.size () - 1] == '"');
...
template< class T >
boost::shared_ptr< PropertyBase > makeProperty (const std::string& key,
                                                const std::string& description, const T& value)
{
    return boost::shared_ptr< PropertyBase > (new Property< T > (key, description, value));
}

boost::shared_ptr< PropertyBase > getProperty (const std::string& key, const iter_t& node)
{
...
Tag getTag (const std::string& file, const iter_t& node)
{
    ...
    iter_t node_attributes = node->children.begin ();
    ++node_attributes;
    ...
    // A single attribute.
    else if (node_attributes->value.id () == TULGrammar::attributeID ||
    ...
    // Multiple attributes.
        ...
        for (iter_t p = node_attributes->children.begin (); p != node_attributes->children.end ();
             ++p) {
...
void getTagOrTagFile (const std::string& file, const iter_t& node, std::vector< Tag >& result)
{
    ...
    // If tag:
        ...
        result.push_back (getTag (file, node));
    }
...
// list of tags.
void getTagList (const std::string& file, const iter_t& node, std::vector< Tag >& result)
{
    // If just a single tag or single file command:
    if (node->value.id () == TULGrammar::tagID || node->value.id () == TULGrammar::fileID) {
...
namespace {
template< typename T > const T* getPropPtr (const PropertyBase& base)
{
...
template< typename T > bool ifHasThenEmit (std::ostream& out, const PropertyBase& prop)
{
...
void emitTagProperty (std::ostream& out, const PropertyBase& prop)
{
    ...
    const bool ok = ifHasThenEmit< std::string > (out, prop) ||
                    ifHasThenEmit< double > (out, prop) || ifHasThenEmit< Q > (out, prop);
...
void emitPropertyList (std::ostream& out, const Tag::PropertyList& vals)
{
...
bool NS::hasAttribute (const Tag& tag, const std::string& key)
{
    return tag.getPropertyMap ().find (key) != tag.getPropertyMap ().end ();
}
...
int NS::getAttributeSize (const Tag& tag, const std::string& key)
{
    ...
    if (p == tag.getPropertyMap ().end ()) {
        RW_THROW ("No property named " << StringUtil::quote (key) << " in tag "
