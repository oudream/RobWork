...
namespace rw { namespace kinematics {
     ...
     * present.
     */
    template< typename T > class FrameProperty
    {
      public:
        /**
     ...
     */
    template< typename T > class FramePropertyImpl : public FrameProperty< T >
    {
      public:
        /**
         ...
         * @return \b true iff the property exists in \b frame.
         */
        bool has (const Frame& frame) const { return this->getPtr (frame) != NULL; }
        ...
        T& get (Frame& frame) const
        {
            ...
            if (!value) {
                ...
                // the property also.
                RW_THROW ("No key (of some type T) named "
                          << rw::core::StringUtil::quote (key ()) << " in frame "
         ...
         * @param value [in] The value to assign to the property.
         */
        void set (Frame& frame, const T& value) const { frame.getPropertyMap ().set (_key, value); }

        /**
           @brief Erase the property of the frame.
         */
        void erase (Frame& frame) const { frame.getPropertyMap ().erase (_key); }
...
string msgHeader (const Tag& tag, const string& attribute)
{
    stringstream buf;
    buf << "Attribute " << quote (attribute) << " (frame: " << quote (tag.getName ())
        << ", file: " << quote (tag.getFile ()) << "): ";
    return buf.str ();
}
...
{
    stringstream buf;
    buf << "(frame: " << quote (tag.getName ()) << ", file: " << quote (tag.getFile ()) << "): ";
    return buf.str ();
}

const FrameProperty< Tag >& tagAccessor ()
{
    static FramePropertyImpl< Tag > accessor ("#TAG", "the tag of the frame");
    return accessor;
}
...
const FrameProperty< Transform3D<> >& movableFrameTransformAccessor ()
{
    static FramePropertyImpl< Transform3D<> > accessor (
        "#MOVABLE_FRAME_TRANSFORM", "the initial relative transform of the movable frame");
    return accessor;
}
...
template< typename T > class TagProperty
{
  public:
    TagProperty (const string& key) : _key (key) {}

    const string& key () const { return _key; }

    bool has (const Tag& tag) const { return hasAttribute (tag, _key); }

    bool has (const Frame& frame) const { return has (getTag (frame)); }

    const T& get (const Tag& tag, int pos) const { return getAttribute< T > (tag, _key, pos); }

    const T& get (const Frame& frame, int pos) const { return get (getTag (frame), pos); }

    // A templated version of get(). I wish I had a better name.
    template< class R > const R& getT (const Tag& tag, int pos) const
    {
        return getAttribute< R > (tag, _key, pos);
    }

    template< class R > const R& getT (const Frame& frame, int pos) const
    {
        return getT< R > (getTag (frame), pos);
    }

    int size (const Tag& tag) const { return getAttributeSize (tag, _key); }
    int size (const Frame& frame) const { return size (getTag (frame)); }

  private:
    string _key;
};

template< class T > vector< T > getAsVector (const TagProperty< T >& prop, const Tag& tag)
{
    vector< T > result;
...
template< class T > vector< T > getAsVector (const TagProperty< T >& prop, const Frame& frame)
{
...
Vector3D<> getVectorOrThrow (const TagProperty< Q >& getter, const Tag& tag)
{
    const Q& q = getter.get (tag, 0);
    if (q.size () != 3)
        RW_THROW (msgHeader (tag, getter.key ())
                  << "Vector of length " << (int) q.size () << " should have been a 3D vector.");
...
Rotation3D<> getRotation (const Tag& tag)
{
    if (tagPropRPY ().has (tag)) {
        const Vector3D<> angles = Deg2Rad * getVectorOrThrow (tagPropRPY (), tag);

        const RPY<> rpy (angles[0], angles[1], angles[2]);
        return rpy.toRotation3D ();
    }
    else if (tagPropI ().has (tag) || tagPropJ ().has (tag) || tagPropK ().has (tag)) {
...
CraigDH getCraigDH (const Tag& tag)
{
    ...
    if (vals.size () != 4) {
        RW_THROW (msgHeader (tag) << "Exactly 4 Craig DH parameters expected. "
                                  << "The number of parameters is " << (int) vals.size ());
    }
...
Transform3D<> getTransform (const Tag& tag)
{
    if (tagPropCraigDH ().has (tag) &&
        (tagPropPosition ().has (tag) || tagPropI ().has (tag) || tagPropJ ().has (tag) ||
         tagPropK ().has (tag) || tagPropRPY ().has (tag))) {
        RW_THROW (msgHeader (tag) << "Attribute 'CraigDH' is in conflict "
        ...
        return Transform3D<> (getPosition (tag), getRotation (tag));
    }
}
 ...
 * directory \a dir.
 */
string getFileNameOfDirectory (const string& dir, const string& file)
{
 ...
 */
string getFileNameOfTag (const Tag& tag, const string& file)
{
    return getFileNameOfDirectory (StringUtil::getDirectoryName (tag.getFile ()), file);
}

/**
 * @brief The file name \a file in the context of \a frame.
 */
string getFileNameOfFrame (const Frame& frame, const string& file)
{
...
class Prefix
{
  ...
  public:
    Prefix () : _frameMap (new TULFrameMap ()) {}
    ...
    Frame* resolve (const string& name) const
    {
        ...
        for (int cnt = 0; cnt < maxCnt; ++cnt, --end) {
            const string frameName = getFrameName (begin, end, name);
    ...
    {
        _frameMap->insert (std::make_pair (frame->getName (), frame));
    }
    ...
    string getPrefixDescription () const { return getFrameName (""); }

  private:
    /**
       A string of the form x0.x1.x2...xN.name.
    */
    static string getFrameName (VI from, VI to, const string& name)
    {
        stringstream buf;
 ...
 */
CollisionSetup makeCollisionSetup (const WorkCell& workcell)
{
    ...
    for (I p = frames.begin (); p != frames.end (); ++p) {
        ...
        if (tagPropCollisionSetup ().has (frame)) {
            const string& setupFile = tagPropCollisionSetup ().get (frame, 0);
            ...
            result = CollisionSetup::merge (result, CollisionSetupLoader::load (prefix, file));
        }
    }
...
std::string getAccessorID (const Frame& frame, const TagProperty< string >& accessor, int pos)
{
    const string& geo = accessor.get (frame, pos);

    if (geo.empty ())
        RW_THROW ("Empty model identifier.");
...
std::vector< std::string > getAccessorIDs (const Frame& frame,
...
std::vector< std::string > getGeoIDAndAccessorIDs (const Frame& frame,
                                                   const TagProperty< string >& accessor)
{
    std::vector< std::string > result = getAccessorIDs (frame, accessor);

    std::vector< std::string > other = getAccessorIDs (frame, tagPropGeoID ());
...
void addDrawableModelInfo (Frame& frame)
{
    ...
    const std::vector< std::string > ids = getGeoIDAndAccessorIDs (frame, tagPropDrawableID ());
...
void addDHSetProperty (Frame& frame)
{
    if (tagPropCraigDH ().has (frame)) {
        ...
        DHParameterSet::set (DHParameterSet (dh.alpha, dh.a, dh.d, dh.theta), &frame);
    }
}
...
struct DeviceStruct
{
    DeviceStruct (const string& name, Frame* first, Frame* last, const vector< Joint* >& joints,
                  const Q& q_home) :
        name (name),
        first (first), last (last), joints (joints), q_home (q_home)
    {}
...
{
    CompositeDeviceStruct (const string& name, const vector< string >& device_names,
                           const Q& q_home) :
        name (name),
        device_names (device_names), q_home (q_home)
    {
        if (device_names.empty ())
            RW_THROW ("Can't construct composite device " << quote (name)
...
struct WorkCellStruct
{
    ...
    WorkCellStruct () : tree (new StateStructure ()), world_frame (0), last_frame (0) {}
};
...
Joint* makeJoint (const Tag& tag, const string& frame_name, const Transform3D<>& transform)
{
    if (tagPropRevolute ().has (tag))
        ...
        RW_THROW (msgHeader (tag) << "The frame is neither revolute nor prismatic.");
        return 0;
    }
}
...
*/
Frame* makeTagFrameHelper (const Tag& tag, const string& frame_name, const Transform3D<>& transform,
                           vector< Joint* >& activeJoints, const Prefix& prefix)
{
    // Frame* parent;
    ...
    if (tagPropMovable ().has (tag) || tagPropMoveable ().has (tag)) {
    ...
    if (isPassiveRevolute || isPassivePrismatic) {
        const TagProperty< string >& prop =
            isPassiveRevolute ? tagPropPassiveRevolute () : tagPropPassivePrismatic ();
        ...
        if (!ownerFrame)
            RW_THROW (msgHeader (tag, prop.key ())
                      << "No frame for context " << quote (prefix.getPrefixDescription ())
                      << " of name " << quote (ownerName));

        // The owner should be a joint.
        Joint* owner = dynamic_cast< Joint* > (ownerFrame);
        if (!owner) {
            RW_THROW (msgHeader (tag, prop.key ())
                      << "The controlling frame " << quote (ownerName) << " must be a Joint.");
...
*/
Frame* makeTagFrame (Frame* parent, const Tag& tag, vector< Joint* >& activeJoints,
                     const Prefix& prefix)
{
    ...
    Frame* tag_frame = makeTagFrameHelper (tag, frame_name, frame_transform, activeJoints, prefix);
...
*/
void makeWorldFrame (WorkCellStruct& workcell, const Prefix& prefix, vector< Tag >& tags)
{
...
CompositeDeviceStruct makeCompositeDeviceStruct (const Frame& frame, const Prefix& prefix)
{
    const vector< string > raw_names = getAsVector (tagPropCompositeDevice (), frame);
    ...
    return CompositeDeviceStruct (frame.getName (), names, getOptionalDeviceHomePos (frame));
}

void loadWorkCellStruct (const string& filename, Frame* root, const Prefix& prefix,
                         WorkCellStruct& workcell, const Q& q_home)
{
    ...
    for (I p = tags.begin (); p != tags.end (); ++p) {
        ...
        if (referenceFrame == "") {
            ...
            if (!parent)
                RW_THROW (msgHeader (tag, tagPropReferenceFrame ().key ())
                          << "No frame for context " << quote (prefix.getPrefixDescription ())
                          << " of name " << quote (referenceFrame));
        }

        // A tag.
        Frame* tag_frame = makeTagFrame (parent, tag, activeJoints, prefix);
        ...
        if (tagPropDevice ().has (tag)) {
            ...
            const string& deviceFile = getFileNameOfTag (tag, tagPropDevice ().get (tag, 0));

            // The device is loaded with a parent of tag_frame.
            loadWorkCellStruct (deviceFile,
                                tag_frame,
                                // Enter a new scope.
        ...
        // Or load a composite device instead.
            ...
            workcell.composite_devices.push_back (makeCompositeDeviceStruct (*tag_frame, prefix));
        }
    }

    if (!activeJoints.empty ()) {
        workcell.devices.push_back (DeviceStruct (
...
State homeState (Frame* world, StateStructure* tree)
{
    ...
    for (I p = frames.begin (); p != frames.end (); ++p) {
        ...
        if (tagPropJointHomePos ().has (frame)) {
            ...
            const double val = tagPropJointHomePos ().get (frame, 0);

            const int dof = frame.getDOF ();
            if (dof != 1) {
                RW_THROW (msgHeader (getTag (frame), tagPropJointHomePos ().key ())
                          << "DOF is " << dof << ". Only a DOF of one is supported.");
            }
            // Assign the value.
...
State movableFrameState (Frame* world, const State& initial_state)
{
    ...
    for (I p = frames.begin (); p != frames.end (); ++p) {
        ...
        if (movable) {
            const Transform3D<> transform = movableFrameTransformAccessor ().get (*movable);
...
State initialState (Frame* world, StateStructure* tree)
{
    ...
    // state to be correct.
    return movableFrameState (world, homeState (world, tree));
}
...
vector< Device::Ptr > makeDevices (const vector< DeviceStruct >& devices, const State& state)
{
    vector< Device::Ptr > result;
    typedef vector< DeviceStruct >::const_iterator I;
    for (I p = devices.begin (); p != devices.end (); ++p) {
        result.push_back (ownedPtr (new SerialDevice (p->first, p->last, p->name, state)));
    }
    return result;
}

Device::Ptr findDevice (const std::string& name, const vector< Device::Ptr >& devices)
{
...
Device::Ptr makeCompositeDevice (const Prefix& prefix,
                                 const vector< Device::Ptr >& standard_devices,
                                 const vector< Device::Ptr >& composite_devices,
                                 const CompositeDeviceStruct& composite, const State& state)
{
    vector< Device::Ptr > devices;

    typedef vector< string >::const_iterator I;
    for (I p = composite.device_names.begin (); p != composite.device_names.end (); ++p) {
        Device::Ptr device = findDevice (*p, standard_devices);
        if (!device)
            device = findDevice (*p, composite_devices);
        if (!device) {
            RW_THROW ("No device named " << quote (*p) << " for composite device "
...
vector< Device::Ptr >
makeCompositeDevices (const Prefix& prefix, const vector< Device::Ptr >& standard_devices,
                      const vector< CompositeDeviceStruct >& composite_devices, const State& state)
{
    vector< Device::Ptr > result;

    typedef vector< CompositeDeviceStruct >::const_iterator I;
    for (I p = composite_devices.begin (); p != composite_devices.end (); ++p)
        result.push_back (makeCompositeDevice (prefix,
                                               standard_devices,
...
void addDevices (const vector< Device::Ptr >& devices, WorkCell& workcell)
{
...
void setOptionalDeviceHomePos (const Device& device, const Q& q_home, State& state)
{
    if (q_home.size () == 0) {
    }
    else if (q_home.size () != device.getDOF ()) {
        RW_THROW ("'DeviceHomePos' configuration with "
                  << q_home.size () << " DOF given for device " << quote (device.getName ())
                  << " with " << device.getDOF () << " DOF.");
    }
    else {
...
WorkCell::Ptr TULLoader::load (const string& filename)
{
    ...
    loadWorkCellStruct (filename,    // The resolved file to load.
    ...
    vector< Device::Ptr > composite_devices =
        makeCompositeDevices (prefix, devices, workcell.composite_devices, state);

    // Assign q_home values for the devices.
    for (int i = 0; i < (int) workcell.devices.size (); i++) {
        setOptionalDeviceHomePos (*devices[i], workcell.devices[i].q_home, state);
    }
    ...
    for (int i = 0; i < (int) workcell.composite_devices.size (); i++) {
        setOptionalDeviceHomePos (
            *composite_devices[i], workcell.composite_devices[i].q_home, state);
    }
