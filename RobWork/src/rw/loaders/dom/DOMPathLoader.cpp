...
DOMPathLoader::DOMPathLoader(const std::string& filename, rw::models::WorkCell::Ptr workcell,
    ...
    for(DOMElem::Iterator it = itpair.first; it != itpair.second; ++it) { readPath(*it); }
...
template<> TimedQ ElementReader<TimedQ>::readElement(DOMElem::Ptr element) {
    ...
    for(DOMElem::Ptr child : element->getChildren()) {
        if(element->isName(DOMPathLoader::idTime())) { time = element->getValueAsDouble(); }
        else if(element->isName(DOMBasisTypes::idQ())) { q = DOMBasisTypes::readQ(child, false); }
    }
