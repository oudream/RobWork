...
DOMTrajectoryLoader::DOMTrajectoryLoader(std::istream& instream,
                                         const std::string& schemaFileName) {
    DOMParser::Ptr parser = DOMParser::make();
    parser->setSchema(schemaFileName);
  ...
  public:
    static LinearInterpolator<T>* read(DOMElem::Ptr element) {
        ...
        for(DOMElem::Ptr child : element->getChildren()) {
            if(index > 1) RW_THROW("Expected 2 via points for LinearInterpolator. Found: >2");
            vias[index] = ElementReader<T>::readElement(child);
            index++;
        }
        if(index < 1) RW_THROW("Expected 3 via points for CircularInterpolator. Found:" << index);
...
CircularInterpolatorParser<Vector3D<>>::read(DOMElem::Ptr element) {
    ...
    int index = 0;
    for(DOMElem::Ptr child : element->getChildren()) {
        if(index > 3) RW_THROW("Expected 3 via points for CircularInterpolator. Found: >3");
        vias[index] = ElementReader<Vector3D<>>::readElement(child);
        index++;
    }
    if(index < 3) RW_THROW("Expected 3 via points for CircularInterpolator. Found:" << index);
...
template<class T>
LloydHaywardBlend<T>* LloydHaywardBlendParser<T>::read(DOMElem::Ptr element, Interpolator<T>* int1,
                                                       Interpolator<T>* int2) {
    double tau   = element->getAttributeValueAsDouble(DOMTrajectoryLoader::idTauAttribute());
...
template<class T> Ptr<InterpolatorTrajectory<T>> read(DOMElem::Ptr element, Identifiers* ids) {
    ...
    for(DOMElem::Ptr child : element->getChildren()) {
        if(ids->isInterpolator(child->getName())) {
            if(blend == NULL) { result->add(ownedPtr(interpolators[interpolatorIndex])); }
            else {
