...
class TriMeshList : public TriMesh
{
  public:
    ...
    virtual rw::geometry::Triangle<double> getTriangle(size_t idx) const {
        size_t i = 0;
        while(idx >= _list[i]->size()) { idx -= _list[i++]->size(); }
...
bool isFrameInDevice(rw::core::Ptr<Frame> frame, Device* dev) {
    if(isFrameInDevice(frame)) return frameToDevice[frame.get()] == dev;
    return false;
}
        ...
        if(object->getGeometry().empty() && !object->getModels().empty()) {
                ...
                if(!mod->getFilePath().empty() &&
                   mod->getFilePath().compare(0, t.length(), t) != 0) {
                    ...
                    if(type == "#Plane") {
                        ...
                        for(auto& o : mod->getObjects()) { list.push_back(o.cast<TriMesh>()); }
                        rw::loaders::STLFile::save(TriMeshList(list),
                                                   dir + "/model_" + mod->getName() + ".stl");

                        DOMElem::Ptr polytope_element = draw_element->addChild("Polytope");
                        polytope_element->addAttribute("file")->setValue("./model_" +
                                                                         mod->getName() + ".stl");
                    }
                }
            }
            ...
            for(auto geom : object->getGeometry()) {
                ...
                if(!geom->getFilePath().empty()) {
                    ...
                    else if(rw::geometry::Box* const box =
                                dynamic_cast<rw::geometry::Box*>(geom->getGeometryData().get())) {
                    ...
                    else if(rw::geometry::Cone* cone =
                                dynamic_cast<rw::geometry::Cone*>(geom->getGeometryData().get())) {
                    ...
                    else if(rw::geometry::Tube* tube =
                                dynamic_cast<rw::geometry::Tube*>(geom->getGeometryData().get())) {
...
template<>
DOMElem::Ptr
ElementCreator::createElement<RevoluteJoint*>(RevoluteJoint* frame,
...
template<>
DOMElem::Ptr
ElementCreator::createElement<MovableFrame*>(MovableFrame* frame,
                                             rw::core::Ptr<const rw::models::WorkCell> workcell,
                                             const State state, DOMElem::Ptr parent) {
...
ElementCreator::createElement<FixedFrame*>(FixedFrame* frame,
    ...
    if(fname.length() > dname.length()) {
        ...
        else { element->addAttribute("name")->setValue(fname); }
    }
    else { element->addAttribute("name")->setValue(fname); }
    ...
    if(fname.length() > dname.length()) {
        if(dname == fname.substr(0, dname.length())) {
            element->addAttribute("refframe")
                ->setValue(fname.substr(dname.length() + 1, fname.length() - (dname.length() + 1)));
        }
        else { element->addAttribute("refframe")->setValue(fname); }
    }
    else { element->addAttribute("refframe")->setValue(fname); }
...
void writeDeviceFrame(rw::core::Ptr<Frame> frame, ElementCreator& creator,
    ...
    if(FixedFrame* ff = frame.cast<FixedFrame>().get()) {
        ...
        if(isFrameInDevice(frame)) creator.createElement<FixedFrame*>(ff, workcell, dev, parent);
        else creator.createElement<FixedFrame*>(ff, workcell, parent);
    }
...
void createDOMDocument(DOMElem::Ptr rootDoc, rw::core::Ptr<const rw::models::WorkCell> workcell,
    ...
    for(Device::Ptr dev : devices) {
        ...
        while(!frames.empty()) {
            ...
            for(Frame::Ptr child : frame->getChildrenList(state)) {
                if(!isFrameInDevice(child.get(), dev.get())) continue;
    ...
    if(workcell->getPropertyMap().has(id_main)) {
        ...
        else { RW_WARN("ProximitySetup file not found at: " << p_file.string()); }
    }
}
}    // end of anonymous namespace

void DOMWorkCellSaver::save(rw::core::Ptr<const rw::models::WorkCell> workcell, const State& state,
                            std::string fileName) {
...
void DOMWorkCellSaver::save(rw::core::Ptr<const rw::models::WorkCell> workcell, const State& state,
    ...
    if(path.empty()) path = "./WorkCell.wc.xml";
