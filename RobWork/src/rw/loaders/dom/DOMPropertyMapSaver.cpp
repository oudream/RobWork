    ...
    switch(value.getType().getId()) {
        ...
        case PropertyType::PropertyMap: {
            const PropertyValue<PropertyMap>* prop =
                dynamic_cast<const PropertyValue<PropertyMap>*>(&value);
            save(prop->getValue(), parent);
            break;
        }
        case PropertyType::PropertyMapPtr: {
            const PropertyValue<PropertyMap::Ptr>* prop =
                dynamic_cast<const PropertyValue<PropertyMap::Ptr>*>(&value);
            save(*prop->getValue(), parent);
            break;
        }
        case PropertyType::PropertyValueBasePtrList: {
            const PropertyValue<std::vector<PropertyValueBase::Ptr>>* prop =
                dynamic_cast<const PropertyValue<std::vector<PropertyValueBase::Ptr>>*>(&value);
            DOMElem::Ptr element = parent->addChild(DOMPropertyMapFormat::idPropertyValueList());
            for(const PropertyValueBase::Ptr& p : prop->getValue()) { save(*p, element); }
            break;
        }
        case PropertyType::String: {
            const PropertyValue<std::string>* prop =
                dynamic_cast<const PropertyValue<std::string>*>(&value);
            DOMBasisTypes::createString(prop->getValue(), parent);
            break;
        }
        case PropertyType::StringList: {
            const PropertyValue<std::vector<std::string>>* prop =
                dynamic_cast<const PropertyValue<std::vector<std::string>>*>(&value);
        ...
        case PropertyType::Quaternion: {
            const PropertyValue<Quaternion<>>* prop =
                dynamic_cast<const PropertyValue<Quaternion<>>*>(&value);
        ...
        case PropertyType::Rotation2D: {
            const PropertyValue<Rotation2D<>>* prop =
                dynamic_cast<const PropertyValue<Rotation2D<>>*>(&value);
        ...
        case PropertyType::QPathPtr: {
            const PropertyValue<QPath::Ptr>* prop =
                dynamic_cast<const PropertyValue<QPath::Ptr>*>(&value);
            DOMPathSaver::createQPath(*prop->getValue(), parent);
            break;
        }
        case PropertyType::Transform3DPath: {
            const PropertyValue<Transform3DPath>* prop =
                dynamic_cast<const PropertyValue<Transform3DPath>*>(&value);
            DOMPathSaver::createTransform3DPath(prop->getValue(), parent);
            break;
        }
        case PropertyType::Transform3DPathPtr: {
            const PropertyValue<Transform3DPath::Ptr>* prop =
                dynamic_cast<const PropertyValue<Transform3DPath::Ptr>*>(&value);
            DOMPathSaver::createTransform3DPath(*prop->getValue(), parent);
            break;
        }
        default:
