...
PropertyValueBase::Ptr DOMPropertyMapLoader::readPropertyValue(DOMElem::Ptr child) {
    if(child->isName(DOMPropertyMapFormat::idPropertyMap())) {
        return ownedPtr(
            new PropertyValue<PropertyMap>(DOMPropertyMapLoader::readProperties(child, true)));
    }
    else if(child->isName(DOMPropertyMapFormat::idPropertyValueList())) {
        std::vector<PropertyValueBase::Ptr> list;
        for(DOMElem::Ptr val : child->getChildren()) { list.push_back(readPropertyValue(val)); }
        return ownedPtr(new PropertyValue<std::vector<PropertyValueBase::Ptr>>(list));
    }
    else if(child->isName(DOMBasisTypes::idString())) {
        return ownedPtr(new PropertyValue<std::string>(DOMBasisTypes::readString(child)));
    }
    else if(child->isName(DOMBasisTypes::idStringList())) {
        return ownedPtr(
            new PropertyValue<std::vector<std::string>>(DOMBasisTypes::readStringList(child)));
    }
    else if(child->isName(DOMBasisTypes::idIntList())) {
        return ownedPtr(new PropertyValue<std::vector<int>>(DOMBasisTypes::readIntList(child)));
    }
    else if(child->isName(DOMBasisTypes::idDoubleList())) {
        return ownedPtr(
            new PropertyValue<std::vector<double>>(DOMBasisTypes::readDoubleList(child)));
    }
    else if(child->isName(DOMBasisTypes::idDouble())) {
        return ownedPtr(new PropertyValue<double>(DOMBasisTypes::readDouble(child)));
    }
    else if(child->isName(DOMBasisTypes::idFloat())) {
        return ownedPtr(new PropertyValue<float>(DOMBasisTypes::readFloat(child)));
    }
    else if(child->isName(DOMBasisTypes::idInteger())) {
        return ownedPtr(new PropertyValue<int>(DOMBasisTypes::readInt(child)));
    }
    else if(child->isName(DOMBasisTypes::idBoolean())) {
        return ownedPtr(new PropertyValue<bool>(DOMBasisTypes::readBool(child)));
    }
    else if(child->isName(DOMBasisTypes::idVector3D())) {
        return ownedPtr(new PropertyValue<Vector3D<>>(DOMBasisTypes::readVector3D(child)));
    }
    else if(child->isName(DOMBasisTypes::idVector2D())) {
        return ownedPtr(new PropertyValue<Vector2D<>>(DOMBasisTypes::readVector2D(child)));
    }
        ...
        return ownedPtr(new PropertyValue<Transform3D<>>(DOMBasisTypes::readTransform3D(child)));
    }
    else if(child->isName(DOMBasisTypes::idRotation3D())) {
        return ownedPtr(new PropertyValue<Rotation3D<>>(DOMBasisTypes::readRotation3D(child)));
    }
    else if(child->isName(DOMBasisTypes::idEAA())) {
        return ownedPtr(new PropertyValue<EAA<>>(DOMBasisTypes::readEAA(child)));
    }
    else if(child->isName(DOMBasisTypes::idRPY())) {
        return ownedPtr(new PropertyValue<RPY<>>(DOMBasisTypes::readRPY(child)));
    }
    else if(child->isName(DOMBasisTypes::idQuaternion())) {
        return ownedPtr(new PropertyValue<Quaternion<>>(DOMBasisTypes::readQuaternion(child)));
    }
    else if(child->isName(DOMBasisTypes::idRotation2D())) {
        return ownedPtr(new PropertyValue<Rotation2D<>>(DOMBasisTypes::readRotation2D(child)));
    }
    else if(child->isName(DOMBasisTypes::idVelocityScrew6D())) {
        return ownedPtr(
            new PropertyValue<VelocityScrew6D<>>(DOMBasisTypes::readVelocityScrew6D(child)));
    }
        ...
        DOMPathLoader loader(child);
        return ownedPtr(new PropertyValue<Transform3DPath>(*loader.getTransform3DPath()));
    }
    else { return nullptr; }
}
...
PropertyBase::Ptr DOMPropertyMapLoader::readProperty(DOMElem::Ptr element, bool checkHeader) {
    ...
    for(DOMElem::Ptr child : element->getChildren()) {
        if(child->isName(DOMPropertyMapFormat::idPropertyName())) { name = child->getValue(); }
        else if(child->isName(DOMPropertyMapFormat::idPropertyDescription())) {
            description = child->getValue();
        }
        else if(child->isName(DOMPropertyMapFormat::idPropertyValue())) { value = child; }
        else {
    ...
    if(name == "") RW_THROW("Parse Error: name element not defined in Property!");
    if(value == NULL)
