...
void writeFile(const std::vector<char>& input, const std::string& file) {
    ...
    if(!out.is_open()) RW_THROW("Can't write file " << quote(file));

    if(!input.empty()) out.write(&input.front(), input.size());
}
...
Frame& findFrame(const WorkCell& workcell, const std::string& name) {
    ...
    if(!frame) RW_THROW("No frame named " << quote(name) << " in work cell.");
    return *frame;
}
...
class Reader
{
  public:
    ...
    int getInt() {
        ...
        if(!canRead(len)) die("Integer expected.");
    ...
    const char* getCharArray(int n) {
        const int len = n;
        if(!canRead(len)) die("Char array expected.");
    ...
    double getDouble() {
        ...
        if(!canRead(len)) die("Double expected.");
    ...
    const double* getDoubleArray(int n) {
        ...
        if(!canRead(len)) die("Double array expected.");
    ...
    StatePath getStatePath(const WorkCell& workcell) {
        StatePath path;
        const int n = getInt();
        for(int i = 0; i < n; i++) path.push_back(getState(workcell));
    ...
    TimedStatePath getTimedStatePath(const WorkCell& workcell) {
        ...
        TimedStatePath path;
        for(int i = 0; i < n; i++) { path.push_back(getTimedState(workcell)); }
    ...
    QPath getPath() {
        ...
        for(int i = 0; i < m; i++) {
            Q q(n);
            for(int j = 0; j < n; j++) { q(j) = getDouble(); }
            path.push_back(q);
        }
        return path;
    }
...
class Writer
{
  public:
    ...
    void putDoubleArray(const double* qs, int len) { put((const char*) qs, len * sizeof(double)); }
    ...
    void putTreeState(const State& state, const std::vector<Frame*>& frames) {
        ...
        for(I p = frames.begin(); p != frames.end(); ++p) {
            ...
            if(isDAF(*frame, state)) dafs.push_back(frame.get());
        }
    ...
    void putQ(const Q& q) {
        for(size_t i = 0; i < q.size(); i++) { putDouble(q(i)); }
    }
    ...
    void putStatePath(const StatePath& path, const std::vector<Frame*>& frames) {
        ...
        for(I p = path.begin(); p != path.end(); ++p) putState(*p, frames);
    }
    ...
    void putTimedStatePath(const TimedStatePath& path, const std::vector<Frame*>& frames) {
        ...
        for(I p = path.begin(); p != path.end(); ++p) { putTimedState(*p, frames); }
    }

    void putPath(const QPath& path) {
        putInt((int) path.size());
        if(path.size() > 0) putInt((int) path.front().size());
        else putInt((int) 0);

        for(QPath::const_iterator it = path.begin(); it != path.end(); ++it) putQ(*it);
    }
...
void PathLoader::storeVelocityTimedStatePath(const WorkCell& workcell, const StatePath& path,
                                             const std::string& file) {
    if(path.empty()) RW_THROW("Can't store empty state path");
