...
Model3D::Ptr LoaderAC3D::load(const std::string& filename) {
    ...
    if(!in.is_open()) RW_THROW("Can't open file " << StringUtil::quote(filename));
    ...
    try {
        ...
        while(!mobjects.empty()) {
            ...
            if(rwobj->_hasTexture) rwobj->_mappedToFaces = true;
            ...
            for(size_t i = 0; i < nrFaces; i++) {
                ...
                if(rwobj->hasTexture()) {
                    ...
                    if(materials[s.mat].texId < 0) materials[s.mat].texId = obj->texture;
                ...
                if(s.vertrefs.size() < 3) {
                    ...
                    for(size_t j = 0; j < s.vertrefs.size(); j++) poly[j] = s.vertrefs[j];
            ...
            if(parent != NULL) parent->_kids.push_back(ownedPtr(rwobj));
            else objects.push_back(ownedPtr(rwobj));
            // add all kids here
...
LoaderAC3D::AC3DMaterial LoaderAC3D::read_material(std::istream& in) {
    ...
    else { RW_THROW("Expected \"rgb\" token not found - " << token); }
    ...
    else { RW_THROW("Expected \"amb\" token not found"); }
    ...
    else { RW_THROW("Expected \"emis\" token not found"); }
    ...
    else RW_THROW("Expected \"spec\" token not found");

    // Read the shininess
    in >> token;
    if(token == "shi") in >> m.shininess;
    else RW_THROW("Expected \"shi\" token not found");
    ...
    else RW_THROW("Expected \"trans\" token not found");
    return m;
}

LoaderAC3D::OBJECT_TYPE LoaderAC3D::string_to_objecttype(const std::string& s) {
    if(s == "world") return OBJECT_WORLD;
    if(s == "group") return OBJECT_GROUP;
    if(s == "light") return OBJECT_LIGHT;
...
void LoaderAC3D::read_surface(std::istream& in, AC3DSurface& surf, AC3DObject* ob) {
    ...
    while(!in.eof()) {
        ...
        // else if (token == "mat") {
        else if(streq(token, "mat")) { sscanf(value, "%d", &surf.mat); }
        // else if (token == "refs") {
            ...
            if(surf.vertref_cnt >= 3) {
                ...
                surf.normal = normalize(cross(Vector3D<float>(v2 - v1), Vector3D<float>(v3 - v1)));
    ...
    while(!in.eof()) {
        ...
        switch(token[0]) {
            case('c'):
                ...
                break;
            case('d'):
                if(token == "data") { read_data(in, ob->data); }
                break;
            case('k'):
                if(token == "kids") {
                    in >> ob->num_kids;
                    if(ob->num_kids > 0) {
                        ...
                        for(int i = 0; i < ob->num_kids; i++) {
                            ...
                            if(k != NULL) { ob->kids[i] = k; }
                            else { RW_THROW("Could not find the specified kid!"); }
                        }
                    }
            ...
            case('l'):
                ...
                break;
            case('M'):
                if(token == "MATERIAL") { model->_materials.push_back(read_material(in)); }
                break;
            case('n'):
                if(token == "name") {
                    in >> ob->name;
                    ...
                    // long time = TimerUtil::currentTimeMs();
                    for(int i = 0; i < ob->surf_cnt; i++) { read_surface(in, ob->surfaces[i], ob); }
                    // long timel = TimerUtil::currentTimeMs();
                    // std::cout << "- time: " << timel-time << std::endl;
                }
                break;
...
void LoaderAC3D::calc_vertex_normals(AC3DObject* ob) {
    ...
    for(Vector3f& normal : ob->normals) { normal = Vector3f(0, 0, 0); }
    ...
    for(size_t i = 0; i < ob->surfaces.size(); i++) {
        ...
        for(int& ref : surf.vertrefs) {
            if(ob->normals[ref].toV3D().normInf() < 0.01) { ob->normals[ref] = surf.normal; }
            else { ob->normals[ref] = surf.normal; }
        }
    }
...
int LoaderAC3D::loadTexture(const std::string& filename, ModelAC3D* model) {
    ...
    for(size_t i = 0; i < model->_textures.size(); i++) {
        if(model->_textures[i]->getName() == filename) { return (int) i; }
    }
