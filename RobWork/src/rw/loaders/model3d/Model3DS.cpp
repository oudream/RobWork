...
void Model3DS::Load(const std::string& name) {
    ...
    if(!bin3ds) RW_THROW("Can't open file " << StringUtil::quote(name));
...
void Model3DS::EditChunkProcessor(long length, long findex) {
    ...
    if(numMaterials > 0) {
        ...
        for(int d = 0; d < numMaterials; d++) Materials.at(d).textured = false;
    ...
    if(numObjects > 0) {
        ...
        for(int k = 0; k < numObjects; k++) Objects.at(k).textured = false;
        ...
        for(int n = 0; n < numObjects; n++) Objects.at(n).numTexCoords = 0;
...
void Model3DS::VertexListChunkProcessor(long length, long findex, int objindex) {
    ...
    for(int j = 0; j < numVerts * 3; j++) Objects.at(objindex).Normals.at(j) = 0.0f;
...
void Model3DS::FacesMaterialsListChunkProcessor(long length, long findex, int objindex,
    ...
    for(material = 0; material < numMaterials; material++) {
        if(strcmp(name, Materials.at(material).name) == 0) break;
    }
