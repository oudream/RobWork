...
void OBJReader::parse_g(char** next_token) {
    _objects.push_back(Object());
    if(strlen(*next_token) != 0) { _objects.back().name = *next_token; }
    else { _objects.back().name = "Default" + std::to_string(_objects.size()); }
}
...
void OBJReader::parse_usemtl(char** next_token) {
    ...
    if(!foundMat) { RW_THROW("Material '" << token << "' not defined"); }
}

void OBJReader::parse_mtllib(char** next_token) {
    char* token;
    while((token = rwStrtok(NULL, " \t", next_token)) != NULL) { parseMtlFile(_dirName + token); }
}
...
void OBJReader::parse_mtl_map_Kd(char** next_token) {
    ...
    while(strlen(*next_token) != 0) {
        ...
        if(token[0] == '-') {
            ...
            if(boost::filesystem::exists(_dirName + token)) {
                _textures.push_back(
...
void OBJReader::parse_mtl_map_opacity(char** next_token) {
    if(strlen(*next_token) != 0) { RW_WARN("mlt type 'map_opacity' not implemented"); }
}

void OBJReader::parse_mtl_map_bump(char** next_token) {
    if(strlen(*next_token) != 0) { RW_WARN("mlt type 'map_bump' not implemented"); }
}

void OBJReader::parse_mtl_map_d(char** next_token) {
    if(strlen(*next_token) != 0) { RW_WARN("mlt type 'map_d' not implemented"); }
}

void OBJReader::parse_mtl_refl(char** next_token) {
    if(strlen(*next_token) != 0) { RW_WARN("mlt type 'refl' not implemented"); }
}

void OBJReader::parse_mtl_map_kS(char** next_token) {
    if(strlen(*next_token) != 0) { RW_WARN("mlt type 'map_kS' not implemented"); }
}

void OBJReader::parse_mtl_map_kA(char** next_token) {
    if(strlen(*next_token) != 0) { RW_WARN("mlt type 'map_kA' not implemented"); }
}

void OBJReader::parse_mtl_map_Ns(char** next_token) {
    if(strlen(*next_token) != 0) { RW_WARN("mlt type 'map_Ns' not implemented"); }
}

int OBJReader::parseInt(char** next_token) {
    char* token = rwStrtok(NULL, " \t", next_token);
    if(token == NULL) RW_THROW("Parse error");
    return atoi(token);
}
...
float OBJReader::parseOptionalFloat(char** next_token, float defaultVal) {
    ...
    else return static_cast<float>(atof(token));
}
...
void OBJReader::scale(float factor) {
    for(Vector3D<float>& v : this->_vertexes) { v *= factor; }
}
...
void OBJReader::writeFile(const std::string& filename) const {
    ...
    if(!out) RW_THROW("Unable to write '" << objFilename << "'");
...
void OBJReader::writeMtlFile(const std::string& filename) const {
    ...
    if(!out) RW_THROW("Unable to write '" << filename << "'");
    out.precision(6);
    out.setf(std::ios_base::fixed);
...
namespace {
template<class T>
void triangulatePolygon(IndexedPolygonN<T>& poly, std::vector<rw::math::Vector3D<float>>& verts,
    ...
    else { RW_WARN("Could not triangulate polygon face. Check face for overlapping points!"); }
}
}    // namespace
...
Model3D::Ptr LoaderOBJ::load(const std::string& name) {
    ...
    for(size_t i = 0; i < objects.size(); i++) {
        ...
        for(size_t j = 0; j < obj_src.MatFaces.size(); j++) {
            ...
            if(model->_materials[matFace.MatIndex].hasTexture()) obj->_hasTexture = true;
            ...
            for(const OBJReader::Face& subFace : matFace.subFaces) {
                ...
                if(0 <= subFace.vn) { obj->_normals.push_back(reader._normals[subFace.vn]); }
                if(0 <= subFace.vt) {
            ...
            if(matFace.subFaces.size() < 3) {
                ...
                if(vertexPoints < UINT8_MAX) {
                    obj8->addTriangle(
                        rw::geometry::IndexedTriangle<uint8_t>(static_cast<uint8_t>(index_s + 0),
