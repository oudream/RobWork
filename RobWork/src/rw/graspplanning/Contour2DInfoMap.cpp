...
void Contour2DInfoMap::reset(const Contour2D& contour) {
    ...
    int _avgFilterLen     = 8;
    if(contourSize < 2 * _avgFilterLen) RW_THROW("Contour is too small");
    ...
    for(int idx = contourSize - _avgFilterLen; idx < contourSize + _avgFilterLen; idx++) {
        int sampleIdx = idx;
        if(idx >= contourSize) sampleIdx = idx - contourSize;
        ...
        if(idx >= 0) curvAvg2.addSample(curvAvg1.getAverage());
    }
    // main loop
    for(int idx = 0; idx < contourSize; idx++) {
        ...
        if(sampleIdx >= contourSize) sampleIdx = sampleIdx - contourSize;
        ...
        if(_minCurvature > c.avgCurvature) _minCurvature = c.avgCurvature;
        if(_maxCurvature < c.avgCurvature) _maxCurvature = c.avgCurvature;
    ...
    if(startIdx > stopIdx) {
        for(; startIdx < _res; startIdx++) {
            for(Contact2D* cPtr : _normalToContactsMap[startIdx]) {
                if(cPtr->avgCurvature < thres) list.push_back(cPtr);
            }
        }
        startIdx = 0;
    }
