...
bool SemiForceClosureFilter::isValid(const Grasp3D& grasp) {
    if(_nrContacts != grasp.contacts.size()) RW_THROW("The number of contacts does not match!");
    // calculate the average of all contact normals and take the opposite
    // as guess for Fext
    Vector3D<> fext2;
    for(size_t i = 0; i < _nrContacts; i++) { fext2 += grasp.contacts[i].n; }
    ...
    for(size_t i = 0; i < _nrContacts; i++) {
        ...
        // std::endl;
        if(fabs(angle) < Pi / 2 + atan(grasp.contacts[i].mu)) { return true; }
    }
    return false;
}
