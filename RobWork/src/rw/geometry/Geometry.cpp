...
Geometry::Geometry(GeometryData::Ptr data, double scale) :
    _refFrame(NULL), _data(data), _transform(rw::math::Transform3D<>::identity()), _scale(scale),
...
Geometry::Geometry(GeometryData::Ptr data, const std::string& name, double scale) :
    _refFrame(NULL), _data(data), _transform(rw::math::Transform3D<>::identity()), _scale(scale),
    _name(name), _filePath(""), _mask(Geometry::CollisionGroup), _colorRGB{0.8f, 0.8f, 0.8f} {}

Geometry::Geometry(GeometryData::Ptr data, const rw::math::Transform3D<>& t3d, double scale) :
    _refFrame(NULL), _data(data), _transform(t3d), _scale(scale), _name(makeName(data->getType())),
    _filePath(""), _mask(Geometry::CollisionGroup), _colorRGB{0.8f, 0.8f, 0.8f} {}
...
Geometry::Ptr Geometry::makeGrid(int dim_x, int dim_y, double size_x, double size_y,
    ...
    for(int dy = 0; dy <= dim_y - 1; dy++) {
        for(int dx = 0; dx <= dim_x - 1; dx++) {
            imesh->add(
                IndexedTriangle<>(dx + 1 + dy * dim_y, dx + dy * dim_y, dx + 1 + (dy + 1) * dim_y));
