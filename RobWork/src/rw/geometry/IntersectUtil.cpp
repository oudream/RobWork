...
bool IntersectUtil::intersetPtRayPlane(const rw::math::Vector3D<>& p1,
    ...
    if(10000000 < fabs(t) || Math::isNaN(t)) return false;
    // compute projected position
    dst = p1 + t * ab;
    return true;
}
...
bool IntersectUtil::intersetPtRayTri(const rw::math::Vector3D<>& p1, const rw::math::Vector3D<>& p2,
                                     const Triangle<>& tri, rw::math::Vector3D<>& dst) {
...
bool IntersectUtil::intersetPtRayTri(const rw::math::Vector3D<>& ray1,
                                     ...
                                     const rw::math::Vector3D<>& p1, const rw::math::Vector3D<>& p2,
                                     const rw::math::Vector3D<>& p3, rw::math::Vector3D<>& dst) {
    Vector3D<> tmp_dst;
    Plane p(p1, p2, p3);
    // test if ray intersects the supporting plane of the triangle
    if(!intersetPtRayPlane(ray1, ray2, p, tmp_dst)) return false;
    // next we need to determine if intersection point is inside triangle
    if(dot(cross((p2 - p1), (tmp_dst - p1)), p.normal()) < 0) return false;
    if(dot(cross((p3 - p2), (tmp_dst - p2)), p.normal()) < 0) return false;
    if(dot(cross((p1 - p3), (tmp_dst - p3)), p.normal()) < 0) return false;
    dst = tmp_dst;
    return true;
}
...
bool IntersectUtil::intersetPtLineTri(const rw::math::Vector3D<>& ray1,
    ...
    Plane p(p1, p2, p3);
    // test if ray intersects the supporting plane of the triangle
    if(!intersetPtLinePlane(ray1, ray2, p, tmp_dst)) return false;
    // next we need to determine if intersection point is inside triangle
    if(dot(cross((p2 - p1), (tmp_dst - p1)), p.normal()) < 0) return false;
    if(dot(cross((p3 - p2), (tmp_dst - p2)), p.normal()) < 0) return false;
    if(dot(cross((p1 - p3), (tmp_dst - p3)), p.normal()) < 0) return false;
...
bool IntersectUtil::intersetPtTriTri(const Triangle<>& triA, const Triangle<>& triB,
    ...
    */
    ...
    if(intersetPtLineTri(triA[0], triA[1], triB, *dst)) dst = &dst2;
    if(intersetPtLineTri(triA[1], triA[2], triB, *dst)) dst = &dst2;
    if(intersetPtLineTri(triA[2], triA[0], triB, *dst)) {
        if(dst == &dst2) return true;
        dst = &dst2;
    }
