...
class Polygon2D
{
  ...
  public:
    Polygon2D(const Polygon<Vector2D<>>& polygon) :
        ...
        for(std::size_t i = 0; i < polygon.size(); i++) { _p[i] = polygon[i]; }
        reversed = PolygonUtil::area(polygon) > 0;
        if(reversed) { std::reverse(_p.begin(), _p.end()); }
    }
    ~Polygon2D() {}
    ...
    std::vector<std::vector<std::size_t>> decompose(LogWriter& log) {
        ...
        for(CutList::const_iterator cit = cuts.begin(); cit != cuts.end(); cit++) {
            ...
            for(std::size_t vi = (i + 1) % _p.size(); vi != j; vi = (vi + 1) % _p.size()) {
                if(remains[vi]) { res[ci].push_back(vi); }
                remains[vi] = false;
            }
            res[ci].push_back(j);
            ci++;
    ...
    void print(LogWriter& writer) const {
        writer << "_p:" << std::endl;
        for(const Vector2D<>& p : _p) writer << " " << p << std::endl;
        writer << "_visible:" << std::endl;
        for(std::size_t i = 0; i < _p.size(); i++) {
            writer << " " << i << "\t";
            for(std::size_t j = 0; j < _p.size(); j++) writer << _visible[i][j] << " ";
            writer << std::endl;
        }
        writer << "_valid:" << std::endl;
        for(std::size_t i = 0; i < _p.size(); i++) {
            writer << " " << i << "\t";
            for(std::size_t j = 0; j < _p.size(); j++) writer << _valid[i][j] << " ";
            writer << std::endl;
        }
        writer << "_decRunning:" << std::endl;
        for(std::size_t i = 0; i < _p.size(); i++) {
            writer << " " << i << "\t";
            for(std::size_t j = 0; j < _p.size(); j++) writer << _decRunning[i][j] << " ";
            writer << std::endl;
        }
  ...
  private:
    CutList decompose(std::size_t i, std::size_t j, LogWriter& log) {
        for(int i = 0; i < logTabLevel; i++) log << " ";
        log << "Decompose " << i << " " << j << std::endl;
        if((i + 1) % _p.size() == j) { return CutList(); }
        if(_optimal[i][j].size() != 0) { return _optimal[i][j]; }
        _decRunning[i][j] = true;
        std::vector<std::size_t> visible(1, i);
        for(std::size_t k = (i + 1) % _p.size(); k != (j + 1) % _p.size();
            k             = (k + 1) % _p.size()) {
            if(_visible[i][k] && _visible[k][j]) visible.push_back(k);
        }
    ...
    CutList best(std::size_t pivot, std::size_t extension, std::vector<DecomposeData>& stack,
        ...
        while(stack[pivot].stack.size() > 0) {
            ...
            else if(old.polygons.size() < res.size()) { res = old.polygons; }
    ...
    bool visibleTo(std::size_t vertex, std::size_t visibleTo) const {
        ...
        for(std::size_t i = 0; i < _p.size(); i++) {
            ...
            if(i == vertex || j == vertex || j == visibleTo || i == visibleTo) continue;
            const double den = (_p[i][0] - _p[j][0]) * (_p[vertex][1] - _p[visibleTo][1]) -
                               (_p[i][1] - _p[j][1]) * (_p[vertex][0] - _p[visibleTo][0]);
            if(den < std::numeric_limits<double>::epsilon()) continue;
            Vector2D<> cr;
            cr[0] =
        ...
        if(visibleTo != next && visibleTo != prev) {
            ...
            const Vector3D<> vprev =
                normalize(Vector3D<>(_p[vertex][0] - _p[prev][0], _p[vertex][1] - _p[prev][1], 0));
            const Vector3D<> vnext =
                normalize(Vector3D<>(_p[next][0] - _p[vertex][0], _p[next][1] - _p[vertex][1], 0));
...
PolygonUtil::convexDecomposition(const Polygon<Vector2D<>>& polygon) {
    ...
    for(std::vector<std::vector<std::size_t>>::const_iterator it = polygons.begin();
        ...
        for(std::size_t k = 0; k < subPoly.size(); k++) { res[i].addVertex(polygon[subPoly[k]]); }
        i++;
    }
    return res;
}
...
bool PolygonUtil::isInsideConvex(const Vector2D<>& point, const Polygon<Vector2D<>>& polygon,
    ...
    for(std::size_t i = 0; i < polygon.size(); i++) {
        if(polygon[i] == point) return false;
