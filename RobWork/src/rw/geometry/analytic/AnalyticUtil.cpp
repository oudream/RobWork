...
std::list<std::vector<Vector3D<>>> AnalyticUtil::combinePolygons(
    const std::vector<Vector3D<>>& border, const std::list<std::vector<std::size_t>>& subborder,
    const std::vector<QuadraticCurve>& curves, const double stepsPerRevolution) {
    ...
    for(curPol = subborder.begin(); curPol != subborder.end(); curPol++) {
        ...
        if(seg.size() > 1) {
            ...
            else { fullPolygon.insert(fullPolygon.end(), ++seg.begin(), --seg.end()); }
        }
    }
    return fullPolygons;
}
