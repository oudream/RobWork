  ...
  public:
    ...
    QuadraticFace::CPtr getFace(std::size_t idx) const {
        const QuadraticFace::Ptr bface =
            ownedPtr(new QuadraticFace(_brep->getSurface(idx).clone(), _brep->getVertices(idx)));
    ...
    void getFace(std::size_t idx, QuadraticFace& dst) const {
        ...
        const std::vector<QuadraticCurve::Ptr> curves = _brep->getCurves(idx);
        for(std::size_t i = 0; i < curves.size(); i++) { dst.setCurve(i, curves[i]); }
    }
...
void QuadraticBREP::scale(double factor) {
    for(std::size_t i = 0; i < _vertices.size(); i++) { _vertices[i]->point *= factor; }
    for(std::size_t i = 0; i < _surfaces.size(); i++) {
        _surfaces[i] = _surfaces[i]->scale(factor);
    }
    for(std::size_t i = 0; i < _curves.size(); i++) { _curves[i] = _curves[i]->scale(factor); }
}
...
QuadraticBREP::Ptr QuadraticBREP::clone() const {
    ...
    for(std::size_t i = 0; i < _curves.size(); i++) { brep->_curves[i] = _curves[i]->clone(); }
    return brep;
}
