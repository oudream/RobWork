...
bool QuadraticUtil::closestPointsApproximation(const QuadraticSurface& s, const QuadraticCurve& c,
    ...
    if(c.type() == QuadraticCurve::Elliptic) {
        for(std::size_t i = 0; i < minSols.size(); i++) minSols[i] = std::atan(minSols[i]) * 2;
    }
    ...
    if(minSols.size() > 0) {
        ...
        if(dfz >= -epsIn && dfz <= epsOut) {
            ...
            result.second = z;
            if(s.insideTrimmingRegion(result.first) && c.inLimits(min)) return true;
        }
        else { return false; }
    }
...
QuadraticUtil::closestPointsApproximation(const QuadraticSurface& sa, const QuadraticSurface& sb,
    ...
    for(std::size_t i = 0; i < times.size(); i++) {
        if(!c.inLimits(times[i])) continue;
...
QuadraticUtil::closestTimesApproximation(const QuadraticSurface& sa1, const QuadraticSurface& sa2,
    ...
    for(std::size_t i = 0; i < closest.size(); i++) {
        if(!cb.inLimits(closest[i])) continue;
        const std::vector<double> times = ca.closestTimes(cb(closest[i]));
        for(std::size_t k = 0; k < times.size(); k++) {
            if(!ca.inLimits(times[k])) continue;
