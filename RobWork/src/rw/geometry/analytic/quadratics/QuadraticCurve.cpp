...
QuadraticCurve::Ptr QuadraticCurve::transform(const Transform3D<>& T) const {
    ...
    if(_hasLimits) curve->setLimits(_limits);
    return curve;
}
...
QuadraticCurve::Ptr QuadraticCurve::reverse() const {
    ...
    if(_hasLimits) curve->setLimits(std::make_pair(-_limits.second, -_limits.first));
    return curve;
}

QuadraticCurve::Ptr QuadraticCurve::clone() const {
    const QuadraticCurve::Ptr curve = ownedPtr(new QuadraticCurve(_c, _u, _v, _type));
    if(_hasLimits) curve->setLimits(std::make_pair(_limits.first, _limits.second));
    return curve;
}
...
std::pair<double, double> QuadraticCurve::extremums(const Vector3D<>& dir) const {
    ...
    switch(_type) {
        case Line:
            if(_hasLimits) {
                result.first =
                    std::min(dot(dir, (*this)(_limits.first)), dot(dir, (*this)(_limits.second)));
                result.second =
                    std::max(dot(dir, (*this)(_limits.first)), dot(dir, (*this)(_limits.second)));
            }
            else {
        ...
        case Elliptic:
            ...
            if(tMax > 5) {
                ...
                else result.second = std::numeric_limits<double>::max();
            }
            else { result.second = dot(dir, (*this)(tMax)); }
            if(tMin < -5) {
                if(_hasLimits)
                    result.first = std::min(dot(dir, (*this)(_limits.first)),
                                            dot(dir, (*this)(_limits.second)));
                else result.first = -std::numeric_limits<double>::max();
            }
            else { result.first = dot(dir, (*this)(tMin)); }
            break;
        case Hyperbola:
            ...
            if(ev != 0 && std::abs(ev) >= std::abs(eu)) {
                ...
                if(t >= _limits.first && t <= _limits.second) {
                    if(eu * std::sinh(t) + ev * std::cosh(t) < 0) { tMax = t; }
                    else { tMin = t; }
                }
                // std::cout << " tmin tmax " << tMin << " " << tMax << std::endl;
        ...
        case Parabola:
            t = -eu / ev / 2;
            if(_hasLimits && t < _limits.first) { tLim = dot(dir, (*this)(_limits.first)); }
            else if(_hasLimits && t > _limits.second) { tLim = dot(dir, (*this)(_limits.second)); }
            else { tLim = dot(dir, (*this)(t)); }
            if(ev < 0) {
                result.second = tLim;
                if(_hasLimits)
                    result.first = std::min(dot(dir, (*this)(_limits.first)),
...
std::list<Vector3D<>> QuadraticCurve::discretizeAdaptive(double stepsPerRevolution) const {
    ...
    if(_type == Line) {
        ...
        else if(_limits.first == 0) { points.push_back(p1); }
        if(_limits.second > 0) {
            K  = curvature(0);
            dt = Pi * 2 / K / stepsPerRevolution;
...
OBB<> QuadraticCurve::obr() const {
    ...
    if(_type == Line) {
        ...
        return OBB<>(
            Transform3D<>(c, Rotation3D<>(normalize(_u), normalize(_v), normalize(cross(_u, _v)))),
            Vector3D<>(uLen, 0, 0));
    }
...
std::vector<Vector3D<>> QuadraticCurve::closestPoints(const Vector3D<>& p) const {
    ...
    for(std::size_t i = 0; i < times.size(); i++) { res[i] = (*this)(times[i]); }
    return res;
}
...
bool QuadraticCurve::equals(Curve::CPtr curve, double eps) const {
    ...
    if((other->hasLimits() && !_hasLimits) || (!other->hasLimits() && _hasLimits)) return false;
    ...
    switch(_type) {
        case Elliptic: {
            if((_c - other->c()).norm2() > eps) return false;
        ...
        case Line: {
            ...
            if((tdir - odir).norm2() > eps) return false;
        ...
        case Parabola: {
            ...
            if((tudir - oudir).norm2() > eps || (tvdir - ovdir).norm2() > eps) return false;
            const double tulen = _u.norm2();
            const double tvlen = _v.norm2();
            const double oulen = other->u().norm2();
            const double ovlen = other->v().norm2();
...
bool QuadraticCurve::inLimits(double t) const {
    if(t >= _limits.first && t <= _limits.second) return true;
    if(_type == Elliptic) {
        while(t < 0) { t += Pi * 2; }
        while(t > Pi * 2) { t -= Pi * 2; }
...
double QuadraticCurve::curvature(double t) const {
    ...
    switch(_type) {
        case Elliptic:
            return std::abs(
                ku * kv /
                std::pow(ku * ku * std::pow(std::cos(t), 2.) + kv * kv * std::pow(std::sin(t), 2.),
                         1.5));
        case Hyperbola:
...
std::vector<double> QuadraticCurve::closestTimes(const Vector3D<>& p) const {
    ...
    if(sols.size() > 0) {
        for(std::size_t i = 0; i < sols.size(); i++) {
            if(pol.evaluateDerivatives(sols[i], 1)[1] > 0) { minSols.push_back(sols[i]); }
        }
    }
    if(_type == QuadraticCurve::Elliptic) {
        for(std::size_t i = 0; i < minSols.size(); i++) {
            if(switched) minSols[i] = Pi / 2. - std::atan(minSols[i]) * 2;
            else minSols[i] = std::atan(minSols[i]) * 2;
        }
    }
    else if(_type == QuadraticCurve::Hyperbola) {
        for(std::size_t i = 0; i < minSols.size(); i++) { minSols[i] = std::atanh(minSols[i]) * 2; }
    }

    if(!_hasLimits) return minSols;
    ...
    for(std::size_t i = 0; i < minSols.size(); i++) {
        ...
        else { limitedSols.push_back(minSols[i]); }
    }
...
double QuadraticCurve::closestTime(const Vector3D<>& p) const {
    const std::vector<double> times = closestTimes(p);
    if(times.size() == 0) RW_THROW("No closest times found!");
    double res = times[0];
    for(std::size_t i = 1; i < times.size(); i++) {
        if(times[i] < res) res = times[i];
    }
    return res;
}
...
std::list<double> QuadraticCurve::discretizeEllipse(double stepsPerRevolution) const {
    ...
    if(_limits.first < -Pi) {
        ...
        if(_limits.first <= -Pi * 3 / 2) times.push_back(-Pi * 3 / 2);
        // treat t=-3Pi/2 to t=-Pi
        for(double t = -Pi; std::cos(t) < 0 && t > _limits.first; t -= dt) {
            K  = curvature(t);
            dt = Pi * 2 / K / stepsPerRevolution / rMax;
            if(t != -Pi) { segment.push_front(t); }
        }
        if(_limits.first > -Pi * 3 / 2) segment.push_front(_limits.first);
    ...
    if(_limits.first < -Pi / 2) {
        ...
        if(_limits.second < -Pi / 2) times.push_back(_limits.second);
        else times.push_back(-Pi / 2);
    }
    // treat t=-Pi/2 to t=0
    if(_limits.first < 0 && _limits.second > -Pi / 2) {
        for(double t = 0; std::cos(t) > 0 && t > _limits.first && t <= _limits.second; t -= dt) {
            K  = curvature(t);
            dt = Pi * 2 / K / stepsPerRevolution / rMax;
            if(t != 0) { segment.push_front(t); }
        }
        if(_limits.first == -Pi / 2) { segment.push_front(-Pi / 2); }
        if(_limits.second == 0) { segment.push_back(0); }
        times.insert(times.end(), segment.begin(), segment.end());
        segment.clear();
    }
    // treat t=0 to t=Pi/2
    if(_limits.first < Pi / 2 && _limits.second > 0) {
        ...
        if(_limits.second >= Pi / 2) times.push_back(Pi / 2);
    }
    // treat t=Pi to t=Pi/2
    ...
    if(_limits.second == Pi) times.push_back(Pi);
    if(_limits.second > Pi) {
        ...
        if(_limits.second < Pi * 3 / 2) times.push_back(_limits.second);
        else times.push_back(Pi * 3 / 2);
