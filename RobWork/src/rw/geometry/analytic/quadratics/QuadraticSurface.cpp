...
QuadraticSurface::QuadraticSurface(
    const Eigen::SelfAdjointView<const Eigen::Matrix3d, Eigen::Upper>& A, const Eigen::Vector3d& a,
...
QuadraticSurface::QuadraticSurface(const Eigen::SelfAdjointView<Eigen::Matrix3d, Eigen::Upper>& A,
                                   const Eigen::Vector3d& a, double u,
                                   const std::vector<TrimmingRegion>& conditions) :
    _A(A),
    _a(a), _u(u), _determinantA(_A.determinant()), _diagonal(false), _conditions(conditions),
...
QuadraticSurface::QuadraticSurface(
    const Eigen::SelfAdjointView<const Eigen::Matrix3d, Eigen::Lower>& A, const Eigen::Vector3d& a,
    double u, const std::vector<TrimmingRegion>& conditions) :
    _A(A),
    _a(a), _u(u), _determinantA(_A.determinant()), _diagonal(false), _conditions(conditions),
    _stepsPerRevolution(10) {}

QuadraticSurface::QuadraticSurface(const Eigen::SelfAdjointView<Eigen::Matrix3d, Eigen::Lower>& A,
                                   const Eigen::Vector3d& a, double u,
                                   const std::vector<TrimmingRegion>& conditions) :
    _A(A),
    _a(a), _u(u), _determinantA(_A.determinant()), _diagonal(false), _conditions(conditions),
...
std::pair<double, double> QuadraticSurface::extremums(const Vector3D<>& dir) const {
    if(_diagonal) return extremumsDiagonal(dir);
...
TriMesh::Ptr QuadraticSurface::getTriMesh(const std::vector<Vector3D<>>& border) const {
    if(_diagonal) return getTriMeshDiagonal(border);
    ...
    for(std::size_t i = 0; i < border.size(); i++) borderRot[i] = R * border[i];
...
bool QuadraticSurface::equals(const Surface& surface, double threshold) const {
    ...
    if(qsurf != nullptr) {
        if((_A - qsurf->_A).cwiseAbs().maxCoeff() > threshold) return false;
        if((_a - qsurf->_a).cwiseAbs().maxCoeff() > threshold) return false;
        if(std::fabs(_u - qsurf->_u) > threshold) return false;
...
bool QuadraticSurface::insideTrimmingRegion(const Vector3D<>& P) const {
    for(std::size_t i = 0; i < _conditions.size(); i++) {
        if((*_conditions[i])(P) > 0) return false;
    }
    return true;
}
...
std::pair<QuadraticSurface, Rotation3D<>> QuadraticSurface::diagonalize() const {
    if(_diagonal) return std::make_pair(*this, Rotation3D<>::identity());
    ...
    return std::make_pair(QuadraticSurface(Eigen::DiagonalMatrix<double, 3, 3>(eigen.eigenvalues()),
                                           _diagonal,
                                           _determinantA,
...
TriMesh::Ptr QuadraticSurface::getTriMeshDiagonal(const std::vector<Vector3D<>>& border,
    ...
    if(u > 2 || v > 2 || e > 2) RW_THROW("Error happened. Could not choose explicit variable!");
    ...
    if(std::fabs(_A(e, e)) >= EPS) {
        ...
        // Front
        {
            ...
            if(last[e] >= eSplit - EPS) cur.push_back(0);
            for(std::size_t i = 1; i < border.size(); i++) {
                if(last[e] >= eSplit - EPS && border[i][e] >= eSplit - EPS) { cur.push_back(i); }
                else if(last[e] < eSplit - EPS) {
                    if(cur.size() > 0) {
                        borderFront.push_back(cur);
                        cur.clear();
                    }
                    if(border[i][e] >= eSplit - EPS) cur.push_back(i);
                }
                last = border[i];
            }
            ...
            if(cur.size() > 0) { borderFront.push_back(cur); }
        }
        // Back
        {
            Vector3D<> last = border.front();
            ...
            if(cur.size() > 0) { borderBack.push_back(cur); }
        }
        ...
        // them if they are part of a larger back segment.
        {
            ...
            for(std::list<std::vector<std::size_t>>::iterator it = listA.begin();
                ...
                for(std::size_t i = 0; i < (*it).size() && within; i++) {
                    if(std::abs(border[(*it)[i]][e] - eSplit) > EPS) { within = false; }
                }
                if(within) {
                    bool match = false;
                    for(std::list<std::vector<std::size_t>>::const_iterator itB = listB.begin();
                        ...
                        for(std::size_t i = 0; i < (*itB).size() && !outsideB; i++) {
                            if(std::abs(border[(*itB)[i]][e] - eSplit) > EPS) { outsideB = true; }
                        }
                        if(outsideB) {
                            ...
                            for(std::size_t j = 0; j <= (*itB).size() && !match; j++) {
                                ...
                                if(!firstMatched) {
                                    ...
                                    if(i >= (*it).size()) { match = true; }
                                    else if((*itB)[j] != (*it)[i]) {
                                        match = false;
                                        break;
                                    }
                                    else if((*itB)[j] == (*it)[i]) { i++; }
                                }
                            }
                        }
                    }
                    if(match) { it = listA.erase(it); }
                    else { it++; }
                }
                else { it++; }
            }
    ...
    else { makeSurface(border, u, v, e, 0, BOTH, R, mesh); }
...
std::vector<QuadraticCurve> QuadraticSurface::findSilhouette(std::size_t u, std::size_t v,
    ...
    if(std::fabs(_A(u, u)) < EPS && std::fabs(_a[u]) < EPS) {
        ...
        if(s >= 0) {
            ...
            if(s != 0) silhouette.push_back(QuadraticCurve(c2, uDir, vDir, QuadraticCurve::Line));
        }
    }
        ...
        if(s >= 0) {
            ...
            if(s != 0) silhouette.push_back(QuadraticCurve(c2, vDir, uDir, QuadraticCurve::Line));
        }
    }
    else if(std::fabs(_A(u, u)) < EPS) { RW_THROW("Polynomial not yet implemented!"); }
    else if(std::fabs(_A(v, v)) < EPS) { RW_THROW("Polynomial not yet implemented!"); }
...
void QuadraticSurface::makeSurface(const std::vector<Vector3D<>> fullPolygon, std::size_t u,
            ...
            for(std::size_t k = 0; k < subPoly.size(); k++) {
                ...
                for(std::vector<std::vector<std::size_t>>::const_iterator itB =
                    ...
                    for(std::size_t kB = 0; kB < subPolyB.size() && inOther == false; kB++) {
                        ...
                        if(subPolyB[kB] == subPoly[k] && idxNextB == idxNext) inOther = true;
                        else if(subPolyB[kB] == idxNext && idxNextB == subPoly[k]) inOther = true;
                    }
                }
                if(inOther) {
                    // Sample inner border
    ...
    for(std::size_t i = 0; i < convexPolygons.size(); i++) {
        ...
        for(std::size_t k = 0; k < convexPoly.size(); k++) {
            if(convexPoly[k][0] < minU) minU = convexPoly[k][0];
            if(convexPoly[k][0] > maxU) maxU = convexPoly[k][0];
            if(convexPoly[k][1] < minV) minV = convexPoly[k][1];
            if(convexPoly[k][1] > maxV) maxV = convexPoly[k][1];
        }
    }
    ...
    if(std::abs(_A(u, u)) >= EPS) nrPointsU = nrPoints;
    if(std::abs(_A(v, v)) >= EPS) nrPointsV = nrPoints;
    ...
    for(std::size_t i = 0; i < convexPolygons.size(); i++) {
        ...
        for(std::size_t pi = 0; pi < convexPoly.size(); pi++) { points2d[pi] = convexPoly[pi]; }
        ...
        if(std::fabs(_A(2, 2)) < EPS && std::fabs(_A(1, 1)) < EPS && std::fabs(_A(0, 0)) < EPS) {
            ...
            for(std::size_t triI = 0; triI < meshProj->size(); triI++) {
                ...
                const Vector3D<> normal = R * Vector3D<>((u == 0)   ? nu
                                                         : (v == 0) ? nv
                                                                    : ne,
                                                         (u == 1)   ? nu
                                                         : (v == 1) ? nv
                ...
                else { mesh->add(TriangleN1<>(tri[0], tri[1], tri[2], normal)); }
            }
        }
                    ...
                    double eVal       = _a[e] * _a[e] / _A(e, e) / _A(e, e) -
                                  (_A(u, u) * uVal * uVal + _A(v, v) * vVal * vVal +
                                   _a[u] * uVal * 2 + _a[v] * vVal * 2 + _u) /
                                      _A(e, e);
                    if(eVal >= 0) {
                        if(place == FRONT) eVal = -_a[e] / _A(e, e) + std::sqrt(eVal);
                        else if(place == BACK) eVal = -_a[e] / _A(e, e) - std::sqrt(eVal);
                    }
                    else if(eVal >= -EPS) { eVal = -_a[e] / _A(e, e); }
                    else {
        ...
        if(std::fabs(_A(u, u)) < EPS || std::fabs(_A(v, v)) < EPS) {
            ...
            if(_A(u, u) * _A(v, v) < 0) {
                ...
                const double xe = -(_A(u, u) * xu * xu + _A(v, v) * xv * xv + _a[u] * xu * 2 +
                                    _a[v] * xv * 2 + _u) /
                                  (_a[e] * 2);
                const Vector3D<> P = Vector3D<>((u == 0)   ? xu
                                                : (v == 0) ? xv
                                                           : xe,
                                                (u == 1)   ? xu
                                                : (v == 1) ? xv
                ...
                if(insideTrimmingRegion(P)) {
                    ...
                    if(dir[e] * _A(u, u) / _a(e) > 0) { extremums.second = extremum; }
                    else { extremums.first = extremum; }
                }
            }
        }
    }
        ...
        if(((_A(u, u) >= EPS) || (_A(u, u) < EPS && _a[u] < EPS)) &&
           ((_A(v, v) >= EPS) || (_A(v, v) < EPS && _a[v] < EPS))) {
            if(std::fabs(dir[u]) < EPS && std::fabs(dir[v]) < EPS) {
                ...
                double s =
                    _a[e] / _A(e, e) * _a[e] / _A(e, e) - (_A(u, u) * xu * xu + _A(v, v) * xv * xv +
                                                           _a[u] * xu * 2 + _a[v] * xv * 2 + _u) /
                                                              _A(e, e);
                if(s < 0) RW_THROW("What should happen here?");
...
QuadraticSurface::Ptr QuadraticSurface::makeCircularCylinder(const double radius,
    ...
    static const Eigen::Vector3d a = Eigen::Vector3d::Zero();
    const double u                 = -radius * radius;
    if(outward) return ownedPtr(new QuadraticSurface(Aout, a, u));
    else return ownedPtr(new QuadraticSurface(Ain, a, -u));
}
