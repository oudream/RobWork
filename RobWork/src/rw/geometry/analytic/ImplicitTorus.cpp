...
ImplicitTorus::ImplicitTorus(double R1, double R2, double r1, double r2,
    ...
    _R1(R1),
    _R2(R2), _r1(r1), _r2(r2), _conditions(conditions), _stepsPerRevolution(stepsPerRevolution),
    _transform(transform), _isNormalForm(false) {}
...
TriMesh::Ptr ImplicitTorus::getTriMesh(const std::vector<Vector3D<>>& border) const {
    if(_isNormalForm) return getTriMeshNormalForm(border);

    const Transform3D<> Tinv = inverse(_transform);
    std::vector<Vector3D<>> borderRot(border.size());
    for(std::size_t i = 0; i < border.size(); i++) borderRot[i] = Tinv * border[i];
    std::vector<TrimmingRegion> conditions;
    for(std::size_t i = 0; i < _conditions.size(); i++)
        conditions.push_back(_conditions[i]->transform(Tinv));
    const ImplicitTorus normalForm(_R1, _R2, _r1, _r2, _transform, conditions, _stepsPerRevolution);
...
TriMesh::Ptr ImplicitTorus::getTriMeshNormalForm(const std::vector<Vector3D<>>& border,
    ...
    if(_R1 != _R2) RW_THROW("ImplicitTorus can not yet convert to trimesh when R1 != R2");
    ...
    // Front
    {
        ...
        if(last[2] >= -EPS) cur.push_back(0);
        for(std::size_t i = 1; i < border.size(); i++) {
            if(last[2] >= -EPS && border[i][2] >= -EPS) { cur.push_back(i); }
            else if(last[2] < -EPS) {
                if(cur.size() > 0) {
                    borderFront.push_back(cur);
                    cur.clear();
                }
                if(border[i][2] >= -EPS) cur.push_back(i);
            }
            last = border[i];
        }
        if(last[2] >= -EPS) {
    ...
    // they are part of a larger back segment.
    {
        ...
        for(std::list<std::vector<std::size_t>>::iterator it = listA.begin();
            ...
            for(std::size_t i = 0; i < (*it).size() && within; i++) {
                if(std::abs(border[(*it)[i]][2]) > EPS) { within = false; }
            }
            if(within) {
                bool match = false;
                for(std::list<std::vector<std::size_t>>::const_iterator itB = listB.begin();
                    ...
                    for(std::size_t i = 0; i < (*itB).size() && !outsideB; i++) {
                        if(std::abs(border[(*itB)[i]][2]) > EPS) { outsideB = true; }
                    }
                    if(outsideB) {
    ...
    for(std::size_t i = 0; i < convexPolygons.size(); i++) {
        ...
        for(std::size_t triI = 0; triI < meshProj->size(); triI++) {
            ...
            for(std::size_t pi = 0; pi < 3; pi++) {
                ...
                else { RW_THROW("R1 != R2 is not yet implemented!"); }
...
bool ImplicitTorus::equals(const Surface& surface, double threshold) const {
    ...
    if(qsurf != nullptr) {
        ...
        if(std::fabs(_R1 - qsurf->_R1) > threshold) return false;
        if(std::fabs(_R2 - qsurf->_R2) > threshold) return false;
        if(std::fabs(_r1 - qsurf->_r1) > threshold) return false;
        if(std::fabs(_r2 - qsurf->_r2) > threshold) return false;
        if(!_transform.equal(qsurf->_transform, threshold)) return false;
...
double ImplicitTorus::operator()(const Vector3D<>& x) const {
    ...
    double rTerm;
    if(_r1 == _r2) rTerm = r1sq - x[2] * x[2];
    else rTerm = r1sq * (1. - x[2] * x[2] / (_r2 * _r2));
    if(_R1 == _R2) {
...
Vector3D<> ImplicitTorus::gradient(const Vector3D<>& x) const {
    ...
    if(_R1 == _R2) {
        ...
        const double r = _R2 * x0sq + _R1 * x1sq - (_R1 + _R2) * (R1R2 + rTerm);
        double drTerm;
        if(_r1 == _r2) drTerm = -2. * x[2];
        else drTerm = -2. * r1sq / r2sq * x[2];
        Vector3D<> g;
