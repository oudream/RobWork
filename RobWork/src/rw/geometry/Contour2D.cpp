...
void Contour2D::write(Contour2D& objC, std::string file) {
    ...
    if(!ostr.is_open()) RW_THROW("Can't read file " << rw::core::StringUtil::quote(file));
...
Contour2D Contour2D::read(std::string file) {
    ...
    if(!inp.is_open()) RW_THROW("Can't read file " << rw::core::StringUtil::quote(file));
    std::string strToken;
    inp >> strToken;
...
{
  public:
    MovingAverage(int len) : _len(len), _cb(_len, Vector2D<>(0.0, 0.0)), _sum(0.0, 0.0), _idx(0) {}
...
rw::math::Vector2D<> Contour2D::calcCentroid(const Contour2D& contour) {
    Vector2D<> c(0, 0);
    for(const Contour2D::Point& p : contour.points()) { c += p.P(); }
...
double Contour2D::getCurvature(int idx, int pixelStep, const Contour2D& contour) {
    ...
    if(l1len < 0.00000001) return 0;
    if(l2len < 0.00000001) return 0;
    double dotangle = dot(l1, l2) / (l1len * l2len);
    if(fabs(1 - dotangle) < 0.0000001) return 0;
...
rw::math::Vector2D<> Contour2D::calcNormal(int idx, int pixelStep, const Contour2D& contour,
    ...
    if(!counterClock) std::swap(left, right);
...
void Contour2D::recalcNormal(Contour2D& contour) {
    ...
    for(int i = (int) contour.size() - _avgFilterLen; i < _avgFilterLen; i++) {
        int sampleIdx = i;
        if(i >= (int) contour.size()) sampleIdx = i - (int) contour.size();
        Vector2D<> normal = Contour2D::calcNormal(sampleIdx, 6, contour, true);

        curvAvg1.addSample(normal);
        if(i >= 0) curvAvg2.addSample(curvAvg1.getAverage());
    }
    ...
    for(size_t i = 0; i < contour.size(); i++) {
        ...
        if(sampleIdx >= (int) contour.size()) sampleIdx = sampleIdx - (int) contour.size();
...
TriMesh::Ptr Contour2D::toTriMesh(double height) {
    if(this->points().size() < 3) RW_THROW("No contour.");
