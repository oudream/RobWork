      ...
      public:
        ...
        IndexedTriMeshN0() :
            IndexedTriMesh<T>(rw::core::ownedPtr(new VertexArray()),
                              rw::core::ownedPtr(new VertexArray()), (uint8_t) sizeof(tri_type),
                              (uint8_t) sizeof(S)),
        ...
        IndexedTriMeshN0(rw::core::Ptr<VertexArray> vertices, rw::core::Ptr<VertexArray> normals) :
            IndexedTriMesh<T>(vertices, normals, (uint8_t) sizeof(tri_type), (uint8_t) sizeof(S)),
            _triangles(rw::core::ownedPtr(new TriangleArray())) {}
        ...
        rw::geometry::IndexedTriangle<uint32_t> getIndexedTriangle(size_t i) const {
            ...
            return rw::geometry::IndexedTriangle<uint32_t>(
                static_cast<uint32_t>(v1), static_cast<uint32_t>(v2), static_cast<uint32_t>(v3));
        }
        ...
        void resize(size_t nsize) {
            if(_triangles != NULL) _triangles->resize(nsize);
            if(_triangles != NULL && _triangles->size() > 0)
                this->setTriArray((uint8_t*) &((*_triangles)[0].getVertexIdx(0)));
        }
        ...
        TriMesh::Ptr clone() const {
            return rw::core::ownedPtr(new rw::geometry::IndexedTriMeshN0<T, S>(*this));
        }
    };
#if !defined(SWIG)
