...
{
    if (vertice._mesh != _mesh)
        return false;
    if (idx (0) == vertice.idx ())
        return true;
    if (idx (1) == vertice.idx ())
        return true;
    if (idx (2) == vertice.idx ())
        return true;
    return false;
}
...
{
    if (vertice._mesh != _mesh) {
        RW_THROW ("vertice is not part of same mesh as triangle");
    }
    if (vertice.idx () == idx (0))
        return edge (1);
    if (vertice.idx () == idx (1))
        return edge (2);
    if (vertice.idx () == idx (2))
        return edge (0);
...
{
    if (edge._mesh != _mesh) {
        RW_THROW ("Edge is not part of same mesh as triangle");
    }

    if (idx (0) == edge._edgeIdx1) {
        if (idx (1) == edge._edgeIdx2) {
            return (*this)[2];
        }
        else if (idx (2) == edge._edgeIdx2) {
            return (*this)[1];
        }
    }
    else if (idx (1) == edge._edgeIdx1) {
        if (idx (0) == edge._edgeIdx2) {
            return (*this)[2];
        }
        else if (idx (2) == edge._edgeIdx2) {
            return (*this)[0];
...
bool ReferencedTriangle::intersects (const ReferencedTriangle& t2) const
{
    ...
    if (t1._mesh != t2._mesh)
        return false;
    if (t1._triIndex == t2._triIndex)
        return false;
