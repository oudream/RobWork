...
{
    FaceIntegrals() :
        Fa(0), Fb(0), Fc(0), Faa(0), Fbb(0), Fcc(0), Faaa(0), Fbbb(0), Fccc(0), Faab(0), Fbbc(0),
        Fcca(0) {}
...
FaceIntegrals compFaceIntegrals(const Triangle<double>& f, const Transform3D<>& t3d,
    ...
    if(!noInertia) {
        ...
        res.Fccc =
            -k4 *
            (std::pow(n[A], 3) * p.Paaa + 3 * std::pow(n[A], 2) * n[B] * p.Paab +
             3 * n[A] * std::pow(n[B], 2) * p.Pabb + std::pow(n[B], 3) * p.Pbbb +
             3 * w *
                 (std::pow(n[A], 2) * p.Paa + 2 * n[A] * n[B] * p.Pab + std::pow(n[B], 2) * p.Pbb) +
...
void compVolumeIntegrals(const rw::core::Ptr<const TriMesh> p, const Transform3D<>& t3d, double& T0,
                         Vector3D<>& T1, Vector3D<>& T2, Vector3D<>& TP, bool noInertia = false) {
    std::size_t used = 0;
    for(std::size_t i = 0; i < p->size(); i++) {
        ...
        B = (A + 1) % 3;
        if(!(nabs[C] > 0)) {
            if(face.calcArea() == 0) { continue; }
            else { RW_THROW("Could not determine normal for face with non-zero area."); }
        }
        else { used++; }
...
std::vector<Frame*> GeometryUtil::getAnchoredFrames(Frame& frame, const State& state) {
    ...
    if(parent.cast<FixedFrame>() == NULL) { return getAnchoredChildFrames(parent, state); }
...
Vector3D<> GeometryUtil::estimateCOG(const std::vector<Geometry::Ptr>& geoms) {
    if(geoms.size() == 0) RW_THROW("At least one geometry is required!");
    double totalVolume = 0;
    ...
    if(totalVolume == 0.0) RW_THROW("There are no valid geometries!");
...
Vector3D<> GeometryUtil::estimateCOG(const std::vector<Geometry::Ptr>& geoms,
    ...
    for(Geometry::Ptr geom : geoms) {
        ...
        const TriMesh::Ptr trimesh    = gdata->getTriMesh(false);
        Transform3D<> t3d;
        if(geom->getFrame() != NULL) t3d = Kinematics::frameTframe(ref, geom->getFrame(), state);
...
InertiaMatrix<> GeometryUtil::estimateInertia(double mass, const std::vector<Geometry::Ptr>& geoms,
    ...
    for(std::size_t i = 0; i < geoms.size(); i++) {
        ...
        for(std::size_t i = 0; i < 3; i++) {
            for(std::size_t j = 0; j < 3; j++) { inertia(i, j) += m * J(i, j); }
        }
    }
    return inertia;
}
