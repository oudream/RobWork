...
TriMesh::Ptr Pyramid::createMesh(int resolution) const {
    ...
    (*mesh)[0].getVertex(0) =
        Vector3D<float>(static_cast<float>(-_widthX / 2.0), static_cast<float>(-_widthY / 2), 0);
    ...
    (*mesh)[1].getVertex(1) =
        Vector3D<float>(static_cast<float>(_widthX / 2.0), static_cast<float>(-_widthY / 2), 0);
    (*mesh)[1].getVertex(2) =
        Vector3D<float>(static_cast<float>(-_widthX / 2.0), static_cast<float>(-_widthY / 2.0), 0);
...
void Pyramid::setParameters(const rw::math::Q& q) {
    if(q.size() != 3) { RW_THROW("Size of parameter list must equal 3!"); }
