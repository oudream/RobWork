...
template<class T> BSphere<T> BSphere<T>::fitEigen(const rw::geometry::TriMesh& tris) {
    ...
    if(nrOfTris == 0) { return BSphere<T>(); }
    ...
    for(size_t j = 0; j < 3; j++)
        for(size_t k = 0; k < 3; k++) covar(j, k) = covar(j, k) - centroid[j] * centroid[k] / n;
    ...
    for(unsigned int i = 0; i < tris.size(); i++) {
        for(unsigned int p = 0; p < 3; p++) {
            ...
            if((point - midPoint).norm2() > halfLength) { halfLength = (point - midPoint).norm2(); }
        }
    }
