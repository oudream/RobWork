...
void Plane::setParameters(const rw::math::Q& q) {
    if(q.size() != 4) { RW_THROW("Size of parameter list must equal 4!"); }
...
double Plane::refit(const std::vector<rw::math::Vector3D<>>& data) {
    if(data.size() < 3) RW_THROW("Data size must be 3 or more!");

    if(data.size() == 3) {
        if(MetricUtil::dist2(data[1], data[0]) < 0.1 || MetricUtil::dist2(data[2], data[1]) < 0.1 ||
        ...
        if(_normal(2) < 0) { _normal = -_normal; }
        ...
        for(const Vector3D<>& v : data) {
            ...
            for(size_t j = 0; j < 3; j++) {
                for(size_t k = 0; k < 3; k++) { covar(j, k) += v(j) * v(k); }
            }
        }
