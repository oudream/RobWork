...
IndexedTriMesh<>::Ptr Delaunay::triangulate(const std::vector<Vector2D<>>& vertices,
    ...
    delete[] vertArray;
    if(exitcode) RW_THROW("Delaunay triangulation returned with exit code " << exitcode);
    ...
    // Create the mesh
    if(vertices.size() <= 256) { return makeMesh<uint8_t>(triangVertices, faceIdxs, values); }
    ...
    else { return makeMesh<uint64_t>(triangVertices, faceIdxs, values); }
}
