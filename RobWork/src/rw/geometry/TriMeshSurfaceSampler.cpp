...
TriMeshSurfaceSampler::TriMeshSurfaceSampler(
    ...
    for(Geometry::Ptr geom : geoms) { add(geom); }
}
...
void TriMeshSurfaceSampler::add(rw::geometry::TriMesh::Ptr mesh) {
    ...
    _sAreaSum = 0;
    if(start_idx > 0) _sAreaSum = _surfaceArea[start_idx - 1];
...
rw::geometry::Triangle<> TriMeshSurfaceSampler::getTriangle(const int idx) {
    ...
    if(_meshes.size() == 1) { return _meshes[0]->getTriangle(idx); }
    else if(_meshes.size() == 2) {
        if(idx < _surfaceAreaMesh[0]) return _meshes[0]->getTriangle(idx);
        else return _meshes[1]->getTriangle(idx - _surfaceAreaMesh[0]);
    }
    ...
    do {
        ...
        if(idx < _surfaceAreaMesh[split]) { upper = split; }
        else { lower = split + 1; }
    } while(upper != lower);
...
rw::math::Transform3D<> TriMeshSurfaceSampler::sample() {
    ...
    do {
        ...
        if(_genRandomRotation) { target.R() = Math::ranRotation3D<double>(); }
        if(!_filterByDirection) return target;
