    ...
    template<class T = uint16_t> class Object3D : public Object3DGeneric
    {
      public:
        ...
        void addTriangles(const std::vector<rw::geometry::IndexedTriangle<T>>& tris) {
            ...
            if(newSize > 65535) RW_THROW("Model3D has two many faces! - max is 65535.");
            _faces.resize(newSize);
            for(size_t i = 0; i < tris.size(); i++) { _faces[startIdx + i] = tris[i]; }
        ...
        void scale(double scale) {
            for(rw::math::Vector3D<float>& v : _vertices) { v *= (float) scale; }

            for(rw::math::Vector2D<float>& v : _texCoords) { v *= (float) scale; }
            _transform.P() *= (float) scale;
            _texOffset *= (float) scale;
            _texRepeat *= (float) scale;

            for(Object3DGeneric::Ptr kid : _kids) { kid->scale(scale); }
        }
