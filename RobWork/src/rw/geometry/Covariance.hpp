      ...
      public:
        ...
        void initialize(rw::geometry::Geometry& geom) {
            ...
            if(dynamic_cast<rw::geometry::IndexedTriMesh<T>*>(mesh.get())) {
                rw::geometry::IndexedTriMesh<T>* imesh =
                    dynamic_cast<rw::geometry::IndexedTriMesh<T>*>(mesh.get());
        ...
        void doInitialize(RandomAccessIterator first, RandomAccessIterator last) {
            ...
            for(; first_tmp != last; ++first_tmp) {
                nrOfPoints++;
                for(size_t j = 0; j < DIM; j++) centroid[j] += (*first_tmp)[j];
            }
            ...
            for(; first != last; ++first) {
                for(size_t k = 0; k < DIM; k++) { p[k] = (*first)[k] - centroid[k]; }

                for(size_t j = 0; j < DIM; j++) {
                    for(size_t k = j; k < DIM; k++) { _covar(k, j) += (*first)[k] * (*first)[j]; }
                }
            }

            // fill in all covariance elements
            for(size_t j = 1; j < DIM; j++)
                for(size_t k = 0; k < j; k++) _covar(k, j) = _covar(j, k);
        }
