...
bool Triangulate::snip(const std::vector<rw::math::Vector2D<>>& contour, int u, int v, int w, int n,
                       int* V) {
    int p;
...
bool Triangulate::processPolygon(Polygon<>::Ptr polygon, std::vector<int>& result,
    ...
    for(size_t i = 0; i < polygon->size(); i++) { contour.push_back(polygon->getVertex(i)); }

    return processPoints(contour, result, colinearCriteria, precision);
}

bool Triangulate::processPoints(const std::vector<Vector3D<>>& contour, std::vector<int>& result,
                                double colinearCriteria, double precision) {
    ...
    do {
        ...
        if(i == contour.size()) { RW_THROW("The points on the contour appears to be colinear"); }
    ...
    int maxIdx        = 0;
    if(normal[0] > normal[1] && normal[0] > normal[2]) { maxIdx = 0; }
    else if(normal[1] > normal[0] && normal[1] > normal[2]) { maxIdx = 1; }
    else { maxIdx = 2; }
