...
namespace rw { namespace proximity {
    ...
    class BVTreeFactory
    {
      public:
        ...
        template<class T> struct ShellAccessor : public PrimArrayAccessor<rw::geometry::GenericFace>
        {
            /**
             * @brief Constructor.
         ...
         */
        template<class OBV> rw::core::Ptr<BVSplitterStrategy<OBV>> makeOBVSpatialMedianSplitter() {
        ...
        rw::core::Ptr<geometry::BVFactory<rw::geometry::OBB<T>>> makeOBBCovarFactory() {
            return rw::core::ownedPtr(new geometry::OBBFactory<T>(geometry::OBBFactory<T>::PCA));
        }
        ...
        template<class BVTREE>
        BVTREE* makeTopDownOBBTreeCovarMean(rw::geometry::TriMesh::Ptr mesh,
                                            int maxTrisInLeaf = 1) {
            typedef typename Traits<BVTREE>::BVType BVType;
            rw::core::Ptr<geometry::BVFactory<BVType>> bvfactory =
                makeOBBCovarFactory<typename Traits<BVType>::value_type>();
            rw::core::Ptr<BVSplitterStrategy<BVType>> splitter = makeOBVMeanSplitter<BVType>();
            return makeTopDownTree<BVTREE>(mesh, *bvfactory, *splitter, maxTrisInLeaf);
        }
        ...
        template<class BVTREE>
        static BVTREE*
        makeTopDownTree(rw::geometry::Shell::CPtr shell,
                        geometry::BVFactory<typename Traits<BVTREE>::BVType>& bvFactory,
        ...
        template<class BVTREE>
        static void
        recursiveTopDownTree(BVTree<BVTREE>* tree, typename Traits<BVTREE>::NodeIterator& node,
            ...
            if(mesh.getSize() == 0) {
                ...
                if(k > 0) {
                    ...
                    recursiveTopDownTree(
                        tree, leftnode, mesh.getSubRange(0, k), bvFactory, splitter, maxTrisInLeaf);
                }
         ...
         */
        template<class BV> struct OBVMedianSplitter : public BVTreeFactory::BVSplitterStrategy<BV>
        {
          public:
            /**
             * @brief
            ...
            size_t partitionMesh(rw::geometry::IndexedTriArray<>& mesh, const BV& obb) const {
                ...
                do {
                    ...
                    if(score + mesh.getSize() / 8 >= mesh.getSize()) { break; }

                    splitAxis++;
                    if(splitAxis == 3) {
                        // No axis was goood, so we use the best one
                        if(bestSplitAxis != 2) mesh.sortAxis(bestSplitAxis, t3d);
                        break;
                    }
          ...
          public:
            size_t partitionShell(rw::geometry::IndexedFaceArray& shell, const BV& obb) const {
                ...
                do {
                    ...
                    // criteria for an okay splitting point
                    if(score + shell.size() / 8 >= shell.size()) { break; }

                    splitAxis++;
                    if(splitAxis == 3) {
                        // No axis was goood, so we use the best one
                        if(bestSplitAxis != 2) shell.sortAxis(bestSplitAxis, t3d);
                        break;
                    }
        ...
        template<class BV> class OBVMeanSplitter : public BVTreeFactory::BVSplitterStrategy<BV>
        {
          public:
            ...
            size_t partitionMesh(rw::geometry::IndexedTriArray<>& mesh, const BV& obb) const {
                ...
                if(meanSplit == 0) return 1;
                if(meanSplit == mesh.size() - 1) return meanSplit - 1;
                return meanSplit;
            }
        };
        ...
        struct OBVSpatialMedianSplitter : public BVTreeFactory::BVSplitterStrategy<BV>
        {
          public:
            ...
            size_t partitionMesh(rw::geometry::IndexedTriArray<>& mesh, const BV& obb) const {
                ...
                if(closest == mesh.size() - 1) return closest - 1;
                return closest;
            }
        };
    };
