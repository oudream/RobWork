...
bool ProximityStrategyRW::addGeometry(rw::proximity::ProximityModel* model,
    ...
    if(_modelCache.has(key)) { rwmodel = _modelCache.get(key); }
    else {
...
std::vector<std::string> ProximityStrategyRW::getGeometryIDs(rw::proximity::ProximityModel* model) {
    ...
    RWProximityModel* pmodel = (RWProximityModel*) model;
    for(Model::Ptr& m : pmodel->models) { res.push_back(m->geo->getId()); }
    return res;
}
...
ProximityStrategyRW::getGeometries(rw::proximity::ProximityModel* model) {
    ...
    RWProximityModel* pmodel = (RWProximityModel*) model;
    for(Model::Ptr& m : pmodel->models) { res.push_back(m->geo); }
    return res;
}
