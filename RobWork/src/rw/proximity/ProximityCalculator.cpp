...
std::vector<std::string> searchName(const std::map<std::string, Frame*>& frameMap,
    ...
    for(const std::pair<std::string, Frame*> x : frameMap) {
        if(std::regex_match(x.first, reg)) { ret.push_back(x.first); }
    }
    return ret;
}
...
namespace rw { namespace proximity {
    ...
    template<class T> void ProximityCalculator<T>::setStrategy(rw::core::Ptr<Strategy> strategy) {
        RW_ASSERT(strategy);
        _strategy = strategy;
    }
    ...
    ProximityCalculator<T>::calculate(const rw::kinematics::State& state,
                                      rw::core::Ptr<ProximityStrategyData> settings,
    ...
    ProximityStrategyData ProximityCalculator<CollisionStrategy>::calculate(
        ...
        if(!settings.isNull()) { data = *settings; }
        ...
        while(!filter->isEmpty()) {
            ...
            if(!res) {
                ...
                if(res && !result.isNull()) { result->push_back(data); }
            }
            if(res) {
                if(stopAtFirstContact) { return data; }
...
#pragma omp critical
            ...
            if(distance.distance == DBL_MAX || _thresholdStrategy == NULL || result != NULL) {
                ...
                dist =
                    _thresholdStrategy->distance(a.get(), ta, b.get(), tb, distance.distance, data);
            }
...
#pragma omp critical
#endif
            {
                if(dist.distance < distance.distance) { distance = dist; }
    ...
    ProximityStrategyData ProximityCalculator<DistanceMultiStrategy>::calculate(
        ...
        if(settings != NULL) { tolerance = settings->getMultiDistanceTolerance(); }
...
#pragma omp critical
    ...
    bool ProximityCalculator<T>::addGeometry(rw::core::Ptr<Frame> frame,
        ...
        if(geometry.isNull()) { RW_THROW("Unable to add NULL as geometry"); }
        bool res = true;
        if(!_strategy.isNull()) {
        ...
        else { _proxFilterStrat->addGeometry(frame.get(), geometry); }
    ...
    template<class T>
    std::vector<std::string> ProximityCalculator<T>::getGeometryIDs(rw::core::Ptr<Frame> frame) {
        if(!_frameToModels.has(*frame)) { return std::vector<std::string>(); }
        ProximityModel::Ptr model = _frameToModels[*frame];
        if(model == NULL) { return std::vector<std::string>(); }
    ...
    bool ProximityCalculator<T>::hasGeometry(rw::core::Ptr<Frame> frame,
        ...
        for(const std::string& name : names) {
            if(name == geometryId) return true;
        }
        return false;
    }
    ...
    rw::core::Ptr<Geometry> ProximityCalculator<T>::getGeometry(rw::core::Ptr<Frame> frame,
                                                                const std::string& geometryId) {
        if(!_frameToModels.has(*frame)) { return NULL; }

        ProximityModel::Ptr model = _frameToModels[*frame];
        if(model == NULL) { return NULL; }

        for(rw::core::Ptr<Geometry>& geom : model->getGeometries()) {
            if(geom->getId() == geometryId) return geom;
        }
        return NULL;
    }
    ...
    template<class T> void ProximityCalculator<T>::initDistPairs(const State& state) {
        ...
        for(I from = frames.cbegin(); from != frames.cend(); ++from) {
            if(_strategy->hasModel(*from)) {
                ...
                for(++to; to != frames.cend(); ++to) {
                    if(_strategy->hasModel(*to)) { pairs.push_back(FramePair(*from, *to)); }
                }
            }
        }
        ...
        for(PLI p = pairs.cbegin(); p != pairs.cend(); ++p) {
            if(!isInList(*p, exclude_pairs)) _distancePairs.push_back(*p);
        }
    }
