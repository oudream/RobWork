...
bool CollisionStrategy::inCollision(const rw::core::Ptr<Frame> a, const Transform3D<>& wTa,
...
bool CollisionStrategy::inCollision(const rw::core::Ptr<Frame> a, const Transform3D<>& wTa,
                                    const rw::core::Ptr<Frame> b, const Transform3D<>& wTb,
                                    ProximityStrategyData& data, QueryType type) {
    if(getModel(a) == NULL || getModel(b) == NULL) return false;
...
CollisionStrategy::Ptr CollisionStrategy::Factory::makeStrategy(const std::string& strategy) {
    ...
    for(Extension::Ptr& ext : exts) {
        ...
        std::transform(id.begin(), id.end(), id.begin(), ::toupper);
        if(id == upper) { return ext->getObject().cast<CollisionStrategy>(); }
    }
    return NULL;
}
