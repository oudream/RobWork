...
void BasicFilterStrategy::addGeometry(rw::core::Ptr<rw::kinematics::Frame> frame,
    ...
    if(geo == NULL) RW_THROW("Unable to add NULL as geometry");
    if(frame == NULL) RW_THROW("Unable to add geometry to NULL frame");
...
void BasicFilterStrategy::removeGeometry(rw::core::Ptr<rw::kinematics::Frame> frame,
    ...
    if(geoNames.empty()) { _frameToGeoIdMap.erase(*frame); }
    initializeCollisionFramePairs(_workcell->getDefaultState());
}
...
FramePair orderPair(const FramePair& pair) {
    if(pair.first->getName() < pair.second->getName()) return pair;
    else return FramePair(pair.second, pair.first);
}
...
void BasicFilterStrategy::initializeCollisionFramePairs(const State& state) {
    ...
    if(_psetup.useIncludeAll()) {
        ...
        for(FrameList::iterator it = allFrames.begin(); it != allFrames.end();) {
            if(*it == NULL) continue;
            if(!_frameToGeoIdMap.has(*(*it))) { it = allFrames.erase(it); }
            else { ++it; }
        }
