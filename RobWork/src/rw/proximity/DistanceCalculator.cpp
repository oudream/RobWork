...
DistanceCalculator::DistanceCalculator(const FramePairList& pairs, DistanceStrategy::Ptr strategy) :
    _cnt(0), _root(nullptr), _strategy(strategy), _distancePairs(pairs) {
    RW_ASSERT(strategy);
...
DistanceCalculator::distance(const State& state,
    ...
    for(I p = _distancePairs.cbegin(); p != _distancePairs.cend(); ++p) {
        ...
        if(result != NULL) result->push_back(*dist);
    }
...
#pragma omp critical
        ...
        else { dist = &_thresholdStrategy->distance(a, ta, b, tb, distance.distance, data); }
...
bool DistanceCalculator::addDistanceModel(const rw::core::Ptr<Frame> frame,
                                          const rw::geometry::Geometry& faces) {
    bool res = _strategy->addModel(frame, faces);
    if(res) initializeDistancePairs();
    return res;
}
