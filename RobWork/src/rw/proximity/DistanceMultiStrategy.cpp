...
DistanceMultiStrategy::Result DistanceMultiStrategy::distances(const rw::core::Ptr<Frame> a,
...
DistanceMultiStrategy::distances(const rw::core::Ptr<Frame> a, const Transform3D<>& wTa,
...
DistanceMultiStrategy::Factory::makeStrategy(const std::string& strategy) {
    ...
    for(Extension::Ptr& ext : exts) {
        ...
        std::transform(id.begin(), id.end(), id.begin(), ::toupper);
        if(id == upper) { return ext->getObject().cast<DistanceMultiStrategy>(); }
    }
    return NULL;
}
