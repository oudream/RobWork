...
template<> Q::Q(size_t n, const double* values) : _vec(n) {
    for(size_t i = 0; i < n; i++) { _vec(i) = values[i]; }
}
...
template<> Q::Q(int n, const double* values) : _vec(n) {
    for(int i = 0; i < n; i++) { _vec(i) = values[i]; }
}
...
template<> Q::Q(int n, double value) : _vec(n) {
    for(int i = 0; i < n; i++) { _vec(i) = value; }
}

template<> Q::Q(size_t n, double value) : _vec(n) {
    for(size_t i = 0; i < n; i++) { _vec(i) = value; }
}

Q::Q(const std::vector<double>& r) : _vec(r.size()) {
    for(size_t i = 0; i < r.size(); i++) _vec(i) = r[i];
}

Q::~Q() {}
namespace rw { namespace math {
    std::ostream& operator<<(std::ostream& out, const Q& v) {
        if(v.size() == 0) { return out << "Q[0]{}"; }
        else {
            out << "Q[" << (int) v.size() << "]{";
            for(size_t i = 0; i < v.size() - 1; i++) out << v[i] << ", ";
...
std::istream& rw::math::operator>>(std::istream& in, Q& q) {
    ...
    if(ch1 == 'Q') {
        in.get(ch2);
        if(ch1 != 'Q' || ch2 != '[') RW_THROW("Content of input stream does not match format of Q");
        in >> size;

        in.get(ch1);
        in.get(ch2);
        if(ch1 != ']' || ch2 != '{') RW_THROW("Content of input stream does not match format of Q");
    }
    else if(ch1 != '{') { RW_THROW("Content of input stream does not match format of Q"); }

    std::vector<double> res;
    while(ch1 != '}') {
        double d;
        in >> d;
        if(!in.eof()) { res.push_back(d); }
        in.get(ch1);
    }

    if(ch1 != '}') RW_THROW("Content of input stream does not match format of Q");

    if(size > -1 && (int) res.size() != size) { RW_THROW("Length of Q does not match device"); }
...
rw::math::Q rw::math::concat(const Q& q1, const Q& q2) {
    ...
    for(size_t i = 0; i < q2.size(); i++) q(q1.size() + i) = q2(i);
    return q;
}
...
rw::core::Ptr<Property<Q>> PropertyMap::findProperty(const std::string& identifier) const {
    ...
    if(!pb.isNull() && pb->getType().getId() == PropertyType::DoubleList) {
        ...
        if(findPropertyBase(id).isNull()) {
            std::vector<double> q =
                findPropertyBase(identifier).cast<Property<std::vector<double>>>()->getValue();

            rw::core::Ptr<Property<Q>> prop =
                ownedPtr(new Property<Q>(pb->getIdentifier() + "<Q>", pb->getDescription(), Q(q)));
