...
template<class T> void Rotation3D<T>::normalize() {
    ...
    for(size_t i = 0; i < 3; i++) {
        for(size_t j = 0; j < 3; j++) {
            if(_m(i, j) > 1) _m(i, j) = 1;
            else if(_m(i, j) < -1) _m(i, j) = -1;
        }
    }
}

template<class T>
void Rotation3D<T>::multiply(const Rotation3D<T>& a, const Vector3D<T>& b, Vector3D<T>& result) {
    const T a00 = a(0, 0);
    const T a01 = a(0, 1);
    const T a02 = a(0, 2);
...
template<class T> const Rotation3D<T> Rotation3D<T>::inverse(bool copy) const {
    if(!copy) { RW_THROW("You can't modefy a constant object"); }
    return Rotation3D<T>(
...
namespace rw { namespace common { namespace serialization {
    ...
    template<class T> void readImpl(Rotation3D<T>& tmp, InputArchive& iar, const std::string& id) {
        std::vector<T> data;
        iar.read(data, id);
    ...
    template<> void write(const Rotation3D<float>& tmp, OutputArchive& oar, const std::string& id) {
        writeImpl(tmp, oar, id);
    }
