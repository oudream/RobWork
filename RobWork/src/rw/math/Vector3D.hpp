...
namespace rw { namespace math {
    ...
    template<class T = double> class Vector3D
    {
      public:
        ...
        Vector3D(const std::vector<T>& vec) {
            ...
            for(size_t i = 0; i < 3u; i++) { _vec[i] = vec[i]; }
        }
        ...
        template<class R> Vector3D<T> elemDivide(const Eigen::MatrixBase<R>& rhs) const {
            Vector3D<T> ret = *this;
            for(size_t i = 0; i < size(); i++) { ret._vec[i] /= rhs[i]; }
            return ret;
        }
        ...
        Vector3D<T> elemDivide(const Vector3D<T>& rhs) const {
            return Vector3D<T>(_vec[0] / rhs._vec[0], _vec[1] / rhs._vec[1], _vec[2] / rhs._vec[2]);
        }
     ...
     * @relates Vector3D
     */
    template<class T> void cross(const Vector3D<T>& v1, const Vector3D<T>& v2, Vector3D<T>& dst) {
    ...
    template<class T> const Vector3D<T> normalize(const Vector3D<T>& v) {
        T length = v.norm2();
        if(length != 0) return Vector3D<T>(v(0) / length, v(1) / length, v(2) / length);
        else return Vector3D<T>(0, 0, 0);
    }
    ...
    template<class Q, class T> const Vector3D<Q> cast(const Vector3D<T>& v) {
        return Vector3D<Q>(static_cast<Q>(v(0)), static_cast<Q>(v(1)), static_cast<Q>(v(2)));
    }
#if !defined(SWIG)
...
namespace rw { namespace common {
    ...
    namespace serialization {
         ...
         */
        template<>
        void write(const rw::math::Vector3D<double>& sobject, rw::common::OutputArchive& oarchive,
                   const std::string& id);
