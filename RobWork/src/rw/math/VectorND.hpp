...
namespace rw { namespace math {
    ...
    template<size_t N, class T = double> class VectorND : public rw::common::Serializable
    {
      public:
         ...
         */
        VectorND() {
            if(N <= 0) { RW_THROW("Vector to small, N must be larger than 0"); }
        ...
        template<typename... ARGS> VectorND(T arg0, ARGS... args) {
            if(N <= 0u) { RW_THROW("Vector to small, N must be larger than 0"); }
            size_t i = 1;
        ...
        VectorND(const std::vector<T>& vec) {
            ...
            for(size_t i = 0; i < N; i++) { _vec[i] = vec[i]; }
        }
        ...
        template<class R> VectorND<N, T> elemMultiply(const Eigen::MatrixBase<R>& rhs) const {
            ...
            for(size_t i = 0; i < N; i++) { ret._vec[i] *= rhs[i]; }
            return ret;
        }
        ...
        template<class R> VectorND<N, T> elemDivide(const Eigen::MatrixBase<R>& rhs) const {
            VectorND<N, T> ret;
            for(size_t i = 0; i < N; i++) { ret[i] = (*this)[i] / rhs[i]; }
            return ret;
        }
        ...
        VectorND<N, T> elemDivide(const VectorND<N, T>& rhs) const {
            VectorND<N, T> ret = *this;
            for(size_t i = 0; i < N; i++) { ret._vec[i] /= rhs._vec[i]; }
            return ret;
        }
        ...
        friend VectorND<N, T> operator/(T lhs, const VectorND<N, T>& rhs) {
            ...
            for(size_t i = 0; i < N; i++) { ret._vec[i] = lhs / ret._vec[i]; }
            return ret;
        }
#endif
        ...
        VectorND<N, T> elemSubtract(const T& rhs) const {
            VectorND<N, T> ret = *this;
            for(size_t i = 0; i < N; i++) { ret._vec[i] = ret._vec[i] - rhs; }
            return ret;
        }
...
#undef VECTORND
        ...
        std::vector<T> toStdVector() const {
            std::vector<T> ret;
            for(size_t i = 0; i < N; i++) { ret.push_back(_vec[i]); }
            return ret;
        }
      ...
      private:
        void ParamExpansion(size_t& i) {
            if(i > N) { RW_THROW("Vector to big, argc(" << i << ") != N(" << N << ")"); }
            else if(i < N) { RW_THROW("Vector to small, argc(" << i << ") != N(" << N << ")"); }
        }
        ...
        template<typename R, typename... ARGS> void ParamExpansion(size_t& i, R arg, ARGS... args) {
            ParamExpansion(++i, args...);
     ...
     * @relates VectorND
     */
    template<class Q, size_t ND, class T> const VectorND<ND, Q> cast(const VectorND<ND, T>& v) {
        VectorND<ND, Q> ret;

        for(size_t i = 0; i < v.size(); i++) { ret[i] = static_cast<Q>(v[i]); }
        return ret;
    }
