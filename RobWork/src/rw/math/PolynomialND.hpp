    ...
    template<typename Coef, typename Scalar = double> class PolynomialND
    {
      public:
        ...
        PolynomialND(const PolynomialND<Coef, Scalar>& p) :
            ...
            for(std::size_t i = 0; i <= p.order(); i++) _coef[i] = p[i];
        }
        ...
        void increaseOrder(std::size_t increase, const Coef& value) {
            ...
            for(std::size_t i = size; i < size + increase; i++) { _coef[i] = value; }
        }
#if !defined(SWIGJAVA)
        ...
        Coef evaluate(const Scalar& x) const {
            // Horner's Method
            Coef res = _coef.back();
            for(int i = static_cast<int>(_coef.size() - 2); i >= 0; i--) res = _coef[i] + res * x;
            return res;
        }
        ...
        std::vector<Coef> evaluateDerivatives(const Scalar& x, std::size_t n = 1) const {
            ...
            for(int i = static_cast<int>(_coef.size() - 2); i >= 0; i--) {
                ...
                for(int j = minJ; j > 0; j--) { res[j] = res[j - 1] + res[j] * x; }
        ...
        PolynomialND<Coef, Scalar> deflate(const Scalar& x) const {
            ...
            res[no] = _coef.back();
            for(int i = (int) no - 1; i >= 0; i--) { res[i] = x * res[i + 1] + _coef[i + 1]; }
            return res;
        }
        ...
        PolynomialND<Coef, Scalar> derivative(std::size_t n = 1) const {
            ...
            for(std::size_t i = 1; i <= order(); i++) der[i - 1] = (Coef) (_coef[i] * double(i));
...
///@}
        ...
        const PolynomialND<Coef, Scalar> operator*(Scalar s) const {
            ...
            for(std::size_t i = 0; i <= order(); i++) { pol[i] = _coef[i] * s; }
            return pol;
        }
        ...
        PolynomialND<Coef, Scalar>& operator*=(Scalar s) {
            for(std::size_t i = 0; i <= order(); i++) { _coef[i] *= s; }
            return *this;
        }
        ...
        PolynomialND<OutCoef, Scalar> multiply(const Coef2& b) const {
            ...
            for(std::size_t i = 0; i <= order(); i++) { pol[i] = _coef[i] * b; }
            return pol;
        }
#if !defined(SWIGPYTHON)
        /**
        ...
        void operator=(const PolynomialND<Coef, Scalar>& b) {
            ...
            for(size_t i = 0; i <= b.order(); i++) _coef[i] = b[i];
        }
#endif
#if !defined(SWIG)
///@}
        ...
        const PolynomialND<Coef, Scalar> operator-() const {
            PolynomialND<Coef, Scalar> pol(order());
            for(std::size_t i = 0; i <= order(); i++) { pol[i] = (Coef) (_coef[i] * double(-1)); }
            return pol;
        }
#if !defined(SWIG)
        /**
...
#endif
    PolynomialND<Eigen::Vector3f, float> operator*(const PolynomialND<Eigen::Matrix3f, float>& A,
...
#endif
    PolynomialND<Eigen::Matrix<float, 1, 3>, float>
    operator*(const PolynomialND<Eigen::Matrix<float, 1, 3>, float>& a, const Eigen::Matrix3f& A);
