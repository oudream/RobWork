...
template<class T> Vector3D<T> angleAxis(const Rotation3D<T>& R) {
    ...
    if(fabs(angle - Pi) < eps) {    // Is the angle close to 180 degree
        ...
        std::size_t k = 0;
        if(std::fabs(axis[1]) > std::fabs(axis[0])) k = 1;
        ...
        if(k == 0) {
            T v1v2 = R(0, 1) + R(1, 0);
            T v1v3 = R(0, 2) + R(2, 0);
            if(v1v2 < 0.) values[1] = -values[1];
            if(v1v3 < 0.) values[2] = -values[2];
        }
        else if(k == 1) {
            T v1v2 = R(0, 1) + R(1, 0);
            T v2v3 = R(1, 2) + R(2, 1);
            if(v1v2 < 0.) values[0] = -values[0];
            if(v2v3 < 0.) values[2] = -values[2];
        }
        else if(k == 2) {
            T v1v3 = R(0, 2) + R(2, 0);
            T v2v3 = R(1, 2) + R(2, 1);
            if(v1v3 < 0.) values[0] = -values[0];
            if(v2v3 < 0.) values[1] = -values[1];
        }
...
EAA<T>::EAA(const rw::math::Vector3D<T>& v1, const rw::math::Vector3D<T>& v2) :
    ...
    if(fabs(dval - 1) < epsilon) {
        ...
        int idx = 0;
        if(fabs(v1(0)) > fabs(v1(1))) idx = 1;
        if(fabs(v1(idx)) > fabs(v1(2))) idx = 2;
        Vector3D<T> v3(0, 0, 0);
        v3(idx) = 1;
