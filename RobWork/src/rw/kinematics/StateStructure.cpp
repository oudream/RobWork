...
void StateStructure::addFrame(Frame::Ptr frame, Frame::Ptr parent) {
    ...
    if(frame == NULL) RW_THROW("Input frame must not be NULL!");
    if(parent == NULL) { parent = getRoot(); }

    RW_ASSERT(frame && parent);

    // and parent must exist in the tree, but not the frame
    if(has(frame.get())) { RW_THROW("The frame has already been added to the state structure!"); }
    ...
    // now add the state data of the frame
    if(frame.isShared()) { addDataInternal(frame); }
    else { addDataInternal(frame.get()); }
...
void StateStructure::addDAF(Frame::Ptr frame, Frame::Ptr parent) {
    ...
    // check if frame name is unique
    if(findFrame(frame->getName()) != NULL) RW_THROW("Frame name is not unique!");
...
void StateStructure::remove(rw::core::Ptr<StateData> data) {
    ...
    if(_frames[id] != NULL) {
        ...
        if(parent != NULL) parent->removeChild(_frames[id]);
...
void StateStructure::cleanup(int ID) {
    ...
    while(iter != _setups.end()) {
        if((*iter).getSharedPtr().use_count() > 1) { ++iter; }
        else {
        ...
        if(data != NULL) {
            if(data.getSharedPtr().use_count() == 1 || data->getID() == ID) {
                ...
                if(_frames[id] != NULL) {
                    ...
                    if(frame->getParent() != NULL) { frame->getParent()->removeChild(frame); }
                    _frames[id] = NULL;
                    data->setID(-1, NULL);
                }
...
Frame* StateStructure::findFrame(const std::string& name) const {
    ...
    if(pos == _frameIdxMap.end()) return NULL;
    int idx = pos->second;
    if(_allDatas[idx] == NULL) return NULL;
...
rw::core::Ptr<StateData> StateStructure::findData(const std::string& name) const {
    ...
    if(pos == _stateIdxMap.end()) return rw::core::Ptr<StateData>();
    int idx = pos->second;
