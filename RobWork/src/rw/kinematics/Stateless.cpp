...
void Stateless::registerIn(StateStructure::Ptr state) {
    if(_registered) RW_THROW("Stateless Object hass allready been initialized to another state!");
...
void Stateless::unregister() {
    ...
    for(size_t i = 0; i < _datas.size(); i++) { _stateStruct->remove(_datas[i].get()); }
    _registered  = false;
    _stateStruct = NULL;
}
