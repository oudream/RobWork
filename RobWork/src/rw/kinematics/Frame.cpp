...
Frame* Frame::getParent(const State& state) {
    ...
    if(f1) return f1.get();
    else return getDafParent(state);
}
...
std::vector<Frame::Ptr> Frame::getChildrenList(const State& state) {
    ...
    for(Frame*& child : _children) { allChildren.push_back(Frame::Ptr(child)); }

    for(size_t i = 0; i < children.size(); ++i) { allChildren.push_back(Frame::Ptr(children[i])); }
    ...
    if(StateData::operator==(rhs) && this->_parent == rhs._parent &&
       this->_children.size() == rhs._children.size()) {
        for(size_t i = 0; i < this->_children.size(); i++) {
            if(this->_children[i] != rhs._children[i]) { return false; }
        }
        return true;
    }
    return false;
