...
#define DEBUGL(ostreamExpression) \
    do { std::cout << ostreamExpression << std::endl; } while(0)
...
WorkCell::Ptr ColladaLoader::loadWorkCell(const std::string& filename) {
    ...
    xercesc::DOMDocument* doc =
        XercesDocumentReader::readDocument(parser, filename, _schemaFileName);
...
WorkCell::Ptr ColladaLoader::loadWorkCell(std::istream& instream) {
    ...
    XercesDOMParser parser;
    xercesc::DOMDocument* doc =
        XercesDocumentReader::readDocument(parser, instream, _schemaFileName);
    DOMElement* elementRoot = doc->getDocumentElement();
    readColladaWorkCell(elementRoot);
    return _workcell;
}
  ...
  public:
    ...
    struct ChildIterator
    {
        ...
        bool operator==(const ChildIterator& other) const {
            return _pos == other._pos && children == other.children;
        }
    ...
    iterator begin() { return ChildIterator(0, children); }

    iterator end() { return ChildIterator(children->getLength(), children); }
...
class DOMElemChildVector
{
  public:
    ...
    static DOMElemChildVector make(DOMElement* parent) { return DOMElemChildVector(parent); }
    ...
    struct ChildIterator
    {
         ...
         * @brief Reference to the T element
         */
        DOMElement* operator*() const { return current; }
         ...
         */
        ChildIterator& operator++() {
            if(current != NULL) current = current->getNextElementSibling();
            return *this;
        }
         ...
         */
        bool operator==(const ChildIterator& other) const {
            return current == other.current && _parent == other._parent;
        }
    ...
    iterator begin() { return ChildIterator(_parent, _parent->getFirstElementChild()); }

    iterator end() { return ChildIterator(_parent, NULL); }
...
bool getAttribBoolean(DOMElement* element, const std::string& name, bool defval) {
    ...
    if(XMLStr(val).str() == "") return defval;
    if(XMLStr(val).str() == "true") { return true; }
    return false;
}
...
double getAttribDouble(DOMElement* element, const char* str, double defVal) {
    ...
    if(val == NULL) { retVal = defVal; }
    else if(XMLStr(val).str() == "") { retVal = defVal; }
    else {
        try {
...
*/
void getArrayValue(std::string& val, const XMLCh* str) {
    val = XMLStr(str).str();
}
void getArrayValue(unsigned int& val, const XMLCh* str) {
    val = (unsigned int) (XMLDouble(str).getValue());
}
void getArrayValue(int& val, const XMLCh* str) {
    val = (int) (XMLDouble(str).getValue());
}
void getArrayValue(double& val, const XMLCh* str) {
    val = XMLDouble(str).getValue();
}
void getArrayValue(bool& val, XMLCh* str) {
    if(XMLString::equals(str, XMLStr("true").uni())) { val = true; }
    else { val = false; }
}

template<class T> void readNArray(DOMElement* elem, std::vector<T>& array, int n) {
    array.resize(n);
...
template<class T> Dae::Array<T>* readArray(DOMElement* elem, ColladaLoader::ParserState& state) {
...
struct ScopedPush
{
    ScopedPush(Dae::Data* d, ColladaLoader::ParserState& state) : _data(d), _state(state) {
        if(d->id == "" && d->sid == "") return;
        _state.push(d);
    }
    ~ScopedPush() {
        if(_data->id == "" && _data->sid == "") return;
        _state.pop();
    }
    Dae::Data* _data;
...
size_t getStride(std::vector<Dae::InputShared*>& inputs) {
    ...
    for(Dae::InputShared* input : inputs) {
        if(input->semantic == "TEXCOORD") { stride += 1; }
        else if(input->semantic == "VERTEX") { stride += 1; }
        else { RW_THROW("Unknown semantic!"); }
    }
    return stride;
}
...
Dae::Accessor* readAccessor(DOMElement* element, ColladaLoader::ParserState& state) {
    ...
    for(DOMElement* child : domChildren) {
        ...
        if(isName(child, "param")) { accessor->params.push_back(readParam(child, state)); }
    }
...
Dae::Source* readSource(DOMElement* element, ColladaLoader::ParserState& state) {
    ...
    for(DOMElement* child : domChildren) {
        ...
        if(isName(child, "bool_array")) { source->boolArray = readArray<bool>(child, state); }
        else if(isName(child, "float_array")) {
            source->floatArray = readArray<double>(child, state);
        }
        else if(isName(child, "int_array")) { source->intArray = readArray<int>(child, state); }
            ...
            DOMElement* achild = dynamic_cast<DOMElement*>(
                child->getElementsByTagName(XMLStr("accessor").uni())->item(0));
            if(achild != NULL) source->accessor = readAccessor(achild, state);
        }
        else if(isName(child, "technique")) { RW_WARN("NOT IMPLEMENTED YET!"); }
    }
    return source;
}
...
Dae::Vertices* readVertices(DOMElement* element, ColladaLoader::ParserState& state) {
    ...
    for(DOMElement* child : domChildren) {
        ...
        if(isName(child, "input")) { vertices->inputs.push_back(readInput(child, state)); }
        else if(isName(child, "extra")) { vertices->extras.push_back(readExtra(child, state)); }
        else { RW_WARN("NOT IMPLEMENTED YET!"); }
    }
    return vertices;
}
...
Dae::Triangles* readTriangles(DOMElement* element, ColladaLoader::ParserState& state) {
    ...
    for(DOMElement* child : domChildren) {
        ...
        if(isName(child, "input")) { tris->inputs.push_back(readInputShared(child, state)); }
...
Dae::Mesh* readMesh(DOMElement* element, ColladaLoader::ParserState& state) {
    ...
    for(DOMElement* child : domChildren) {
        ...
        if(isName(child, "source")) { mesh->sources.push_back(readSource(child, state)); }
        else if(isName(child, "vertices")) { mesh->vertices = readVertices(child, state); }
        else if(isName(child, "lines")) { RW_WARN("NOT IMPLEMENTED YET!"); }
        else if(isName(child, "linestrips")) { RW_WARN("NOT IMPLEMENTED YET!"); }
        else if(isName(child, "polygons")) {
            // mesh->.push_back( readPolygons(child, state));
            RW_WARN("NOT IMPLEMENTED YET!");
        }
        else if(isName(child, "polylist")) { RW_WARN("NOT IMPLEMENTED YET!"); }
        else if(isName(child, "triangles")) { mesh->tris.push_back(readTriangles(child, state)); }
        else if(isName(child, "trifans")) { RW_WARN("NOT IMPLEMENTED YET!"); }
        else if(isName(child, "tristrips")) {
            /// TODO: implement parsing of all primitives
            RW_WARN("NOT IMPLEMENTED YET!");
        }
        else if(isName(child, "extra")) { mesh->extras.push_back(readExtra(child, state)); }
        else { RW_WARN("NOT IMPLEMENTED YET!"); }
    }
    return mesh;
}
...
Dae::InstanceEffect* readInstanceEffect(DOMElement* element, ColladaLoader::ParserState& state) {
    ...
    for(DOMElement* child : domChildren) {
        DEBUGL(XMLStr(child->getNodeName()).str());
        if(isName(child, "technique_hint")) { RW_WARN("NOT IMPL YET"); }
        else if(isName(child, "setparam")) { RW_WARN("NOT IMPL YET"); }
        else if(isName(child, "extra")) { mat->extras.push_back(readExtra(child, state)); }
    }
...
Dae::Material* readMaterial(DOMElement* element, ColladaLoader::ParserState& state) {
    ...
    for(DOMElement* child : domChildren) {
        ...
        if(isName(child, "asset")) { mat->asset = readAsset(child, state); }
...
Dae::InstanceMaterial* readInstanceMaterial(DOMElement* element,
                                            ColladaLoader::ParserState& state) {
    ...
    for(DOMElement* child : domChildren) {
        ...
        else if(isName(child, "extra")) { iMat->extras.push_back(readExtra(child, state)); }
    }
    return iMat;
}
...
Dae::Render* readRender(DOMElement* element, ColladaLoader::ParserState& state) {
    ...
    for(DOMElement* child : domChildren) {
        ...
        else if(isName(child, "extra")) { render->extras.push_back(readExtra(child, state)); }
    }
...
Dae::EvaluateScene* readEvaluateScene(DOMElement* element, ColladaLoader::ParserState& state) {
    ...
    for(DOMElement* child : domChildren) {
        DEBUGL(XMLStr(child->getNodeName()).str());
        if(isName(child, "asset")) { evalScene->asset = readAsset(child, state); }
        else if(isName(child, "render")) { evalScene->renders.push_back(readRender(child, state)); }
        else if(isName(child, "extra")) { evalScene->extras.push_back(readExtra(child, state)); }
    }
...
Dae::Effect* readEffect(DOMElement* element, ColladaLoader::ParserState& state) {
    ...
    for(DOMElement* child : domChildren) {
        DEBUGL(XMLStr(child->getNodeName()).str());
        if(isName(child, "asset")) { effect->asset = readAsset(child, state); }
        else if(isName(child, "newparam")) { RW_WARN(""); }
        else if(isName(child, "extra")) { effect->extras.push_back(readExtra(child, state)); }
    }
    return effect;
}
...
struct Accessor
{
    ...
    int getVal(int i) { return _acc->offset + i * _acc->stride; }

    int getValIndex(const std::string& v) {
        int i = 0;
        for(Dae::Param* param : _acc->params) {
            if(param->name == v) return i;
            i++;
        }
...
struct AccessorFloat
{
    AccessorFloat(Dae::Source* source, std::vector<std::string> params) :
        _source(source), _params(params), _param_offsets(params.size(), 0),
        _floats(params.size(), 0) {
        if(source != NULL) {
    ...
    const std::vector<double>& get(int i) {
        ...
        for(size_t j = 0; j < _floats.size(); j++) _floats[j] = fdata[offset + _param_offsets[j]];
        return _floats;
    }

    template<class ARRAY_TYPE> void get(int i, ARRAY_TYPE& dst) {
        std::vector<double>& fdata = *_source->floatArray->data;
        int offset                 = _accessor.getVal(i);
        for(int j = 0; j < _floats.size(); j++) dst[j] = fdata[offset + _param_offsets[j]];
    }
...
struct TriIndiciesAccessor
{
    TriIndiciesAccessor(std::vector<Dae::InputShared*>& inputs_tmp) : inputs(inputs_tmp) {
        ...
        for(Dae::InputShared* input : inputs) {
            ...
            else { RW_WARN("unsupported semantic!"); }
        }

        std::cout << "STRIDE PER VERTEX: " << stride_per_vertex << "  " << vertOffset << std::endl;
    }
    int getIdx(int i) { return i * stride_per_vertex; }
    int getVertexIdx(int i) { return i * stride_per_vertex + vertOffset; }

    int getOffset(std::string semantic) {
        for(Dae::InputShared* input : inputs) {
            if(input->semantic == semantic) return input->offset;
        }
        return -1;
    }
...
Dae::Source* getSource(const std::string& semantic, std::vector<Dae::Input*>& inputs,
                       ColladaLoader::ParserState& state) {
    for(Dae::Input* input : inputs) {
        if(semantic == input->semantic) return state.get<Dae::Source>(input->source);
    }
    return NULL;
}
...
bool has(const std::string& semantic, std::vector<Dae::Input*>& inputs) {
    for(Dae::Input* input : inputs) {
        if(semantic == input->semantic) return true;
    }
    return false;
}
...
rw::graphics::Model3D::Ptr makeRWMesh(Dae::Mesh* mesh, ColladaLoader::ParserState& state) {
    ...
    for(Dae::Triangles* tri : mesh->tris) {
        ...
        if(posSource) std::cout << "POSITION SOURCE: " << posSource->id << std::endl;
        if(normSource) std::cout << "NORMAL SOURCE  : " << normSource->id << std::endl;
        if(texSource) std::cout << "TEXCOORD SOURCE  : " << texSource->id << std::endl;
        ...
        IndexedTriMeshN0<float, uint16_t>::TriangleArray* rwtris =
            new IndexedTriMeshN0<float, uint16_t>::TriangleArray();
        ...
        for(size_t i = 0; i < tri->count * 3; i += 3) {
            ...
            // std::cout << "Tri: " << indices[i+0] << " " << indices[i+1] << " " <<  indices[i+2]
            // << std::endl;
        }
        ...
        IndexedTriMeshN0<float, uint16_t>::Ptr mymesh =
            ownedPtr(new IndexedTriMeshN0<float, uint16_t>(rwvertices, rwnormals, rwtris));
...
Dae::Library<Dae::Camera>* ColladaLoader::readLibraryCameras(DOMElement* element,
                                                             ColladaLoader::ParserState& state) {
...
Dae::Node* ColladaLoader::readNode(DOMElement* element, ColladaLoader::ParserState& state) {
    ...
    for(DOMElement* child : domChildren) {
        ...
        if(isName(child, "asset")) { node->asset = readAsset(child, state); }
            ...
            transTmp->transform = Transform3D<>(Vector3D<>(arrayTmp[3], arrayTmp[7], arrayTmp[11]),
                                                Rotation3D<>(arrayTmp[0],
                                                             arrayTmp[1],
                                                             arrayTmp[2],
                                                             arrayTmp[4],
                                                             arrayTmp[5],
        ...
        else if(isName(child, "skew")) { RW_WARN("NOT IMPLEMENTED!"); }
        ...
        else if(isName(child, "instance_controller")) { RW_WARN("NOT IMPLEMENTED YET!"); }
        ...
        else if(isName(child, "instance_light")) { RW_WARN("NOT IMPLEMENTED YET!"); }
        else if(isName(child, "instance_node")) {
            node->inodes.push_back(readInstanceNode(child, state));
        }
        else if(isName(child, "node")) { node->nodes.push_back(readNode(child, state)); }
        else if(isName(child, "extra")) { node->extras.push_back(readExtra(child, state)); }
    }
...
Dae::Geometry* ColladaLoader::readGeometry(DOMElement* element, ColladaLoader::ParserState& state) {
    ...
    for(DOMElement* child : domChildren) {
        DEBUGL(XMLStr(child->getNodeName()).str());
        if(isName(child, "asset")) { geom->asset = readAsset(child, state); }
        else if(isName(child, "convex_mesh")) { RW_WARN("NOT IMPLEMENTED YET!"); }
        else if(isName(child, "mesh")) {
            // TODO: do something with mesh
            geom->meshes.push_back(readMesh(child, state));
        }
        else if(isName(child, "spline")) { RW_WARN("NOT IMPLEMENTED YET!"); }
        else if(isName(child, "brep")) {
            /// TODO: brep, spline and convex_mesh
            RW_WARN("NOT IMPLEMENTED YET!");
        }
        else if(isName(child, "extra")) { geom->extras.push_back(readExtra(child, state)); }
    }

    return geom;
}

Dae::Library<Dae::Geometry>*
ColladaLoader::readLibraryGeometries(DOMElement* element, ColladaLoader::ParserState& state) {
    ...
    for(DOMElement* child : domChildren) {
        DEBUGL(XMLStr(child->getNodeName()).str());
        if(isName(child, "asset")) { geomLib->asset = readAsset(child, state); }
        else if(isName(child, "geometry")) {
            geomLib->elements.push_back(readGeometry(child, state));
        }
        else if(isName(child, "extra")) { geomLib->extras.push_back(readExtra(child, state)); }
    }
    return geomLib;
}
...
Dae::VisualScene* ColladaLoader::readVisualScene(xercesc::DOMElement* element, ParserState& state) {
    ...
    for(DOMElement* child : domChildren) {
        DEBUGL(XMLStr(child->getNodeName()).str());
        if(isName(child, "asset")) { scene->asset = readAsset(child, state); }
        else if(isName(child, "node")) { scene->nodes.push_back(readNode(child, state)); }
        else if(isName(child, "evaluate_scene")) {
            scene->evaluateScenes.push_back(readEvaluateScene(child, state));
        }
        else if(isName(child, "extra")) { scene->extras.push_back(readExtra(child, state)); }
    }
    return scene;
}

Dae::Library<Dae::VisualScene>*
ColladaLoader::readLibraryVisualScenes(DOMElement* element, ColladaLoader::ParserState& state) {
    ...
    for(DOMElement* child : domChildren) {
        DEBUGL(XMLStr(child->getNodeName()).str());
        if(isName(child, "asset")) { visualSceneLib->asset = readAsset(child, state); }
...
Dae::Library<Dae::Effect>* ColladaLoader::readLibraryEffects(DOMElement* element,
                                                             ColladaLoader::ParserState& state) {
    ...
    for(DOMElement* child : domChildren) {
        DEBUGL(XMLStr(child->getNodeName()).str());
        if(isName(child, "asset")) { matLib->asset = readAsset(child, state); }
        else if(isName(child, "effects")) { matLib->elements.push_back(readEffect(child, state)); }
        else if(isName(child, "extra")) { matLib->extras.push_back(readExtra(child, state)); }
    }
    return matLib;
}

Dae::Library<Dae::Material>*
ColladaLoader::readLibraryMaterials(DOMElement* element, ColladaLoader::ParserState& state) {
...
Dae::InstanceVisualScene*
ColladaLoader::readInstanceVisualScene(DOMElement* element, ColladaLoader::ParserState& state) {
    ...
    for(Dae::Node* node : vscene->nodes) {
        ...
        // nodes need to be mapped to frames, but in RobWork these frames would either be Fixed,
        // Movable or Joints. for now we use FixedFrame
        ...
        if(node->igeometries.size() > 0) {
            ...
            for(Dae::InstanceGeometry* igeom : node->igeometries) {
                ...
                for(Model3D::Ptr g : rwgeom) {
                    ...
                    if(getScene())
                        getScene()->addModel3D(
                            geom->name, g, frame);    // TODO: which layers should it be visible on
                }
            }
        ...
        // follow child nodes
        for(Dae::InstanceNode* child : node->inodes) { RW_WARN("Not impl yet!"); }
    }

    DOMElemChildVector domChildren(element);
    for(DOMElement* child : domChildren) {
        DEBUGL(XMLStr(child->getNodeName()).str());
        if(isName(child, "extra")) { data->extras.push_back(readExtra(child, state)); }
    }
...
void ColladaLoader::readScene(DOMElement* element, ColladaLoader::ParserState& state) {
    ...
    for(DOMElement* child : domChildren) {
        DEBUGL(XMLStr(child->getNodeName()).str());
        if(isName(child, "instance_physics_scene")) { RW_WARN("NOT IMPLEMENTED YET!"); }
        else if(isName(child, "instance_visual_scene")) { readInstanceVisualScene(child, state); }
        else if(isName(child, "instance_kinematics_scene")) { RW_WARN("NOT IMPLEMENTED YET!"); }
        else if(isName(child, "extra")) {
...
void ColladaLoader::readCollada(DOMElement* element, ColladaLoader::ParserState& state) {
    ...
    for(DOMElement* child : domChildren) {
        DEBUGL(XMLStr(child->getNodeName()).str());
        if(isName(child, "asset")) { state.data.asset = readAsset(child, state); }
        else if(isName(child, "library_cameras")) { readLibraryCameras(child, state); }
        else if(isName(child, "library_lights")) { RW_WARN("NOT IMPLEMENTED YET!"); }
        ...
        else if(isName(child, "extra")) { RW_WARN("NOT IMPLEMENTED YET!"); }
...
void ColladaLoader::readColladaWorkCell(DOMElement* element) {
    ...
    if(getScene() != NULL) getScene()->setWorkCell(collada.wc);
    // potentially make a WorkCellScene such as to avoid
    // std::vector<PluginFactory<T>::Ptr> factories =
    // SceneGraphRobWork::getInstance()->getPluginRepository().getPlugins<rw::graphics::SceneGraph>();

    if(isName(element, "COLLADA")) { readCollada(element, collada); }
