...
namespace rw { namespace graphics {
    ...
    class IvgVec3d
    {
      public:
        ...
        IvgVec3d(double x, double y, double z) {
            _v[0] = x;
            _v[1] = y;
            _v[2] = z;
        }
        ...
        inline const IvgVec3d operator-() const { return IvgVec3d(-_v[0], -_v[1], -_v[2]); }
    ...
    class CIVGBody : public CIVGNode
    {
      ...
      private:
        ...
        CIVGBody(const CIVGBody& t) { nSubCount = t.nSubCount; }
    ...
    class CIVGPoly : public CIVGNode
    {
      ...
      private:
        // Prefer 'private'.
        CIVGPoly(const CIVGPoly& t) { nBoundaries = t.nBoundaries; }
    ...
    class CIVGTess : public CIVGNode
    {
      public:
        CIVGTess() :
            nVertices(0), pVertices(0), nTriangles(0), pTriangles(0), nNormals(0), pNormals(0) {}
    ...
    {
      public:
        CIVGCone() : bShowBottom(false), dBottomRadius(0.0), bShowTop(false), dTopRadius(0.0) {}
    ...
    {
      public:
        CIvgEntity() : cName(), nListSize(0), pFirst(0), pLast(0) {}
...
Model3D::Ptr LoaderIVG::load(const std::string& filename) {
    FILE* fp;
    if(!(fp = fopen(filename.c_str(), "rb"))) { RW_THROW("Couldn't open '" << filename << "'"); }
...
int IVGReader::ParseIVGGeometry(char* pArray, std::list<CIvgEntity>& pScene) {
    ...
    if((version = IVGCheckHeader(&pIdx)) == -1) return -1;    // no valid ivg file
        ...
        for(int m = 0; m < nSubCount; m++)    // for each sub-entity
        {
            ...
            else    // add the type information, for picking feedback etc..
            {
                ...
                if(nSubType == IVG_TYPE_POLY) {
                    //				MessageBox(NULL,_T("Polyformat not supported "),_T("RView
                    //Warning"),MB_OK);
                }
                else if(nSubType == IVG_TYPE_TESS) { ivgNode = AddTess(&pIdx); }
                else if(nSubType == IVG_TYPE_WIRE) { ivgNode = AddCurve(&pIdx); }
                else if(nSubType == IVG_TYPE_SHADED_WIRE) { ivgNode = AddCurve(&pIdx); }
                else if(nSubType == IVG_TYPE_SPHERE) { ivgNode = AddSphere(&pIdx); }
                else if(nSubType == IVG_TYPE_CONE) { ivgNode = AddCone(&pIdx); }
                else {
                    //				MessageBox(NULL,_T("Wrong ivg format"),_T("RView
                    //Error"),MB_OK);
                    return -5;
                }
...
int IVGReader::IVGCheckHeader(char** pArray) {
    ...
    for(int i = 0; i < 7; i++) { header[i] = (*pArray)[i]; }
    cVersion[0] = (*pArray)[4];
    cVersion[1] = (*pArray)[6];
    cVersion[2] = 0;
    ...
    if(!strcmp(header, "IVG#")) {
        if(!((version == 20) || (version == 21) || (version == 30))) { version = -1; }
        else *pArray += sizeof(char) * 7;
    }
    else { version = -1; }
...
CIVGCone* IVGReader::AddCone(char** ppIdx) {
    ...
    *ppIdx += sizeof(char);
    ...
    if(showBottom) ivgCone->bShowBottom = true;
    else ivgCone->bShowBottom = false;

    if(showTop) ivgCone->bShowTop = true;
    else ivgCone->bShowTop = false;
