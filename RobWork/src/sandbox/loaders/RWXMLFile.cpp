...
{
  public:
    bool isFrameInDevice(Frame* frame) { return frameToDevice.find(frame) != frameToDevice.end(); }

    bool isFrameInDevice(Frame* frame, Device* dev) {
        if(isFrameInDevice(frame)) return frameToDevice[frame] == dev;
        return false;
    }
        ...
        for(Device::Ptr devp : devices) {
            ...
            for(Frame* dframe : dframes) {
                ...
                if(fname.length() > dname.length()) {
                    if(dname == fname.substr(0, dname.length())) { frameToDevice[dframe] = dev; }
                }
            }

            if(TreeDevice* tdev = dynamic_cast<TreeDevice*>(dev)) {
                for(Frame* endf : tdev->getEnds()) isEndEffector[endf] = true;
            }
            else { isEndEffector[dev->getEnd()] = true; }
...
std::string scopedName(DummyWorkcell& dwc, Frame* frame) {
    if(dwc.isFrameInDevice(frame)) { return scopedName(dwc.frameToDevice[frame], frame); }
...
std::string getDeviceType(Device& dev) {
    if(dynamic_cast<SerialDevice*>(&dev)) { return std::string("SerialDevice"); }
    else if(dynamic_cast<TreeDevice*>(&dev)) { return std::string("TreeDevice"); }
    else if(dynamic_cast<ParallelDevice*>(&dev)) { return std::string("ParallelDevice"); }
    else if(dynamic_cast<MobileDevice*>(&dev)) { return std::string("MobileDevice"); }
    else { RW_THROW("unknown device type!"); }
    return "";
}
...
void writeTransform(Transform3D<> t3d, std::ostream& ostr) {
    ...
    Vector3D<> p = t3d.P();
    ostr << wlvl();
    ostr << "<RPY>" << rpy(0) * Rad2Deg << " " << rpy(1) * Rad2Deg << " " << rpy(2) * Rad2Deg
         << "</RPY> ";
    ostr << "<Pos>" << p(0) << " " << p(1) << " " << p(2) << "</Pos>\n";
}

void writeRevoluteJoint(DummyWorkcell& dwc, RevoluteJoint& joint, std::ostream& ostr) {
    ostr << wlvl() << "<Joint name=\"" << scopedName(dwc, &joint) << "\" refframe=\""
         << scopedName(dwc, &joint, joint.getParent(dwc.state)) << "\" type=\"Revolute\">\n";
...
void writePrismaticJoint(DummyWorkcell& dwc, PrismaticJoint& joint, std::ostream& ostr) {
    ostr << wlvl() << "<Joint name=\"" << scopedName(dwc, &joint) << "\" refframe=\""
         << scopedName(dwc, &joint, joint.getParent(dwc.state)) << "\" type=\"Prismatic\">\n";
    clvl++;
    writeTransform(joint.getTransform(dwc.state), ostr);
    Q pmin = joint.getBounds().first;
    Q pmax = joint.getBounds().second;
...
void writeDependentRevoluteJoint(DummyWorkcell& dwc, DependentRevoluteJoint& joint,
                                 std::ostream& ostr) {
    ostr << wlvl() << "<Joint name=\"" << scopedName(dwc, &joint) << "\" refframe=\""
         << scopedName(dwc, &joint, joint.getParent(dwc.state)) << "\" type=\"Revolute\">\n";
    ...
    ostr << wlvl() << "<Depend on\"" << scopedName(dwc, &joint.getOwner(), &joint) << "\" gain=\""
         << joint.getScale() << "\" offset=\"" << joint.getOffset() * Rad2Deg << "\" />\n";
    clvl--;
    ostr << wlvl() << "</Joint>\n";
}

void writeDependentPrismaticJoint(DummyWorkcell& dwc, DependentPrismaticJoint& joint,
                                  std::ostream& ostr) {
    ostr << wlvl() << "<Joint name=\"" << scopedName(dwc, &joint) << "\" refframe=\""
         << scopedName(dwc, &joint, joint.getParent(dwc.state)) << "\" type=\"Prismatic\">\n";
    clvl++;
    writeTransform(joint.getFixedTransform(), ostr);
    ostr << wlvl() << "<Depend on\"" << scopedName(dwc, &joint.getOwner(), &joint) << "\" gain=\""
         << joint.getScale() << "\" offset=\"" << joint.getOffset() << "\" />\n";
    clvl--;
    ostr << wlvl() << "</Prismatic>\n";
}
...
void writeFixedFrame(DummyWorkcell& dwc, FixedFrame& frame, std::ostream& ostr) {
    ostr << wlvl() << "<Frame name=\"" << scopedName(dwc, &frame) << "\" refframe=\""
         << scopedName(dwc, &frame, frame.getParent(dwc.state)) << "\">\n";
...
void writeMovableFrame(DummyWorkcell& dwc, MovableFrame& frame, std::ostream& ostr) {
    ostr << wlvl() << "<Frame name=\"" << scopedName(dwc, &frame) << "\" refframe=\""
         << scopedName(dwc, &frame, frame.getParent(dwc.state)) << "\" type=\"Movable\">\n";
...
void writeFrame(DummyWorkcell& dwc, Frame& frame, std::ostream& ostr) {
    ...
    if(&frame == dwc.wc->getWorldFrame()) return;
    if(frame.getParent() == NULL) { RW_WARN("frame: " << frame.getName() << " is DAF!"); }

    if(FixedFrame* ff = dynamic_cast<FixedFrame*>(&frame)) { writeFixedFrame(dwc, *ff, ostr); }
    ...
    else { RW_THROW("unknown Frame type!"); }
}
/*
   void writeCollisionInfo(DummyWorkcell& dwc,CollisionModelInfo &info, Frame *frame, std::ostream
   &ostr){ std::stringstream sstr; sstr << scopedName(dwc, frame) << "Geo"; ostr << wlvl()
...
*/
/*
void writeDrawableInfo(DummyWorkcell& dwc,DrawableModelInfo &info, Frame *frame, std::ostream
&ostr){ std::stringstream sstr; sstr << scopedName(dwc, frame) << "Geo"; ostr << wlvl()
...
void writeDevice(DummyWorkcell& wc, Device& dev, std::ostream& ostr) {
    ...
    while(!frames.empty()) {
        ...
        for(Frame& child : frame->getChildren(wc.state)) {
            if(!wc.isFrameInDevice(&child, &dev)) continue;
            frames.push(&child);
        }
    }
    ostr << "\n";
...
void writeWorkcell(DummyWorkcell& dwc, std::ostream& ostr) {
    ...
    for(Frame* frame : dwc.frames) {
        if(dwc.isFrameInDevice(frame)) continue;
        if(dwc.isFrameParentToDevice(frame)) continue;
...
void RWXMLFile::saveWorkCell(rw::models::WorkCell& wc, const rw::kinematics::State& initState,
