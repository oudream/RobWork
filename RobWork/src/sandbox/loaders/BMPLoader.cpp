...
class CBitmap
{
  ...
  private:
    unsigned int ShiftRightByMask(unsigned int Color, unsigned int Mask,
                                  unsigned int DistributeToBits = 8) {
        if(Mask == 0) return 0;
        ...
        if(DistributeToBits > BitCount) {
            ...
            while(BitsToFill--) {
                BaseColor <<= 1;
                if(BaseColor & 1) { BaseColor |= 1; }
            }
        }
        else if(DistributeToBits < BitCount) { BaseColor >>= (BitCount - DistributeToBits); }
        return BaseColor;
    }
  ...
  public:
    ...
    CBitmap(char* Filename) :
        m_BitmapData(0), m_BitmapSize(0), m_ColorTableSize(0), m_ColorTable(0) {
        Load(Filename);
    }
    ...
    /* Copies internal RGBA buffer to user specified buffer and convertes into destination bit
     * format. Supported Bit depths are: 16 (5-5-5), 24, 32
     */
    ...
    /* See GetBits().
     * It creates a corresponding color table (palette) which have to be destroyed by the user after
     * usage. Supported Bit depths are: 4, 8 Todo: Optimize, use optimized palette, do ditehring
     */

    bool GetBitsWithPalette(void* Buffer, unsigned int& Size, unsigned int BitCount, BGRA*& Palette,
    ...
    void SetBits(void* Buffer, unsigned int Width, unsigned int Height, unsigned int RedMask,
                 unsigned int GreenMask, unsigned int BlueMask, unsigned int AlphaMask = 0) {
        unsigned char* BufferPtr = (unsigned char*) Buffer;
        ...
        while(BitCount) {
            if((RedMask | GreenMask | BlueMask | AlphaMask) & Test) { break; }
            Test >>= 1;
            BitCount--;
        }
    ...
    void SetAlphaBits(unsigned char Alpha) {
        for(int i = 0; i < m_BitmapSize; i++) { m_BitmapData[i].Alpha = Alpha; }
    }
};
}    // namespace
...
rw::sensor::ImagePtr ImageLoader::load(const std::string& filename) {
    ...
    if(!loadsucess) { RW_THROW("Error parsing file: " << filename); }
    ...
    if(p.maxgrayval < 256) coding = Image::MONO8;

    return ownedPtr(
        new Image((std::vector<unsigned char>*) output.release(), p.width, p.height, coding));
}
