...
#define DAE_DATA_TYPE(dae_type)                                                 \
    ...
    {                                                                           \
      public:                                                                   \
        ...
        dae_type(const std::string& id, const std::string& sid, Data* parent) : \
            Data(id, sid, parent) {}
...
template<class T> DAE_DATA_TYPE(Library) std::string name;
// Asset *asset;
...
template<class T> DAE_DATA_TYPE(Array) unsigned int count;
std::string name;
...
Data* getData(const std::string& url) {
    ...
    if(idToData.find(url) == idToData.end()) RW_THROW("Could Not find data: " << url);
    return idToData[url];
}

void addData(Data* data) {
    // std::cout << "ADDING DATA: " << data->id << " : " << data->sid << std::endl;
    if(data->id != "") idToData[data->id] = data;
}

void printAllData() {
    typedef std::pair<std::string, Data*> DataPair;
    std::cout << "\n All data saved so far:" << std::endl;
    for(const DataPair& idAndData : idToData) { std::cout << idAndData.first << std::endl; }
}
