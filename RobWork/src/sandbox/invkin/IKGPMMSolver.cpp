...
IKGPMMSolver::IKGPMMSolver(const rw::models::JointDevice* device,
    ...
    _controller(device, foi, state, makeQHome(device), DT), _device(device), _state(state),
    _maxQuatStep(0.8), _foi(foi), _fkranges(createFKRanges(device->getBase(), _foi, state)),
    _returnBestFit(false) {}

IKGPMMSolver::IKGPMMSolver(const rw::models::TreeDevice* device,
                           const rw::kinematics::State& state) :
    IterativeMultiIK(device->getEnds().size()),
    _controller(device, state, makeQHome(device), DT), _device(device), _state(state),
    _maxQuatStep(0.8), _foi(device->getEnds()),
    _fkranges(createFKRanges(device->getBase(), device->getEnds(), state)), _returnBestFit(false) {}

bool IKGPMMSolver::solveLocal(const std::vector<Transform3D<>>& bTed, std::vector<double>& maxError,
                              State& state, int maxIter, bool untilSmallChange) const {
    ...
    for(int cnt = 0; cnt < maxIterations; ++cnt) {
        ...
        if(lastError < error) {
            ...
            nrOfNonDiverges++;
            if(nrOfNonDiverges > 3) { nrOfDiverges = 0; }
        }
...
std::vector<Q> IKGPMMSolver::solve(const std::vector<Transform3D<>>& bTeds,
...

{
    ...
    for(int step = 1; step < steps; step++) {
        ...
        if(!solveLocal(bTedVia, maxTmpError, state, maxIterations, true)) return result;
    }
    ...
    else if(_returnBestFit) { result.push_back(_device->getQ(state)); }
    return result;
}
