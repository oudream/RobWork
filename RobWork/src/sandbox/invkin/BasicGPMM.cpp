...
BasicGPMM::BasicGPMM(const rw::models::TreeDevice* device, const rw::kinematics::State& state,
                     const rw::math::Q& qhome, double dt) :
    _device(device),
    _foi(device->getEnds()), _fkranges(createFKRanges(device->getBase(), _foi, state)),
    _state(state), _qhome(qhome), _dof((int) device->getDOF()), _dt(dt),
    _P(MatrixXd::Identity(6 * _foi.size(), 6 * _foi.size())), _space(BaseFrame) {
...
BasicGPMM::BasicGPMM(const rw::models::JointDevice* device,
    ...
    _device(device),
    _foi(foi), _fkranges(createFKRanges(device->getBase(), _foi, state)), _state(state),
    _qhome(qhome), _dof((int) device->getDOF()), _dt(dt),
    _P(MatrixXd::Identity(6 * _foi.size(), 6 * _foi.size())), _space(BaseFrame) {
    _jacCalc = _device->baseJCframes(_foi, state);
...
Q BasicGPMM::solve(const Q& q, const Q& dq,
                   const std::vector<rw::math::VelocityScrew6D<>>& tcpvellist) {
    ...
    if(_space == ControlFrame) {
        ...
        for(size_t i = 0; i < _foi.size(); i++) {
            ...
            // matrix_range<matrix<double> > rot1(R, range(0+offset,3+offset),
            // range(0+offset,3+offset)); rot1 = rot.m(); matrix_range<matrix<double> > rot2(R,
            // range(3+offset,6+offset), range(3+offset,6+offset)); rot2 = rot.m();
        }
        P = _P * R;
    }
    else { P = _P; }
    ...
    for(int i = 0; i < dim; i++) {
        ...
        for(int j = 0; j < 6; j++) tcpscrew(j + i * 6) = screw(j);
    }
...
Q BasicGPMM::applyJointVelocityConstraint(const Q& q, const Q& dq, const Q& dqnew) {
    ...
    double scale = 1;
    for(int i = 0; i < _dof; i++) {
        if(dqnew(i) > upper(i)) scale = std::max(scale, upper(i) / dqnew(i));
        if(dqnew(i) < lower(i)) scale = std::max(scale, lower(i) / dqnew(i));
    }
    return dqnew * scale;
}

void BasicGPMM::calculatePosAndVelLimits(VectorXd& lower, VectorXd& upper, const Q& q,
                                         const Q& dq) {
    Q joint_pos = q;
    Q joint_vel = dq;
    ...
    for(int i = 0; i < _dof; i++) {
        ...
        if(upper(i) < lower(i)) { lower(i) = upper(i); }
    }
}
