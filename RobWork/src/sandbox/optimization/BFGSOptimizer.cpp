...
namespace rwlibs { namespace optimization {
    ...
    BFGSOptimizer::BFGSOptimizer(typename FunctionType::Ptr function, double stepSize,
                                 StopCondition::Ptr stopCondition, LineSearch::Ptr strategy) :
        GradientOptimizer(function),
        _stepSize(stepSize), _lineSearchStopCondition(stopCondition) {
    ...
    void BFGSOptimizer::newOptimization(const VectorType& initialGuess, ResultType& initialValue,
                                        double& initialError) {
    ...
    void BFGSOptimizer::step(VectorType& currentGuess, ResultType& currentValue,
        ...
        VectorType newGuess = _lineSearch->optimize(currentGuess, *_lineSearchStopCondition);
    ...
    void BFGSOptimizer::updateHessian(const vector& sk, const vector& yk) {
        ...
        matrix leftTmp = matrix::Identity(n, n) - (sk * yk.transpose()) / (yk.transpose() * sk);

        // calculate right temporary
        matrix rightTmp = matrix::Identity(n, n) - (yk * sk.transpose()) / (yk.transpose() * sk);
