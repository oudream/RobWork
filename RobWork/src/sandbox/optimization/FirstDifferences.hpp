...
namespace rw { namespace math {
    ...
    namespace { /* anonymous namespace */
        ...
        GRAD_T doFirstDifferences(typename Function<RES_T, ARG_T>::Ptr func, ARG_T q, double step) {
            GRAD_T diffs(q.size());
            ...
            for(unsigned i = 0; i < q.size(); ++i) {
                ...
                diffs(i) = (1.0 / step) * (func->f(q + 0.5 * step1) - func->f(q - 0.5 * step1));
            }
         ...
         */
        template<>
        double doFirstDifferences<double, double, double>(typename Function<>::Ptr func, double q,
                                                          double step) {
    ...
    class FirstDifferences : public NumericalDerivative<RES_T, ARG_T, GRAD_T>
    {
      ...
      public:
         ...
         */
        FirstDifferences(double step = 1.0) : _step(step) {}

        /**
         * @brief Calculates gradient using first differences.
         */
        virtual GRAD_T derivative(typename Function<RES_T, ARG_T>::Ptr function, ARG_T x) {
            GRAD_T gradient = doFirstDifferences<RES_T, ARG_T, GRAD_T>(function, x, _step);

            return gradient;
        }

        double getStepSize() const { return _step; }

        void setStepSize(double step) { _step = step; }
