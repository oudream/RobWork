...
namespace rwlibs { namespace optimization {
    ...
    class SimulatedAnnealingOptimizer : public Optimizer
    {
      ...
      public:
         ...
         * @brief Constructor.
         */
        SimulatedAnnealingOptimizer(typename FunctionType::Ptr function, double dev = 1.0,
                                    double T = 1.0, double dT = 0.01);

        //! Destructor.
        virtual ~SimulatedAnnealingOptimizer();

        //! Set deviation for calculating new states.
        void setDeviation(double dev) { _deviation = dev; }

        //! Set initial temperature.
        void setInitialTemperature(double T) { _initialT = T; }
      ...
      protected:
         ...
         */
        VectorType generateNewState(const VectorType& currentState, double deviation);

        /**
         * @brief Acceptance function.
         *
         * Calculates the probability of accepting a new state based on current energy, new state
         ...
         */
        double calculateAcceptanceProbability(ResultType e0, ResultType e1, double T);
