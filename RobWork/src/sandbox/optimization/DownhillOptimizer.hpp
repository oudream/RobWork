...
namespace rwlibs { namespace optimization {
    ...
    class DownhillOptimizer : public Optimizer
    {
      ...
      public:
         ...
         * @brief Constructor.
         */
        DownhillOptimizer(typename FunctionType::Ptr function, double simplexSize = 1.0,
                          const Parameters& parameters = {1.0, 2.0, -0.5, 0.5});

        //! Destructor.
        virtual ~DownhillOptimizer();

        //! Set starting simplex size.
        void setSimplexSize(double simplexSize) { _simplexSize = simplexSize; }

        //! Set Downhill optimizer parameters.
        void setParameters(const Parameters& parameters) { _parameters = parameters; }
