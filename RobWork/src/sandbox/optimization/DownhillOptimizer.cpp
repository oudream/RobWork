...
namespace rwlibs { namespace optimization {
    ...
    DownhillOptimizer::DownhillOptimizer(typename FunctionType::Ptr function, double simplexSize,
                                         const Parameters& parameters) :
        Optimizer(function),
        _simplexSize(simplexSize), _parameters(parameters) {}
    ...
    void DownhillOptimizer::newOptimization(const VectorType& initialGuess,
        ...
        for(Vertex& v : _vertices) { v.second = getFunction()->f(v.first); }
    ...
    void DownhillOptimizer::step(VectorType& currentGuess, ResultType& currentValue,
        ...
        Vertex reflected =
            std::make_pair(centroid + _parameters.kReflection * (centroid - v.first), 0.0);
        ...
        if(reflected.second < _vertices.front().second) {
            Vertex expanded =
                std::make_pair(centroid + _parameters.kExpansion * (centroid - v.first), 0.0);
            expanded.second = f->f(expanded.first);

            // if expanded point is better than reflected point, then assign v := expanded
            if(expanded.second < reflected.second) { v = expanded; }
            else { v = reflected; }
        }
        else {
            /*
            ...
            if(reflected.second < w.second) { v = reflected; }
            else {
                /*
                 * 4. Try contraction on the v vertex
                 */
                Vertex contracted =
                    std::make_pair(centroid + _parameters.kContraction * (centroid - v.first), 0.0);
                contracted.second = f->f(contracted.first);

                // if contracted point is better than v, assign v := contracted
                if(contracted.second < v.second) { v = contracted; }
                else {
                    ...
                    for(std::vector<Vertex>::iterator i = _vertices.begin() + 1;
                        ...
                        ++i) {
                        (*i) = std::make_pair(
                            best.first + _parameters.kReduction * (best.first - i->first), 0.0);
                    }
                }
            }
        }
         ...
         */
        for(Vertex& v : _vertices) { v.second = f->f(v.first); }
        std::sort(_vertices.begin(), _vertices.end(), vertexComp);
        ...
        /*
         * Error is calculated as the difference in values of best & worst points divided by their
         * distance
         */
        double dValue   = _vertices.back().second - _vertices.front().second;
        double distance = (_vertices.front().first - _vertices.back().first).norm2();
