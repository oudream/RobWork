...
namespace rwlibs { namespace optimization {
    ...
    Optimizer::Optimizer(typename FunctionType::Ptr function) :
        _function(function), _currentGuess(VectorType()), _currentValue(ResultType()),
        _stepCount(0), _currentError(std::numeric_limits<double>::max()), _logging(false) {}

    Optimizer::~Optimizer() {}

    VectorType Optimizer::optimize(VectorType initialGuess, const StopCondition& stopCondition) {
        ...
        while(!stopCondition.check(this)) {
            ...
            // log step
            if(_logging) {
                State state = {_stepCount, _currentGuess, _currentValue, _currentError};
