...
namespace rwlibs { namespace optimization {
    ...
    TaxiCabOptimizer::TaxiCabOptimizer(typename FunctionType::Ptr function, double stepSize,
                                       StopCondition::Ptr stopCondition, LineSearch::Ptr strategy) :
        DirectionSetOptimizer(function, stepSize, stopCondition, strategy),
        _dirIdx(0) {}

    TaxiCabOptimizer::~TaxiCabOptimizer() {}

    void TaxiCabOptimizer::newOptimization(const VectorType& initialGuess, ResultType& initialValue,
                                           double& initialError) {
        /*
