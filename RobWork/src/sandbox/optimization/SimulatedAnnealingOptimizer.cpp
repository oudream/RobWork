...
namespace rwlibs { namespace optimization {
    ...
    SimulatedAnnealingOptimizer::SimulatedAnnealingOptimizer(typename FunctionType::Ptr function,
                                                             double dev, double T, double dT) :
        Optimizer(function),
        _deviation(dev), _initialT(T), _dT(dT) {}

    SimulatedAnnealingOptimizer::~SimulatedAnnealingOptimizer() {}

    void SimulatedAnnealingOptimizer::newOptimization(const VectorType& initialGuess,
    ...
    void SimulatedAnnealingOptimizer::step(VectorType& currentGuess, ResultType& currentValue,
                                           double& currentError) {
        /*
        ...
        VectorType newState = generateNewState(previousGuess, _deviation * (_T / _initialT));
    ...
    VectorType SimulatedAnnealingOptimizer::generateNewState(const VectorType& currentState,
                                                             double deviation) {
    ...
    double SimulatedAnnealingOptimizer::calculateAcceptanceProbability(ResultType e0, ResultType e1,
                                                                       double T) {
        if(e1 < e0) { return 1.0; }
        else if(T == 0.0) { return 0.0; }
        else { return exp(-fabs(e0 - e1) / T); }
    }
