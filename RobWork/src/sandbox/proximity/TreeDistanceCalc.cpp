...
struct BVJob
{
  public:
    ...
    BVJob() : treeA(NULL), treeB(NULL), isAfromA(true) {}
...
class BalancedTreeDistanceCalc : public TreeDistanceCalc
{
  public:
    ...
    double distance(const rw::math::Transform3D<>& fTA, const CollisionModel& cmA,
        ...
        while(!empty()) {
            ...
            if(job.dist > sdistance) continue;

            RW_DEBUG("after Get JOB: " << _BVstackIdx);
            const OBB<>& cobbA = job.treeA->getOBB();
            Transform3D<> ATtB;
            if(job.isAfromA) { ATtB = inverse(cobbA.getTransform()) * tATtB; }
            else { ATtB = inverse(cobbA.getTransform()) * tBTtA; }
            RW_DEBUG("1: ");
            ...
            for(int i = 0; i < nrOfChildren; i++) {
                ...
                if(job.isAfromA)
                    ...
                    dists[i] = _bvdistcalc->calcDistance(
                        cobbB, cobbA, inverse(ATtB * cobbB.getTransform()));
            }
            ...
            for(int i = 0; i < nrOfChildren; i++) {
                ...
                if(dist[i] > sdistance) continue;
                if(child.isLeaf() && job.treeA->isLeaf()) {
                    // TODO: test primitives against each other
                }
                else if(job.treeA->isLeaf()) { push(BVJob(job.treeA, &child, job.isAfromA)); }
                else { push(BVJob(&child, job.treeA, !job.isAfromA)); }
            }
            ...
            for(int i = 0; i < nrOfChildren; i++) {
                ...
                if(child.isLeaf() && job.treeA->isLeaf()) {
                    ...
                    incollision |= true;
                    if(_firstContact && incollision) return true;
                    continue;
                }
                else if(job.treeA->isLeaf()) { push(BVJob(job.treeA, &child, job.isAfromA)); }
                else { push(BVJob(&child, job.treeA, !job.isAfromA)); }
