...
namespace rw { namespace proximity {
     ...
     *
     */
    template<class BVTYPE> class BVTree
    {
      public:
        ...
        struct nodeiterator
        {
            ...
            inline nodeiterator left() const {
                return nodeiterator(tree->getChildIdx(cidx, 0), tree);
            };
            inline nodeiterator right() const {
                return nodeiterator(tree->getChildIdx(cidx, 1), tree);
            };
        ...
        virtual ~BVTree() { delete _bvnodes; };

        inline int getN() { return _N; }

        inline int getTrisPerLeaf() const { return _trisPerLeaf; }

        inline int getRootIdx() const { return 0; }
        ...
        inline bool hasChildIdx(int parentIdx, int childNr) const { return !isLeafIdx(parentIdx); }

        inline bool has(int idx) const { return (0 <= idx) && (idx < (int) _bvnodes->size()); }

        inline BVNode& get(int idx) {
            if(has(idx)) return (*_bvnodes)[idx];
        ...
        inline BVNode& getChild(int parentIdx, int childNr) {
            if(hasChildIdx(parentIdx, childNr)) return (*_bvnodes)[getChildIdx(parentIdx, childNr)];
        ...
        inline BVNode* getChildren(int parentIdx) {
            if(hasChildIdx(parentIdx, 0)) return (*_bvnodes)[getChildIdx(parentIdx, 0)];
            return NULL;
        }
        ...
        */
        ...
        inline int* getTri(int idx) { return &((*_triIndexes)[idx]); }
        ...
        // static BVTree<T>* buildBalancedNaryTree(int n, const rw::geometry::IndexedTriMesh<T>&
        // imesh);

        /**
         * @brief creates a
         */
        BVTree(std::vector<BVNode>* bvnodes, int n, std::vector<int>* triIdx, int trisPerLeaf,
               int depth) :
            _bvnodes(bvnodes),
            _triIndexes(triIdx), _size(bvnodes->size()), _N(n), _trisPerLeaf(trisPerLeaf),
            _depth(depth),
