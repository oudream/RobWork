...
namespace rw { namespace proximity {
     ...
     *
     */
    template<class PRIM, class T = size_t> class IndexedPrimArray : public PrimArrayAccessor<PRIM>
    {
      private:
      ...
      public:
        ...
        IndexedPrimArray(rw::core::Ptr<PrimArrayAccessor<PRIM>> objArr) :
            _objArr(objArr), _idxArr(rw::core::ownedPtr(new std::vector<T>(objArr->getSize()))),
            _first(0), _last(objArr->getSize()) {
            for(size_t i = 0; i < _idxArr->size(); i++) (*_idxArr)[i] = i;
        }
        ...
        IndexedPrimArray(rw::core::Ptr<PrimArrayAccessor<PRIM>> objArr,
            ...
            _objArr(objArr),
            _idxArr(idxArr), _first(0), _last(idxArr->size()) {}
         ...
         */
        IndexedPrimArray(rw::core::Ptr<PrimArrayAccessor<PRIM>> objArr,
                         rw::core::Ptr<std::vector<T>> idxArr, size_t first, size_t last) :
            _objArr(objArr),
            _idxArr(idxArr), _first(first), _last(last) {}
      ...
      private:
        struct PrimIdxSort
        {
            PrimIdxSort(const int splitAxis, const rw::math::Transform3D<>& t3d,
            ...
            bool operator()(const size_t& i0, const size_t& i1) {
                ...
                Vector3D<PRIM::value_type> c1 =
                    _t3d * p2.calcCenter();    //((t1[0]+t1[1]+t1[2])/3.0);
      ...
      public:
        ...
        void sortAxis(int axis, const rw::math::Transform3D<>& t3d) {
            ...
            std::sort(&((*_idxArr)[_first]),
                      &((*_idxArr)[_first]) + size(),
                      PrimIdxSort(axis, t3d, *_objArr));
            /*if(size()<10){
                std::cout << "POST\n-- ";
                for(int i=0;i<size();i++)
                    std::cout << ", " << (*_idxArr)[i];
        ...
        PRIM operator[](size_t i) const { return _objArr->getPrimitive((*_idxArr)[_first + i]); }

        //! @copydoc TriMesh::getTriangle
        PRIM getPrimitive(size_t idx) const { return (*this)[idx]; }
