    ...
    class RANSACFitting
    {
      public:
        ...
        */
        ...
        static std::vector<MODEL_T> fit2(std::vector<DATA>& data, int k, int d, double t,
                                         double s) {
            using namespace rw::math;
            // std::cout << "Fitting data! " << data.size() << std::endl;
            int n =
                MODEL_T::getMinReqData();    // the minimum number of data required to fit the model
                                             /*
            ...
            while(++iterations < k) {
                ...
                try {
                    ...
                    if(maybeModel.invalid()) continue;
                }
                catch(...) {
                    continue;
                }
                // add the maybe inliers to the conensus set
                for(int i = 0; i < n; i++) { consensusSet.push_back(maybeInliers[i]); }

                // check if any point in data fits the model with an error smaller than t
                for(size_t i = 0; i < data.size(); i++) {
                    if(maybeModel.fitError(data[i]) < t) { consensusSet.push_back(data[i]); }
                }
            ...
            if(models.size() < 0) return std::vector<MODEL_T>();
            if(models.size() == 1) return std::vector<MODEL_T>(1, models[0].first);
            ...
            for(size_t i = 0; i < modelsPtr.size() - 1; i++) {
                if(modelsPtr[i].first == NULL) continue;
                ...
                for(size_t j = i + 1; j < modelsPtr.size(); j++) {
                    ...
                    if(modelsPtr[j].first == NULL) continue;
                ...
                if(bestCloseModel.first == NULL) continue;
