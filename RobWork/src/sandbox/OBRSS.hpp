...
namespace geometry {
     ...
     */
    template<class T = double> class OBRSS : public OBV<OBRSS<T>>
    {
      private:
      ...
      public:
         ...
         */
        OBRSS() :
            _t3d(rw::math::Transform3D<T>::identity()), _halfLng(rw::math::Vector3D<T>(0, 0, 0))
        ...
        T calcVolumne() const { return _halfLng(0) * 2 * _halfLng(1) * 2 * _halfLng(2) * 2; }
            ...
            for(int i = 0; i < nrOfTris; i++) {
                ...
                for(size_t j = 0; j < 3; j++)
                    for(size_t k = 0; k < 3; k++)
                        covar(j, k) += t[0](j) * t[0](k) + t[1](j) * t[1](k) + t[2](j) * t[2](k);
            }
...
template<typename T> struct Traits<geometry::OBRSS<T>>
{ typedef T value_type; };
