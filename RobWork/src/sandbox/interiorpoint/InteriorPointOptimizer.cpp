...
namespace internal {
void compute_f_info_EXT(const Eigen::VectorXd& x, double& f, Eigen::VectorXd& df,
                        Eigen::MatrixXd& ddf) {}

void compute_con_info_i_EXT(int i, const Eigen::VectorXd& x, Eigen::VectorXd& a,
...
InteriorPointOptimizer::InteriorPointOptimizer(size_t n, size_t m, ObjectFunction objectFunction,
                                               ConstraintFunction constraintFunction) :
    compute_f_info_EXT(objectFunction),
    compute_con_info_i_EXT(constraintFunction), N(n), M(m) {
    // compute_f_info_EXT = boost::bind(&InteriorPointOptimizer::objectFunction, this, _1, _2, _3,
    // _4); compute_con_info_i_EXT = boost::bind(&InteriorPointOptimizer::constraintFunction, this,
    // _1, _2, _3, _4, _5);
    initialize();
}
...
InteriorPointOptimizer::InteriorPointOptimizer(size_t n, size_t m) :
    ...
    compute_con_info_i_EXT(
        boost::bind(&InteriorPointOptimizer::constraintFunction, this, _1, _2, _3, _4, _5)),
    N(n), M(m) {
    initialize();
}
...
void InteriorPointOptimizer::objectFunction(const Eigen::VectorXd& x, double& f,
                                            Eigen::VectorXd& df, Eigen::MatrixXd& ddf) {
    RW_THROW("Object function undefined for InteriorPointMethod");
}

void InteriorPointOptimizer::constraintFunction(const Eigen::VectorXd& x, int i, Eigen::VectorXd& a,
                                                Eigen::MatrixXd& da, Eigen::MatrixXd& dda) {
...
void InteriorPointOptimizer::verify_user_defined_objective_and_constraints() {
    ...
    for(size_t j = 0; j < M; j++) {
        ...
        if(_a(j) <= 0) RW_THROW("Initial value: Violation of constraint ");
        else std::cout << "Constraint " << j << " has value " << _a(j) << std::endl;
    }
    delta = 0.0;
    h     = 0.00001;
    ...
    std::cout << "Objective. "
              << "First order:"
              << "h: " << h << " Rel. error:" << (ftest - _f) / (_f - fold) << std::endl;
    for(size_t jj = 0; jj < N; ++jj)
        for(size_t kk = 0; kk < N; ++kk) ftest += 0.5 * _ddf(jj, kk) * dxtest(jj) * dxtest(kk);
    std::cout << "Objective. "
              << "Second order:"
              << "h: " << h << " Rel. error:" << (ftest - _f) / (_f - fold) << std::endl;
    _x -= dxtest;
    ...
    for(size_t j = 0; j < M; j++) {
        ...
        compute_con_info_i_EXT(_x, j, _a, _da, _dda);
        atest = aold(j);
        for(size_t i = 0; i < N; i++) atest += daold(j, i) * dxtest(i);
        std::cout << "Constraint no. " << j << ". First order:"
                  << "h: " << h << " Rel. error:" << (atest - _a(j)) / (_a(j) - aold(j))
                  << std::endl;
        for(size_t jj = 0; jj < N; ++jj)
            for(size_t kk = 0; kk < N; ++kk) atest += 0.5 * _dda(jj, kk) * dxtest(jj) * dxtest(kk);
        std::cout << "Constraint no. " << j << ". Second order:"
                  << "h: " << h << " Rel. error:" << (atest - _a(j)) / (_a(j) - aold(j))
                  << std::endl;
        _x -= dxtest;
    }
...
void InteriorPointOptimizer::choleskySolve(int n_e, int bw, Eigen::MatrixXd& A, Eigen::VectorXd& b,
                                           Eigen::VectorXd& x) {
    ...
    for(int k = 0; k < n_e; ++k) {
        ...
        for(int ii = k + 1; ii < ist; ++ii) {
            jst = k - bw;
            if(jst < 0) jst = 0;
            for(int jj = jst; jj < k; ++jj) A(ii, k) -= A(ii, jj) * A(k, jj);
            A(ii, k) = A(ii, k) / A(k, k);
        }
    }
    ...
    for(int k = n_e - 1; k >= 0; --k) {
        ...
        ist  = k + bw + 1;
        if(ist > n_e) ist = n_e;
        for(int ii = k + 1; ii < ist; ++ii) x(k) -= A(ii, k) * x(ii);
        x(k) /= A(k, k);
    }
}
...
void InteriorPointOptimizer::compute_f_info(Eigen::MatrixXd& A, Eigen::VectorXd& RHS) {
    compute_f_info_EXT(_x, _f, _df, _ddf);
    for(size_t jj = 0; jj < N; ++jj) {
        for(size_t kk = 0; kk < N; ++kk) A(jj, kk) += _ddf(jj, kk);
        RHS(jj) -= _df(jj);
    }
}
...
// merit_info and differentiated merit_info implements the merit function given by Eq. 19.26 in
// Nocedal and Wright
void InteriorPointOptimizer::merit_info(Eigen::VectorXd& x, Eigen::VectorXd& s, double& phi,
                                        double& eta) {
    phi = 0;
    phi += _f;
    for(size_t i = 0; i < M; ++i) {
        if(_a(i) > s(i)) phi += eta * (_a(i) - s(i)) - _mu * log(s(i));
        else phi += eta * (s(i) - _a(i)) - _mu * log(s(i));
    }
}

void InteriorPointOptimizer::Dmerit_info(Eigen::VectorXd& x, Eigen::VectorXd& s,
                                         Eigen::VectorXd& dx, Eigen::VectorXd& ds, double& Dphi,
                                         double& eta) {
    Dphi = 0;
    for(size_t i = 0; i < N; ++i) Dphi += _df(i) * dx(i);

    for(size_t j = 0; j < M; ++j) {
        Dphi -= _mu * ds(j) / s(j);
        if(_a(j) > s(j)) {
            Dphi -= eta * ds(j);
            for(size_t i = 0; i < N; ++i) Dphi += eta * _da(j, i) * dx(i);
        }
        else {
            Dphi += eta * ds(j);
            for(size_t i = 0; i < N; ++i) Dphi -= eta * _da(j, i) * dx(i);
...
// trust region based update. Method used is Nocedal and Wright p. 572-577. NOTICS: Adaptive updates
// of eta for efficiency is still not implemented
void InteriorPointOptimizer::update(Eigen::VectorXd& x, Eigen::VectorXd& dx, Eigen::VectorXd& s,
                                    Eigen::VectorXd& z) {
    ...
    for(i = 0; i < M; ++i) {
        ds(i) = _a(i) - s(i);
        for(jj = 0; jj < N; ++jj) ds(i) += _da(i, jj) * dx(jj);
    ...
    for(size_t j = 0; j < M; ++j) compute_con_info_i_EXT(x, j, _a, _da, _dda);
    ...
    for(size_t j = 0; j < M; ++j) z(j) = _mu / s(j);
}
...
int InteriorPointOptimizer::solve(const Eigen::VectorXd& x_init) {
    ...
    std::cout << "Search finished. Result is " << _x << "The accuracy is at least " << sqrt(diff)
              << ". The number of LE solves was " << n_eq_solves << std::endl;
