    ...
    class AsciiPathSaver
    {
      public:
        ...
        struct Format
        {
            ...
            Format(std::string pre, std::string post, std::string numberSeparator,
                   std::string entrySeparator) :
                entryPreToken(pre),
                entryPostToken(post), numberSeparator(numberSeparator),
         ...
         */
        template<class T>
        static void save(std::ostream& outstream, const rw::trajectory::Path<T>& path,
                         const Format& format = AsciiPathSaver::CommaSeparated) {
            for(typename rw::trajectory::Path<T>::const_iterator it = path.begin();
                it != path.end();) {
                const T& t = (*it);
                ++it;
         ...
         */
        template<class T>
        static void save(const std::string& filename, const rw::trajectory::Path<T>& path,
                         const Format& format = AsciiPathSaver::CommaSeparated) {
            std::ofstream outstream(filename.c_str());
            if(!outstream.is_open()) RW_THROW("Unable to open file " << filename);
            save(outstream, path, format);
        }

      private:
        static void write(std::ostream& outstream, const rw::math::Rotation3D<>& r,
                          const Format& format, bool lastEntry) {
            outstream << format.entryPreToken << format.entryPreToken;
            for(size_t i = 0; i < 3; i++) {
                for(size_t j = 0; j < 3; j++) {
                    outstream << r(j, i);
                    if(j != 2 || i != 2) outstream << format.numberSeparator;
                }
                outstream << format.entryPostToken;
            }
            outstream << format.entryPostToken;
            if(!lastEntry) outstream << format.entrySeparator;
        }

        static void write(std::ostream& outstream, const rw::math::Transform3D<>& t,
                          const Format& format, bool lastEntry) {
            outstream << format.entryPreToken << format.entryPreToken;
            for(size_t i = 0; i < 4; i++) {
                for(size_t j = 0; j < 3; j++) {
                    outstream << t(j, i);
                    if(j != 2 || i != 3) outstream << format.numberSeparator;
                }
                outstream << format.entryPostToken;
            }
            outstream << format.entryPostToken;
        ...
        template<class T>
        static void write(std::ostream& outstream, const rw::trajectory::Timed<T>& q,
                          const Format& format, bool lastEntry) {
        ...
        template<class T>
        static void write(std::ostream& outstream, const T& q, const Format& format,
                          bool lastEntry) {
            outstream << format.entryPreToken;
            for(size_t i = 0; i < q.size(); i++) {
                outstream << q(i);
                if(i < q.size() - 1) outstream << format.numberSeparator;
            }
            outstream << format.entryPostToken;
            if(!lastEntry) outstream << format.entrySeparator;
        }
