...
SDHGraspPlanner2D::SDHGraspPlanner2D(const rw::models::TreeDevice& hand,
                                     rw::models::JointDevice& robot,
                                     rw::proximity::CollisionDetector& colDect,
                                     const rw::kinematics::State& state) :
    _hand(hand),
    _robot(robot), _graspGen(hand, state), _detector(&colDect),
    _hbTwrist(Kinematics::frameTframe(hand.getBase(), robot.getEnd(), state)), _clerance(0.03),
    _maxObjHeight(0.07), _minQuality(0.5), _graspHeight(0.04), _maxCurvature(0.05),
...
SDHGraspPlanner2D::SDHGraspPlanner2D(const rw::models::TreeDevice& hand,
                                     ...
                                     const rw::kinematics::State& state) :
    _hand(hand),
    _robot(robot), _graspGen(hand, state), _detector(NULL),
    _hbTwrist(Kinematics::frameTframe(hand.getBase(), robot.getEnd(), state)), _clerance(0.03),
    _maxObjHeight(0.07), _minQuality(0.5), _graspHeight(0.04), _maxCurvature(0.05),
    _maxAppAngle(8 * Deg2Rad) {}

void SDHGraspPlanner2D::setContour(const Contour2D& contour,
...
std::vector<SDHGraspPlanner2D::GraspResult> SDHGraspPlanner2D::query(const State& initState,
                                                                     int maxNrOfQs) {
    ...
    for(const Grasp2D& grasp : _grasps) {
        ...
        // make sure that the approach grasp is collision free, so test a few clerances before
        // discarding an approach grasp
        ...
        for(badGrasps = 0; badGrasps < MAX_BAD_GRASPS; badGrasps++) {
            ...
            if(ikMetaRes.size() == 0) continue;
            nrAppIkRobot++;
            break;
        }
        nrBadGrasps += badGrasps;
        if(badGrasps == MAX_BAD_GRASPS) continue;
