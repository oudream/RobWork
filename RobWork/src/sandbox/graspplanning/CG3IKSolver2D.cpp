...
std::pair<double, double> calcFingerConfig(const Vector3D<>& fbPftcp, const double a,
                                           const double b, const State& state,
                                           std::pair<Q, Q> bounds) {
    ...
    if(a + b - 0.005 < c) { return std::make_pair(-1.0, -1.0); }
    // std::cout << "Vector: " << fbPftcp2d << std::endl;
    const double acosTmp = (a * a + b * b - c * c) / (2 * a * b);
    if(acosTmp >= 1 || acosTmp <= -1) return std::make_pair(-1.0, -1.0);
    const double angleTip = acos(acosTmp);

    const double asinTmp = (sin(angleTip) * b) / c;
    if(asinTmp >= 1 || asinTmp <= -1) return std::make_pair(-1.0, -1.0);
    ...
    if(j3Angle < bounds.first(0) || bounds.second(0) < j3Angle) return std::make_pair(-1.0, -1.0);
...
double angleToPlane(Vector3D<> v, Vector3D<> n) {
    ...
    if(dotProd >= 1 || dotProd <= -1) return 0;
    return std::acos(dotProd) - Pi / 2;
}

}    // namespace

CG3IKSolver2D::CG3IKSolver2D(const rw::models::TreeDevice& device,
                             const rw::kinematics::State& state) :
    _device(device),
    _maxWristToFingerTip(0.18), _minWristHeight(0.06) {
...
CG3IKSolver2D::IKResult CG3IKSolver2D::solve(const Transform3D<>& bTobj, const Grasp2D& grasp,
    ...
    // TODO: could be nice if the pose was not too fixed... say changing the pose a bit could
    // perhaps generate better solutions
    ...
    for(double z = _maxWristToFingerTip; z > _minWristHeight; z -= 0.005) {
        // project c2 onto a1 and take the half to get the x value as the resulting length of the
        // vector
    ...
    if(semires.size() == 0) return result;
    int bestIdx = 0;
    for(size_t i = 1; i < semires.size(); i++) {
        if(semires[i].first < semires[bestIdx].first) bestIdx = i;
    }
