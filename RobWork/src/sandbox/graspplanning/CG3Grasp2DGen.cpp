...
int curvatureFilter(Contour2DInfoMap::ContactPtrList& clist, double scale, double curvthres) {
    int left = clist.size();
    // std::cout << "List Size: " << left << std::endl;
    for(size_t i = 0; i < clist.size(); i++) {
        if(clist[i] == NULL) { left--; }
        else if(fabs(clist[i]->avgCurvature) * scale > curvthres) {
            clist[i] = NULL;
            left--;
        }
        else {
            // std::cout << "Curvature: " << fabs(clist[i]->avgCurvature) << " " << scale << "<" <<
            // curvthres << std::endl;
        }
    }
    return left;
...
{
  public:
    int operator()(const GraspItem& p1, const GraspItem& p2) { return p1.second > p2.second; }
};

}    // namespace

CG3Grasp2DGen::CG3Grasp2DGen(const TreeDevice& hand, const rw::kinematics::State& state,
                             int thetaRes, bool counterclock) :
    _hand(hand),
    _state(state), _counterClock(counterclock), _thetaRes(thetaRes), _psiRes(100), _phiRes(30),
    _thetaStep(2 * Pi / thetaRes), _psiStep(2 * Pi / _psiRes), _phiStep((Pi / 2) / _phiStep),
    //_sqrCurvThres(0.03*0.00136124),
    //_sqrCurvThres(0.1),
    _sqrCurvThres(1), _acceptUniform(0.005), _acceptPerp(8 * Deg2Rad), _acceptDirs(0.05),
    _maxGraspWidth(0.18)
...
std::vector<Grasp2D> CG3Grasp2DGen::computeGrasps(const QualityMeasure2D& measure,
                                                  double minQuality,
    ...
    for(Grasp2D& candidate : _graspCandidates) {
        ...
        if(minQuality < quality) {
            ...
            if(queue.size() > nrOfGrasps) queue.pop();
        }
    ...
    std::cout << "CG3Grasp2DGen: grasps computed " << std::endl
              << " - Found best " << grasps.size() << " out of " << _graspCandidates.size()
              << " candidates " << std::endl
...
void CG3Grasp2DGen::init(const Contour2D& contour, int psiRes, int phiRes) {
    ...
    for(int psiIdx = 0; psiIdx < psiRes; psiIdx++) {
        ...
        if(thumList.size() == 0) continue;

        // remove any contacts from list that has bad curvature
        if(curvatureFilter(thumList, _sqrL, _sqrCurvThres) == 0) { continue; }
        ...
        for(int phiIdx = 0; phiIdx < phiRes; phiIdx++) {
            ...
            // the thumb
            Contour2DInfoMap::ContactPtrList f1List =
                _infoMap.getCNormals(psi + phi, _acceptPerp, _sqrCurvThresScale);
            if(curvatureFilter(f1List, _sqrL, _sqrCurvThres) == 0) { continue; }

            Contour2DInfoMap::ContactPtrList f2List =
                _infoMap.getCNormals(psi + Pi - phi, _acceptPerp, _sqrCurvThresScale);
            if(curvatureFilter(f2List, _sqrL, _sqrCurvThres) == 0) { continue; }

            // now comes the real filtering
            for(Contact2D* thumCon : thumList) {
                if(thumCon == NULL) continue;
                for(Contact2D* f1Con : f1List) {
                    if(f1Con == NULL) continue;

                    // constrain the distance between f1 and f2 contacts to L/2
                    const double thumToF1dist = (thumCon->p - f1Con->p).norm2();
                    if(thumToF1dist < _L / 2 || _maxGraspWidth < thumToF1dist) continue;

                    const Vector2D<> f1Dir = rotPsiPhi * _f1Dir;
                    for(Contact2D* f2Con : f2List) {
                        if(f2Con == NULL) continue;

                        // constrain the distance between f1 and f2 contacts to L/2
                        const double thumToF2dist = (thumCon->p - f2Con->p).norm2();
                        if(thumToF2dist < _L / 2 || _maxGraspWidth < thumToF2dist) continue;

                        const Vector2D<> f1Vf2  = f2Con->p - f1Con->p;
                        const double f1ToF2dist = f1Vf2.norm2();
                        if(f1ToF2dist < _w * 2.0 || _maxGraspWidth < f1ToF2dist) continue;

                        Rotation2D<> rotf2PsiPhi(clampAnglePi(psi - phi));
                        const Vector2D<> f2Dir = rotf2PsiPhi * _f2Dir;

                        // filter out contacts where thumb direction is not somewhat perpendicular
                        // on f1.c-f2.c
                        double dotangle =
                            fabs(dot(thumDir, f1Vf2) / (thumDir.norm2() * f1ToF2dist));
                        ...
                        double scale =
                            (dot(thumCon->p - f1Con->p, f1Vf2) / Math::sqr(f1ToF2dist)) - 0.5;
                        if(Math::sqr(scale) > _acceptUniform) continue;

                        // check that the f1 dosn't point toward f2
                        const double f1scale = dot(f1Dir, f1Vf2) / (Math::sqr(f1ToF2dist));
                        if(f1scale > 0) continue;
                        ...
                        // if this
                        if(distc1center > 2 * _w) continue;

                        double distc2center = (center - c2).norm2();
                        if(distc2center > 2 * _w) continue;
    ...
    std::cout << "- CG3Grasp2DGen: found " << _graspCandidates.size() << " grasp candidates."
              << std::endl;
}
