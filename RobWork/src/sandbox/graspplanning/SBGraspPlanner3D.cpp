...
{
    MyCollisionDetector(TreeDevice* hand, rw::core::Ptr<Frame> object,
                        CollisionStrategy* strategy) :
        _hand(hand),
        _obj(object), _strategy(strategy) {}
    ...
    bool query(std::vector<Joint*>& frames, int startIdx, State& state) {
        ...
        for(size_t i = startIdx; i < frames.size(); i++) {
            ...
            if(_strategy->inCollision(frame, wTf, _obj, wTobj)) { return true; }
        }
        return false;
    }
...
SBGraspPlanner3D::SBGraspPlanner3D(rw::models::TreeDevice* hand,
    ...
    _hand(hand),
    _obj(NULL), _fingers(fingers), _preshapeSampler(preshapeSampler), _roll(Pi / 2), _pitch(Pi / 2),
    _yaw(Pi / 2)
...
void SBGraspPlanner3D::reset(rw::kinematics::MovableFrame* obj,
                             const rw::kinematics::State& state) {
    RW_ASSERT(obj);
    ...
    if(geoms.size() == 0) RW_THROW("No geometry associated with frame: " << _obj->getName());
...
SBGraspPlanner3D::query(const rw::kinematics::State& initstate, int maxNrOfQs) {
    ...
    for(Frame* frame : _fingerTipFrames) { std::cout << frame->getName() << "\n-"; }

    std::cout << "End effector frames: \n-";
    for(Frame* frame : _endEffFrames) { std::cout << frame->getName() << "\n-"; }
    ...
    for(int tries = 0; tries < NR_OF_TRIES; tries++) {
        ...
        if(_preshapeSampler->empty()) break;
        ...
        for(size_t fingerIdx = 0; fingerIdx < _fingers.size(); fingerIdx++) {
            ...
            do {
                for(size_t jointIdx = 0; jointIdx < finger.size(); jointIdx++) {
                    ...
                    if(q < bound.first(0) || bound.second(0) < q) { outOfBounds = true; }
                }
