...
Planning::Planning(WorkCell::Ptr wc, Device::Ptr device, Device::Ptr gripper,
                   rw::core::Ptr<Frame> objectFrame, Transform3D<> tcpTgoal, GraspDB::Ptr db) :
    _db(db),
    _device(device), _gripper(gripper),
    _gripperBase(wc->findFrame<MovableFrame>(gripper->getBase()->getName())),
    _objectFrame(objectFrame), _targetTtcp(inverse(tcpTgoal)), _strategy(RANDOM) {
    _detector = ownedPtr(
...
Planning::Status Planning::inverseKin(std::vector<Q>& sol, const Transform3D<>& target,
    ...
    if(solutions.size() <= 0) return INVKIN_FAIL;
    while(solutions.size() > 0) {
        ...
        if(!constraint.inCollision(*bestSolution)) { sol.push_back(*bestSolution); }
        solutions.erase(bestSolution);
    }
    if(sol.size() > 0) return SUCCESS;
...
Planning::Status Planning::getPath(std::pair<unsigned int, QPath>& res, const State& state,
                                   double approach, std::vector<unsigned int> blacklist) const {
    if(_detector->inCollision(state)) return COLLISION_INITIALLY;
    ...
    if(_strategy == RANDOM) { std::random_shuffle(ind.begin(), ind.end()); }
        ...
        for(unsigned int i = 0; i < list.size(); i++) {
            ...
            for(unsigned int j = 0; j < ids.size(); j++) { ind.push_back(ids[j]); }
        }
    }
    ...
    for(std::vector<unsigned int>::iterator it = ind.begin(); it != ind.end(); it++) {
        ...
        for(std::vector<unsigned int>::iterator bit = blacklist.begin(); bit != blacklist.end();
            bit++) {
            if((*bit) == (*it)) blacklisted = true;
        }
        if(!blacklisted) {
...
Planning::Status Planning::getPath(std::pair<unsigned int, QPath>& res, unsigned int id,
                                   const State& state, double approach) const {
    if(_detector->inCollision(state)) return COLLISION_INITIALLY;

    std::vector<GraspSubTask> tasks = _db->getTasks();
    if(id >= tasks.size()) return NO_TASK;
...
Planning::Status Planning::getPath(QPath& res, const rw::math::Q qTo, const rw::math::Q qFrom,
                                   const rw::kinematics::State& state) const {
    State fromState = state;
    _device->setQ(qTo, fromState);
    State toState = state;
    _device->setQ(qTo, toState);
    if(_detector->inCollision(fromState)) return COLLISION_INITIALLY;
    if(_detector->inCollision(toState)) return COLLISION_END;

    QPath path;
    PlannerConstraint constraint = PlannerConstraint::make(_detector, _device, state);
    QSampler::Ptr sampler =
        QSampler::makeConstrained(QSampler::makeUniform(_device), constraint.getQConstraintPtr());
    ...
    if(planner->query(qFrom, qTo, path, 10.)) {
        ...
        if(path.size() > 2) path = optimizer.partialShortCut(path, 10, 0.1, 0.25);
        path = optimizer.pathPruning(path);
        res.clear();
        for(unsigned int i = 0; i < path.size(); i++) { res.push_back(path[i]); }
        return SUCCESS;
    }
    else { return NO_PATH_FOUND; }
}

Planning::Status Planning::getPath(QPath& res, const Transform3D<> pose, const Q gripper,
                                   const State& state) const {
    ...
    for(FramePairSet::iterator it = fps.begin(); it != fps.end(); it++) {
        ...
        if(!(!dev1.compare(deviceName) && !dev2.compare(gripperName)) &&
           !(!dev1.compare(gripperName) && !dev2.compare(deviceName)))
            handColl = true;
    }
    delete query;
    if(handColl) { return COLLISION_END; }
    else {
        std::vector<Q> qs;
        State invState = state;
        _gripper->setQ(gripper, invState);
        Planning::Status invKinSuc = inverseKin(qs, pose, invState);
        if(invKinSuc != SUCCESS) return INVKIN_FAIL;
        else {
            for(unsigned int i = 0; i < qs.size(); i++) {
                ...
                if(!_detector->inCollision(tempState)) {
                    ...
                    PlannerConstraint constraint =
                        PlannerConstraint::make(_detector, _device, tempState);
                    QSampler::Ptr sampler = QSampler::makeConstrained(
                        QSampler::makeUniform(_device), constraint.getQConstraintPtr());
                    QToQPlanner::Ptr planner =
                        RRTQToQPlanner::makeConnect(constraint, sampler, _metric, 0.1);
