...
RandomSampler::RandomSampler(const WorkCell::Ptr wc, const Device::Ptr device,
                             const Transform3D<> endTtcp, Pair radius, Pair x, Pair y,
                             double height, double maxangleZ) :
    _wc(wc),
    _device(device), _tcpTend(inverse(endTtcp)) {
...
Q RandomSampler::doSample() {
    ...
    while(!inverseOK) {
        ...
        while(resample) {
            pos =
                Vector3D<>(Math::ran(_x.first, _x.second), Math::ran(_y.first, _y.second), _height);
            bool test1 =
                (pos - dot(pos, Vector3D<>::z()) * Vector3D<>::z()).norm2() > _radius.second;
            bool test2 =
                (pos - dot(pos, Vector3D<>::z()) * Vector3D<>::z()).norm2() < _radius.first;
...
bool RandomSampler::inverseKin(std::vector<Q>& sol, const Transform3D<>& wTtarget,
                               const State& state) const {
    PlannerConstraint constraint = PlannerConstraint::make(_detector, _device, state);
    const Transform3D<>& target =
        inverse(Kinematics::worldTframe(_device->getBase(), state)) * wTtarget;
