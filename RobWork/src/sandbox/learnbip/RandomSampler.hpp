...
class RandomSampler : public rw::pathplanning::QSampler
{
  public:
    ...
    RandomSampler(const rw::models::WorkCell::Ptr wc, const rw::models::Device::Ptr device,
                  const rw::math::Transform3D<> endTtcp = rw::math::Transform3D<>::identity(),
                  Pair radius = Pair(0.3, 0.7), Pair x = std::pair<double, double>(-0.2, 0.4),
                  Pair y = std::pair<double, double>(-1.1, -0.5), double height = 0.45,
                  double maxangleZ = 60 * rw::math::Deg2Rad);
  ...
  private:
    ...
    bool inverseKin(std::vector<rw::math::Q>& sol, const rw::math::Transform3D<>& wTtarget,
