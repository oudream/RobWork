...
rwlibs::task::CartesianTask::Ptr Experiments::toCartesianTask() {
    ...
    for(Experiment& exp : _experiments) {
        ...
        if(!(exp.worldTobjectReal.equal(Transform3D<>::identity())))
            experiment->getPropertyMap().set<Transform3D<>>("WorldTObjectReal",
                                                            exp.worldTobjectReal);
        if(!(exp.worldTobjectDetected.equal(Transform3D<>::identity())))
            experiment->getPropertyMap().set<Transform3D<>>("WorldTObjectDetected",
                                                            exp.worldTobjectDetected);

        if(exp.gripperConfigurationTarget.size() > 0)
            experiment->getPropertyMap().set<Q>("GripperConfigurationTarget",
                                                exp.gripperConfigurationTarget);
        if(exp.gripperConfigurationGrasp.size() > 0)
            experiment->getPropertyMap().set<Q>("GripperConfigurationGrasp",
                                                exp.gripperConfigurationGrasp);
        if(exp.gripperConfigurationLift.size() > 0)
            experiment->getPropertyMap().set<Q>("GripperConfigurationLift",
                                                exp.gripperConfigurationLift);
        if(exp.gripperConfigurationRelease.size() > 0)
            experiment->getPropertyMap().set<Q>("GripperConfigurationRelease",
                                                exp.gripperConfigurationRelease);

        if(!(exp.objectTtcpTarget.equal(Transform3D<>::identity())))
            experiment->getPropertyMap().set<Transform3D<>>("ObjectTtcpTarget",
                                                            exp.objectTtcpTarget);
        ...
        if(!(exp.objectTtcpRelease.equal(Transform3D<>::identity())))
            experiment->getPropertyMap().set<Transform3D<>>("ObjectTtcpRelease",
                                                            exp.objectTtcpRelease);

        if(exp.mark != -1) experiment->getPropertyMap().set<int>("Mark", exp.mark);
...
Experiments::Experiments(rwlibs::task::CartesianTask::Ptr experiments) {
    ...
    for(size_t i = 0; i < experiments->getTasks().size(); i++) {
        ...
        _experiments[i].testStatus = (GraspTask::TestStatus) experiment->getPropertyMap().get<int>(
            "TestStatus", GraspTask::UnInitialized);

        _experiments[i].worldTobjectReal = experiment->getPropertyMap().get<Transform3D<>>(
            "WorldTObjectReal", Transform3D<>::identity());
        _experiments[i].worldTobjectDetected = experiment->getPropertyMap().get<Transform3D<>>(
            "WorldTObjectDetected", Transform3D<>::identity());

        _experiments[i].gripperConfigurationTarget =
            experiment->getPropertyMap().get<Q>("GripperConfigurationTarget", Q());
        _experiments[i].gripperConfigurationGrasp =
            experiment->getPropertyMap().get<Q>("GripperConfigurationGrasp", Q());
        _experiments[i].gripperConfigurationLift =
            experiment->getPropertyMap().get<Q>("GripperConfigurationLift", Q());
        _experiments[i].gripperConfigurationRelease =
            experiment->getPropertyMap().get<Q>("GripperConfigurationRelease", Q());

        _experiments[i].objectTtcpTarget = experiment->getPropertyMap().get<Transform3D<>>(
            "ObjectTtcpTarget", Transform3D<>::identity());
        _experiments[i].objectTtcpGrasp = experiment->getPropertyMap().get<Transform3D<>>(
            "ObjectTtcpGrasp", Transform3D<>::identity());
        _experiments[i].objectTtcpLift = experiment->getPropertyMap().get<Transform3D<>>(
            "ObjectTtcpLift", Transform3D<>::identity());
        _experiments[i].objectTtcpRelease = experiment->getPropertyMap().get<Transform3D<>>(
            "ObjectTtcpRelease", Transform3D<>::identity());

        _experiments[i].mark = experiment->getPropertyMap().get<int>("Mark", -1);
        _experiments[i].poseEstimateError =
            experiment->getPropertyMap().get<double>("PoseEstimateError", -1);
        _experiments[i].inlierFraction =
            experiment->getPropertyMap().get<double>("InlierFraction", -1);
        _experiments[i].temperature = experiment->getPropertyMap().get<std::vector<double>>(
            "Temperature", std::vector<double>());
    }
}
