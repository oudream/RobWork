...
def norm_inf(vektor):
    print("\n vektor", vektor )
    vektor2 = sdurw_math.Vector3D(vektor)          # Make a copy of vektor using c++ copy constructor
...
class Transform3DTest(unittest.TestCase):
    ...
    def test_Transform3D(self):
        d = sdurw_math.Vector3D(1, 2, 3)
        t = sdurw_math.Transform3D(d)               # Make a copy of d using c++ copy constructor
        v1 = sdurw_math.Vector3D(0, 0, 0)

        self.assertEqual(norm_inf(t * v1 - d) , 0)

        eaa = sdurw_math.EAA(math.pi / 2, 0.0, 0.0)
        r1 = eaa.toRotation3D()
        t2 = sdurw_math.Transform3D(d, r1)
        t3 = t2 * sdurw_math.inverse(t2)
        xeaa = sdurw_math.EAA(t3.R ())
