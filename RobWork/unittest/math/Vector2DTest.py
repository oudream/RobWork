...
class Vector2DTest(unittest.TestCase):
    ...
    def test_Vector2DTest_ADL(self):
        vd1 = sdurw_math.Vector2D(0.4, -0.7)
        vd2 = sdurw_math.Vector2D(0.9, 0.1)
        v5 = sdurw_math.Vector2D(3.0, 4.0)

        # Test Argument-Dependent Lookup (ADL)
        self.assertLess(math.fabs(sdurw_math.dot(vd1, vd2) - 0.29), 1e-15)

        cr = sdurw_math.cross(sdurw_math.Vector2D(2, 1), sdurw_math.Vector2D(1, 3))
    ...
    def test_Vector2D(self):
        v1 = sdurw_math.Vector2D(1.0, 2.0)
        v2 = sdurw_math.Vector2D(v1)
        v3 = v1+v2
        v4 = sdurw_math.Vector2D(2.0, 4.0)
        ...
        cr = sdurw_math.cross(sdurw_math.Vector2D(2.0, 1.0), sdurw_math.Vector2D(1.0, 3.0))   # qualified
        self.assertEqual(cr, 5)

        vd = sdurw_math.Vector2D(1.51, -2.51)
