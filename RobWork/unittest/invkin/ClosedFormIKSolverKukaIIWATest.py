...
class ClosedFormIKSolver(unittest.TestCase):

    def test_KukaIIWA(self):
        stateStructure = kinematics.StateStructure()
        state = stateStructure.getDefaultState()
        device = getKukaIIWA(stateStructure)
        solver = invkin.ClosedFormIKSolverKukaIIWA(device.cptr(), state)    # Forklar mig lige, hvad cptr() gør?
        ...
        for refI in range(0, len(qRefs), 1):
            ...
            for i in range(0, len(solutions), 1):
                ...
                print("\n solution[", i, "] ", solutions[i], sep="" )
                Tfound = device.baseTend(state)
                self.assertAlmostEqual(0, (Tfound.P()-T.P()).normInf(), delta = 1e-15)
                self.assertTrue(T.R().equal(Tfound.R(), 1e-15))
