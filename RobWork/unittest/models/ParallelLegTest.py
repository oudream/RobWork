...
class ParallelLeg(unittest.TestCase):
    ...
    def test_Common(self):
        ...
        A    = sdurw_models.RevoluteJoint("A",sdurw_math.Transform3D(sdurw_math.Vector3D.zero() , sdurw_math.RPY(0.0, 0.0, math.pi/2)) )
        Aend = sdurw_kinematics.FixedFrame("Aend",sdurw_math.Transform3D.identity())
        B    = sdurw_models.RevoluteJoint("B",sdurw_math.Transform3D(sdurw_math.Vector3D(0,0,lAB) , sdurw_math.RPY(0.0, 0.0, -math.pi/2)) )
        ...
        self.assertTrue(leg.baseTend(state).equal( sdurw_math.Transform3D(sdurw_math.Vector3D(lAB,0,0),sdurw_math.RPY(angleFb,0,0) ) ) )
        ...
        print("\n MANGLER   Wrong result        DET VIRKER IKKE")
        #print(leg.baseJend(state).asNumpy())
        #print(jacRef)
        #print(leg.baseJend(state).asNumpy()-jacRef)
#        self.assertTrue( (leg.baseJend(state).asNumpy() - jacRef).all()  )
...
#        print("\n jacRef", jacRef )
        ...
        self.assertEqual(0.2,B.getData(state)[0])
        self.assertEqual(0.3,Fa.getData(state)[0])
        self.assertEqual(0.4,Fb.getData(state)[0])
