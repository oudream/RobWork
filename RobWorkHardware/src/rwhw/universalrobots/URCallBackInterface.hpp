...
class URCallBackInterface
{
  public:
     ...
     *
     * Check the controller box to see which version. Notice that not all commands are supported in
     * all versions.
     */
     ...
     *
     * @param host [in] IP address of the host (to which the robot will connect), e.g.
     * 192.168.100.1.
     ...
     */
    void startCommunication (const std::string& callbackIP, const unsigned int callbackPort = 33334,
                             ControllerBox cb = CB2, const std::string& filename = "");
     ...
     * @param filename [in] UR script filename; if not specified, a default bundled script is used.
     */
    void startCommunication (const unsigned int callbackPort = 33334, ControllerBox cb = CB2,
     ...
     *
     * The coordinates are send directly to the robot, hence the inverse kinematics is the robots
     * internal model.
     ...
     * @param selection [in] 6D vector containing 0 and 1, where one indicates that the robot will
     * be compliant in that direction.
     * @param wrench [in] 6D wrench to apply
     * @param limits [in] For compliant axis the limit is the maximal tcp speed along/around the
     * axis. For non-compliant axes it is the maximal deviation allowed for the TCP.
     */
    void forceModeStart (const rw::math::Transform3D<>& base2ref, const rw::math::Q& selection,
                         const rw::math::Wrench6D<>& wrench, const rw::math::Q& limits);
    ...
    class URScriptCommand
    {
      public:
        ...
        URScriptCommand (CmdType type, const rw::math::Q& q, float speed, float blend) :
            _type (type), _q (q), _speed (speed), _blend (blend)
        {}

        URScriptCommand (CmdType type, const rw::math::Q& q, float speed) :
            _type (type), _q (q), _speed (speed)
        {}

        URScriptCommand (CmdType type) : _type (type) {}

        URScriptCommand (CmdType type, const rw::math::Transform3D<>& transform, float speed,
                         float blend) :
            _type (type),
            _transform (transform), _speed (speed), _blend (blend)
        {}
        ...
        URScriptCommand (CmdType type, const rw::math::Q& selection,
                         const rw::math::Wrench6D<>& wrench, const rw::math::Q& limits) :
            _type (type),
            _selection (selection), _wrench (wrench), _limits (limits)
        {}

        URScriptCommand (CmdType type, const rw::math::Transform3D<>& base2ref,
                         const rw::math::Q& selection, const rw::math::Wrench6D<>& wrench,
                         const rw::math::Q& limits) :
            _type (type),
            _transform (base2ref), _selection (selection), _wrench (wrench), _limits (limits)
        {}

        URScriptCommand (CmdType type, double mass, const rw::math::Vector3D<>& centerOfGravity) :
            _type (type), _mass ((float) mass), _centerOfGravity (centerOfGravity)
        {}

        URScriptCommand (CmdType type, const rw::math::Wrench6D<>& wrench) :
            _type (type), _wrench (wrench)
        {}

        URScriptCommand (CmdType type, int id, bool bValue) :
            _type (type), _id (id), _bValue (bValue)
        {}
