...
URCallBackInterface::URCallBackInterface () :
    _cb (CB3), _thread (NULL), _callbackPort (33334), _stopServer (false), _robotStopped (true),
    _isMoving (false), _isServoing (false), _isConnected (false)
{
...
/* Deprecated function */
// void URCallBackInterface::startInterface(const unsigned int callbackPort, const std::string&
// filename) {
//    startCommunication("192.168.100.1", callbackPort, filename);
//}

void URCallBackInterface::startCommunication (const unsigned int callbackPort, ControllerBox cb,
                                              const std::string& filename)
{
    startCommunication (_urPrimary.getLocalIP (), callbackPort, cb, filename);
}

void URCallBackInterface::startCommunication (const std::string& callbackIP,
                                              const unsigned int callbackPort, ControllerBox cb,
                                              const std::string& filename)
{
    _cb           = cb;
    _callbackPort = callbackPort;
    /* There should be some better error checking at the conversion of the IP from string to
     * ip::address - but the documentation is limited with what error codes that can be returned /
     * exception(s) that can be thrown - [look at the source for more specific error
    ...
    if (n == std::string::npos) {
        ...
        sstr_host << script.substr (0, n) << "\"" << _callbackIP.to_string () << "\""
                  << script.substr (n + 4);
    }
    ...
    std::string cbId;
    switch (cb) {
        case CB2: cbId = "CB2"; break;
        case CB3: cbId = "CB3"; break;
        default: RW_THROW ("Unknown controller box specified: " << cb); break;
    }
...
void URCallBackInterface::handleCmdRequest (tcp::socket& socket)
{
    ...
    switch (cmd._type) {
        case URScriptCommand::STOP: break;
        case URScriptCommand::MOVEQ:
            q2intVector (cmd._q, integers, 1);
            integers[7] =
                static_cast< int > ((cmd._speed * NORMALIZE_PERCENTAGE) * FLOAT_TO_INT_SCALE);
        ...
        case URScriptCommand::FORCE_MODE_UPDATE:
            ...
            //_isMoving = true;
            break;
        case URScriptCommand::FORCE_MODE_END: break;
        case URScriptCommand::TEACH_MODE_START: break;
        case URScriptCommand::TEACH_MODE_END: break;
...
/* Commented out due to missing error code to catch the case where there are no errors, but 0 bytes are available to be read - could catch a posix specific 0=successful error code from the boost system error codes, but that seem a bit too hackish... */
                    ...
                    if (_robotStopped) {
                        ...
                        // RW_LOG_DEBUG("Going to handle command request - Time: " <<
                        // timer.getTime()); timer.resetAndResume();
                        handleCmdRequest (socket);
                    }

                    // if (cnt++ % 100 == 0) {
                    //	std::cout<<"Avg/min/max/stddev Time =
                    //"<<stats.mean()<<"/"<<stats.minValue()<<"/"<<stats.maxValue()<<"/"<<std::sqrt(stats.variance())<<std::endl;
                    //	stats.clear();
                    //}
                }
...
void URCallBackInterface::popAllUpdateCommands ()
{
    while ((_commands.size () > 0) &&
           ((_commands.front ()._type == URScriptCommand::SERVOQ) ||
            (_commands.front ()._type == URScriptCommand::FORCE_MODE_UPDATE))) {
        _commands.pop ();
    }
}
...
void URCallBackInterface::forceModeStart (const rw::math::Transform3D<>& base2ref,
                                          const rw::math::Q& selection,
                                          const rw::math::Wrench6D<>& wrench,
                                          const rw::math::Q& limits)
{
    RW_LOG_DEBUG ("rwhw::URCallBackInterface::forceModeStart("
                  << base2ref << "," << selection << "," << wrench << "," << limits << ")");
    boost::mutex::scoped_lock lock (_mutex);

    popAllUpdateCommands ();

    _commands.push (
        URScriptCommand (URScriptCommand::FORCE_MODE_START, base2ref, selection, wrench, limits));
    _robotStopped = false;
}
