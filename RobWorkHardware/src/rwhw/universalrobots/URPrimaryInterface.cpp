...
URPrimaryInterface::URPrimaryInterface () :
    _thread (NULL), _stop (false), _haveReceivedSize (false), _messageLength (0),
    _messageOffset (0), _socket (0), _hostName (""), _connected (false), _hasURData (false)
{}
...
bool URPrimaryInterface::sendCommand (const std::string& str)
{
    ...
    bytesTransfered =
        boost::asio::write (*_socket, boost::asio::buffer (str.c_str (), str.size ()));
    RW_LOG_DEBUG ("Bytes to be transfered '" << str.size () << "' and bytes actually transfered '"
                                             << bytesTransfered << "'");
...
bool URPrimaryInterface::readPrimaryInterfacePacket ()
{
    ...
    while (_messageOffset < dataLength) {
        switch (messageType) {
            ...
            case ROBOT_MESSAGE: readRobotMessage (_dataStorage, dataLength); break;
            case HMC_MESSAGE:
            default:
...
void URPrimaryInterface::readRobotMessage (const std::vector< char >& data,
                                           unsigned int messageLength)
{
    ...
    switch (robotMessageType) {
        case URMessage::ROBOT_MESSAGE_VERSION: {
            ...
            _messages.push (URMessage (URMessage::ROBOT_MESSAGE_VERSION,
                                       majorVersion,
                                       minorVersion,
                                       "Project Name: ",
                                       projectName));
        ...
        case URMessage::ROBOT_MESSAGE_SECURITY: {
            ...
            switch (safetyModeType) {
                case 1: safetyModeTypeText = "NORMAL"; break;
                case 2: safetyModeTypeText = "REDUCED"; break;
                case 3: safetyModeTypeText = "PROTECTIVE STOP"; break;
                case 4: safetyModeTypeText = "RECOVERY"; break;
                case 5: safetyModeTypeText = "SAFE GUARD STOP"; break;
                case 6: safetyModeTypeText = "SYSTEM EMERGENCY STOP"; break;
                case 7: safetyModeTypeText = "ROBOT EMERGENCY STOP"; break;
                case 8: safetyModeTypeText = "VIOLATION"; break;
                case 9: safetyModeTypeText = "FAULT"; break;
                default: safetyModeTypeText = "UNKNOWN"; break;
            }
            ...
            _messages.push (URMessage (URMessage::ROBOT_MESSAGE_SECURITY,
                                       robotMessageCode,
        ...
        case URMessage::ROBOT_MESSAGE_ERROR_CODE: {
            ...
            std::string levelMsg     = "";
            switch (warningLevel) {
                case 1: levelMsg = "INFO"; break;
                case 2: levelMsg = "WARNING"; break;
                case 3: levelMsg = "VIOLATION"; break;
                case 4: levelMsg = "FAULT"; break;
                default: levelMsg = "UNKNOWN"; break;
            }
            std::string textmsg;
            if (messageLength > _messageOffset) {
                size_t txtLength = messageLength - _messageOffset;
            ...
            _messages.push (URMessage (URMessage::ROBOT_MESSAGE_ERROR_CODE,
                                       robotMessageCode,
                                       robotMessageArgument,
                                       levelMsg,
                                       textmsg));
        ...
        case URMessage::ROBOT_MESSAGE_KEY:
            _messages.push (
                URMessage (URMessage::ROBOT_MESSAGE_KEY, 0, 0, "ROBOT MESSAGE KEY", ""));
            break;
        case URMessage::ROBOT_MESSAGE_PROGRAM_LABEL:
            _messages.push (URMessage (
                URMessage::ROBOT_MESSAGE_PROGRAM_LABEL, 0, 0, "ROBOT_MESSAGE_PROGRAM_LABEL", ""));
            break;
        case URMessage::ROBOT_MESSAGE_POPUP: {
            ...
            _messages.push (URMessage (URMessage::ROBOT_MESSAGE_PROGRAM_LABEL,
                                       warning,
                                       error,
                                       std::string (title),
                                       std::string (msg)));
        ...
        case URMessage::ROBOT_MESSAGE_TEXT: {
            ...
            _messages.push (
                URMessage (URMessage::ROBOT_MESSAGE_TEXT, 0, 0, "TEXT MESSAGE", textmsg));
        } break;
        case URMessage::ROBOT_MESSAGE_VARIABLE: break;
        default: break;
    }
    _messageOffset = startOffset + messageLength;
}
...
void URPrimaryInterface::readRobotsState (const std::vector< char >& data)
{
    ...
    switch (packetType) {
        case ROBOT_MODE_DATA:
            _data.controllerTimeStamp =
                static_cast< long > (URCommon::getUInt64 (data, _messageOffset));
        ...
        case JOINT_DATA:
            ...
            for (unsigned int j = 0; j < 6; j++) {
                ...
                // store Array
                _data.jointCurrent[j] =
                    static_cast< float > (URCommon::getFloat (data, _messageOffset));
                _data.jointVoltage[j] =
                    static_cast< float > (URCommon::getFloat (data, _messageOffset));
                _data.jointMotorTemperature[j] =
                    static_cast< float > (URCommon::getFloat (data, _messageOffset));
                _data.jointMicroTemperature[j] =
                    static_cast< float > (URCommon::getFloat (data, _messageOffset));
                _data.jointMode[j] = URCommon::getUChar (data, _messageOffset);
            }
            // Store rw::math::Q
            // Encoder adjustment
            // jointPosition = rw::models::EncoderDecentralization::calcRealAngle(jointPosition,
            // _tau, _sigma); targetJointPosition =
            // rw::models::EncoderDecentralization::calcRealAngle(targetJointPosition, _tau, _sigma);
        ...
        case MASTERBOARD_DATA: {
            ...
            _data.masterTemperature =
                static_cast< float > (URCommon::getFloat (data, _messageOffset));

            // float robotVoltage48V;
            _data.robotVoltage48V =
                static_cast< float > (URCommon::getFloat (data, _messageOffset));
        ...
        case CARTESIAN_INFO: _messageOffset += packetLength - 5; break;
        case LASER_POINTER_POSITION: _messageOffset += packetLength - 5; break;
        default:
            _messageOffset += packetLength - 5;    //-5 because the header has already been read.
            // RW_WARN("Unknown package type: "<<(int)packetType<<"  "<<packetLength<<"
            // MessageOffset = "<<_messageOffset<<std::endl);
    }
}
