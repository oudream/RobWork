...
class URCommon
{
  public:
    ...
    static inline int getData (boost::asio::ip::tcp::socket* socket, size_t cnt,
    ...
    static inline std::string getString (boost::asio::ip::tcp::socket* socket, int cnt,
    ...
    static inline std::string readUntil (boost::asio::ip::tcp::socket* socket, char terminator,
                                         uint32_t& messageOffset)
    {
        char buffer[1024];
        size_t cnt = 0;
    ...
    static inline unsigned char getUChar (boost::asio::ip::tcp::socket* socket,
                                          uint32_t& messageOffset)
    {
    ...
    static inline std::string getString (const std::vector< char >& data, int length,
                                         uint32_t& messageOffset)
    {
    ...
    // Extract a 3d vector
    static inline rw::math::Vector3D< double > getVector3D (boost::asio::ip::tcp::socket* socket,
                                                            uint32_t& messageOffset)
    {
    ...
    static inline rw::math::Vector3D< double > getVector3D (const std::vector< char >& data,
                                                            uint32_t& messageOffset)
    {
        rw::math::Vector3D< double > output;
    ...
    static inline rw::math::Q getQ (boost::asio::ip::tcp::socket* socket, size_t cnt,
                                    uint32_t& messageOffset)
    {
        rw::math::Q res (cnt);
    ...
    static inline void send (boost::asio::ip::tcp::socket* socket,
                             const std::vector< int >& integers)
    {
        ...
        try {
            ...
            /* TODO:
             * This call to memcpy(...) is unnecessary? as the two (not-commented) for-loops are
             * doing the same thing, just flipping the "word order" ?
             */
             ...
             * Rewrite to dynamic sizes using sizeof(int) and verify that the content in buffer will
             * be the same
             */
            for (size_t i = 0; i < integers.size (); i++) {
...
//            ibuf[i] = integers.at(i);
                    std::cout<<"Int = "<<integers.at(i)<<" = "<<(unsigned int)ibuf[4*i]<<"
"<<(unsigned int)ibuf[4*i+1]<<" "<<(unsigned int)ibuf[4*i+2]<<" "<<(unsigned
int)ibuf[4*i+3]<<std::endl; std::cout<<"Int from buffer = "<<*(int*)(&buffer[4*i])<<std::endl;
            }*/
            //        ibuf[0] = 254;
    ...
    socket->send(boost::asio::buffer(buffer, n));*/
            ...
            if (static_cast< int > (bytesTransfered) == n) {
                ...
                /* Unsuccessful send */
                RW_LOG_ERROR ("Unable to send all the '" << n << "' bytes - only sent '"
     ...
     * Consider changing the void to int/bool and return status on whether every byte was transfered
     * or not (should also look at the function(s) above)
     */
    ...
    static inline double driverTimeStamp () { return rw::common::TimerUtil::currentTime (); }
};
