...
class URRTDE
{
  public:
     ...
     * @param q_path with joint configurations that includes acceleration, speed and blend for each
     * configuration
     */
    ...
    /**
     * @brief Joint speed - Accelerate linearly in joint space and continue with constant joint
     * speed
    ...
    /**
     * @brief Tool speed - Accelerate linearly in Cartesian space and continue with constant tool
     * speed. The time t is optional;
     ...
     * @param time time where the command is controlling the robot. The function is blocking for
     * time t [S]
     * @param lookahead_time time [S], range [0.03,0.2] smoothens the trajectory with this lookahead
     * time
     * @param gain proportional gain for following target position, range [100,2000]
     */
    bool servoJ (const rw::math::Q& q, double speed, double acceleration, double time,
     ...
     * @param selection_vector A 6d Q vector of 0s and 1s. 1 means that the robot will be compliant
     ...
     * pointing from the robot tcp towards the origin of the force frame. 2: The force frame is not
     * transformed. 3: The force frame is transformed in a way such that its x-axis is the
     * projection of the robot tcp velocity vector onto the x-y plane of the force frame.
     * @param limits (Float) 6d Q vector. For compliant axes, these values are the maximum allowed
     ...
     */
    bool forceMode (const rw::math::Transform3D<>& task_frame, const rw::math::Q& selection_vector,
