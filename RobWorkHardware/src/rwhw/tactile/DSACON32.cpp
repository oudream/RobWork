...
void createData (unsigned char id, unsigned int size, unsigned char data[], int offset = 0)
{
    data[offset]     = 0xAA;
    data[offset + 1] = 0xAA;
    data[offset + 2] = 0xAA;
...
DSACON32::DSACON32 (SerialPort& port, const ControllerConfig& cConfig, const SensorConfig& sConfig,
                    MatrixConfig* mConfigs, const std::vector< TactileMaskMatrix >& staticMasks) :
    _sPort (&port),
    _timeStamp (0), _useCompression (false), _fps (5), _cConfig (cConfig), _sConfig (sConfig),
    _mConfigs (mConfigs), _staticMasks (staticMasks),
...
std::string DSACON32::toString () const
{
    ...
    for (unsigned short n = 0; n < _sConfig.numMatrices; n++) {
        ostr << "\nMatrix #" << n << ": (" << _mConfigs[n].descrStr << ")" << std::endl
             << "---------------------" << std::endl;
...
bool DSACON32::ReadMatrixMask (SerialPort* sPort, unsigned char maskType, unsigned char matrixNum,
                               TactileMaskMatrix* mat)
{
    unsigned char data[10];
...
void DSACON32::ParsePacket (unsigned char* payload, unsigned short payloadSize, unsigned char id,
                            void* answer)
{
    int errorCode;
    switch (id) {
        ...
        case QUERY_CONTROLLER_CONFIG:
            ...
            break;
        case QUERY_SENSOR_CONFIG: static_cast< SensorConfig* > (answer)->parse (payload); break;
        case QUERY_MATRIX_CONFIG: static_cast< MatrixConfig* > (answer)->parse (payload); break;
        ...
        case READ_DESCRIPTOR:
            ...
            *static_cast< std::string* > (answer) =
                payloadSize > 2 ? std::string ((char*) &(payload[2]), payloadSize - 2) : "N/A";
            break;
        ...
        //  break;
        default: RW_THROW ("BAD ID RECIEVED" << (int) id);
    }
}
...
unsigned short DSACON32::CalcCrc16Checksum (unsigned char* data, unsigned short size,
                                            unsigned short crc)
{
    unsigned long c;
