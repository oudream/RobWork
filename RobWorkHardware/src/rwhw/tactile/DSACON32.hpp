...
class DSACON32
{
  ...
  private:
    DSACON32 (rwhw::SerialPort& port, const ControllerConfig& cConfig, const SensorConfig& sConfig,
              MatrixConfig* mConfigs, const std::vector< TactileMaskMatrix >& staticMasks);
  ...
  public:
    struct ControllerConfig
    {
      public:
        ...
        bool isDataAcqRunning () { return ((statusFlags & 64) == 64); }
    };
    ...
    void setUseCompression (bool useCompression) { _useCompression = useCompression; }

    void setFrameRate (unsigned short fps) { _fps = fps; }
    ...
    const ControllerConfig& getControllerConfig () { return _cConfig; }

    const SensorConfig& getSensorConfig () { return _sConfig; }

    const std::vector< TactileMatrix >& getPads () { return _pads; }

    const TactileMatrix& getPad (int indx) { return _pads[indx]; }

    const std::vector< TactileMaskMatrix >& getStaticMasks () { return _staticMasks; }

    const TactileMaskMatrix& getStaticMask (int indx) { return _staticMasks[indx]; }
  ...
  private:
    ...
    // static bool SetPropertiesSampleRate(rwlibs::io::SerialPort *sPort, unsigned short
    // sampleRate); static bool SetPropertiesVectorMatrix(rwlibs::io::SerialPort *sPort,
    ...
    static bool ReadAck (unsigned char cmdId, unsigned int timeout, rwhw::SerialPort* sPort,
                         void* answer = NULL);
    static bool ReadHeader (unsigned char& id, unsigned short& payloadSize, unsigned char* header,
                            unsigned int timeout, rwhw::SerialPort* sPort);
    static void ParsePacket (unsigned char* payload, unsigned short payloadSize, unsigned char id,
                             void* answer);
    ...
    static void SendBlocking (unsigned char* data, int len, unsigned char id, int timeout,
                              rwhw::SerialPort* sPort)
    {
