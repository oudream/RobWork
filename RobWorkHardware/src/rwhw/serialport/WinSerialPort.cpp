...
bool SerialPort::open (const std::string& port, Baudrate baudrate, DataBits dbits, Parity parity,
                       StopBits sbits)
{
    // cfd = CreateFile(TEXT("COM4"), GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0,
    // NULL);
    cfd = CreateFileW (
        s2ws (port).data (), GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL);
    ...
    switch (baudrate) {
        ...
        case (Baud460800):
        case (Baud921600):
        default: RW_WARN ("Unsupported baudrate configuration!!"); return false;
    }
    switch (sbits) {
        ...
        default: RW_WARN ("Unsupported StopBit configuration!!!"); return false;
    }
    ...
    switch (parity) {
        ...
        default: RW_WARN ("Unsupported parity configuration!!!"); return false;
    }
    ...
    switch (dbits) {
        ...
        default: RW_WARN ("Unsupported data bit size!"); return false;
    }
...
int SerialPort::read (char* buf, int len)
{
    ...
    BOOL res         = ReadFile (cfd,            // handle of file to read
