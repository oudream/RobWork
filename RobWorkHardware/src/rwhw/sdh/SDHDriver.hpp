...
class SDHDriver
{
  public:
     ...
     */
    bool connect (int canNetId = 0, int canBaudRate = 1000000, double canTimeOut = 0.5,
                  int id_read = 43, int id_write = 42);
     ...
     * @return
     */
    bool connect (const std::string& device, int canBaudRate = 1000000, double canTimeOut = 0.5,
                  int id_read = 43, int id_write = 42);

    /**
     * @brief connects to the SDH hardware using the CAN interface. The SDH can be connected both
     * through CAN and SerialPort depending on the configuration of the SDH.
     */
    bool connect (CanPort::Ptr cport, double canTimeOut = 0.5, int id_read = 43, int id_write = 42);

    /**
     * @brief connects to the SDH hardware using the RS232 interface. The SDH can be connected both
     * through CAN and SerialPort depending on the configuration of the SDH.
     */
    bool connect (int port, unsigned long baudrate = 115200, double timeout = 0.5,
                  char const* device_format_string = NULL);
