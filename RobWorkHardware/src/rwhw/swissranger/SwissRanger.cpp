...
SwissRanger::SwissRanger (const std::string& name, float maxrange) :
    Sensor (NULL, name, "swissranger sensor", 3), _nIntTime (30), _isOpen (false),
    _maxrange (maxrange)
{}
...
void SwissRanger::openCamera ()
{
    if (!_isOpen) {
        ...
        if (swissranger_send (_srHandle, (char) 0x2, (char) 0xC0) < 0)
            RW_THROW ("Could not initialize swissranger camera!!!");
...
void SwissRanger::acquire (float* points, float* accuracies, float* intensities, unsigned short n)
{
    if (!isOpen ()) {
        RW_WARN ("Could not Acquire image, Open Camera first.");
        return;
    ...
    unsigned int d;
    for (unsigned short i = 0; i < IMG_SIZE; i++) {
        accuracy = L / sqrt (8.0f) * sqrt ((float) _intensityBuffer[i] / n) /
                   (2 * _amplitudeBuffer[i] / n);
...
void SwissRanger::setIntegrationTime (unsigned char intTime)
{
    ...
    if (isOpen ()) {
        if (swissranger_send (_srHandle, 0x9, intTime) < 0)
            RW_THROW ("Could not send integration time to camera");
    }
}
