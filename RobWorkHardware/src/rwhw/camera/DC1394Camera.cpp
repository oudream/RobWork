...
namespace rwhw { namespace camera {
    ...
    DC1394Camera::DC1394Camera (dc1394camera_t* dc1394cam) :
        CameraFirewire (dc1394cam->model, dc1394cam->vendor), _policy (SINGLE_SHOT),
        _isAcquired (false), _dccamera (dc1394cam), _captureMode (M640x480), _colorMode (MONO8),
        _frameRate (DC1394_FRAMERATE_30), _bufferSize (10)
    {
        _initialized = false;
        _started     = false;
        _f7PosLeft   = 0;
    ...
    std::string DC1394Camera::getCameraName () { return std::string (_dccamera->model); }

    std::string DC1394Camera::getCameraVendor () { return std::string (_dccamera->vendor); }
    ...
    bool DC1394Camera::initialize ()
    {
        ...
        //	if(dc1394_video_set_operation_mode(_dccamera,DC1394_OPERATION_MODE_1394B) !=
        //DC1394_SUCCESS) { 		fprintf(stderr,"Unable to setup camera in OPERATION_MODE_1394B\n");
        //		return false;
        //	}
        ...
        switch (_captureMode) {
            case (M160x120):
                switch (_colorMode) {
                    ...
                    default:
                        RW_WARN ("Unable to setup camera resolution with selected color mode.");
                        return false;
                }
                break;
            ...
            case (CameraFirewire::MFORMAT7):
                ...
                // if(dc1394_format7_set_color_coding(_dccamera, (dc1394video_mode_t) res,
                // ColorCodeConverter(_colorMode)) != DC1394_SUCCESS){ 	RW_WARN("Unable to send the
                //selected Format7 color mode"); 	return false;
                //}
                if (dc1394_format7_set_roi (_dccamera,
                                            res,
                                            ColorCodeConverter (_colorMode),
                                            packetSize,
                                            _f7PosLeft,
                                            _f7PosTop,
                                            _f7width,
                                            _f7height) != DC1394_SUCCESS) {
                    RW_WARN ("Unable to send the selected Format7 resolution, color mode, image "
                             "size and position\nCheck the settings and connection!");
                    return false;
                }
                std::cout << "dc1394 Format7 set " << _f7width << "x" << _f7height
                          << " Pos: " << _f7PosLeft << "x" << _f7PosTop << std::endl;
                break;
        ...
        //	if(dc1394_format7_get_packet_parameters(_dccamera, (dc1394video_mode_t) res, &pmin,
        //&pmax) != DC1394_SUCCESS){ 		RW_WARN("Unable to get packet size parameters!"); 		return false;
        //	}
        //
    ...
    bool DC1394Camera::start ()
    {
        ...
        switch (mode) {
            ...
            case (M1600x1200):
                ...
                _height = 1200;
                break;
            case (MFORMAT7): getFormat7ImageSize (_f7Mode, _width, _height); break;
            default: RW_WARN ("Unsupported capturing mode."); return false;
        }
        ...
        switch (getColorMode ()) {
            case (CameraFirewire::MONO8):
                encoding = Image::GRAY;
                depth    = Image::Depth8U;
                break;
            case (CameraFirewire::MONO16):
                encoding = Image::GRAY;
                depth    = Image::Depth16U;
                break;
            case (CameraFirewire::MONO16S):
                encoding = Image::GRAY;
                depth    = Image::Depth16S;
                break;
            case (CameraFirewire::RGB8):
                encoding = Image::RGB;
                depth    = Image::Depth8U;
                break;
            case (CameraFirewire::RGB16):
                encoding = Image::RGB;
                depth    = Image::Depth16U;
                break;
            case (CameraFirewire::RGB16S):
                encoding = Image::RGB;
                depth    = Image::Depth16S;
                break;
            case (CameraFirewire::RGB24):
                encoding = Image::RGB;
                depth    = Image::Depth32S;
                break;
            case (CameraFirewire::YUV444):
                encoding = Image::RGB;
                depth    = Image::Depth8U;
                break;
            case (CameraFirewire::YUV411):
                encoding = Image::RGB;
                depth    = Image::Depth8U;
                break;
            case (CameraFirewire::YUV422):
                encoding = Image::RGB;
                depth    = Image::Depth8U;
                break;
            case (CameraFirewire::RAW8):
                encoding = Image::RGB;
                depth    = Image::Depth8U;
                break;
            case (CameraFirewire::RAW16):
                encoding = Image::RGB;
                depth    = Image::Depth16U;
                break;
            default:
                std::cout << " No valid image mode selected - mode: " << getColorMode ()
                          << std::endl;
                return false;
        }
        ...
        /* Setup capture */
        if (dc1394_capture_setup (_dccamera, _bufferSize, DC1394_CAPTURE_FLAGS_DEFAULT) !=
            DC1394_SUCCESS) {
            RW_WARN ("Unable to setup camera for DMA capture.\n"
    ...
    {
        switch (_policy) {
            case (SINGLE_SHOT): acquireOneShot (); break;
            case (CONTINUES):
            case (CONTINUES_BUFFERED): acquireContinues (); break;
            default: RW_WARN ("Unknown capture policy selected."); break;
        }
    }
    ...
    void DC1394Camera::acquireOneShot ()
    {
        ...
        if (_dccamera->one_shot_capable) {
            ...
            if (dc1394_capture_dequeue (_dccamera, DC1394_CAPTURE_POLICY_WAIT, &frame) !=
                DC1394_SUCCESS) {
                RW_WARN ("Unable to dequeue a frame.");
                return;
            }

            memcpy (_image->getImageData (), frame->image, _image->getDataSize ());
    ...
    bool DC1394Camera::isImageReady () { return _isAcquired; }
    ...
    void DC1394Camera::setFrameRate (double framerate)
    {
        ...
        if (framerate == 1.875)
            res = DC1394_FRAMERATE_1_875;
        else if (framerate == 3.75)
            res = DC1394_FRAMERATE_3_75;
        else if (framerate == 7.5)
            res = DC1394_FRAMERATE_7_5;
        else {
    ...
    CameraFirewire::CaptureMode DC1394Camera::getCaptureMode ()
    {
        ...
        switch (captureMode) {
            ...
            case DC1394_VIDEO_MODE_FORMAT7_7:
                mode = MFORMAT7;
                std::cout << "getCaptureMode=Format7 - " << mode << std::endl;
                break;

            case DC1394_VIDEO_MODE_EXIF:
            default: RW_THROW ("Unsupported capturemode!!");
        }
        return mode;
    }
    ...
    CameraFirewire::ColorCode DC1394Camera::getColorMode ()
    {
        ...
        switch (mode) {
            ...
            case DC1394_VIDEO_MODE_EXIF:
            default: RW_THROW ("Unsupported colormode!!"); break;
        }
    ...
    CameraFirewire::CapturePolicy DC1394Camera::getCapturePolicy () { return _policy; }
    ...
    dc1394feature_t DC1394Camera::settingsConverter (CameraFirewire::CameraFeature setting)
    {
        switch (setting) {
            ...
            case CameraFirewire::AUTOEXPOSURE:
                return DC1394_FEATURE_EXPOSURE;
                //		case CameraFirewire::TEMPERATURE    : return
                //DC1394_FEATURE_TEMPERATURE; 		case CameraFirewire::TRIGGER    	: return
                //DC1394_FEATURE_TRIGGER; 		case CameraFirewire::TRIGGER_DELAY  : return
                //DC1394_FEATURE_TRIGGER_DELAY; 		case CameraFirewire::WHITE_SHADING  : return
                //DC1394_FEATURE_WHITE_SHADING; 		case CameraFirewire::FRAME_RATE    	: return
                //DC1394_FEATURE_FRAME_RATE; 		case CameraFirewire::PAN    		: return
                //DC1394_FEATURE_PAN; 		case CameraFirewire::TILT    		: return
                //DC1394_FEATURE_TILT; 		case CameraFirewire::OPTICAL_FILTER : return
                //DC1394_FEATURE_OPTICAL_FILTER; 		case CameraFirewire::CAPTURE_SIZE   : return
                //DC1394_FEATURE_CAPTURE_SIZE; 		case CameraFirewire::CAPTURE_QUALITY: return
    ...
    CameraFirewire::CameraFeature DC1394Camera::settingsConverter (dc1394feature_t setting)
    {
        switch (setting) {
            ...
            case DC1394_FEATURE_EXPOSURE:
                return CameraFirewire::AUTOEXPOSURE;
                //		case DC1394_FEATURE_TEMPERATURE    : return
                //DC1394_FEATURE_TEMPERATURE; 		case DC1394_FEATURE_TRIGGER    	: return
                //DC1394_FEATURE_TRIGGER; 		case DC1394_FEATURE_TRIGGER_DELAY  : return
                //DC1394_FEATURE_TRIGGER_DELAY; 		case DC1394_FEATURE_WHITE_SHADING  : return
                //DC1394_FEATURE_WHITE_SHADING; 		case DC1394_FEATURE_FRAME_RATE    	: return
                //DC1394_FEATURE_FRAME_RATE; 		case DC1394_FEATURE_PAN    		: return
                //DC1394_FEATURE_PAN; 		case DC1394_FEATURE_TILT    		: return
                //DC1394_FEATURE_TILT; 		case DC1394_FEATURE_OPTICAL_FILTER : return
                //DC1394_FEATURE_OPTICAL_FILTER; 		case DC1394_FEATURE_CAPTURE_SIZE   : return
                //DC1394_FEATURE_CAPTURE_SIZE; 		case DC1394_FEATURE_CAPTURE_QUALITY: return
                //DC1394_FEATURE_CAPTURE_QUALITY;
            default:
    ...
    {
        uint32_t value = 0;
        if (dc1394_feature_get_value (_dccamera, settingsConverter (setting), &value) !=
            DC1394_SUCCESS) {
    ...
    bool DC1394Camera::setFeature (CameraFirewire::CameraFeature setting,
                                   std::vector< double > values)
    {
        ...
        if (dc1394_feature_is_switchable (_dccamera, settingsConverter (setting), &is_switchable) !=
            DC1394_SUCCESS) {
            RW_WARN ("Unable to read if the feature is switchable.");
            return false;
        }
        if (is_switchable == DC1394_TRUE) {
            if (dc1394_feature_set_power (_dccamera, settingsConverter (setting), DC1394_ON) !=
                DC1394_SUCCESS) {
        ...
        if (dc1394_feature_set_mode (_dccamera,
                                     settingsConverter (setting),
        ...
        switch (setting) {
            case WHITEBALANCE:
                ...
                return dc1394_feature_whitebalance_set_value (_dccamera, vals[0], vals[1]) ==
                       DC1394_SUCCESS;
            default:
                return dc1394_feature_set_value (_dccamera, settingsConverter (setting), vals[0]) ==
                       DC1394_SUCCESS;
        }
    ...
    bool DC1394Camera::getFormat7ImageSize (const CameraFirewire::Format7Mode mode,
                                            unsigned int& width, unsigned int& heigth)
    {
        if (dc1394_format7_get_image_size (_dccamera, modeConverter (mode), &width, &heigth) !=
            DC1394_SUCCESS) {
    ...
    bool DC1394Camera::getFormat7ImageMaxSize (const CameraFirewire::Format7Mode mode,
                                               unsigned int& width, unsigned int& heigth)
    {
        if (dc1394_format7_get_max_image_size (_dccamera, modeConverter (mode), &width, &heigth) !=
            DC1394_SUCCESS) {
    ...
    bool DC1394Camera::getFormat7ImagePos (unsigned int& left, unsigned int& top)
    {
        ...
        if (dc1394_format7_get_image_position (_dccamera, captureMode, &left, &top) !=
            DC1394_SUCCESS) {
    ...
    bool DC1394Camera::getFormat7ColorCoding (CameraFirewire::ColorCode& color)
    {
        ...
        if (dc1394_format7_get_color_coding (_dccamera, captureMode, &colorCode) !=
            DC1394_SUCCESS) {
    ...
    bool DC1394Camera::getFormat7PacketSize (unsigned int& packetSize)
    {
        ...
        if (dc1394_format7_get_packet_size (_dccamera, captureMode, &packetSize) !=
            DC1394_SUCCESS) {
    ...
    bool DC1394Camera::getFormat7RecommendedPacketSize (const CameraFirewire::Format7Mode mode,
                                                        unsigned int& packetSize)
    {
        if (dc1394_format7_get_recommended_packet_size (
                _dccamera, modeConverter (mode), &packetSize) != DC1394_SUCCESS) {
            RW_WARN ("Unable to read packet size from camera.");
            return false;
        }
        return true;
