...
const std::string CMU1394CameraC::sFrameRates[6] = {
...
CMU1394CameraC::CMU1394CameraC (CameraID cmuCam, std::string camName, std::string vendorName) :
    CameraFirewire (camName, vendorName), _policy (CONTINUES), _isAquired (false), _camID (cmuCam),
    _image (NULL)
{
...
void CMU1394CameraC::stop ()
{
    ...
    switch (StopImageCapture_c (
        _camID))    // edr: changed from StopImageAcquisition() to StopImageCapture()
    // switch( StopImageAcquisition_c(_camID) ) // edr: changed from StopImageAcquisition() to
    // StopImageCapture()
    {
        case CAM_SUCCESS:
            _started = false;
...
bool CMU1394CameraC::start ()
{
    ...
    switch (StartImageCapture_c (_camID)) {
        ...
        case CAM_SUCCESS: {
            ...
            TimerUtil::sleepMs (CAM_DELAY_SLEEP);
            switch (
                CaptureImage_c (_camID))    // edr: Changed from AcquireImage() to CaptureImage()!
            // switch(AcquireImage_c(_camID)) // edr: Changed from AcquireImage() to CaptureImage()!
            {
                case CAM_SUCCESS: status = true; break;
                case CAM_ERROR_NOT_INITIALIZED: Log::errorLog () << "Camera not initialized"; break;
                default: Log::errorLog () << "Initialization of image buffer failed"; break;
            }
            _started = true;
            status   = true;
            break;
        }
        case CAM_ERROR_NOT_INITIALIZED: Log::errorLog () << "Must call Initialize() first."; break;
        case CAM_ERROR_INVALID_VIDEO_SETTINGS:
            Log::errorLog ()
                << "The current video settings (format, mode, and/or rate) are not supported.";
            break;
        case CAM_ERROR_BUSY:
        ...
        case ERROR_OUTOFMEMORY:
            ...
            break;
        default: Log::errorLog () << "Error in device driver."; break;
    }
    return status;
}
...
const rw::sensor::Image* CMU1394CameraC::grab ()
{
    ...
    switch (AcquireImage_c (_camID)) {
        case CAM_SUCCESS: status = true; break;
        case CAM_ERROR_NOT_INITIALIZED:
            Log::errorLog () << "CCameraFirewireCMU::Acquire() StartImageCapture() has not been "
                                "successfully called.";
            break;
        default: Log::errorLog () << "CCameraFirewireCMU::Acquire() Error acquiring frame."; break;
    }

    // make sure the image has the right dimensions
    // if ((Image.GetWidth()!=m_Width) || (Image.GetHeight()!=m_Height) ||
    // (Image.GetBits()!=m_BitsPerPixel))
    //{
    ...
    if (status) {
        ...
        if (dataLen > width * height) {
            std::cout << "Image is toooo small: " << width << " " << height << " _-> " << dataLen
                      << std::endl;
            // resize image
        ...
        if (_image->getBitsPerPixel () == 8) {
            ...
            for (size_t lineNo = 0; lineNo < height; lineNo++) {
                ...
                memcpy (
                    &destination[lineNo * width], &source[(height - lineNo - 1) * width], width);
            }
        }
        ...
        //              //copies a line at a time
        //              memcpy(destination->ppAllScanLines[lineNo],
        //              &tempData[(m_Height-lineNo-1)*m_Width*3], m_Width*3);
        //          free(tempData);
        //      }
        // else if(m_BitsPerPixel == 24)
