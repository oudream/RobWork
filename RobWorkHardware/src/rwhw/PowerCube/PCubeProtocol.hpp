...
#define PCUBE_DefLinRatio \
    2    // 0x02 // Transmission factor for transforming rotary in linear motion (Default value)
         // float (read)
...
#define PCUBE_DefMaxDeltaPos \
    5    // 0x05  Maximum following error (tow) for the digital servo filter (Default value)
         // float x
#define PCUBE_DefMaxDeltaVel \
    6    // 0x06  Maximum following error for velocity control (Default value)
         // float (read)
#define PCUBE_DefTorqueRatio 7    // 0x07  Transmission factor for transforming current to torque
                                  // (Default value) float (read)
#define PCUBE_DefCurRatio \
    8    // 0x08 Transmission factor for current measurement (Default value) float x
...
#define PCUBE_DefPulsesPerTurn \
    28    // 0x1c Number of Encoder ticks per revolution (Default value) UInt32 x
...
#define PCUBE_DefBrakeTimeOut \
    31                          // 0x1f Delay for releasing the brake in ms (Default value) UInt16 x
...
#define PCUBE_RefPosCount \
    37    // 0x25 Absolute Counter value at Homing position (Actual value) Int32 x
...
#define PCUBE_TargetVelInc \
    41    // 0x29 Target velocity in Encoder ticks/s (Target value) UInt32 x x
#define PCUBE_TargetAccInc \
    42    // 0x2a Target accedleration in Encoder ticks/s� (Target value) UInt32 x x
#define PCUBE_StepInc \
    43    // 0x2b Step mode target position in Encoder ticks (Actual value) UInt32 x
#define PCUBE_HomeOffsetInc 44    // 0x2c Home offset in Encoder ticks (Actual value) Int32 x
#define PCUBE_RawCur \
    53    // 0x35 Commanded Current in Digits [-500...+500] (Actual value) Int16 x x
...
#define STATE_ERROR \
    0x00000001    // An error occured. The module stop immediately and does not accept motion
                  // commands anymore. The reason for the error state can be found reading the error
                  // flags. In many cases the error state can be reset by the user sending a Reset
                  // command. After a successful Reset the module is ready again to accept motion
                  // commands.
#define STATE_HOME_OK \
    0x00000002    // This flag is set after a successful homing procedure. It means that the drive
                  // has successfully found its zero position. All limitations for the operation
                  // range are valid now. If the user sends another Home-Command the flag will be
                  // reset until the homing procedure has been finished successfully.
#define STATE_HALTED \
    0x00000004    // This flag is set in conjunction with an emergency stop. It means that the cube
                  // is in a secure state, not moving and not accepting motion commands. Only after
                  // a reset command which resets this flag, the module will return to the normal
                  // operation mode. An emergency stop can be caused automatically by the module in
                  // case of an error or by the user when sending a Halt command.
#define STATE_POWERFAULT \
    0x00000008    // This flag defines an error of the servo amplifier. This flag si set in
                  // conjuction with
...
#define STATE_TOW_ERROR \
    0x00000010    // Tow error: The servo loop was not able to follow the target position within the
                  // given limit. The maximum tow can be adjsuted using the parameter �MaxDeltaPos".
                  // Check if the module was overloaded.
#define STATE_COMM_ERROR \
    0x00000020    // A data transmission error occured. If this error flag is set you should check
                  // the transmission line (cable, end resistor, grounding).
#define STATE_SWR \
    0x00000040    // This flag shows the state of the home switch. Flag set means home switch is
                  // active, This is no error flag.
#define STATE_SW1 \
    0x00000080    // This flag shows the state of the Limit switch 1. Flag set means limit switch 1
                  // is active. This is no error flag.
#define STATE_SW2 \
    0x00000100    // This flag shows the state of the Limit switch 2. Flag set means limit switch 2
                  // is active. This is no error flag.
#define STATE_BRAKEACTIVE \
    0x00000200    // This flag shows the state of the brake. Flag set means brake is active and
                  // servo loop is open. This is no error flag and it is used only if a brake is
                  // installed.
#define STATE_CURLIMIT \
    0x00000400    // This flag is a warning of the servo loop. It has reached the maximum current
                  // output. The drive is working at its limits. This flag can be reset by the Reset
                  // command. It is no error flag
#define STATE_MOTION \
    0x00000800    // This flag indicates the drive is in motion. It is set and reset automatically.
#define STATE_RAMP_ACC \
    0x00001000    // This flag indicates the drive is in acceleration when controlled by ramp motion
                  // commands. It is automatically reset when the ramp motion profile has ended.
#define STATE_RAMP_STEADY \
    0x00002000    // This flag indicates the drive is moving at constant speed when controlled by
                  // ramp motion commands. It is automatically reset when the ramp motion profile
                  // has ended.
#define STATE_RAMP_DEC \
    0x00004000    // This flag indicates the drive is in deceleration when controlled by ramp motion
                  // commands. It is automatically reset when the ramp motion profile has ended.
#define STATE_RAMP_END \
    0x00008000    // This flag indicates the end of a ramp motion profile. The drive is not moving.
#define STATE_INPROGRESS \
    0x00010000    // This flag is only used in Step motion control. It indicates a Step motion
                  // command is in progress.
#define STATE_FULLBUFFER \
    0x00020000    // This flag is only used in Step motion control. It indicates a Step motion
                  // command was pushed to the command stack. This happens when the module receives
                  // a Step motion command while STATE_INPROGRESS is set. Upon completion of the
                  // currently executed step command, the buffered one will automatically be
                  // executed.
#define STATE_POW_VOLT_ERR \
    0x00040000    // This flag is set in conjunction with STATE_POWERFAULT. It indicates the a
                  // voltage drop in the servo loop. This error can be reset after the normal
                  // voltage level has been restored. Check your power supply.
#define STATE_POW_FET_TEMP \
    0x00080000    // This flag is set in conjunction with STATE_POWERFAULT. The power transistors
...
#define STATE_POW_WDG_TEMP \
    0x00100000    // This flag is set in conjunction with STATE_POWERFAULT. The motor has overheated
...
#define STATE_POW_SHORTCUR \
    0x00200000    // This flag is set in conjunction with STATE_POWERFAULT. A short curcuit occured.
                  //  The servo loop has been disabled. The power must be switched of to reset this
                  //  error. The module has been overlaoded. If this error cannot be reset consult
                  //  your service partner.
#define STATE_POW_HALLERR \
    0x00400000    // This flag is set in conjunction with STATE_POWERFAULT. An error occured in
                  // reading the hall effect sensors of the motor. The motor has been overheated.
                  // Power must be switched off to reset this error.
#define STATE_POW_INTEGRALERR \
    0x00800000    // This flag is set in conjunction with STATE_POWERFAULT. The drive has been
...
#define STATE_CPU_OVERLOAD \
    0x01000000    // Communication breakdown between CPU and current controller. Power must be
                  // switched off. Please consult your service partner.
#define STATE_BEYOND_HARD \
    0x02000000    // This flag indicates the module has reached the hard limit. An emergency stop
                  // has been executed automatically. To remove the module from this position you
                  // need to follow the procedure described in "Plustronik� Operation System:
                  // Disorder".
#define STATE_BEYOND_SOFT \
    0x04000000    // This flag indicates the module has reached the soft limit. An emergency stop
                  // has been executed automatically. This flag can be reset by a Reset command.

#define STATE_POW_SETUP_ERR \
    0x08000000    // Error in initializing the current controller. Module settings disaccord with
                  // controller configuration (5A/10A types). Power must be switched off. Please
                  // consult your service partner. Available from version 3.5.14
...
struct Cmd
{
    ...
    Cmd (CMDType cmd_Type, std::vector< unsigned char > data) : cmdType (cmd_Type), data (data) {}
};
}    // namespace rwhw
...
{
  public:
    static std::vector< unsigned char > makeData (int commandId, int motionId, int val);

    static std::vector< unsigned char > makeData (int commandId, int motionId, float val);

    static std::vector< unsigned char > makeData (int commandId, int motionId, float x, int y);

    static std::vector< unsigned char > makeData (int commandId, int motionId, int x, int y);

    static std::vector< unsigned char > makeData (int commandId, int motionId);

    // B�gild
    static std::vector< unsigned char > makeData (int commandId, int parameterId, unsigned int val);

    static std::vector< unsigned char > makeData (unsigned char x);

    static float toFloat (unsigned char b0, unsigned char b1, unsigned char b2, unsigned char b3)
    {
        ToData tofloat (1);
        tofloat.data[0] = b0;
        tofloat.data[1] = b1;
    ...
    static int toInt (unsigned char b0, unsigned char b1, unsigned char b2, unsigned char b3)
    {
        ToData toint (1);
        toint.data[0] = b0;
        toint.data[1] = b1;
