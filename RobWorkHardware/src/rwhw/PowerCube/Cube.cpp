...
Cube::Cube (int moduleId, CubePort* port) : _moduleId (moduleId), _port (port)
{}
...
bool Cube::ping (int moduleNr, CubePort* port)
{
    ...
    /*
        emitp( Cmd( Cmd::PUT,
              ...
              moduleNr);
    */
    emitp (Cmd (Cmd::GET, PCubeProtocol::makeData (PCUBE_GetExtended, PCUBE_DefCubeVersion)),
           port,
           moduleNr);
...
{
    emitp (Cmd (Cmd::ALL, PCubeProtocol::makeData (PCUBE_ResetAll)), port);
}

void Cube::homeAllCmd (CubePort* port)
{
    emitp (Cmd (Cmd::ALL, PCubeProtocol::makeData (PCUBE_HomeAll)), port);
}

void Cube::haltAllCmd (CubePort* port)
{
    emitp (Cmd (Cmd::ALL, PCubeProtocol::makeData (PCUBE_HaltAll)), port);
}
// watchdog port, _moduleId )h all
void Cube::watchdogAllCmd (CubePort* port)
{
    emitp (Cmd (Cmd::ALL, PCubeProtocol::makeData (PCUBE_WatchdogRefreshAll)), port);
}

void Cube::setBaudrateAll (CubeBaudrate rate, CubePort* port)
{
    emitp (Cmd (Cmd::ALL, PCubeProtocol::makeData (PCUBE_SetBaudAll, rate)), port);
}

void Cube::savePosAllCmd (CubePort* port)
{
    emitp (Cmd (Cmd::ALL, PCubeProtocol::makeData (PCUBE_SavePosAll)), port);
}

void Cube::syncMotionAllCmd (CubePort* port)
{
    emitp (Cmd (Cmd::ALL, PCubeProtocol::makeData (PCUBE_SyncMotionAll)), port);
}
...
/*void Cube::setTargetVel( float paramval ){
        emitCmd( Cmd(Cmd::PUT, PCubeProtocol::makeData(PCUBE_SetExtended, PCUBE_TargetVel ,
paramval)) ); io::CubePort::Message msg; if( !ackext( Cmd(Cmd::ACK,
PCubeProtocol::makeData(PCUBE_SetExtended, PCUBE_Config)), msg) ) RW_WARN("SET command did not
function correct!!");
}

// Bogild
void Cube::setTargetAcc( float paramval ){
        emitCmd( Cmd(Cmd::PUT, PCubeProtocol::makeData(PCUBE_SetExtended, PCUBE_TargetAcc ,
paramval)) ); io::CubePort::Message msg; if( !ackext( Cmd(Cmd::ACK,
PCubeProtocol::makeData(PCUBE_SetExtended, PCUBE_Config)), msg) ) RW_WARN("SET command did not
function correct!!");
}*/
