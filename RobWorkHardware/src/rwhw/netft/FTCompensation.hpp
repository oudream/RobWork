...
class FTCompensation
{
  public:
    ...
    /**
     * @brief Constructor for an input device, a workcell state which is copied and a calibration
     * file.
     *
     ...
     */
    FTCompensation (rw::models::Device::Ptr dev, rw::kinematics::State state,
                    const std::string& calibfile = "ftcalib.xml",
                    const Wrench3D& thres        = Wrench3D (rw::math::Vector3D<> (5, 5, 5),
                                                      rw::math::Vector3D<> (0.5, 0.5, 0.5)));
     ...
     *
     * The use of this function is in general discouraged since higher-order differentials
     * destabilize.
     *
    ...
    inline void update (const Wrench3D& ft, const rw::math::Q& q)
    {
        ...
        const boost::posix_time::ptime currentT =
    ...
    /**
     * @brief Updates the collision flag based on current joint configuration, velocity and F/T
     * vector. Joint velocity and acceleration are estimated based on input and an internal timer.
     *
     * @param ft current wrench
     * @param q current joint configuration
     ...
     */
    void update (const Wrench3D& ft, const rw::math::Q& q, const rw::math::Q& dq,
                 const rw::math::Q& ddq, double dt);
     ...
     */
    static bool LoadCalib (const std::string& filename, FTCalib& calib,
                           rw::math::Transform3D<>& eTft);

    /**
     * Update the F/T sensor bias based on a set of recent measurements, which can be necessary
     * under e.g. temperature drift.
     *
     * It is the intention that this function be used from a set of recent outputs using \code
     * getFT() \endcode, which should ideally be zero. Therefore, it is advised to keep the sensor
     * steady while retrieving a set of e.g. 20 measurements as input for this function.
     *
    ...
    inline void updateBias (const std::vector< Wrench3D >& biases)
    {
        ...
        Wrench3D meanBias;
        for (std::vector< Wrench3D >::const_iterator it = biases.begin (); it != biases.end ();
             ++it) {
     ...
     */
    inline void resetBias (const Wrench3D& bias) { _calib.bias = bias; }
