...
class NetFTLogging
{
  public:
    ...
    {
        // Constructor
        NetFTData (unsigned int statuss, unsigned int lostt, unsigned int countt,
                   const Wrench3D& dataa, double timestampp) :
            status (statuss),
            lost (lostt), count (countt), data (dataa), timestamp (timestampp)
        {}
    ...
    // Constructor
    NetFTLogging (const std::string& address = "192.168.1.1", unsigned short port = 49152,
                  unsigned int countsPerForce = 1000000, unsigned int countsPerTorque = 1000000);
  ...
  private:
    ...
    struct Command
    {
        ...
        Command (uint16_t ct = CMD_START_HIGH_SPEED_STREAMING, uint32_t sz = INFINITE_SAMPLES) :
            _header (0x1234), _command (ct), _count (sz)
        {}
         ...
         */
        enum COMMAND_TYPE { CMD_STOP_STREAMING = 0, CMD_START_HIGH_SPEED_STREAMING = 2 };

        // Special values for misc. sizes
        enum SIZES { INFINITE_SAMPLES = 0, RDT_COMMAND_SIZE = 8 };
