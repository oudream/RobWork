...
FTCompensation::FTCompensation (Device::Ptr dev, State state, const std::string& calibfile,
                                const Wrench3D& thres) :
    _dev (dev),
    _state (state), _thres (thres)
{
...
void FTCompensation::update (const Wrench3D& ft, const Q& q, const Q& dq, const Q& ddq, double dt)
{
    ...
    _collision =
        std::abs (_ft.first[0]) > _thres.first[0] || std::abs (_ft.first[1]) > _thres.first[1] ||
        std::abs (_ft.first[2]) > _thres.first[2] || std::abs (_ft.second[0]) > _thres.second[0] ||
        std::abs (_ft.second[1]) > _thres.second[1] || std::abs (_ft.second[2]) > _thres.second[2];
}
...
Vector3D<> FTCompensation::ddp (double dt)
{
    ...
    ts[0].reserve (_horizon);
    ts[1].reserve (_horizon);
    ts[2].reserve (_horizon);
    ...
    tlpfs[0].reserve (_horizon - 1);
    tlpfs[1].reserve (_horizon - 1);
    tlpfs[2].reserve (_horizon - 1);
    for (circular_buffer< Vector3D<> >::const_iterator it = _tlpfs.begin (); it != _tlpfs.end ();
         ++it) {
...
bool FTCompensation::LoadCalib (const std::string& filename, FTCalib& calib, Transform3D<>& eTft)
{
    try {
        ...
        // Get calibrated tool roll angle and distance to COG and set transformation from robot tool
        // to F/T tool
        ...
        std::cerr << "Parsing of the XML file failed due to a bad path:" << std::endl
                  << "\t" << e.what () << std::endl;
    }
    catch (const ptree_error& e) {
        std::cerr << "Parsing of the XML file failed:" << std::endl
                  << "\t" << e.what () << std::endl;
    }
    catch (const std::exception& e) {
        std::cerr << "Reading of the XML file failed:" << std::endl
                  << "\t" << e.what () << std::endl;
    }
    catch (...) {
