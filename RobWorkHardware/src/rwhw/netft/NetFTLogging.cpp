...
NetFTLogging::NetFTLogging (const std::string& address, unsigned short port,
                            unsigned int countsPerForce, unsigned int countsPerTorque) :
    _address (address),
    _port (port), _socket (_ioservice), _countsF (countsPerForce), _countsT (countsPerTorque),
    _threadRunning (false), _stopThread (false), _lastRdtSequence (0), _systemStatus (0),
    _outOfOrderCount (0), _lostPackets (0), _packetCount (0)
{
    // Set scaling
    ...
    try {
        ...
        while (!_stopThread) {
            ...
            if (len == Message::RDT_RECORD_SIZE) {
                ...
                RW_WARN ("Packet size: " << len << " does not match expected size: "
                                         << Message::RDT_RECORD_SIZE);
            }
        }
    }
...
void NetFTLogging::print (std::ostream& os, const NetFTLogging::NetFTData& netftAllData) const
{
    ...
    os << "Data {Fx, Fy, Fz, Tx, Ty, Tz}: {" << data.first[0] << ", " << data.first[1] << ", "
       << data.first[2] << ", " << data.second[0] << ", " << data.second[1] << ", "
       << data.second[2] << "}" << std::endl;
}
...
void NetFTLogging::print (std::ostream& os, const Wrench3D& netftData) const
{
    os << netftData.first[0] << " " << netftData.first[1] << " " << netftData.first[2] << " "
       << netftData.second[0] << " " << netftData.second[1] << " " << netftData.second[2]
       << std::endl;
}
