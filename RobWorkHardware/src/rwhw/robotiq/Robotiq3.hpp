...
class Robotiq3 : public Robotiq
{
  public:
    ...
    /*
     Global Gripper Status - A global Gripper Status is available. This gives information such as
     which Operation Mode is currently active or if the Gripper is closed or open.

     Object Status - There is also an Object Status that let you know if there is an object in the
     Gripper and, in the affirmative, how many fingers are in contact with it.

     Fault Status - The Fault Status gives additional details about the cause of a fault.

     Position Request Echo - The Gripper returns the position requested by the robot to make sure
     that the new command has been received correctly.

     Motor Encoder Status - The information of the encoders of the four motors is also available.

     Current Status - The current of the motors can also be known. Since the torque of the motor is
     a linear function of the current, this gives information about the torque applied.
     *
     */
    ...
    virtual bool isGripperBlocked ()
    {
        return _statusDataValid && (_gripperStatus.data.gSTA == 2 || _gripperStatus.data.gSTA == 1);
    }
