...
Robotiq::Robotiq (rw::math::Q currentQ, rw::math::Q currentCurrent, rw::math::Q target,
                  rw::math::Q speed, rw::math::Q force, unsigned int numberOfJoints) :
    _stop (true),
    _socket (0), _connected (false), _packageIDCounter (0), _currentQ (currentQ),
    _currentCurrent (currentCurrent), _target (target), _speed (speed), _force (force),
...
bool Robotiq::activate (unsigned int timeout)
{
    ...
    if (n != 6 || answer.data[0] != 0x00 || answer.data[1] != 0x00 || answer.data[2] != 0x00 ||
        answer.data[3] != 0x03) {
        RW_THROW ("Received message is wrong size (is " << n << " should be " << 6
                                                        << ") or wrong content");
    }
        ...
        if (_connected) {
            // read status package from gripper
            {
                ...
                if (bytesReady > 7) {
                    ...
                    //                    // todo Move this to trace level instead of debug once
                    //                    trace is implemented if (Log::log().isEnabled(Log::Debug))
                    //                    {
                    //                        std::stringstream traceMessage;
                    //                        traceMessage << "Received Header: ";
                    //                        traceMessage.fill('0');
                    //                        for (int i = 0; i < 8; i++) {
                    //                            traceMessage << std::setw(2) << std::hex <<
                    //                            (unsigned int) package.header.value[i] << " ";
                    //                        }
                    //                        traceMessage << ": ";
                    //
                    //                        for (int i = 0; i < n - 2; i++) {
                    //                            traceMessage << std::setw(2) << std::hex <<
                    //                            (unsigned int) package.data[i] << " ";
                    //                        }
            ...
            while (_packagesOutgoing.size () != 0) {
                ...
                //                // todo Move this to trace level instead of debug once trace is
                //                implemented if (Log::log().isEnabled(Log::Debug)) {
                //                    std::stringstream traceMessage;
                //                    traceMessage << "Sending Header: ";
                //                    traceMessage.fill('0');
                //                    for (int i = 0; i < 8; i++) {
                //                        traceMessage << std::setw(2) << std::hex << (unsigned int)
                //                        package.header.value[i] << " ";
                //                    }
                //                    traceMessage << ": ";
                //                    for (int i = 0; i < n - 2; i++) {
                //                        traceMessage << std::setw(2) << std::hex << (unsigned int)
                ...
                if (bytesTransfered == bytesToBeTransfered) {
                    ...
                    /* Unsuccessful sent */
                    RW_LOG_DEBUG ("Unable to send all the '" << bytesToBeTransfered
                                                             << "' bytes - only sent '"
