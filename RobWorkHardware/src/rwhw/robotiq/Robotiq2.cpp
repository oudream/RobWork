...
Robotiq2::Robotiq2 () :
    Robotiq (rw::math::Q (1, 0.0), rw::math::Q (1, 0.0), rw::math::Q (1, 0.0),
             rw::math::Q (1, 255.0), rw::math::Q (1, 255.0), 1),
...
void Robotiq2::validateStatusPackageAndUpdateState (const ModbusPackage& package)
{
    ...
    if (package.data[0] != 6 || n != 6 + 3)
        RW_THROW ("answer should contain 6 bytes and not "
                  << (int) package.data[0] << " and the length should be 9 instead of " << n);
