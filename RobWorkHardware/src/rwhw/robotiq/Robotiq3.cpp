...
Robotiq3::Robotiq3 () :
    Robotiq (rw::math::Q (4, 0, 0, 0, 0), rw::math::Q (4, 0, 0, 0, 0), rw::math::Q (4, 0, 0, 0, 0),
             rw::math::Q (4, 0xFF, 0xFF, 0xFF, 0xFF), rw::math::Q (4, 0xFF, 0xFF, 0xFF, 0xFF), 4),
...
void Robotiq3::validateStatusPackageAndUpdateState (const ModbusPackage& package)
{
    ...
    if (package.data[0] != 16 || n != 16 + 3)
        RW_THROW ("answer should contain 16 bytes and not "
                  << (int) package.data[0] << " and the length should be 19 instead of " << n);
