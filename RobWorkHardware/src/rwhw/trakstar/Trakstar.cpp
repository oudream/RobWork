...
void Trakstar::initializeBird ()
{
    ...
    if (_errorCode != BIRD_ERROR_SUCCESS) {
        ...
        // eg. 120 / (3*80) = 0.500seconds per update. Similarly 1 / (3*80) = 4ms per update or
        // 240hz
        ...
        if (_errorCode != BIRD_ERROR_SUCCESS) {
            ...
            for (int i = 0; i < _ATC3DG.numberSensors; i++) {
                ...
                /*						HEMISPHERE_TYPE buffer;
                                _errorCode = GetSensorParameter(i, HEMISPHERE, &buffer,
                   sizeof(buffer)); if(_errorCode!=BIRD_ERROR_SUCCESS) errorHandler(_errorCode,
                   __LINE__); cout << buffer << endl;*/
            }
        }
    }
...
void Trakstar::startPolling ()
{
    ...
    for (int i = 0; i < _ATC3DG.numberSensors; i++) {
        DATA_FORMAT_TYPE type =
            DOUBLE_POSITION_QUATERNION_TIME_Q_BUTTON;    // DOUBLE_POSITION_QUATERNION_TIME_Q_BUTTON_RECORD
    ...
    _errorCode = GetSynchronousRecord (
        ALL_SENSORS, &_rawValues[0], sizeof (_rawValues[0]) * _rawValues.size ());
    if (_errorCode != BIRD_ERROR_SUCCESS)
        errorHandler (_errorCode, __LINE__);
        ...
        if (!_flagStopPoll) {
            ...
            for (_sensorID = 0; _sensorID < _ATC3DG.numberSensors; _sensorID++) {
                ...
                if (status == VALID_STATUS) {
                    ...
                    // This would be a place to debug for sensor-data not read if that error
                    // present.
