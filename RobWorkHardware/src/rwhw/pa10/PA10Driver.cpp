...
bool PA10Driver::receive_pos ()
{
    smsc_packet b;
    Q joint_pos_real (7);
    while (!smsc_recvpacket (0, &b))
        TimerUtil::sleepUs (100);
    if (b.type == 'C') {
        for (unsigned int i = 0; i < joint_pos_real.size (); i++) {
            joint_pos_real (i) =
                (double) ((int) ((b.data[i * 8 + 2] & 0xff) + (b.data[i * 8 + 3] << 8) +
                                 (b.data[i * 8 + 4] << 16) + (b.data[i * 8 + 5] << 24)));
...
{
    for (int i = 0; i < 7; i++) {
        long val = data->axis[i].angle[0] + (data->axis[i].angle[1] << 8) +
                   (data->axis[i].angle[2] << 16) + (data->axis[i].angle[3] << 24);
...
void PA10Driver::pa10_send_S ()
{
    ...
    p.type = 'S';
    p.len  = 0;
    while (!smsc_sendpacket (0, &p))
        std::cout << ".";    // TimerUtil::SleepUs(WAIT);
    TimerUtil::sleepUs (5);
}
