...
void SchunkPG70::close ()
{
    try {
        _port->clean ();
        while (!_cube->resetCmd ()) {
            rw::common::TimerUtil::sleepMs (100);
            _port->clean ();
        }
        _port->clean ();

        while (!_cube->moveCurCmd (_graspCurrent)) {
            rw::common::TimerUtil::sleepMs (100);
            _port->clean ();
        }
    }
...
bool SchunkPG70::setQ (const rw::math::Q& q)
{
    ...
    try {
        ...
        _port->clean ();
        while (!_cube->setTargetVel (VEL)) {
            rw::common::TimerUtil::sleepMs (100);
            _port->clean ();
        };
        _port->clean ();
        while (!_cube->setTargetAcc (ACC)) {
            rw::common::TimerUtil::sleepMs (100);
            _port->clean ();
        };
        _port->clean ();
        while (!_cube->moveRampCmd ((float) qTmp[0])) {
            rw::common::TimerUtil::sleepMs (100);
            _port->clean ();
        };
        unsigned int statusMem = 0;
        ...
        do {
            ...
            if ((statusMem & (STATE_BEYOND_SOFT | STATE_BEYOND_HARD)) > 0) {
                ...
                break;
                  ...
                  (STATE_MOTION | STATE_RAMP_ACC | STATE_RAMP_STEADY | STATE_RAMP_DEC)) != 0 ||
                 (!getStatusMem));
