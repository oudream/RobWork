...
FanucDriver::FanucDriver (std::string ipNr, int updateRate) :
    _updateRate (updateRate), _ipNr (ipNr), _isLibraryOpen (false), _isConnected (false),
    _qCurrent (Q::zero (6)), _qdCurrent (Q::zero (6)), _timeStamp (CurrentTimeMs ()),
    _nextCmd (IdleCMD), _lastCmd (IdleCMD), _accCnt (100), _accFine (100), _speedCnt (100),
    _speedFine (100), _globalSpeed (10), _accCntChanged (false), _accFineChanged (false),
    _speedCntChanged (false), _speedFineChanged (false), _globalSpeedChanged (false),
    _isCallFinished (true), _notifyProgram (false), _isCallWaiting (false), _error (0.0),
    _updateCnt (0), _resetError (false)
{
    ...
    switch (_nextCmd) {
        ...
        case (CallCMD): {
            if (_cmdChanged) {
                ...
                std::cout << "Call program: " << _progNrToCall << "(" << _callArg[0] << " , "
                          << _callArg[1] << " , " << _callArg[2] << ")\n";
