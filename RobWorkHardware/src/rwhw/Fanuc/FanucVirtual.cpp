...
FanucVirtual::FanucVirtual (rw::models::SerialDevice* fanucModel, const State& state) :
    _model (fanucModel), _state (state), _isConnected (false), _qCurrent (Q::zero (6)),
    _dqCurrent (Q::zero (6)), _timeStamp (currentTimeMs ()), _nextCmd (IdleCMD), _lastCmd (IdleCMD),
    ...
    _speedFineChanged (false), _globalSpeedChanged (false)
{
    ...
    for (std::size_t i = 0; i < dof; i++) {
        posLimit.push_back (
            std::make_pair (_model->getBounds ().first[i], _model->getBounds ().second[i]));
...
{
    std::cout << "Robot program nr. " << progNr << " called!\n"
              << "  --- with arg (" << _callArg[0] << "," << _callArg[1] << "," << _callArg[2]
              << ")\n";
}
