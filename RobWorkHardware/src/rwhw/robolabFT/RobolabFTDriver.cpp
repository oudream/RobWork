...
RobolabFT::RobolabFT () : _isRunning (false), _dataInit (false)
{}
RobolabFT::~RobolabFT ()
{
    _serialPort.close ();
...
RobolabFT::RobolabFTData RobolabFT::read ()
{
    ...
    if (lenght == 0)
        std::cout << "lenght: " << lenght << " found: " << found << "_dataOut is:" << _dataOut
                  << "str is:" << str << std::endl;
    ...
    std::cout << "foces: " << tmpData.data.first << "tourques: " << tmpData.data.second
              << std::endl;
    return tmpData;
}
