...
SceneViewer::View::Ptr SceneOpenGLViewer::createView(const std::string& name,
    ...
    if(enableBackground) { nview->_viewCamera->setClearBufferEnabled(false); }
    else { nview->_viewCamera->setClearBufferEnabled(true); }
...
void SceneOpenGLViewer::setWorldNode(rw::graphics::GroupNode::Ptr wnode) {
    ...
    if(wnode == NULL) { _mainCam->setEnabled(false); }
    else { _mainCam->setEnabled(true); }
...
void SceneOpenGLViewer::renderView(View::Ptr view) {
    ...
    if(isGuiThread) { renderViewThreadSafe(view); }
    else {
...
void SceneOpenGLViewer::initializeGL() {
    ...
    if(_pmap->getValue().add<bool>("GL_LIGHTING", "", true)->getValue()) glEnable(GL_LIGHTING);
    if(_pmap->getValue().add<bool>("GL_NORMALIZE", "", true)->getValue()) glEnable(GL_NORMALIZE);
    if(_pmap->getValue().add<bool>("GL_DEPTH_TEST", "", true)->getValue()) glEnable(GL_DEPTH_TEST);
...
void SceneOpenGLViewer::resizeGL(int width, int height) {
    _width  = width * this->window()->devicePixelRatio();
    _height = height * this->window()->devicePixelRatio();
...
void SceneOpenGLViewer::updateState(const State& state) {
    if(_state == NULL) _state = rw::core::ownedPtr(new State());
    *_state            = state;
...
DrawableNode::Ptr SceneOpenGLViewer::pickDrawable(rw::graphics::SceneGraph::RenderInfo& info, int x,
                                                  int y) {
    makeCurrent();
...
void SceneOpenGLViewer::mousePressEvent(QMouseEvent* event) {
    ...
    else { _cameraCtrl->handleEvent(event); }
...
void SceneOpenGLViewer::propertyChangedListener(PropertyBase* base) {
    ...
    if(id == "ShowCollisionModels") {
        ...
        if(p == NULL) return;
        if(p->getValue()) { dmask = dmask | DrawableNode::CollisionObject; }
        else { dmask = dmask & ~DrawableNode::CollisionObject; }
        ...
        int dmask         = getViewCamera()->getDrawMask();
        if(p == NULL) return;
        if(p->getValue()) { dmask = dmask | DrawableNode::Virtual; }
        else { dmask = dmask & ~DrawableNode::Virtual; }
        getViewCamera()->setDrawMask(dmask);
    }
        ...
        int dmask         = getViewCamera()->getDrawMask();
        if(p == NULL) return;
        if(p->getValue()) { dmask = dmask | DrawableNode::Physical; }
        else { dmask = dmask & ~DrawableNode::Physical; }
        getViewCamera()->setDrawMask(dmask);
    }
        ...
        int dmask         = getViewCamera()->getDrawMask();
        if(p == NULL) return;
        if(p->getValue()) { dmask = dmask | DrawableNode::DrawableObject; }
        else { dmask = dmask & ~DrawableNode::DrawableObject; }
        getViewCamera()->setDrawMask(dmask);
    }
        ...
        int dmask         = getViewCamera()->getDrawMask();
        if(p != NULL) {
            if(p->getValue()) { dmask = dmask | DrawableNode::ALL; }
            else {
                int dmask = 0;
                if(_pmap->getValue().get<bool>("ShowCollisionModels", false))
                    dmask |= DrawableNode::CollisionObject;
    ...
    else if(base == _viewBackground) { _backgroundnode->setVisible(_viewBackground->getValue()); }
