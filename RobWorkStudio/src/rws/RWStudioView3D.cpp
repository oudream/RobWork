...
void RWStudioView3D::setupActions() {
    ...
    _rearViewAction =
        new QAction(QIcon(":/images/rear_view_100.png"), tr("Rear (Ctrl + E)"), this);    // owned
    connect(_rearViewAction, SIGNAL(triggered()), this, SLOT(setCheckAction()));

    _leftViewAction =
        new QAction(QIcon(":/images/left_view_100.png"), tr("Left (Ctrl + L)"), this);    // owned
    ...
    // outline action
    _zoomAutoAction =
        new QAction(QIcon(":/images/zoom_auto.png"), tr("Autozoom (Ctrl + *)"), this);    // owned
...
RWStudioView3D::RWStudioView3D(RobWorkStudio* rwStudio, QWidget* parent) :
    QWidget(parent), _view(NULL), _wcscene(NULL), _wc(NULL), _rws(rwStudio), _viewLogo("RobWork") {
...
void RWStudioView3D::setWorkCell(rw::models::WorkCell::Ptr wc) {
    ...
    _wc = wc;
    if(_wc == NULL) { RW_THROW("Workcell is null!"); }
...
void RWStudioView3D::mouseDoubleClickEvent(QMouseEvent* event) {
    if(event->button() == Qt::LeftButton && event->modifiers() == Qt::ControlModifier) {
        ...
        else { Log::debugLog() << "Frame is NULL." << std::endl; }
    }
}
...
void RWStudioView3D::keyPressEvent(QKeyEvent* e) {
    if(e->key() == Qt::Key_G && e->modifiers() == Qt::ControlModifier) { saveBufferToFileDialog(); }
    ...
    // change camera view according to the keyboard inputs
        ...
        switch(e->key()) {
            ...
            case(Qt::Key_Left):
                ...
                if(camNr < 0) camNr = int(_sensorCameraViews.size());
                break;
            case(Qt::Key_Right):
                camNr = (int) currentView + 1;
                if(camNr > int(_sensorCameraViews.size())) camNr = 0;
                break;
            default: return;
        }
        ...
        if(camNr == 0) {
            ...
            else { Log::warningLog() << "The selected camera view is not available!\n"; }
            e->accept();
        }
...
void RWStudioView3D::resetCameraViewMenu() {
    ...
    for(QAction* action : actions) { _cameraViewMenu->removeAction(action); }
...
void RWStudioView3D::setMaskCheckAction() {
    ...
    if(_physicalMaskEnabled->isChecked()) mask = mask | DrawableNode::Physical;
    if(_virtualMaskEnabled->isChecked()) mask = mask | DrawableNode::Virtual;
    if(_drawableMaskEnabled->isChecked()) mask = mask | DrawableNode::DrawableObject;
    if(_collisionMaskEnabled->isChecked()) mask = mask | DrawableNode::CollisionObject;
    if(_user1MaskEnabled->isChecked()) mask = mask | DrawableNode::User1;
    if(_user2MaskEnabled->isChecked()) mask = mask | DrawableNode::User2;
    if(_user3MaskEnabled->isChecked()) mask = mask | DrawableNode::User3;
    if(_user4MaskEnabled->isChecked()) mask = mask | DrawableNode::User4;
...
void RWStudioView3D::setCheckAction() {
    ...
    if(obj == _axometricViewAction) {
        _view->setTransform(Transform3D<>::makeLookAt(
            Vector3D<>(v2c.norm2() / sqrt(3.0), v2c.norm2() / sqrt(3.0), v2c.norm2() / sqrt(3.0)),
            center,
            Vector3D<>::z()));
    }
...
void RWStudioView3D::setDrawTypeSlot() {
    if(_showSolidAction->isChecked()) setDrawType(DrawableNode::SOLID);
    else if(_showWireAction->isChecked()) setDrawType(DrawableNode::WIRE);
    else if(_showOutlineAction->isChecked()) setDrawType(DrawableNode::OUTLINE);
    _view->updateView();
}

void RWStudioView3D::setTransparentSlot() {
    double alpha;
    if(_showTransparentAction->isChecked()) alpha = 0.5;
    else alpha = 1.0;

    // set alpha for all Drawable in the view
    for(DrawableNode::Ptr da : _wcscene->getDrawables()) { da->setTransparency(alpha); }
...
void RWStudioView3D::saveBufferToFileDialog() {
    ...
    if(!filename.isEmpty()) {
        try {
            ...
            if(fi.suffix().isEmpty()) fi = QFileInfo(filename + ".png");
