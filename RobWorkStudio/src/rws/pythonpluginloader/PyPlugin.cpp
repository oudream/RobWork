...
bool PyPlugin::initialize(std::string pythonFilePath, std::string pluginName) {
    ...
    if(exsist) {
        ...
        else { RW_THROW("Could not open PyPlugin.py"); }
...
void PyPlugin::open(rw::models::WorkCell* workcell) {
    if(_isPythonInit) { _python.runCode("rws_cpp_link.openWorkCell()\n"); }
}

void PyPlugin::close() {
    if(_isPythonInit) { _python.runCode("rws_cpp_link.closeWorkCell()\n"); }
}

void PyPlugin::stateChangedListener(const State& state) {
    if(_isPythonInit) { _python.runCode("rws_cpp_link.stateChanged()\n"); }
}
