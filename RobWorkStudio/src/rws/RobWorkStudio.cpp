...
RobWorkStudio::RobWorkStudio(const PropertyMap& map) :
    ...
    if(state_vector.size() > 0) {
        ...
        for(int i = 0; i < mainAppState.size(); i++) { mainAppState[i] = (char) state_vector[i]; }
...
void RobWorkStudio::closeEvent(QCloseEvent* e) {
    ...
    for(int i = 0; i < mainAppState.size(); i++) { state_vector[i] = mainAppState[i]; }
    ...
    for(I it = _plugins.begin(); it != _plugins.end(); ++it) { (*it)->QWidget::close(); }
...
void RobWorkStudio::updateLastFiles() {
    ...
    std::vector<std::string> nfiles =
        _settingsMap->get<std::vector<std::string>>("LastOpennedFiles", std::vector<std::string>());
    // remove old actions
    for(size_t i = 0; i < fileactions.size(); i++) { filemenu->removeAction(fileactions[i].first); }
    fileactions.clear();
    ...
    for(size_t i = 0; i < nfiles.size(); i++) {
        ...
        if(!skip) tmp.push_back(nfiles[idx]);
...
void RobWorkStudio::setupFileActions() {
    ...
    QAction* openAction = new QAction(QIcon(":/images/open.png"), tr("&Open..."), this);    // owned
    connect(openAction, SIGNAL(triggered()), this, SLOT(open()));

    QAction* closeAction = new QAction(QIcon(":/images/close.png"), tr("&Close"), this);    // owned
...
void RobWorkStudio::printCollisions() {
    ...
    if(res.collidingFrames.size() > 0) {
        for(const FramePair& pair : res.collidingFrames) {
            std::cout << "Colliding: " << pair.first->getName() << " -- " << pair.second->getName()
                      << std::endl;
...
void RobWorkStudio::setupPluginsMenu(bool create) {
    ...
    if(_pluginsMenu == nullptr) { create = true; }

    if(create) { _pluginsMenu = menuBar()->addMenu(tr("&Plugins")); }
    else { _pluginsMenu->clear(); }
    ...
    else { _pluginsToolBar->clear(); }
}

void RobWorkStudio::loadPlugin(std::string pluginFile, bool visible, int dock) {
    if(boost::filesystem::exists(pluginFile)) { setupPlugin(pluginFile.c_str(), visible, dock); }
}
...
void RobWorkStudio::unloadPlugin() {
    QStringList list;
    for(RobWorkStudioPlugin* pl : _plugins) { list.append(pl->name()); }
...
void RobWorkStudio::closeAllPlugins() {
    ...
    for(PI p = _plugins.begin(); p != _plugins.end(); ++p) closePlugin(**p);
}
...
void RobWorkStudio::openPlugin(RobWorkStudioPlugin& plugin) {
    ...
    try {
        ...
        buf << "Exception in opening of plugin " << StringUtil::quote(plugin.name().toStdString());
#if !defined(RW_MACOS)
...
void RobWorkStudio::closePlugin(RobWorkStudioPlugin& plugin) {
    try {
        ...
        std::stringstream buf;
        buf << "Exception in closing of plugin " << StringUtil::quote(plugin.name().toStdString());
...
void RobWorkStudio::setupPlugin(const QString& pathname, const QString& filename, bool visible,
    ...
    if(py) { setupPyPlugin(pfilename, filename, visible, dock); }
    else { setupPlugin(pfilename, visible, dock); }
}
...
void RobWorkStudio::setupPlugin(const QString& fullname, bool visible, int dock) {
    ...
    if(ext == "py" || ext == ".py") {
        ...
        if(pluginObject != NULL) {
            ...
            if(plugin) { addPlugin(plugin, visible, dockarea); }
            else {
...
void RobWorkStudio::dragEnterEvent(QDragEnterEvent* event) {
    ...
    else if(event->mimeData()->hasHtml()) { event->acceptProposedAction(); }
    else if(event->mimeData()->hasUrls()) { event->acceptProposedAction(); }
    else { event->ignore(); }
}
...
void RobWorkStudio::dropEvent(QDropEvent* event) {
    if(event->mimeData()->hasUrls()) {
        ...
        if(urls.size() == 1) { openFile(urls[0].toLocalFile().toStdString()); }
    }
    else if(event->mimeData()->hasHtml()) { Log::debugLog() << "html dropped: " << std::endl; }
...
void RobWorkStudio::openWorkCellFile(const QString& filename) {
    ...
    try {
        ...
        if(wc == NULL) { RW_THROW("Loading of workcell failed!"); }
    }
...
void RobWorkStudio::setWorkcell(rw::models::WorkCell::Ptr workcell) {
    // Always close the workcell.
    if(_workcell && workcell != _workcell) { closeWorkCell(); }
    ...
    if(workcell && workcell != _workcell) {
        ...
        if(sceneProp.has("ZoomScale")) { sceneProp.set("ZoomScale", scale); }
        else {
            sceneProp.add(
                "ZoomScale", "value [0-1] scaling the zoom factor of the cameracontroller", scale);
        }
    }
}
...
class RobWorkStudioEvent : public QEvent
{
  public:
    ...
    virtual ~RobWorkStudioEvent() { done(); }

    void done() {
        if(_hs != NULL) { *_hs = true; }
    }
};
...
bool RobWorkStudio::event(QEvent* event) {
    ...
    else if(event->type() == QEvent::Close) { QApplication::closeAllWindows(); }
    else {
        // event->ignore();
    }
...
boost::any RobWorkStudio::waitForAnyEvent(const std::string& id, double timeout) {
    ...
    genericAnyEvent().add(
        boost::bind(&AnyEventListener::cb, &listener, boost::arg<1>(), boost::arg<2>()), &listener);
    ...
    // now return result
    if(reachedTimeout) RW_THROW("Timeout!");
...
rw::geometry::AABB<double> RobWorkStudio::calculateWorkCellSize() {
    ...
    for(Object::Ptr object : objects) {
        for(rw::geometry::Geometry::Ptr geom : object->getGeometry(state)) {
            ...
            RW_ASSERT(frame);
            spheres.push_back(rw::geometry::BSphere<double>::fitEigen(geom->getGeometryData()));
    ...
    for(Vector3D<double>& p : points) {
        for(size_t i = 0; i < p.size(); i++) {
            if(p[i] > axis_max[i]) { axis_max[i] = p[i]; }
            if(p[i] < axis_min[i]) { axis_min[i] = p[i]; }
        }
    }
