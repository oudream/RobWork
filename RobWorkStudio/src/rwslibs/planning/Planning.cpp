...
QToQPlanner::Ptr getPlanner(WorkCell* workcell, Device::Ptr device, const State& state,
    ...
    if(type == ARW) { return ARWPlanner::makeQToQPlanner(constraint, device); }
    else if(type == RRT) { return RRTPlanner::makeQToQPlanner(constraint, device); }
    else if(type == SBL) {
    ...
    else if(type == Z3) { return Z3Planner::makeQToQPlanner(constraint, device); }
    else { RW_THROW("Unknown planner " << StringUtil::quote(type.toStdString())); }
}
...
*/
    ...
    for(std::string id : ids) { _cmbCollisionDetectors->addItem(id.c_str()); }
...
*/
    ...
    if(deviceIndex >= _workcell->getDevices().size()) RW_THROW("Index out of bounds");
...
Device::Ptr Planning::getDevice() {
    ...
    if(planForAll) {
        ...
        _compositeDevice = ownedPtr(new rw::models::CompositeDevice(
            devices.front()->getBase(), devices, devices.back()->getEnd(), "Composite", _state));

        return _compositeDevice;
    }
    else { return _workcell->getDevices().at(deviceIndex); }
}
...
void Planning::plan() {
    ...
    else { log().info() << "Could not find a solution" << std::endl; }
...
*/
    ...
    if(_path.size() > 2) { setAsTimedStatePath(); }
    //    delete dialog;
}
