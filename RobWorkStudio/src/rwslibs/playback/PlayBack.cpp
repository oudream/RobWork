...
void PlayBack::savePath() {
    ...
    if(!filename.isEmpty()) {
        ...
        if(StringUtil::getFileExtension(filename.toStdString()) != ".rwplay") filename += ".rwplay";
...
void PlayBack::openPlayFile(const std::string& file) {
    ...
    else { RW_THROW("Unknown file extension - expected either .csv or .rwplay!"); }
}
...
void PlayBack::reloadPlay() {
    if(_file.empty()) openPath();
    else openPlayFile(_file);
}

void PlayBack::keyPressEvent(QKeyEvent* e) {
    if(e->key() == Qt::Key_F) _player->step(true);
    else if(e->key() == Qt::Key_B) _player->step(false);
}
