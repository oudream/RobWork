...
Slider::Slider(const std::string& title, double low, double high, QGridLayout* layout, int row,
    ...
    unsigned int col = 0;
    if(!title.empty()) layout->addWidget(_title, row, col++, Qt::AlignLeft);
...
void Slider::boxValueChanged(double val) {
    ...
    if(!_sliderChanged) { setSliderValueFromBox(val); }
...
void Slider::sliderValueChanged(int val) {
    ...
    if(!isBoxSame) { setBoxValueFromSlider(val); }
...
void JointSliderWidget::setup(const std::vector<std::string>& titles, const std::pair<Q, Q>& bounds,
                              const Q& q, bool enablers, bool enableAngularCombined) {
    ...
    */
    for(std::size_t i = 0; i < _sliders.size(); i++) {
        if(_sliders[i] != NULL) { delete _sliders[i]; }
    }
    for(std::size_t i = 0; i < _enablers.size(); i++) {
        if(_enablers[i] != NULL) { delete _enablers[i]; }
    }
    _sliders.clear();
    _enablers.clear();
...
void JointSliderWidget::paste() {
    ...
    do {
        ...
        try {    // Transform3D(Vector3D(0.2, 0.598, 0.85), Rotation3D(1, 0, 0, 0, 1, 0, -0, 0, 1))
            ...
            if(_isNotADevice) {
                std::cmatch cm;
                if(regex_match(txt.toStdString().c_str(),
                               cm,
                    ...
                    doQ = false;
                    double v[12];
                    for(unsigned i = 1; i < cm.size(); ++i) { v[i - 1] = std::stod(cm[i]); }
                    Transform3D<> t(
...
void JointSliderWidget::copy() {
    ...
    for(size_t i = 0; i < _sliders.size(); ++i) { q[i] = _sliders[i]->value(); }
...
void JointSliderWidget::updateValues(const rw::math::Q& q) {
    for(size_t i = 0; i < q.size(); i++) { _sliders[i]->setValue(q(i)); }
}

void JointSliderWidget::updateInactiveValues(const Q& q) {
    for(std::size_t i = 0; i < _enablers.size(); i++) {
        if(!_enablers[i]->isChecked()) { _sliders[i]->setValue(q(i)); }
    }
}
...
std::vector<bool> JointSliderWidget::enabledState() const {
    ...
    for(std::size_t i = 0; i < _enablers.size(); i++) res[i] = _enablers[i]->isChecked();
    return res;
}
...
void TransformSliderWidget::setUnits(const std::vector<double>& converters,
    ...
    else { _jointSliderWidget->setUnits(_converters, _descriptions); }
}
...
Q TransformSliderWidget::qFromTransform(const rw::math::Transform3D<>& transform) {
    ...
    if(_angleType == QUAtype) {
        ...
        for(std::size_t i = 0; i < 3; i++) { q[i] = transform.P()[i]; }
        ...
        else if(_EAAfromSliders.angle() > 0) { eaa = EAA<>(eaa.axis(), _EAAfromSliders.angle()); }
...
rw::math::Transform3D<> TransformSliderWidget::getTransform() {
    ...
    switch(_angleType) {
        ...
        case EAAtype: return Transform3D<>(Vector3D<>(q[0], q[1], q[2]), EAA<>(q[3], q[4], q[5]));
...
VectorND<6, bool> TransformSliderWidget::enabledState() const {
    ...
    if(state.size() == 0) {
        for(std::size_t i = 0; i < 6; i++) res[i] = false;
    }
    else {
        RW_ASSERT(state.size() == 6);
        for(std::size_t i = 0; i < 6; i++) res[i] = state[i];
    }
    return res;
}
    ...
    switch(_angleType) {
        case QUAtype:
            if(_lastTransformType == QUAtype) {
                ...
                for(std::size_t i = 3; i < 7; i++) {
                    if(_last_q[i] != q[i]) { changed = i; }
                }
                Q q_mod = q;

                if(changed >= 3) {
                    double length(0);
                    for(std::size_t i = 3; i < 7; i++) { length += q[i] * q[i]; }
            ...
            if(_lastTransformType == EAAtype) {
                ...
                if(changed == 6) {
                    ...
                    if(largestReduction < 1) {
                        if(not_zero == 1) { largestReduction *= 0.9999; }
                    ...
                    while(length > Pi) { length -= Pi; }
                    if(length < 0) length = 0;
                    _EAAfromSliders = angle;
                    transform =
                        Transform3D<>(Vector3D<>(q[0], q[1], q[2]), EAA<>(angle.axis(), length));
                }
                else {
...
Q TransformSliderWidget::getQ(const AngleType fromType, const AngleType toType) {
    ...
    else { q = _jointSliderWidget->getQ(); }
    return q;
}
...
CartesianDeviceTab::CartesianDeviceTab(const std::pair<rw::math::Q, rw::math::Q>& bounds,
    ...
    else _iksolver = ownedPtr(new JacobianIKSolver(_device, _tcpFrame, _state));
...
void CartesianDeviceTab::transformChanged(const Transform3D<>& refTtcp) {
    ...
    if(solutions.empty()) { doUpdateValues(); }
    else {
