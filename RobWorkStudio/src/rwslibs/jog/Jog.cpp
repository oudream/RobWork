...
formatUnitDescriptions(const std::string& angles, const std::string& distances,
    ...
    for(std::map<std::string, double>::const_iterator it = distanceUnitConverters.begin();
        ...
        ++it)
        if(QString::compare(
               QString(distances.c_str()), QString(it->first.c_str()), Qt::CaseInsensitive) == 0)
...
Jog::Jog() :
    RobWorkStudioPlugin("Jog", getIcon()), _workcell(0), _selectedDevice(0), _jointSliderWidget(0),
    _jointSliderWidgetFull(0), _selectedFrame(0), _cartesianTab(0), _cartesianDeviceTab(0),
    _updating(false), _cartesianBounds(Q::zero(6), Q::zero(6)), _rwsSettings(0) {
...
void Jog::open(WorkCell* workcell) {
    ...
    _rwsSettings =
        getRobWorkStudio()->getPropertyMap().getPtr<rw::core::PropertyMap>("RobWorkStudioSettings");
    if(_rwsSettings) {
...
void Jog::deviceConfigChanged(const Q& q) {
    ...
    if(_selectedDevice != NULL) {
        ...
        else { _selectedDevice->setQ(q, _state); }
...
void Jog::updateValues() {
    ...
    if(_selectedDevice != NULL) {
        if(_jointSliderWidget != NULL) {
            ...
            if(_cartesianDeviceTab != NULL) { _cartesianDeviceTab->updateValues(_state); }
        }
    ...
    if(_selectedFrame != NULL && _cartesianTab != NULL) { _cartesianTab->updateValues(_state); }
    _updating = false;
}
...
void Jog::stateChangedListener(const State& state) {
    _state = state;
    if(isVisible() && !_updating) { updateValues(); }
}
...
void Jog::frameSelectedListener(Frame* frame) {
    ...
    if(_frameToIndex.has(*frame)) { _cmbDevices->setCurrentIndex(_frameToIndex[*frame]); }
}
