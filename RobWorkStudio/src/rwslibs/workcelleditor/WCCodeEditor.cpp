...
WCCodeEditor::LineState WCCodeEditor::getLineState(int lineNr) {
    if((int) _executedLines.size() <= lineNr || lineNr < 0) return Nothing;

    return _executedLines[lineNr];
}

void WCCodeEditor::setCompleter(QCompleter* completer) {
    if(_c) QObject::disconnect(_c, 0, this, 0);
...
void WCCodeEditor::insertCompletion(const QString& completion) {
    ...
    for(int i = 0; i < extra; i++) { tc.deleteChar(); }
    setTextCursor(tc);
}
...
void WCCodeEditor::keyPressEvent(QKeyEvent* e) {
    ...
    if(e->key() == Qt::Key_Tab) {
        if(textCursor().selectedText().count() > 0) { indentSelection(); }
    ...
    if(!_c || (ctrlOrShift && e->text().isEmpty())) return;
