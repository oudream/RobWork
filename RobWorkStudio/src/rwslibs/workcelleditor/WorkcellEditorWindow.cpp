...
WorkcellEditorWindow::WorkcellEditorWindow(rw::core::Log::Ptr output, rws::RobWorkStudio* rwstudio,
    ...
    connect(_tabPane, SIGNAL(tabCloseRequested(int)), this, SLOT(on_actionCloseTriggered(int)));
    ...
    rw::core::Log::infoLog() << "Opening: workcell " << _rws->getWorkcell()->getFilePath().c_str()
                             << "\n";
}
...
void WorkcellEditorWindow::on_actionClose_triggered(bool) {
    ...
    if(_tabPane->count() == 1) return;
    EditorTab::Ptr tab = getCurrentTab();
    if(tab->_editor->document()->isModified()) save();
...
void WorkcellEditorWindow::on_actionAdd_Frame_triggered(bool) {
    ...
    if(InputFormDialog::getInput("Add Frame", data, options)) {
        ...
        QString frame_type = "";
        if(data.at<QString>("Type") == "Movable") frame_type = "type=\"Movable\"";
...
void WorkcellEditorWindow::on_actionAdd_Drawable_triggered(bool) {
    ...
    if(InputFormDialog::getInput("Add Drawable", data, options)) {
        ...
        if(data.at<bool>("CollisionModel")) { col_model = "Enabled"; }
        else { col_model = "Disabled"; }
        ...
        // additional tab
        QString indentation = "";
        if(ref_frame_str.isEmpty()) indentation = "    ";
        else indentation = "  ";
        ...
        if(data.at<QString>("CAD file").isEmpty()) {
            ...
            QTextStream(&model_str)
                << indentation << "  <Polytope file=\"" << data.at<QString>("CAD file") << "\"/>";
        }
        ...
        QTextStream(&drawable_str)
            ...
            << data.at<QVector3D>("Position").y() << " " << data.at<QVector3D>("Position").z()
            << "</Pos>\n"
            // RPY
    ...
    if(InputFormDialog::getInput("Add Collision Model", data, options)) {
        ...
        if(data.at<QString>("CAD file").isEmpty()) {
            ...
            if(data.at<QString>("Geometry Type") == "Box") {
                ...
                QTextStream(&model_str)
                    << "<Cylinder radius=\"" << data.at<QVector2D>("Cylinder (radius, height)").x()
...
bool WorkcellEditorWindow::save(const std::string& filename) {
    ...
    if(filename.find(".temp.wc.xml") < filename.size()) { isTmpFile = true; }
    QFile file;
...
bool WorkcellEditorWindow::openWorkCell(const QString& fileName) {
    ...
    else if(ignoreNextWorkcellOpen) { ignoreNextWorkcellOpen = false; }
...
QAbstractItemModel* WorkcellEditorWindow::modelFromFile(const QString& fileName,
    ...
    while(!file.atEnd()) {
        ...
        int level                           = 0;
        if(!match.hasMatch()) { level = 0; }
        else {
            if(line.startsWith("\t")) { level = match.capturedLength(0); }
            else { level = match.capturedLength(0) / 4; }
        }

        if(level + 1 >= parents.size()) parents.resize(parents.size() * 2);
