...
void GTaskVisPlugin::btnPressed() {
    ...
    else if(obj == _updateBtn) { updateVis(); }
}
...
void GTaskVisPlugin::updateVis() {
    ...
    for(int i = 0; i < nrToShow; i++) {
        ...
        int idx = i;
        if(nrToShow < (int) _ymtargets.size()) { idx = Math::ranI(0, (int) _ymtargets.size()); }
        else {
            if(idx >= (int) _ymtargets.size()) break;
        }
        ...
        if(maxQual < quality(qIdx)) maxQual = quality(qIdx);
        if(minQual > quality(qIdx)) minQual = quality(qIdx);
        if(_showQuality->isChecked()) {
            if(quality(qIdx) < _fromThresSpin->value()) continue;
            if(quality(qIdx) > _toThresSpin->value()) continue;
        }

        if(testStatus == -1) {
            if(!_untestedBox->isChecked()) continue;
        }
        else if(testStatus == GraspResult::ObjectDropped) {
            if(!_droppedBox->isChecked()) continue;
            droppedStat++;
            rt.color[0] = 1.0;
        }
        else if(testStatus == GraspResult::ObjectMissed) {
            if(!_missedBox->isChecked()) continue;
            missedStat++;
            rt.color[0] = 0.5;
        }
        else if(testStatus == GraspResult::Success) {
            if(!_successBox->isChecked()) continue;
            successStat++;
            rt.color[1] = 1.0;
        }
        else if(testStatus == GraspResult::ObjectSlipped) {
            if(!_slippedBox->isChecked()) continue;
            slippedStat++;
            rt.color[0] = 0.0;
            rt.color[1] = 1.0;
            rt.color[2] = 0.5;
            ...
            if(!_collisionsBox->isChecked()) continue;
            if(testStatus == GraspResult::CollisionEnvironmentInitially) collisionEnvStat++;
            else if(testStatus == GraspResult::CollisionObjectInitially) collisionObjStat++;
            else collisionStat++;
            rt.color[0] = 1.0;
            rt.color[1] = 0.0;
            rt.color[2] = 1.0;
            ...
            if(!_otherBox->isChecked()) continue;
            otherStat++;
        ...
        if(_showQuality->isChecked()) { rt.scale = quality(qIdx); }
        else { rt.scale = 1.0; }
        ...
        if(_showEndGraspTargetBox->isChecked()) {
            ...
            if(!showInGripperFrame) { rt.trans = wTo * target.result->objectTtcpGrasp; }
            else { rt.trans = wTtcp * inverse(target.result->objectTtcpGrasp); }
            rt.endTarget = true;
            rtargets.push_back(rt);
        }

        if(_showEndLiftTargetBox->isChecked()) {
            if(!showInGripperFrame) { rt.trans = wTo * target.result->objectTtcpLift; }
            else { rt.trans = wTtcp * inverse(target.result->objectTtcpLift); }
            rt.endTarget = true;
            rtargets.push_back(rt);
        }
    }
    ...
    if(_showQuality->isChecked()) {
        ...
        for(RenderTargets::Target& t : rtargets) {
            if(t.color[0] + t.color[1] + t.color[2] < 0.00001) {
                ...
                if(_lowIsHigh->isChecked()) { sval = 1 - sval; }
    ...
    log().info() << "Total:"
                 << successStat + slippedStat + otherStat + collisionStat + droppedStat + missedStat
...
void GTaskVisPlugin::loadTasks(QString taskFile_tmp) {
    ...
    if(tcpIdx != -1) _tcpSelectBox->setCurrentIndex(tcpIdx);
...
void GTaskVisPlugin::selectGrasp(int index) {
    ...
    if((index < 0) || (index >= (int) targets.size())) { return; }
    ...
    if(object == NULL) RW_THROW("selected object is not movable frame!");
    if(base == NULL) RW_THROW("selected base is not movable frame!");
    if(device == NULL) RW_THROW("selected device is not valid!");
    ...
    if(device != NULL) {
        ...
        if(targets[index].endTarget) {
            ...
            else { q = targets[index].ctask->closeQ; }
        }
        else {
            if(targets[index].ctarget.result != NULL &&
            ...
            else if(targets[index].ctask->openQ.size() > 0) { q = targets[index].ctask->openQ; }
        }
        if(q.size() == device->getDOF()) device->setQ(q, state);
    }
    if(targets[index].ctarget.result != NULL)
