...
TaskSetupDialog::TaskSetupDialog(QWidget* parent, rw::core::Ptr<const WorkCell> wc,
    ...
    connect(_ui->maleTCP, SIGNAL(currentTextChanged(const QString&)), this, SLOT(updateViews()));
    connect(_ui->femaleTCP, SIGNAL(currentTextChanged(const QString&)), this, SLOT(updateViews()));
...
void TaskSetupDialog::updateViews() {
    ...
    if(!male.isNull() && !female.isNull()) _ui->saveBtn->setEnabled(true);
}

void TaskSetupDialog::updatePositions() {
    const Transform3D<> ftmtcpTarget(Vector3D<>(_ui->x->value(), _ui->y->value(), _ui->z->value()),
...
void TaskSetupDialog::stepInsertion() {
    ...
    if(_simulation->cstate.isNull()) _simulation->cstate = _strategy->createState();
    State state;
    ...
    if(response != NULL) { _simulation->response = response; }
    if(!_simulation->response.isNull()) {
        if(_simulation->response->type == AssemblyControlResponse::POSITION_TRAJECTORY) {
...
void TaskSetupDialog::loadTask() {
    ...
    if(!found) {
        if(_ui->female->count() > 0) _ui->female->setCurrentIndex(0);
        if(error == "") error = "Could not find the female object specified in task.";
    }
    ...
    if(!found) {
        if(_ui->male->count() > 0) _ui->male->setCurrentIndex(0);
        if(error == "") error = "Could not find the male object specified in task.";
    }
    ...
    if(!found) {
        if(_ui->femaleTCP->count() > 0) _ui->femaleTCP->setCurrentIndex(0);
        if(error == "") error = "Could not find the female TCP frame specified in task.";
    }
    ...
    if(!found) {
        if(_ui->maleTCP->count() > 0) _ui->maleTCP->setCurrentIndex(0);
        if(error == "") error = "Could not find the male TCP frame specified in task.";
    }
    ...
    // Check strategy
    if(error == "") {
        if(task->strategy.isNull()) { error = "Could not find the strategy specified in task."; }
    ...
    if(task->parameters.isNull()) {
        if(error == "") error = "Did not retrieve parameters from given task.";
    }
    else {
...
void TaskSetupDialog::updateView(SceneOpenGLViewer* widget) {
    ...
    if(world->hasChild("Male")) world->removeChild("Male");
    if(world->hasChild("Female")) world->removeChild("Female");

    if(_wc.isNull()) return;
...
void TaskSetupDialog::updatePosition(SceneOpenGLViewer* widget, const Transform3D<>& fTm) {
    ...
    for(std::list<SceneNode::Ptr>::const_iterator it = world->_childNodes.begin();
        ...
        it++) {
        if((*it)->getName() == "Female") female = it->cast<GroupNode>();
        else if((*it)->getName() == "Male") male = it->cast<GroupNode>();
    }
    if(!female.isNull()) {
        if(_ui->fixFemale->isChecked()) female->setTransform(Transform3D<>::identity());
        else female->setTransform(inverse(fTm));
    }
    if(!male.isNull()) {
        if(_ui->fixMale->isChecked()) male->setTransform(Transform3D<>::identity());
        else male->setTransform(fTm);
    }
    widget->updateView();
}
