...
void StrategyLibraryDialog::step() {
    for(std::size_t i = 0; i < _cells.size(); i++) {
        Cell& cell = _cells[i];
        if(cell.viewWidget == NULL) continue;
        if(!cell.response.isNull()) {
            if(cell.response->done) continue;
        }
        AssemblyState::Ptr real = ownedPtr(new AssemblyState());
        // real->femaleTmale // for done checks
        if(cell.cstate.isNull()) cell.cstate = cell.strategy->createState();
        State state;
        ...
        if(response != NULL) { cell.response = response; }
        if(!cell.response.isNull()) {
            ...
            if(cell.response->done) cell.reset();
        }
