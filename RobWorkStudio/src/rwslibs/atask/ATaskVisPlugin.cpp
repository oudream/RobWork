...
void ATaskVisPlugin::btnPressed() {
    QObject* obj = sender();
    if(obj == _loadTasksBtn) loadTasks();
    else if(obj == _loadResultsBtn) loadResults();
        ...
        if(_real->isChecked()) _showReal = true;
        else _showReal = false;
        if(_currentTask != NULL && _currentResult != NULL && _wc != NULL) constructPlayback();
    }
}
...
void ATaskVisPlugin::genericAnyEventListener(const std::string& event, boost::any data) {
    ...
    try {
        if(event == "PlayBack::TimeRelative") {
            ...
            if(_showReal) path = _currentResult->realState;
            else path = _currentResult->assumedState;
            double endTime                   = path.back().getTime();
            double time                      = timeRelative * endTime;
            Timed<AssemblyState>& foundState = path.front();
            if(time >= endTime) { foundState = path.back(); }
            else {
...
void ATaskVisPlugin::selectResult(std::size_t i) {
    ...
    if(!_real->isEnabled() && !_assumed->isEnabled()) real = true;

    // Disable both and enable the ones possible
    _real->setEnabled(false);
    _assumed->setEnabled(false);
    if(_currentResult->realState.size() > 0) _real->setEnabled(true);
    if(_currentResult->assumedState.size() > 0) _assumed->setEnabled(true);

    if(!_real->isEnabled() && _assumed->isEnabled()) _assumed->setChecked(true);
    if(_real->isEnabled() && !_assumed->isEnabled()) _real->setChecked(true);
    if(_real->isEnabled() && _assumed->isEnabled()) {
        if(real) _real->setChecked(true);
        else _assumed->setChecked(true);
    }
...
void ATaskVisPlugin::constructPlayback() {
    ...
    if(wc != NULL) {
        ...
        if(male == NULL) RW_THROW("Male frame could not be found (the frame must be Movable)!");
        if(female == NULL) RW_THROW("Female frame could not be found (the frame must be Movable)!");
        Frame* maleTCP   = male;
        Frame* femaleTCP = female;
        if(_currentTask->maleTCP != "") maleTCP = wc->findFrame(_currentTask->maleTCP);
        if(_currentTask->femaleTCP != "") femaleTCP = wc->findFrame(_currentTask->femaleTCP);
        if(maleTCP == NULL) RW_THROW("MaleTCP frame could not be found!");
        if(femaleTCP == NULL) RW_THROW("FemaleTCP frame could not be found!");
        if(male != NULL && female != NULL) {
            TimedStatePath rwpath;
            Path<Timed<AssemblyState>> apath;
            if(_showReal) apath = _currentResult->realState;
            else apath = _currentResult->assumedState;
            for(Timed<AssemblyState>& tastate : apath) {
                ...
                if(!(astate.maleOffset == Transform3D<>::identity())) {
                    male->setTransform(male->getTransform(defState) * maleTtcp * astate.maleOffset *
                                           inverse(maleTtcp),
                                       state);
                }
                ...
                const Transform3D<> wTmaleTCPNew = wTfemaleTCP * astate.femaleTmale *
                                                   inverse(maleTtcp) * astate.maleOffset * maleTtcp;
