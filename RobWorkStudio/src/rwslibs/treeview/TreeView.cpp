...
std::string getFrameName(const Frame& frame) {
    ...
    if(index == std::string::npos) return frame.getName();
...
TreeView::TreeView() :
    RobWorkStudioPlugin("TreeView", QIcon(":/treeview.png")), _workcell(NULL), _frameAxisSize(0.25)

{
    ...
    _showWorkCellStructureAction =
        new QAction(QIcon(":/images/workcell.png"), "WorkCell Structure", displayTypes);    // owned
    ...
    connect(
        _treewidget, SIGNAL(itemDoubleClicked(QTreeWidgetItem*, int)), this, SLOT(highlightSlot()));
...
void TreeView::collapseAll(QTreeWidgetItem* item) {
    if(item != NULL) {
        ...
        for(int i = 0; i < item->childCount(); i++) collapseAll(item->child(i));
    }
}
...
void TreeView::expandAll(QTreeWidgetItem* item) {
    if(item != NULL) {
        ...
        for(int i = 0; i < item->childCount(); i++) expandAll(item->child(i));
    }
}
...
void TreeView::clearTreeContent() {
    while(_treewidget->topLevelItemCount() > 0) { delete _treewidget->takeTopLevelItem(0); }
...
void TreeView::setupDrawables(Frame* frame, QTreeWidgetItem* parent) {
    ...
    for(DI p = drawables.begin(); p != drawables.end(); ++p) {
        ...
        if(!(drawable->getMask() & drawMask)) continue;
        ...
        else { item->setIcon(0, QIcon(":/images/drawable.png")); }
    }
}
        ...
        for(MI it = devices.begin(); it != devices.end(); ++it) {
            ...
            if(sdevice) {
                ...
                for(FI it = frames.begin(); it != frames.end(); ++it) {
                    ...
                    else frameItem->setIcon(0, QIcon(":/images/frame.png"));
    ...
    if(_workcell != NULL) {
        ...
        for(MI it = devices.begin(); it != devices.end(); ++it) {
            ...
            if(sdevice) {
                ...
                // Setup Frames
                const std::vector<Frame*>& frames = sdevice->frames();
                if(!frames.empty()) setupFrame(*frames.front(), deviceItem);
                continue;
            }
            TreeDevice::Ptr tdevice = (*it).cast<TreeDevice>();
            if(tdevice) {
            ...
            if(pdevice) {
                ...
                if(frame != NULL) setupFrame(*frame, deviceItem);
                continue;
            }
...
void TreeView::showFrameStructure() {
    ...
    if(_workcell != NULL) { setupFrame(*_workcell->getWorldFrame(), NULL); }
    _treewidget->update();
}
...
void TreeView::addFromFileSlot() {
    ...
    else { std::cout << "could not find frame..... " << std::endl; }
}
...
void TreeView::toggleFramesView(QTreeWidgetItem* item) {
    ...
    for(int i = 0; i < item->childCount(); i++) { toggleFramesView(item->child(i)); }
}
...
void TreeView::toggleFrameLabels(QTreeWidgetItem* item) {
    ...
    for(int i = 0; i < item->childCount(); i++) { toggleFrameLabels(item->child(i)); }
}
...
void TreeView::increaseFrameAxisSlot() {
    if(_treewidget->currentItem() == NULL) return;
    _frameAxisSize += 0.05;
...
void TreeView::constructDrawableList(std::vector<DrawableNode::Ptr>& drawables) {
    ...
    for(int i = 0; i < selected.size(); ++i) {
        ...
        if(drawableIt != _drawableMap.end()) drawables.push_back(drawableIt->second);
    }
}
...
void TreeView::showSolidSlot() {
    ...
    for(DrawableNode::Ptr& node : drawables) node->setDrawType(DrawableNode::SOLID);
...
void TreeView::toggleSlot() {
    ...
    for(DrawableNode::Ptr& node : drawables) node->setVisible(!(node->isVisible()));
...
void TreeView::showWireSlot() {
    ...
    for(DrawableNode::Ptr& node : drawables) node->setDrawType(DrawableNode::WIRE);
...
void TreeView::showOutlineSlot() {
    ...
    for(DrawableNode::Ptr& node : drawables) node->setDrawType(DrawableNode::OUTLINE);
...
void TreeView::showTransparentSlot() {
    ...
    if(ok) {
        ...
        for(DrawableNode::Ptr& node : drawables) node->setTransparency(alpha);
    }
...
void TreeView::scaleSlot() {
    ...
    if(ok) {
        for(DrawableNode::Ptr& node : drawables) node->setScale(scale);
    }
...
void TreeView::highlightSlot() {
    ...
    for(DrawableNode::Ptr& node : drawables) node->setHighlighted(!node->isHighlighted());
...
void TreeView::poseSlot() {
    ...
    if(drawableIt != _drawableMap.end()) {
        ...
        Transform3D<> t3d;
        if(frame == NULL) { t3d = drawable->getTransform(); }
        else {
...
void TreeView::open(WorkCell* workcell) {
    ...
    if(_showWorkCellStructureAction->isChecked()) { showWorkCellStructure(); }
    else if(_showDeviceStructureAction->isChecked()) { showDeviceStructure(); }
    else if(_showFrameStructureAction->isChecked()) { showFrameStructure(); }
...
void TreeView::setupFrame(Frame& frame, QTreeWidgetItem* parentItem) {
    ...
    if(dynamic_cast<Joint*>(&frame)) { item->setIcon(0, QIcon(":/images/joint.png")); }
    else { item->setIcon(0, QIcon(":/images/frame.png")); }

    Frame::iterator_pair children = frame.getChildren(_state);
    for(Frame::iterator it = children.first; it != children.second; ++it) { setupFrame(*it, item); }
...
void TreeView::keyPressEvent(QKeyEvent* event) {
    ...
    else { QDockWidget::keyPressEvent(event); }
}
