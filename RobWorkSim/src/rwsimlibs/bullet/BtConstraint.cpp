...
BtConstraint::BtConstraint(rw::core::Ptr<const Constraint> constraint, const BtBody* const parent,
                           const BtBody* const child, btDynamicsWorld* btWorld) :
...
void BtConstraint::createJoint() {
        ...
        if(spring.enabled) {
            ...
            for(unsigned int dof = 0; dof < 6; dof++) {
                if(spring.compliance(dof, dof) > 0) {
                    ...
                    else btConstraint->setDamping(dof, 0);    // how to set this?!
    ...
    if(_btConstraint != NULL) _btDynamicsWorld->addConstraint(_btConstraint, true);
}
...
btJointFeedback* BtConstraint::getFeedback() {
    if(!_feedback) _feedback = new btJointFeedback();
    return _feedback;
}
