...
BtBody::BtBody(Body::Ptr body, const MaterialDataMap* frictionMap,
    ...
    _rwBody(body),
    _rwBodyDynamic(_rwBody.cast<RigidBody>()), _rwBodyKinematic(_rwBody.cast<KinematicBody>()),
...
BtBody::~BtBody() {
    ...
    if(_btRigidBody != NULL) {
        ...
        if(_btRigidBody->getMotionState()) { delete _btRigidBody->getMotionState(); }
        delete _btRigidBody;
    }
}
...
void BtBody::postupdate(State& state) const {
    if(isDynamic()) {
        ...
        const Vector3D<> lin =
            BtUtil::toVector3D(_btRigidBody->getLinearVelocity()) + cross(ang, wTb.P() - wTcom.P());
...
btCollisionShape* BtBody::createColShape(rw::core::Ptr<const Geometry> geometry) const {
    ...
    if(shape) shape->setUserPointer(new GeometryMetaData(geometry));
...
btCompoundShape* BtBody::getColShape(Body::Ptr body, const Transform3D<>& bTcom) const {
    ...
    for(const Geometry::Ptr& geometry : body->getGeometry()) {
        ...
        if(colShape != NULL) { composite->addChildShape(comTgeo, colShape); }
    }
