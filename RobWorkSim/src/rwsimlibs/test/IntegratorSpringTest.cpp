...
void IntegratorSpringTest::run(TestHandle::Ptr handle, const std::string& engineID,
    ...
    static const TestCallback cb(std::bind(&IntegratorSpringTest::updateResults, _1));
...
DynamicWorkCell::Ptr IntegratorSpringTest::makeIntegratorDWC(const std::string& integratorType) {
    ...
    const Constraint::Ptr constraint = ownedPtr(new Constraint(
        "Spring", Constraint::Free, dwc->findBody("Cylinder").get(), dwc->findBody("Ball").get()));
