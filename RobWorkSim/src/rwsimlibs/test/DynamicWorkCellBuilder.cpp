...
DynamicWorkCellBuilder::ColorScheme::ColorScheme(const rw::math::Vector3D<float>& fixedBodies,
...
void DynamicWorkCellBuilder::addPlane(const DynamicWorkCell::Ptr dwc, const rw::math::Vector3D<>& n,
                                      const double d, const std::string& name,
                                      const bool trimesh) const {
    ...
    Geometry::Ptr geo;
    if(trimesh) geo = ownedPtr(new Geometry(geoData->createMesh(0, 5), name));
    else geo = ownedPtr(new Geometry(geoData, name));
...
void DynamicWorkCellBuilder::addBall(const DynamicWorkCell::Ptr dwc, const double radius,
    ...
    if(parentFrame == NULL) RW_THROW("The given parent frame \"" << parent << "\"does not exist!");
...
void DynamicWorkCellBuilder::addCylinder(const DynamicWorkCell::Ptr dwc, const double radius,
    ...
    if(trimesh) geoData = geoData->getTriMesh(true);
...
void DynamicWorkCellBuilder::addTube(const DynamicWorkCell::Ptr dwc, const double radius,
    ...
    GeometryData::Ptr geoData = ownedPtr(new Tube(
        static_cast<float>(radius), static_cast<float>(thickness), static_cast<float>(height)));
    if(trimesh) geoData = geoData->getTriMesh(true);
...
void DynamicWorkCellBuilder::addBox(const DynamicWorkCell::Ptr dwc, const double x, const double y,
    ...
    else { body = ownedPtr(new KinematicBody(info, robject)); }
...
void DynamicWorkCellBuilder::boxInfo(BodyInfo& info, const double x, const double y, const double z,
                                     const double density) {
    defaultInfo(info);
