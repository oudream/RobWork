...
RWSimPlugin::RWSimPlugin() :
    ...
    _tactileSensorDialog(NULL), _timerShot(NULL), _openAction(NULL), _planarPoseDistAction(NULL),
    _poseDistAction(NULL), _graspSelectionAction(NULL), _graspRestPoseAction(NULL),
    _restPoseAction(NULL), _poseAnalyserAction(NULL) {
    setupUi(this);
...
RWSimPlugin::~RWSimPlugin() {
    if(_timerShot != NULL) _timerShot->stop();
    _timer->stop();
}
...
void RWSimPlugin::setupMenu(QMenu* pluginmenu) {
    ...
    _openAction =
        new QAction(QIcon(":/images/open.png"), tr("&Open DynamicWorkCell..."), this);    // owned
    ...
    if(action.get<1>() != NULL) action.get<0>()->insertAction(action.get<1>(), _openAction);
    ...
    if(action.get<1>() != NULL) menu->insertMenu(action2.get<1>(), dynMenu);
...
void RWSimPlugin::btnPressed() {
    ...
    if(obj == _openDwcBtn || obj == _openAction) {
    ...
    else if(obj == _saveStatePathBtn) { getRobWorkStudio()->setTimedStatePath(_path); }
        ...
        if(ctrl) {
            if(ctrl->getControllerHandle(_sim->getSimulator()) != NULL) {
                ...
                else { RW_WARN("No support for this controller type!"); }
            }
...
void RWSimPlugin::stepCallBack(const rw::kinematics::State& state) {
    ...
    _state = state;
    if(_recordStatePathBox->isChecked()) { _path.push_back(TimedState(_sim->getTime(), state)); }
}
...
void RWSimPlugin::changedEvent() {
    ...
    if(obj == _timer) {
        ...
        if(_debugRender == NULL) {
            if(_debugLevelSpin->value() == 0) return;
            ...
            _debugRender->setDrawMask(_debugLevelSpin->value());
            if(_debugLevelSpin->value() == 0) { _debugDrawable->setVisible(false); }
            else { _debugDrawable->setVisible(true); }
        }
    }
}
...
void RWSimPlugin::open(rw::models::WorkCell* workcell) {
    ...
    rwlibs::swig::LuaState::Ptr lstate =
        getRobWorkStudio()->getPropertyMap().get<rwlibs::swig::LuaState::Ptr>("LuaState", NULL);
    ...
    if(_luastate != NULL && _dwc != NULL) { rwsim::swig::setDynamicWorkCell(_dwc.get()); }
#endif
    ...
    for(DynamicDevice::Ptr device : _dwc->getDynamicDevices()) {
        ...
        if(dynamic_cast<JointDevice*>(dev) == NULL) continue;
        RW_ASSERT(dev);
