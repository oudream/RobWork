...
void SimTaskPlugin::open(WorkCell* workcell) {
    if(workcell == NULL || _dwc == NULL) return;
    ...
    for(RigidBody::Ptr body : bodies) { _objectComboBox->addItem(body->getName().c_str()); }
    ...
    if(idx >= 0) _outputFormatBox->setCurrentIndex(idx);

    _loadTaskBtn->setEnabled(true);

    std::string gripper =
        getRobWorkStudio()->getPropertyMap().get<PropertyMap>("cmdline").get<std::string>("Gripper",
                                                                                          "GS20");
    ...
    _nrOfTargetsToGen = getRobWorkStudio()->getPropertyMap().get<PropertyMap>("cmdline").get<int>(
...
void SimTaskPlugin::close() {
    ...
    _timer->stop();
    if(_graspSim != NULL) { _graspSim->pauseSimulation(); }
    _graspSim = NULL;
    _wc       = NULL;
...
void SimTaskPlugin::btnPressed() {
    ...
    if(obj == _loadTaskBtn) {
    ...
    else if(obj == _saveResultBtn) { saveTasks(false); }
        ...
        if(_graspSim->getSimulator() != NULL) _graspSim->getSimulator()->setRealTimeScale(delay);
    }
        ...
        */
        ...
        if(_graspSim->getSimulator() == NULL) return;
        ...
        if(_graspSim->isFinished() && !_genTasksBox->isChecked()) {
            ...
            if(_continuesBox->isChecked()) {
                ...
                if(_mergedResult->getSubTasks().size() == 0) { _mergedResult = task; }
                else {
...
GraspTask::Ptr SimTaskPlugin::generateTasks(int nrTasks) {
    ...
    if(body == NULL) { RW_THROW("OBJECT DOES NOT EXIST: " << objectName); }
    ...
    if(gripperName == "SchunkHand") {
        ...
        else { tau(0) = std::max(2 * cos(alpha) * tau(3), 0.2); }
        subtask.tauMax = tau;
    }
    ...
    if(type == "GS20" || type == "GS20_WIDE") { ssurf.setBoundsD(-0.02, 0.02); }
    else if(type == "SCUP") { ssurf.setBoundsD(-0.02, 0.005); }
    else { ssurf.setBoundsD(-0.04, 0.04); }
...
void SimTaskPlugin::saveTasks(bool automatic) {
    ...
    if(_mergeResultBox->isChecked()) {
        if(_mergedResult != NULL) { result = _mergedResult; }
        else { result = _graspSim->getResult(); }
    }
    else { result = _graspSim->getResult(); }
    ...
    try {
        if(_outputFormatBox->currentText() == "RWTask") { GraspTask::saveRWTask(result, taskFile); }
        else if(_outputFormatBox->currentText() == "UIBK") {
            GraspTask::saveUIBK(result, taskFile);
        }
        else if(_outputFormatBox->currentText() == "TXT") { GraspTask::saveText(result, taskFile); }
    }
    catch(const Exception&) {
