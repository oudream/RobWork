...
EngineTestPlugin::EngineTestPlugin() :
    ...
    _ui(new Ui::EngineTestPlugin()), _inputEditor(new PropertyViewEditor(this)), _logWidget(NULL),
    _simFailed(false) {
    _ui->setupUi(this);
...
void EngineTestPlugin::initialize() {
    getRobWorkStudio()->genericAnyEvent().add(
        boost::bind(
            &EngineTestPlugin::genericAnyEventListener, this, boost::arg<1>(), boost::arg<2>()),
        this);
...
void EngineTestPlugin::engineChanged(QListWidgetItem* current) {
    ...
    for(int i = 0; i < _ui->testList->count(); i++) {
        ...
        bool disable                = false;
        if(test == NULL) disable = true;
        else if(!test->isEngineSupported(engine)) disable = true;
        if(disable) {
        ...
        else { item->setFlags(Qt::ItemIsEnabled | Qt::ItemIsUserCheckable | Qt::ItemIsSelectable); }
    }
    if(_ui->testList->currentItem() == NULL)
        message("Engine \"" + engine + "\" selected, please select which test to perform.");
    else message("Proceed to Execution.");
}
...
void EngineTestPlugin::testChanged(QListWidgetItem* current) {
    ...
    const EngineTest::Ptr test = EngineTest::Factory::getTest(testName);
    if(test.isNull()) RW_THROW("Could not get EngineTest with name " << testName << "!");
    for(int i = 0; i < _ui->testList->count(); i++) {
        QListWidgetItem* const item = _ui->testList->item(i);
        item->setFlags(Qt::ItemIsEnabled | Qt::ItemIsUserCheckable | Qt::ItemIsSelectable);
    }
...
void EngineTestPlugin::run() {
    if(_ui->run->text() == "Run") {
        ...
        _testHandle->setTimeCallback(
            std::function<void(double, bool, bool)>(std::bind(&EngineTestPlugin::simulatorCallBack,
                                                              this,
                                                              std::placeholders::_1,
                                                              std::placeholders::_2,
                                                              std::placeholders::_3)));
        ...
        bool verbose = false;
        if(_log != NULL) {
            if(_log->children() > 0) verbose = true;
        }
        ...
        if(verbose) {
            ...
            if(_ui->logCheck->isChecked()) _ui->logFile->setEnabled(true);
        }
        _ui->run->setText("Run");
        _ui->run->setEnabled(true);
    }
...
void EngineTestPlugin::verbose() {
    if(_logWidget == NULL) _logWidget = new SimulatorLogWidget(NULL);
...
void EngineTestPlugin::predefinedChoice(int choice) {
    ...
    if(text == "(User)") { return; }
        ...
        if(_ui->predefined->itemText(0) == "(User)") _ui->predefined->removeItem(0);
    ...
    if(event->type() == SimulationTimeEvent::Type) {
        ...
        if(!_simFailed) {
            if(simEvent->failure) {
                ...
                _ui->progress->setStyleSheet(STYLE_PROGRESS_SUCCESS);
                if(simEvent->done) { message("Simulation done."); }
            }
        }
        if(simEvent->done) {
            ...
            if(verbose) _ui->runVerbose->setEnabled(true);
...
void EngineTestPlugin::logCheck(int state) {
    ...
    if(sender == _ui->verboseCheck) {
        ...
        if(state == Qt::Checked) { _ui->logFile->setEnabled(true); }
        else { _ui->logFile->setEnabled(false); }
    }
}

void EngineTestPlugin::genericAnyEventListener(const std::string& event, boost::any data) {
    if(_logWidget == NULL || getRobWorkStudio()->getTimedStatePath().size() == 0) return;
    try {
