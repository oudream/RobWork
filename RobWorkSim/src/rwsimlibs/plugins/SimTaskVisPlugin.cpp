...
void SimTaskVisPlugin::initialize() {
    ...
    getRobWorkStudio()->genericAnyEvent().add(
        boost::bind(
            &SimTaskVisPlugin::genericAnyEventListener, this, boost::arg<1>(), boost::arg<2>()),
        this);
...
void SimTaskVisPlugin::updateVis() {
    ...
    for(int i = 0; i < nrToShow; i++) {
        ...
        int idx = i;
        if(nrToShow < (int) _ymtargets.size()) { idx = Math::ranI(0, _ymtargets.size()); }
        else {
        ...
        if(testStatus == -1) {
            ...
            if(testStatus == GraspTask::CollisionEnvironmentInitially) collisionEnvStat++;
            else if(testStatus == GraspTask::CollisionObjectInitially) collisionObjStat++;
            else collisionStat++;
            rt.color[0] = 1.0;
            rt.color[2] = 1.0;
        }
