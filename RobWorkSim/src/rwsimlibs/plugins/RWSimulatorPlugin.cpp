...
void RWSimulatorPlugin::open(rw::models::WorkCell* workcell) {
    RW_DEBUGRWS("Open");
    if(workcell == NULL || _dworkcell == NULL) return;

    if(_dworkcell->getWorkcell() != workcell) { return; }
    ...
    else { RW_WARN("Physics engine does not support debug rendering!"); }
...
void RWSimulatorPlugin::genericEventListener(const std::string& event) {
    ...
    if(event == "StartSimulation") { startSimulation(); }
    else if(event == "StopSimulation") { stopSimulation(); }
    else if(event == "ResetSimulation") { resetSimulation(); }
}
...
void RWSimulatorPlugin::update() {
    ...
    if(_save) _statePath.push_back(Timed<State>(time, _simulator->getState()));
...
void RWSimulatorPlugin::open(const std::string& file) {
    if(!file.empty()) {
        ...
        if(_dworkcell == NULL) RW_THROW("Dynamic workcell is null");
