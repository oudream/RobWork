...
void nearCallback(void* data, dGeomID o1, dGeomID o2) {
    ...
    else { reinterpret_cast<ODESimulator*>(data)->handleCollisionBetween(o1, o2); }
}
}    // namespace
...
ODESimulator::ODESimulator(DynamicWorkCell::Ptr dwc,
    ...
    if(_dwc)
        _dwc->changedEvent().add(
            boost::bind(&ODESimulator::DWCChangedListener, this, boost::arg<1>(), boost::arg<2>()),
            this);
}
...
void ODESimulator::step(double dt, rw::kinematics::State& state) {
    ...
    if(_useRobWorkContactGeneration) {
        ...
        if(_detector.isNull()) { TIMING("Collision: ", detectCollisionsRW(state, false)); }
        else { TIMING("Collision: ", detectCollisionsContactDetector(state)); }
        {
        ...
        if(doLog) _log->log("Contact Detection Type", __FILE__, __LINE__) << "ODE Internal";
        try {
    ...
    int badLCPcount               = 0;
    if(doLog) _log->beginSection("Rollback Iterations", __FILE__, __LINE__);
    for(i = 0; i < MAX_TIME_ITERATIONS; i++) {
        ...
        for(ODEDevice* dev : _odeDevices) { dev->update(conStepInfo, tmpState); }
        ...
        for(ODEBody* body : _odeBodies) { body->update(conStepInfo, tmpState); }
...
*/
        ...
        if(!badLCPSolution) {
            ...
            if(!inCollision) {
                ...
                if(_log->doLog()) _log->endSection(__LINE__);
                break;
            }
            else {
                if(i > 5) {
        ...
        if(i == MAX_TIME_ITERATIONS - 1) {
            ...
            for(dBodyID body : _allbodies) {
                ...
                else { Log::debugLog() << "--- Body: NoRWBODY"; }
                drealCopy(dBodyGetPosition(body), vec, 3);
                Log::debugLog() << "\n---- pos   : " << printArray(vec, 3);
                drealCopy(dBodyGetQuaternion(body), vec, 4);
                Log::debugLog() << "\n---- rot   : " << printArray(vec, 4);
    ...
    if(doLog) _log->endSection(__LINE__);
    state    = tmpState;
    _oldTime = _time;
    _time += dttmp;
    ...
    for(ODEDevice* dev : _odeDevices) { dev->postUpdate(state); }
    ...
    for(ODETactileSensor* odesensor : _odeSensors) { odesensor->update(conStepInfo, state); }
    ...
    for(ODEUtil::TriGeomData* data : _triGeomDatas) {
        if(!data->isGeomTriMesh) continue;
    ...
    if(doLog) _log->endStep(_time, __LINE__);
    // std::cout << "e";
    RW_DEBUG("----------------------- END STEP --------------------------------");
    // std::cout << "-------------------------- END STEP --------------------------------" <<
    // std::endl;
...
void ODESimulator::addEmulatedContact(const rw::math::Vector3D<>& pos,
    ...
    for(ODETactileSensor* sensor : odesensors) { sensor->addContact(pos, force, normal, b); }
    ContactPoint point;
    point.p      = pos;
    point.n      = normal;
...
void ODESimulator::readProperties() {
    ...
    if(stepStr == "WorldQuickStep") { _stepMethod = WorldQuickStep; }
    else if(stepStr == "WorldStep") { _stepMethod = WorldStep; }
    else if(stepStr == "WorldFast1") { _stepMethod = WorldFast1; }
    else { RW_THROW("ODE simulator property: Unknown step method!"); }
    ...
    if(spaceTypeStr == "QuadTree") _spaceType = QuadTree;
    else if(spaceTypeStr == "Simple") _spaceType = Simple;
    else if(spaceTypeStr == "HashTable") _spaceType = HashTable;
    else _spaceType = QuadTree;
}
...
void ODESimulator::initPhysics(rw::kinematics::State& state) {
    ...
    for(Frame* frame : frames) { _frameToModels[*frame] = _narrowStrategy->getModel(frame); }
    ...
    for(Body::Ptr body : _dwc->getBodies()) {
        ...
        if(!_dwc->inDevice(body)) addBody(body, state);
    }
    ...
    for(DynamicDevice::Ptr device : _dwc->getDynamicDevices()) { addDevice(device, initState); }

    RW_DEBUG("- ADDING CONSTRAINTS ");
    for(Constraint::Ptr constraint : _dwc->getConstraints()) { addConstraint(constraint); }
...
void ODESimulator::addODEBody(ODEBody* odebody) {
    ...
    else { _rwFrameToODEBody[odebody->getFrame()] = odebody; }
    ...
    if(odebody->getType() != ODEBody::FIXED) _allbodies.push_back(odebody->getBodyID());
}
...
void ODESimulator::addBody(rwsim::dynamics::Body::Ptr body, rw::kinematics::State& state) {
    ...
    if(!odeBodyAdded) {
        for(Frame* f : odeBody->getRwBody()->getFrames()) { _rwFrameToODEBody[f] = odeBody; }
        for(ODEUtil::TriGeomData* tgeom : odeBody->getTriGeomData()) {
            _frameToOdeGeoms[odeBody->getFrame()] = tgeom->geomId;
        }
...
void ODESimulator::addConstraint(Constraint::Ptr constraint) {
    ...
    if(ode1 == NULL) RW_THROW("Invalid body1 of Constraint: " << constraint->getName());
    if(ode2 == NULL) RW_THROW("Invalid body2 of Constraint: " << constraint->getName());
...
void ODESimulator::addDevice(rwsim::dynamics::DynamicDevice::Ptr dev,
    ...
    if(base == NULL) RW_THROW("Invalid base of Device: " << device->getModel().getName());
    ...
    else { RW_WARN("Controller not supported!"); }
}
...
void ODESimulator::addSensor(rwlibs::simulation::SimulatedSensor::Ptr sensor,
    ...
    if(!sensor->isRegistered()) sensor->registerIn(state);
    ...
    else { _rwsensorToOdesensor[sensor] = NULL; }
}
...
void ODESimulator::removeSensor(rwlibs::simulation::SimulatedSensor::Ptr sensor) {
    ...
    for(SimulatedSensor::Ptr oldsen : _sensors) {
        if(sensor != oldsen) newsensors.push_back(oldsen);
    }
    _sensors      = newsensors;
    ...
    if(_rwFrameToODEBody.find(bframe) == _rwFrameToODEBody.end()) { return; }
    ...
    if(_rwsensorToOdesensor.find(sensor) != _rwsensorToOdesensor.end()) {
        ...
        if(odesensors.size() == 0) return;

        std::vector<ODETactileSensor*> nodesensors;
        for(size_t i = 0; i < odesensors.size(); i++) {
            if(odesensors[i] != tsensor) nodesensors.push_back(odesensors[i]);
        }
        ...
        for(ODETactileSensor* oldsen : _odeSensors) {
            if(tsensor != oldsen) newOdeSensors.push_back(oldsen);
        }
...
void ODESimulator::detectCollisionsContactDetector(const State& state) {
    ...
    for(rwsim::contacts::Contact& contact : contacts) {
        ...
        if(contact.getDepth() < -_maxSepDistance) continue;
        ...
        if(odeSensorb1.size() > 0 || odeSensorb2.size() > 0) { enableFeedback = true; }
...
bool ODESimulator::detectCollisionsRW(rw::kinematics::State& state, bool onlyTestPenetration) {
    ...
    while(!filter->isEmpty()) {
        ...
        if(_enabledMap[*a_data->getFrame()] == 0 || _enabledMap[*b_data->getFrame()] == 0) continue;
        ...
        if(numc > 0 && _log->doLog()) logDistances.push_back(*res);
        if(_rwcontacts.size() < numc) {
            _rwcontacts.resize(numc);
            _contacts.resize(numc);
        }
...
void ODESimulator::attach(rwsim::dynamics::Body::Ptr b1, rwsim::dynamics::Body::Ptr b2) {
    ...
    if(ob1 == NULL) RW_THROW("Body b1 is not part of the simulation! " << b1->getName());
    if(ob2 == NULL) RW_THROW("Body b2 is not part of the simulation! " << b2->getName());
...
void ODESimulator::handleCollisionBetween(dGeomID o1, dGeomID o2) {
    ...
    ODEBody* dataB1;
    if(b1 == NULL) { dataB1 = (ODEBody*) dGeomGetData(o1); }
    else { dataB1 = (ODEBody*) dBodyGetData(b1); }
    // RW_DEBUG("- get data2")
    ODEBody* dataB2;
    if(b2 == NULL) { dataB2 = (ODEBody*) dGeomGetData(o2); }
    else { dataB2 = (ODEBody*) dBodyGetData(b2); }
    ...
    if(frameB1 == frameB2) return;
    RW_DEBUG("- check enabled Map")
    // if any of the bodies are disabled then discard them
    if(_enabledMap[*frameB1] == 0 || _enabledMap[*frameB2] == 0) return;
    ...
    if(_excludeMap.has(pair)) return;
...
rw::math::Vector3D<> ODESimulator::addContacts(int numc, ODEBody* dataB1, ODEBody* dataB2,
    ...
    if((int) _srcIdx.size() < numc) _srcIdx.resize(numc);
    if((int) _dstIdx.size() < numc) _dstIdx.resize(numc);
...
void ODESimulator::resetScene(rw::kinematics::State& state) {
    ...
    for(ODEBody* body : _odeBodies) { body->reset(state); }
    ...
    for(ODETactileSensor* sensor : _odeSensors) { sensor->clear(); }

    RW_DEBUG("- Resetting devices: " << _odeDevices.size());
    // run through all devices and set the rigid bodies accoringly
    for(ODEDevice* device : _odeDevices) { device->reset(state); }
...
void ODESimulator::exitPhysics() {
    for(ODEConstraint* constraint : _odeConstraints) { delete constraint; }
    _odeConstraints.clear();

    for(ODEDevice* vdev : _odeDevices) { delete vdev; }
    _odeDevices.clear();

    for(ODETactileSensor* sensor : _odeSensors) { delete sensor; }
    _odeSensors.clear();

    while(_odeBodies.size() > 0) {
        ODEBody* body = _odeBodies[0];
        if(!_dwc->inDevice(body->getRwBody())) delete body;
        std::vector<ODEBody*>::iterator it = _odeBodies.begin();
        while(it < _odeBodies.end()) {
            if(*it == body) it = _odeBodies.erase(it);
            else it++;
        }
    }
    _odeBodies.clear();
