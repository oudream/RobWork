...
void ODESuctionCupDevice::update(const rwlibs::simulation::Simulator::UpdateInfo& info,
    ...
    if(!info.rollback) { updateNoRollBack(info, state); }
    ...
    else { sp1 = _dev->getSpringParamsOpen(); }
...
void ODESuctionCupDevice::reset(rw::kinematics::State& state) {
    ...
    Transform3D<> wTend  = Transform3D<>(
        wToff.P() + wToff.R() * (Vector3D<>::z() * _dev->getSpringParamsOpen()(4)), wToff.R());
...
void ODESuctionCupDevice::init(ODEBody* odebase, rwsim::dynamics::SuctionCup* scup,
    ...
    Transform3D<> wTend  = Transform3D<>(
        wToff.P() + wToff.R() * (Vector3D<>::z() * scup->getSpringParamsOpen()(4)), wToff.R());
    Vector3D<> saxis = wToff.R() * (Vector3D<>::z());    // slider axis is along the z-axis
