...
ODEBody::ODEBody(dBodyID odeBody, RigidBody::Ptr rwbody, rw::math::Vector3D<> offset, int matID,
    ...
    _offset(offset) {
    _body->changedEvent().add(boost::bind(&ODEBody::bodyChangedListener, this, boost::arg<1>()),
                              this);
}
...
ODEBody::ODEBody(std::vector<dGeomID> geomIds, dynamics::Body::Ptr body, int matID, int conID) :
    ...
    _contactReductionThreshold(0.005),    // 1cm
    _materialID(matID), _contactID(conID), _geomId(geomIds[0]), _geomIds(geomIds), _rwBody(NULL),
    _kBody(NULL) {
    _body->changedEvent().add(boost::bind(&ODEBody::bodyChangedListener, this, boost::arg<1>()),
                              this);
}
...
ODEBody::ODEBody(dBodyID odeBody, rw::kinematics::Frame* frame) :
    _mframe(dynamic_cast<MovableFrame*>(frame)), _bodyId(odeBody), _body(NULL), _rwframe(frame),
    _type(ODEBody::RigidDummy), _contactReductionThreshold(0.005),    // 1cm
...
void ODEBody::setTransformCOM(const rw::math::Transform3D<>& wTcom) {
    ...
    else { ODEUtil::setODEBodyT3D(_bodyId, wTcom); }
}
...
ODEBody* ODEBody::makeRigidBody(dynamics::Body::Ptr rwbody, dSpaceID spaceId, ODESimulator* sim) {
    ...
    for(ODEUtil::TriGeomData* gdata : gdatas) {
        if(!gdata->isPlaceable) RW_THROW("ODE can not use Plane geometry for rigid objects!");
...
ODEBody* ODEBody::makeKinematicBody(Body::Ptr kbody, dSpaceID spaceid, ODESimulator* sim) {
    ...
    else { odeBody = new ODEBody(bodyId, kbody, mc, mid, oid, ODEBody::KINEMATIC); }
    ...
    for(ODEUtil::TriGeomData* gdata : gdatas) {
        if(!gdata->isPlaceable) RW_THROW("ODE can not use Plane geometry for kinematic objects!");
...
ODEBody* ODEBody::makeFixedBody(Body::Ptr rwbody, dSpaceID spaceid, ODESimulator* sim) {
    ...
    for(size_t i = 0; i < gdatas.size(); i++) { geomids[i] = gdatas[i]->geomId; }
