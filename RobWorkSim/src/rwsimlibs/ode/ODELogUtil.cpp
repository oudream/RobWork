...
void ODELogUtil::addPositions(const std::string& description, const std::vector<ODEBody*>& bodies,
                              const State& state, const char* file, int line) {
    if(!doLog()) return;
...
void ODELogUtil::addVelocities(const std::string& description, const std::vector<ODEBody*>& bodies,
                               const State& state, const char* file, int line) {
    if(!doLog()) return;
