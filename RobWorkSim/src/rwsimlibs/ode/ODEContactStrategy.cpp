...
ODEContactStrategy::findContacts(ProximityModel::Ptr a, const Transform3D<>& wTa,
    ...
    RW_ASSERT(mA != NULL);
    RW_ASSERT(mB != NULL);
    if(!tracking.isInitialized()) tracking.setStrategyData(new ODETracking());
    ...
    for(const ODEContactModel::ODEModel& model : mA->models) {
        if(model.movable) ODEUtil::setODEGeomT3D(model.geomId, wTa * model.transform);
    }
    for(const ODEContactModel::ODEModel& model : mB->models) {
        if(model.movable) ODEUtil::setODEGeomT3D(model.geomId, wTb * model.transform);
    }
...
void ODEContactStrategy::destroyModel(ProximityModel* model) {
    ...
    RW_ASSERT(bmodel);
    for(const ODEContactModel::ODEModel& model : bmodel->models) { dGeomDestroy(model.geomId); }
    bmodel->models.clear();
...
void ODEContactStrategy::nearCallback(void* data, dGeomID o1, dGeomID o2) {
    ...
    if(numc >= MAX_CONTACTS - 1) RW_THROW("ODEContactStrategy uses too small collision buffer!");
    ...
    for(const ODEContactModel::ODEModel& model : contactData->mA->models) {
        if(model1 != NULL) break;
        if(model.geomId == o1) model1 = &model;
    }
    for(const ODEContactModel::ODEModel& model : contactData->mB->models) {
        if(model2 != NULL) break;
        if(model.geomId == o2) model2 = &model;
    }
