...
ODEConstraint::~ODEConstraint() {
    if(_spring != NULL) deleteSpring();
    if(_rwConstraint->getType() != Constraint::Free) destroyJoint();
}
...
void ODEConstraint::createJoint() {
    ...
    else { RW_THROW("Unsupported Constraint type!"); }
}
...
void ODEConstraint::setLimits() const {
    ...
    for(std::size_t i = 0; i < dof; i++) {
        ...
        if(limit.lowOn) setLoStop(i, limit.low);
        if(limit.highOn) setHiStop(i, limit.high);
    }
}
...
void ODEConstraint::decomposeCompliance(const Eigen::MatrixXd& compliance, const Spring& spring,
    ...
    for(Eigen::DenseIndex i = 0; i < sigma.rows(); i++) {
        Eigen::VectorXd vec;
        if(std::fabs((double) sigma(i)) < tolerance) vec = V.col(i);
        else vec = U.col(i);
        bool lin = false;
        bool ang = false;
        ...
        for(Eigen::DenseIndex j = 0; j < vec.rows(); j++) {
            if(j < spring.linComp) {
                linVec[j] = vec(j);
                if(vec(j) > linInfNorm) linInfNorm = vec(j);
            }
            else {
                angVec[j - spring.linComp] = vec(j);
                if(vec(j) > angInfNorm) angInfNorm = vec(j);
            }
            freeVec[j] = vec(j);
        }
        if(linInfNorm < 1e-12) lin = true;
        if(angInfNorm < 1e-12) ang = true;

        if(std::fabs((double) sigma(i)) < tolerance) {
            if(lin && ang)
                RW_THROW("Null space of compliance matrix spans vectors that are mixed linear and "
                         "angular.");
            if(lin) linFixed.push_back(linVec);
            if(ang) angFixed.push_back(angVec);
        }
        else { free.push_back(freeVec); }
    ...
    for(const std::vector<double>& dir : linFixed) {
        Vector3D<> cDir;
        for(std::size_t i = 0; i < dir.size(); i++) { cDir += dir[i] * spring.rowToDir[i]; }
    ...
    for(const std::vector<double>& dir : free) {
        VectorND<6> cDir;
        for(std::size_t i = 0; i < 6; i++) cDir[i] = 0;
        for(std::size_t i = 0; i < dir.size(); i++) {
...
Eigen::VectorXd ODEConstraint::getX(const Transform3D<>& wTchild, const Transform3D<>& wTconstraint,
...
Eigen::VectorXd ODEConstraint::getXd(const Transform3D<>& wTparent,
    ...
    const Vector3D<> linVelP   = (_parent->getType() == ODEBody::FIXED)
                                     ? Vector3D<>::zero()
    ...
    const Vector3D<> angVelP   = (_parent->getType() == ODEBody::FIXED)
                                     ? Vector3D<>::zero()
                                     : ODEUtil::toVector3D(dBodyGetAngularVel(_parent->getBodyID()));
    const Vector3D<> angVelC   = (_child->getType() == ODEBody::FIXED)
                                     ? Vector3D<>::zero()
...
std::pair<Vector3D<>, Vector3D<>> ODEConstraint::toCartesian(const Eigen::VectorXd& reduced) const {
    ...
    Vector3D<> lin;
    Vector3D<> ang;
    for(std::size_t i = 0; i < _spring->linComp; i++) lin += reduced[i] * _spring->rowToDir[i];
...
void ODEConstraint::update(const Simulator::UpdateInfo& dt, State& state) {
    ...
    for(unsigned int i = 0; i < _spring->cDec.linFixedDirs.size(); i++) {
        ...
        dJointSetLMotorParam(
            _spring->motorLin, dParamVel + dParamGroup * i, (1. / dt.dt) * dot(axisCon, pos.first));
    }
...
void ODEConstraint::createSpring() {
    ...
    for(std::size_t i = 0; i < 6; i++) _spring->comp[i] = false;
...
void ODEConstraint::deleteSpring() {
    if(_useSpringFrictionLin && _spring->linComp > 0) dJointDestroy(_spring->motorLin);
    if(_useSpringFrictionAng && _spring->angComp > 0) dJointDestroy(_spring->motorAng);
    delete _spring;
    _spring = NULL;
}
...
void ODEConstraint::setLoStop(std::size_t dof, double limit) const {
    ...
    if(type == Constraint::Fixed) {}
        ...
        if(dof == 0) dJointSetUniversalParam(_jointId, dParamLoStop, limit);
        else if(dof == 1) dJointSetUniversalParam(_jointId, dParamLoStop2, limit);
    }
    else if(type == Constraint::Spherical) {
        ...
        if(dof == 0) dJointSetPRParam(_jointId, dParamLoStop, limit);
        else if(dof == 1) dJointSetPRParam(_jointId, dParamLoStop2, limit);
    }
    else if(type == Constraint::PrismaticUniversal) {
        if(dof == 0) dJointSetPUParam(_jointId, dParamLoStop, limit);
        else if(dof == 1) dJointSetPUParam(_jointId, dParamLoStop2, limit);
    }
    else if(type == Constraint::Free) {
        // Do nothing
    }
    else { RW_THROW("Constraint is unsupported for lower limit"); }
}
void ODEConstraint::setHiStop(std::size_t dof, double limit) const {
    ...
    if(type == Constraint::Fixed) {}
        ...
        if(dof == 0) dJointSetUniversalParam(_jointId, dParamHiStop, limit);
        else if(dof == 1) dJointSetUniversalParam(_jointId, dParamHiStop2, limit);
    }
    else if(type == Constraint::Spherical) {
        ...
        if(dof == 0) dJointSetPRParam(_jointId, dParamHiStop, limit);
        else if(dof == 1) dJointSetPRParam(_jointId, dParamHiStop2, limit);
    }
    else if(type == Constraint::PrismaticUniversal) {
        if(dof == 0) dJointSetPUParam(_jointId, dParamHiStop, limit);
        else if(dof == 1) dJointSetPUParam(_jointId, dParamHiStop2, limit);
    }
    else if(type == Constraint::Free) {
        // Do nothing
    }
    else { RW_THROW("Constraint is unsupported for higher limit"); }
}
