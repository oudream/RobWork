...
void SupportPoseAnalyserDialog::btnPressed() {
    ...
    else if(obj == _ui->_calcBtn) { showPlanarDistribution(); }
    else if(obj == _ui->_saveDistBtn) { saveDistribution(); }
...
void SupportPoseAnalyserDialog::changedEvent() {
    ...
    if(obj == _ui->_drawXBox) {
        ...
        double ang = angle(wTdir_current,
                           wTdir_target,
        ...
        if(xaxis.size() == _xaxis[bodyIdx].size()) {
            ...
            for(std::size_t i = 0; i < length; i++) {
                ...
                if(_startTransforms[bodyIdx][i].P()[2] < 0) continue;
                if(_ui->_rpyBox->isChecked()) {
...
std::vector<CircleModel> refitCircles(std::vector<CircleModel>& circles,
    ...
    for(size_t i = 0; i < circles.size(); i++) {
        ...
        for(size_t j = 0; j < points.size(); j++) {
            if(circle.isClose(points[j], epsilon)) { circPoints.push_back(points[j]); }
        }
        // ignore the circle if the number of points is very low
        if(circPoints.size() < 4 + points.size() / 50) continue;
        ...
        for(size_t j = 0; j < circResult.size(); j++) {
            if(ncirc.isClose(circResult[j], 0.1)) {
                ...
                if(circPoints.size() < pointsInCircle[j]) break;
...
std::vector<CircleModel> extractCircles(std::vector<Vector3D<>>& points, int houghThres) {
    ...
    for(int i = 0; i < lines->total; i++) {
        ...
        for(size_t j = 0; j < lineGroups.size() && !inGroup; j++) {
            ...
            if(inGroup) { group.push_back(line); }
        }
        if(!inGroup) {
            LineGroup nGroup;
...
calculateRegions(const std::vector<Transform3D<>>& data, double dist, double angle) {
        ...
        for(const KDTreeQ<KDTreeValue>::KDNode* nn : result) {
            ...
            if(nnval.get<1>() >= 0 && nnval.get<1>() != currentIndex) {
                ...
                for(KDTreeQ<KDTreeValue>::KDNode& npro : nodes) {
                    ...
                    // check if the node is allready part of a region
                    if(npval.get<1>() == nnval.get<1>()) npval.get<1>() = currentIndex;
                }
            }
        }
    }
    ...
    for(mapType val : regions) {
        if(val.second == true) { validRegions.push_back(val.first); }
    }
...
void SupportPoseAnalyserDialog::process() {
    ...
    for(size_t j = 0; j < _bodies.size(); j++) {
        ...
        for(size_t i = 0; i < _xaxis[j].size(); i++) {
            ...
            if(xcirc < 0 && ycirc < 0 && zcirc < 0) {
            ...
            else if(xcirc < 0 && ycirc < 0) { c1++; }
            else if(xcirc < 0 && zcirc < 0) { c1++; }
            else if(ycirc < 0 && zcirc < 0) { c1++; }
            else if(xcirc < 0) {
                c2++;
                ...
                CircleModel circ = circlesX[xcirc];
                if(circ._r < circlesY[ycirc]._r) circ = circlesY[ycirc];
                if(circ._r < circlesZ[zcirc]._r) circ = circlesZ[zcirc];
            ...
            if(!hasPose) circlePoses.push_back(circlePose);
        }
