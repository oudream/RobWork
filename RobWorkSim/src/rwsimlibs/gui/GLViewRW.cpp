...
GLViewRW::GLViewRW(QWidget* parent) :
    ...
    _viewPos(0, 0, -5), _drawType(DrawableNode::SOLID), _alpha(1), _gl(new GLData()), _width(640),
    _height(480), _arcBall(_width, _height), _zoomFactor(0.0), _zoomScale(1.0),
...
void GLViewRW::drawGLStuff(bool showPivot) {
    ...
    glTranslated(
        -_pivotPoint(0) * _zoomScale, -_pivotPoint(1) * _zoomScale, -_pivotPoint(2) * _zoomScale);

    // TODO: draw arcball, TODO: not really nice looking _arcBall.draw()

    // draw center point
    if(showPivot) { drawPivot(_pivotPoint, _viewPos.norm2() * 0.01, _zoomScale, _gl->sphereObj); }
...
void GLViewRW::wheelEvent(QWheelEvent* event) {
    ...
    if(_zoomFactor >= 0) { _zoomScale = 1.0 + 0.5 * _zoomFactor; }
    else { _zoomScale = -1.0 / (0.5 * _zoomFactor - 1); }
...
void GLViewRW::saveBufferToFile(const QString& filename) {
    const char* type = "PNG";
    if(filename.endsWith(".BMP", Qt::CaseInsensitive)) type = "BMP";
    else if(filename.endsWith(".JPG", Qt::CaseInsensitive)) type = "JPG";
    else if(filename.endsWith(".PNG", Qt::CaseInsensitive)) type = "PNG";
    else {
