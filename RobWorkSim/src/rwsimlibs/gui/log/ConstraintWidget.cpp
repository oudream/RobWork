...
ConstraintWidget::~ConstraintWidget() {
    if(_root != NULL) { _root->removeChild("Constraints"); }
}
...
void ConstraintWidget::setEntry(rw::core::Ptr<const SimulatorLog> entry) {
    ...
    if(!(set == NULL)) _constraints = set;
    else RW_THROW("ConstraintWidget (setEntry): invalid entry!");
}
...
void ConstraintWidget::showGraphics(GroupNode::Ptr root, SceneGraph::Ptr graph) {
    if(root == NULL && _root != NULL) _root->removeChild("Constraints");
    _root  = root;
    _graph = graph;
}
...
ConstraintWidget::Dispatcher::makeWidget(rw::core::Ptr<const SimulatorLog> entry,
    ...
    const rw::core::Ptr<const LogConstraints> set = entry.cast<const LogConstraints>();
    if(!(set == NULL)) return new ConstraintWidget(set, parent);
...
bool ConstraintWidget::Dispatcher::accepts(rw::core::Ptr<const SimulatorLog> entry) const {
    if(!(entry.cast<const LogConstraints>() == NULL)) return true;
    return false;
}
