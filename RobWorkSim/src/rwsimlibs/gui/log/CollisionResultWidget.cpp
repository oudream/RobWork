...
CollisionResultWidget::~CollisionResultWidget() {
    if(_root != NULL) { _root->removeChild("Collisions"); }
}
...
void CollisionResultWidget::setEntry(rw::core::Ptr<const SimulatorLog> entry) {
    ...
    else RW_THROW("CollisionResultWidget (setEntry): invalid entry!");
}
...
void CollisionResultWidget::updateEntryWidget() {
    ...
    if(nrOfEntries > 0)
        _ui->_framePairTable->setRangeSelected(QTableWidgetSelectionRange(0, 0, nrOfEntries - 1, 4),
                                               true);
}

void CollisionResultWidget::showGraphics(GroupNode::Ptr root, SceneGraph::Ptr graph) {
    if(root == NULL && _root != NULL) _root->removeChild("Collisions");
    _root  = root;
    _graph = graph;
}
...
CollisionResultWidget::Dispatcher::makeWidget(rw::core::Ptr<const SimulatorLog> entry,
    ...
    if(!(tentry == NULL)) return new CollisionResultWidget(tentry, parent);
...
bool CollisionResultWidget::Dispatcher::accepts(rw::core::Ptr<const SimulatorLog> entry) const {
    if(!(entry.cast<const LogCollisionResult>() == NULL)) return true;
    return false;
}
