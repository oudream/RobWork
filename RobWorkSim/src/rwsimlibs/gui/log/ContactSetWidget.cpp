...
ContactSetWidget::~ContactSetWidget() {
    if(_root != NULL) { _root->removeChild("Contacts"); }
}
...
void ContactSetWidget::setEntry(rw::core::Ptr<const SimulatorLog> entry) {
    ...
    if(!(set == NULL)) _contactSet = set;
    else RW_THROW("_contactSet (setEntry): invalid entry!");
}
...
void ContactSetWidget::updateEntryWidget() {
    ...
    for(const Contact& c : _contactSet->getContacts()) {
        ...
        if(nameA < nameB) pairs.insert(std::make_pair(nameA, nameB));
        else pairs.insert(std::make_pair(nameB, nameA));
    }
    ...
    for(const FramePair& pair : pairs) {
        ...
        int contacts = 0;
        for(const Contact& c : _contactSet->getContacts()) {
            if(c.getNameA() == pair.first && c.getNameB() == pair.second) contacts++;
            else if(c.getNameA() == pair.second && c.getNameB() == pair.first) contacts++;
        }
    ...
    if(nrOfPairs > 0)
        _ui->_contactBodyPairs->setRangeSelected(QTableWidgetSelectionRange(0, 0, nrOfPairs - 1, 2),
                                                 true);
}

void ContactSetWidget::showGraphics(GroupNode::Ptr root, SceneGraph::Ptr graph) {
    if(root == NULL && _root != NULL) _root->removeChild("Bodies");
    _root  = root;
    _graph = graph;
}
...
void ContactSetWidget::setProperties(const PropertyMap::Ptr properties) {
    ...
    if(!_properties.isNull()) {
        ...
        _ui->_scalingRadius->setValue(_properties->get<double>("ContactSetWidget_PointRadius") *
                                      1000.);
        _ui->_scalingNormal->setValue(_properties->get<double>("ContactSetWidget_NormalLength") *
                                      1000.);
...
void ContactSetWidget::contactSetPairsChanged(const QItemSelection&, const QItemSelection&) {
    ...
    for(std::size_t i = 0; i < _contactSet->size(); i++) {
        ...
        for(const NamePair& name : names) {
            const std::string& nameA = c.getNameA();
            const std::string& nameB = c.getNameB();
            if(nameA == name.first && nameB == name.second) show = true;
            else if(nameB == name.first && nameA == name.second) show = true;
            if(show) break;
        }
        if(show) contactsToShow.push_back(i);
    }
...
void ContactSetWidget::contactSetChanged(const QItemSelection&, const QItemSelection&) {
    ...
    foreach(QModelIndex index, indexes) {
        ...
        const Contact& c    = _contactSet->getContact(i);
        if(c.getDepth() > 0) contactsPen.push_back(c);
        else contactsNon.push_back(c);
    }
...
QString ContactSetWidget::toQString(const Contact& contact) {
    ...
    return "Bodies: " + nameA + " - " + nameB + "<br/>" +
           "Points: " + toQString(contact.getPointA()) + " - " + toQString(contact.getPointB()) +
           "<br/>" + "Normal: " + toQString(contact.getNormal()) + "<br/>" +
...
ContactSetWidget::Dispatcher::makeWidget(rw::core::Ptr<const SimulatorLog> entry,
    ...
    if(!(contactSet == NULL)) return new ContactSetWidget(contactSet, parent);
...
bool ContactSetWidget::Dispatcher::accepts(rw::core::Ptr<const SimulatorLog> entry) const {
    if(!(entry.cast<const LogContactSet>() == NULL)) return true;
    return false;
}
