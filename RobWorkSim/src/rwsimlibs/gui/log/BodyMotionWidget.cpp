...
BodyMotionWidget::BodyMotionWidget(rw::core::Ptr<const LogPositions> entry, QWidget* parent) :
    SimulatorLogEntryWidget(parent), _ui(new Ui::BodyMotionWidget()), _dwc(NULL), _positions(entry),
    _velocities(NULL) {
    _ui->setupUi(this);
...
BodyMotionWidget::BodyMotionWidget(rw::core::Ptr<const LogVelocities> entry, QWidget* parent) :
    ...
    connect(
        _ui->_motionScalingLinA, SIGNAL(valueChanged(double)), this, SLOT(scalingChanged(double)));
    connect(
        _ui->_motionScalingLinB, SIGNAL(valueChanged(double)), this, SLOT(scalingChanged(double)));
    connect(
        _ui->_motionScalingAngA, SIGNAL(valueChanged(double)), this, SLOT(scalingChanged(double)));
    connect(
        _ui->_motionScalingAngB, SIGNAL(valueChanged(double)), this, SLOT(scalingChanged(double)));
}
...
void BodyMotionWidget::setEntry(rw::core::Ptr<const SimulatorLog> entry) {
    ...
    if(!(pos == NULL)) { _positions = pos; }
    else if(!(vel == NULL)) {
        _velocities = vel;
        _positions  = vel->getPositions();
    }
    else { RW_THROW("BodyMotionWidget (setEntry): invalid entry!"); }
}

rw::core::Ptr<const SimulatorLog> BodyMotionWidget::getEntry() const {
    if(!(_velocities == NULL)) return _velocities;
    else if(!(_positions == NULL)) return _positions;
...
void BodyMotionWidget::updateEntryWidget() {
    ...
    if(!(_velocities == NULL)) {
        ...
        for(it = velMap.begin(); it != velMap.end(); it++) {
            ...
            for(int i = 0; i < 6; i++)
                _ui->_motionBodiesTable->setItem(row,
                                                 i + 1,
                                                 new QTableWidgetItem(QString::number(
                                                     vel[static_cast<std::size_t>(i)], 'd', 4)));
            row++;
        }
...
std::string BodyMotionWidget::getName() const {
    if(!(_velocities == NULL)) return "Velocity";
    else if(!(_positions == NULL)) return "Position";
    return "Motion";
}
...
BodyMotionWidget::Dispatcher::makeWidget(rw::core::Ptr<const SimulatorLog> entry,
    ...
    if(!(pos == NULL)) return new BodyMotionWidget(pos, parent);
    else if(!(vel == NULL)) return new BodyMotionWidget(vel, parent);
...
bool BodyMotionWidget::Dispatcher::accepts(rw::core::Ptr<const SimulatorLog> entry) const {
    if(!(entry.cast<const LogPositions>() == NULL)) return true;
    else if(!(entry.cast<const LogVelocities>() == NULL)) return true;
    return false;
}
