...
void SimulatorLogModel::compare(rw::core::Ptr<const SimulatorLog> a,
    ...
    if(*a == *b) return;
    else _bgColor[a.get()] = QColor(255, 0, 0);
    if(aScope.isNull()) return;
    if(!bScope.isNull()) {
    ...
    else { compareFailSubTree(aScope); }
}
...
void SimulatorLogModel::compareFailSubTree(rw::core::Ptr<const SimulatorLog> a) {
    ...
    if(!aScope.isNull()) {
        const std::vector<SimulatorLog::Ptr> aChildren = aScope->getChildren();
        for(const SimulatorLog::Ptr& child : aChildren) { compareFailSubTree(child); }
    }
}
...
QVariant SimulatorLogModel::data(const QModelIndex& index, int role) const {
    ...
    if(role == Qt::DisplayRole) {
        if(index.column() == 0) { return QString::fromStdString(node->getDescription()); }
        else if(index.column() == 1) { return QString::number(node->children()); }
    }
    else if(role == Qt::ToolTipRole) { return QString::fromStdString(node->getDescription()); }
    else if(role == Qt::BackgroundRole) {
        std::map<const SimulatorLog*, QColor>::const_iterator it = _bgColor.find(node);
        if(it != _bgColor.end()) return it->second;
...
const SimulatorLog* SimulatorLogModel::nodeFromIndex(const QModelIndex& index) const {
    ...
    if(index.isValid()) { return static_cast<const SimulatorLog*>(index.internalPointer()); }
    else { return _root.get(); }
}
...
QModelIndex SimulatorLogModel::index(int row, int column, const QModelIndex& parent) const {
    ...
    if(parentComposite == NULL) return QModelIndex();
    ...
    if(childNode == NULL) return QModelIndex();
...
QModelIndex SimulatorLogModel::parent(const QModelIndex& child) const {
    ...
    if(row > grandparentNode->children()) { return QModelIndex(); }
...
bool SimulatorLogModel::setData(const QModelIndex& index, const QVariant& value, int role) {
    if(role != Qt::BackgroundRole || !index.isValid()) return false;
    const SimulatorLog* const entry = static_cast<const SimulatorLog*>(index.internalPointer());
    if(value.isValid()) { _bgColor[entry] = value.value<QColor>(); }
    else {
...
QVariant SimulatorLogModel::headerData(int section, Qt::Orientation orientation, int role) const {
    if(role == Qt::TextAlignmentRole) { return Qt::AlignCenter; }
        ...
        if(section == 0) return "Entry Name";
