...
ForceTorqueWidget::ForceTorqueWidget(rw::core::Ptr<const LogForceTorque> entry, QWidget* parent) :
    SimulatorLogEntryWidget(parent), _ui(new Ui::ForceTorqueWidget()), _forces(entry) {
    _ui->setupUi(this);
...
ForceTorqueWidget::~ForceTorqueWidget() {
    if(_root != NULL) { _root->removeChild("Wrenches"); }
}
...
void ForceTorqueWidget::setEntry(rw::core::Ptr<const SimulatorLog> entry) {
    ...
    if(!(set == NULL)) { _forces = set; }
    else { RW_THROW("ForceTorqueWidget (setEntry): invalid entry!"); }
}
...
void ForceTorqueWidget::showGraphics(GroupNode::Ptr root, SceneGraph::Ptr graph) {
    if(root == NULL && _root != NULL) _root->removeChild("Wrenches");
    _root  = root;
    _graph = graph;
}
...
void ForceTorqueWidget::scalingChanged(double d) {
    if(!_properties.isNull()) {
        if(QObject::sender() == _ui->_scalingLinA) _properties->set("ForceTorqueWidget_LinA", d);
...
ForceTorqueWidget::Dispatcher::makeWidget(rw::core::Ptr<const SimulatorLog> entry,
    ...
    if(!(forces == NULL)) return new ForceTorqueWidget(forces, parent);
...
bool ForceTorqueWidget::Dispatcher::accepts(rw::core::Ptr<const SimulatorLog> entry) const {
    if(!(entry.cast<const LogForceTorque>() == NULL)) return true;
    return false;
}
