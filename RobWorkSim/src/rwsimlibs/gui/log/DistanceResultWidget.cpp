...
DistanceResultWidget::DistanceResultWidget(const LogDistanceResult::CPtr entry,
        QWidget* parent):
    SimulatorLogEntryWidget(parent),
    _ui(new Ui::DistanceResultWidget()),
...
{
    const rw::common::Ptr<const LogDistanceResult> set =
...
void DistanceResultWidget::updateEntryWidget()
{
    ...
    for(const LogDistanceResult::ResultInfo& info : results) {
        ...
        _ui->_framePairTable->setItem(row,2,new QTableWidgetItem(QString::number(info.result.distance)));
        row++;
    }
    _ui->_framePairTable->setSortingEnabled(true);
    if (nrOfEntries > 0)
        _ui->_framePairTable->setRangeSelected(QTableWidgetSelectionRange(0,0,nrOfEntries-1,2),true);
}

void DistanceResultWidget::showGraphics(
        const GroupNode::Ptr root, const SceneGraph::Ptr graph)
{
...
SimulatorLogEntryWidget* DistanceResultWidget::Dispatcher::makeWidget(
        const SimulatorLog::CPtr entry, QWidget* parent) const
{
    const rw::common::Ptr<const LogDistanceResult> tentry = entry.cast<const LogDistanceResult>();
    if (!(tentry == NULL))
...
bool DistanceResultWidget::Dispatcher::accepts(const SimulatorLog::CPtr entry)
const
{
...
void DistanceResultWidget::framePairsChanged(const QItemSelection& newSelection, const QItemSelection& oldSelection) {
    ...
    foreach (const QModelIndex& index, indexes) {
        if (index.column() > 0)
            continue;
        const LogDistanceResult::ResultInfo& result = results[static_cast<std::size_t>(index.row())];
    ...
    const DrawableNode::Ptr drawable = _graph->makeDrawable("Distances",render,DrawableNode::Virtual);
