...
DistanceResultWidget::~DistanceResultWidget() {
    if(_root != NULL) { _root->removeChild("Distances"); }
}
...
void DistanceResultWidget::setEntry(const SimulatorLog::CPtr entry) {
    const rw::core::Ptr<const LogDistanceResult> set = entry.cast<const LogDistanceResult>();
    if(!(set == NULL)) _result = set;
    else RW_THROW("DistanceResultWidget (setEntry): invalid entry!");
}
...
void DistanceResultWidget::showGraphics(const GroupNode::Ptr root, const SceneGraph::Ptr graph) {
    if(root == NULL && _root != NULL) _root->removeChild("Distances");
    _root  = root;
    _graph = graph;
}
...
DistanceResultWidget::Dispatcher::makeWidget(const SimulatorLog::CPtr entry,
                                             QWidget* parent) const {
    const rw::core::Ptr<const LogDistanceResult> tentry = entry.cast<const LogDistanceResult>();
    if(!(tentry == NULL)) return new DistanceResultWidget(tentry, parent);
...
bool DistanceResultWidget::Dispatcher::accepts(const SimulatorLog::CPtr entry) const {
    if(!(entry.cast<const LogDistanceResult>() == NULL)) return true;
    return false;
}
            ...
            for(const Geometry::Ptr& geo : object->getGeometry()) {
                if(geo->getFrame()->getName() == result.frameA &&
                   geo->getId() == result.geoNamesA[geoIdxA]) {
                    ...
                    Transform3D<> T;
                    if(_positions->has(result.frameA)) T = _positions->getPosition(result.frameA);
                    ...
                    const TriMesh::Ptr data = geo->getGeometryData()->getTriMesh();
                    Transform3D<> T;
                    if(_positions->has(result.frameB)) T = _positions->getPosition(result.frameB);
                    T                    = T * geo->getTransform();
