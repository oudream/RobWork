...
namespace { namespace {
    ...
    std::string openFile(std::string& previousOpenDirectory, QWidget* parent) {
        ...
        if(!file.empty()) previousOpenDirectory = rw::core::StringUtil::getDirectoryName(file);

        return file;
    }
}}    // namespace

GraspSelectionDialog::GraspSelectionDialog(const rw::kinematics::State& state, DynamicWorkCell* dwc,
    ...
    connect(
        _ui->_graspTableSlider, SIGNAL(valueChanged(int)), _ui->_graspSpin, SLOT(setValue(int)));
...
void GraspSelectionDialog::initializeStart() {
    ...
    for(RigidBody::Ptr rbody : _bodies) { _frameToBody[*rbody->getMovableFrame()] = rbody; }
...
void GraspSelectionDialog::btnPressed() {
    ...
    if(obj == _ui->_graspTableLoadBtn) {
        ...
        else { _object = dynamic_cast<MovableFrame*>(obj); }
        if(_gtable->size() == 0) {
            _ui->_graspTableSlider->setRange(0, 0);
        ...
        if(fbase != NULL) _handBase = dynamic_cast<MovableFrame*>(fbase);
        else _handBase = NULL;
    }
...
void GraspSelectionDialog::setGraspState(GraspTable::GraspData& data,
    ...
    if(_object) _object->setTransform(wTo, state);
    if(_handBase) _handBase->setTransform(wTo * inverse(hbTo), state);
    ...
    for(std::size_t i = 0; i < _dwc->getSensors().size(); i++) {
        if((tsensor = dynamic_cast<TactileArraySensor*>(_dwc->getSensors()[i].get()))) {
            if(j >= data._tactiledata.size()) continue;
...
void GraspSelectionDialog::updateStatus() {
    ...
    for(size_t i = 0; i < _simulators.size(); i++) {
        ...
        if(!sim->isRunning() && _nrOfTests >= _ui->_nrOfTestsSpin->value()) { continue; }
        ...
        if(!allBelowThres || time < _ui->_minTimeValidSpin->value()) _lastBelowThresUpdate = time;
        ...
        if(time - _lastBelowThresUpdate > _ui->_minRestTimeSpin->value()) {
            ...
            if(_nrOfTests >= _ui->_nrOfTestsSpin->value()) continue;
    ...
    if(_ui->_forceUpdateCheck->isChecked()) stateChanged(state);
