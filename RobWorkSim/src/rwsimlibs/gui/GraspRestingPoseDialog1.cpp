...
void saveRestingPose(std::string& desc, std::vector<matrix<float>>& datas, rw::math::Q& qualities,
                     bool isStable, int preshapeId, rw::math::Q& preq, rw::math::Q& handq,
    ...
    for(matrix<float>& data : datas) {
        for(size_t x = 0; x < data.size1(); x++) {
            for(size_t y = 0; y < data.size2(); y++) { file << data(x, y) << " "; }
            file << "\n";
        }
        file << "\n\n";
    }
...
void saveRestingPose(std::string& desc, std::vector<matrix<float>>& datas, rw::math::Q& qualities,
                     bool isStable, int preshapeId, rw::math::Q& preq, rw::math::Q& handq,
                     rw::math::Vector3D<>& approach, const std::string& filename) {
...
GraspRestingPoseDialog::GraspRestingPoseDialog(const rw::kinematics::State& state,
                                               ...
                                               QWidget* parent) :
    QDialog(parent),
    _defstate(state), _state(state), _dwc(dwc), _colDect(detector), _avgSimTime(4), _avgTime(4),
    _handBase(NULL), _exitHard(false), _graspNotStable(false) {
    RW_ASSERT(_dwc);
    setupUi(this);
    ...
    for(Body* body : _dwc->getBodies()) {
        ...
        if(dynamic_cast<const MovableFrame*>(obj)) { _objectBox->addItem(obj->getName().c_str()); }
    }
...
*/
        ...
        if(_nrOfTests < _nrOfTestsSpin->value()) _startBtn->setDisabled(false);
...
void GraspRestingPoseDialog::changedEvent() {
    ...
    else if(obj == _updateRateSpin) { _timer->setInterval(_updateRateSpin->value()); }
    else if(obj == _forceUpdateCheck) {
        if(!_forceUpdateCheck->isChecked()) { _timer->setInterval(100); }
        else { _timer->setInterval(_updateRateSpin->value()); }
    }
}
...
void GraspRestingPoseDialog::saveRestingState(rw::core::Ptr<ThreadSimulator> sim) {
    ...
    if(isStable) {
        filename << pathPre << "/S_OutDataTest_" << (_preshapeStratBox->currentText().toStdString())
                 << "_" << _nrOfTests << ".txt";
    }
    else {
        filename << pathPre << "/U_OutDataTest_" << (_preshapeStratBox->currentText().toStdString())
                 << "_" << _nrOfTests << ".txt";
    }
...
void GraspRestingPoseDialog::updateStatus() {
    ...
    for(size_t i = 0; i < _simulators.size(); i++) {
        ...
        if(!isSimRunning && _nrOfTests >= _nrOfTestsSpin->value()) { continue; }
        ...
        if(!isSimFinished || time < _minTimeValidSpin->value()) _lastBelowThresUpdate = time;

        if(i == 0) _state = sim->getState();
        ...
        if(time - _lastBelowThresUpdate > _minRestTimeSpin->value()) {
            ...
            if(_nrOfTests >= _nrOfTestsSpin->value()) continue;
    ...
    if(_forceUpdateCheck->isChecked()) { stateChanged(_state); }
...
void GraspRestingPoseDialog::calcRandomCfg(rw::kinematics::State& state) {
    if(_colFreeStart->isChecked()) { calcColFreeRandomCfg(state); }
    else { calcRandomCfg(_bodies, state); }
}
