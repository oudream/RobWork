...
void saveRestingPoseSimple(std::string& desc,
    ...
    // Grasp Qualities
    for(size_t i = 0; i < qualities.size(); i++) file << qualities[i] << "\t";

    // preshape ID
    file << preshapeId << "\t";

    // preshape of hand
    for(size_t i = 0; i < preq.size(); i++) file << preq[i] * Rad2Deg << "\t";

    // resting configuration of hand
    for(size_t i = 0; i < handq.back().size(); i++) file << handq.back()[i] * Rad2Deg << "\t";
...
void saveRestingPose(std::string& desc,
    ...
    for(GraspRestingPoseDialog::TactileSensorData& tdata : datas) {
        file << "# Q " << idx << "/" << datas.size() << "\n";
        for(size_t i = 0; i < handq.back().size(); i++) file << handq[idx][i] * Rad2Deg << " ";
        file << "\n\n";

        file << "# Tdata " << idx << "/" << datas.size() << "\n";
        for(Eigen::MatrixXf& data : tdata) {
            for(Eigen::DenseIndex x = 0; x < data.rows(); x++) {
                for(Eigen::DenseIndex y = 0; y < data.cols(); y++) { file << data(x, y) << " "; }
                file << "\n";
            }
            file << "\n\n";
        }
...
void saveRestingPose(std::string& desc,
    ...
    if(!file.is_open()) RW_THROW("CANNOT OPEN FILE! " << filename);
    saveRestingPose(
...
void GraspRestingPoseDialog::setPreshapeStrategy(const std::string& str) {
    ...
    if(idx >= 0) _ui->_preshapeStratBox->setCurrentIndex(idx);
}
...
void GraspRestingPoseDialog::initializeStart() {
    ...
    if(!file.is_open()) RW_THROW("CANNOT OPEN FILE! " << filename.str());
    ...
    if(_bodies.size() == 0) { RW_THROW("You must choose a valid object to grasp!"); }
    ...
    file << "Preshape configuration type: " << _ui->_preshapeStratBox->currentText().toStdString()
         << "\n";
    file << "preshapes:  \n";
    for(Q pq : _preshapes) { file << "- " << pq << "\n"; }
    file << "\nTargets: \n";
    for(Q pq : _targetQ) { file << "- " << pq << "\n"; }
    ...
    for(int i = 0; i < threads; i++) {
        ...
        // 0.1)); pdctrl->reset( state ); sim->addController(pdctrl);
        ...
        ThreadSimulator::StepCallback cb(
            boost::bind(&CallBackFunctor::stepCallBack, func, boost::arg<1>(), boost::arg<2>()));
...
void GraspRestingPoseDialog::stepCallBack(int i, const rw::kinematics::State& state) {
    try {
        ...
        for(rw::sensor::Contact3D& con : contacts) {
            if(con.normalForce > 1000) largeForces = true;
        }
            ...
            if(TactileArraySensor* tsensor = dynamic_cast<TactileArraySensor*>(sensor.get())) {
                ...
                for(Eigen::DenseIndex i = 0; i < datas.back().rows(); i++)
                    for(Eigen::DenseIndex j = 0; j < datas.back().cols(); j++)
                        if(datas.back()(i, j) > 0.5) sum += datas.back()(i, j);
                if(sum > 1) {
        ...
        if(fingersWithData > 0) { _tactileDataOnAllCnt++; }
        else { _tactileDataOnAllCnt = 0; }

        if(_ui->_continuesLogging->isChecked()) {
            if(fingersWithData > 0) { _fingersInContact[i] = true; }
            else {
        ...
        if(!isSimFinished || time < _ui->_minTimeValidSpin->value()) _lastBelowThresUpdate = time;
           ...
           _tactileDataOnAllCnt > 50) {
            ...
            if(_ui->_inGroupsCheck->isChecked()) {
                if(_nrOfGraspsInGroup == 0 || _nrOfGraspsInGroup == _ui->_groupSizeSpin->value()) {
                    ...
                    else { handForceLimits(0) = 2 * cos(alpha) * handForceLimits(3); }
                    ...
                    if(_ui->_fixedGroupBox->isChecked()) {
                        double devi = 0.005;    //
                        if(_nrOfGraspsInGroup - 1 > 5) devi = Pi / 90.0;
                        if((_nrOfGraspsInGroup - 1) & 0x1) devi = -devi;
                    ...
                    _nrOfGraspsInGroup++;
                    if(_nrOfGraspsInGroup > _ui->_groupSizeSpin->value()) _nrOfGraspsInGroup = 0;
                }
            }
            else {
                nstate                 = _defstate;
...
*/
        ...
        if(_nrOfTests < _ui->_nrOfTestsSpin->value()) _ui->_startBtn->setDisabled(false);
...
void GraspRestingPoseDialog::changedEvent() {
    ...
    if(obj == _timer) {
    ...
    else if(obj == _ui->_updateRateSpin) { _timer->setInterval(_ui->_updateRateSpin->value()); }
    else if(obj == _ui->_forceUpdateCheck) {
        if(!_ui->_forceUpdateCheck->isChecked()) { _timer->setInterval(100); }
        else { _timer->setInterval(_ui->_updateRateSpin->value()); }
    }
        ...
        else { _ui->_groupSizeSpin->setEnabled(true); }
    }
}
...
bool GraspRestingPoseDialog::isSimulationFinished(DynamicSimulator::Ptr sim, const State& state) {
    ...
    if(MetricUtil::normInf(vel) < lVelThres) { return true; }
    return false;
}
...
bool GraspRestingPoseDialog::saveRestingState(int simidx, DynamicSimulator::Ptr sim,
    ...
    // Transform3D<> wTf = Kinematics::worldTframe(_handBase, state);
    for(Contact3D& c : g3d.contacts) { sstr << c.p << "  " << c.n << " "; }
    ...
    for(Contact3D& c : g3d.contacts) { std::cout << "\t" << c.p << "  " << c.n << " \n"; }
...
void GraspRestingPoseDialog::updateStatus() {
    ...
    if(_ui->_forceUpdateCheck->isChecked()) { stateChanged(_state); }
...
void GraspRestingPoseDialog::calcRandomCfg(std::vector<RigidBody::Ptr>& bodies,
    ...
    Rotation3D<> hR =
        RPY<>(Math::ran(-Pi / 2, Pi / 2), Math::ran(-Pi / 2, Pi / 2), Math::ran(-Pi / 2, Pi / 2))
            .toRotation3D() /* * bTo.R()*/;
    hR = rrot;
...
void GraspRestingPoseDialog::calcRandomCfg(rw::kinematics::State& state) {
    if(_ui->_colFreeStart->isChecked()) { calcColFreeRandomCfg(state); }
    else {
