...
void RestingPoseDialog::btnPressed() {
    ...
    if(obj == _ui->_saveBtn1) {
        ...
        if(_ui->_asCommaBtn->isChecked()) {
            ...
            for(RigidBody::Ptr b : bodies) { fout << b->getName() << " ; "; }
            fout << "\n";
            fout.precision(16);
            for(size_t i = 0; i < _startPoses.size(); i++) {
...
void RestingPoseDialog::calcColFreeRandomCfg(rw::kinematics::State& state) {
    ...
    while(_colDect->inCollision(state, &result, false)) {
        ...
        for(rw::kinematics::FramePair pair : result.collidingFrames) {
            ...
            if(!body1.isNull()) bodies.push_back(body1);
            if(!body2.isNull()) bodies.push_back(body2);
        }
        if(bodies.size() == 0) RW_THROW("Collision between non-rigid bodies.");
...
Transform3D<> ranTransform3D(const rw::math::Transform3D<>& point, double maxPos, double maxAngle) {
    Vector3D<> pos(
        Random::ran(-maxPos, maxPos), Random::ran(-maxPos, maxPos), Random::ran(-maxPos, maxPos));
    EAA<> rot(Random::ran(-maxAngle, maxAngle),
              Random::ran(-maxAngle, maxAngle),
              Random::ran(-maxAngle, maxAngle));
...
void RestingPoseDialog::calcRandomCfg(std::vector<RigidBody::Ptr>& bodies,
    ...
    if(_ui->_rotationTabs->currentIndex() == 0) {
        ...
        if(_ui->_normalDistRot->isChecked()) { rot = ranNormalDistRotation3D(angle); }
        else { rot = ranRotation3D(angle); }
    }
    else {
