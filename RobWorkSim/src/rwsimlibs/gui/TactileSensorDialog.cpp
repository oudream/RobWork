...
Moment calcMoments2D(Eigen::MatrixXf& mat, double low_thres) {
        ...
        for(Eigen::DenseIndex y = 0; y < mat.cols(); y++) {
            if(mat(x, y) > low_thres) {
                ...
                for(size_t j = 0; j < 2; j++)
                    for(size_t k = 0; k < 2; k++) covar(j, k) += val(j) * val(k);
            }
        }
    }
    if(centroid(0) < 0.01 && centroid(1) < 0.01) return Moment();

    const size_t n = mat.rows() * mat.cols();
    for(size_t j = 0; j < 2; j++)
        for(size_t k = 0; k < 2; k++) covar(j, k) = covar(j, k) - centroid[j] * centroid[k] / n;
    ...
    if(maxEigVal < minEigVal) std::swap(minEigIdx, maxEigIdx);
...
void TactileSensorDialog::btnPressed() {
    ...
    if(obj == _ui->_updateBtn) {
        ...
        for(int i = 0; i < 13; i++) {
            for(int j = 0; j < 6; j++) ifile >> mat(i, j);
        }
...
void TactileSensorDialog::changedEvent() {
    QObject* obj = sender();
    if(obj == _ui->_maxPressureSpin) { drawTactileInput(); }
...
void TactileSensorDialog::setState(const rw::kinematics::State& state) {
    _values.clear();
    for(TactileArraySensor* sensor : _tsensors) { _values.push_back(sensor->getTexelData(state)); }
    // now detect features in the tactile images enabled
    detectFeatures();
    drawTactileInput();
    if(_ui->_saveCheckBox->isChecked()) {
        if(_renderingToImage) return;
...
void TactileSensorDialog::detectFeatures() {
    if(_ui->_centerOfMassBtn->isChecked() || _ui->_momentsBtn->isChecked()) { detectCenterMass(); }
    if(_ui->_momentsBtn->isChecked()) { findMoments(); }
}
