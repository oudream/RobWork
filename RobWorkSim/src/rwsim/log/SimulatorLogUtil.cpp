...
void SimulatorLogUtil::endStep(double time, int line) {
    ...
    if(step == NULL) RW_THROW("Could not end step! - Not in correct scope.");
...
void SimulatorLogUtil::endSection(int line) {
    ...
    if(section == NULL) RW_THROW("Could not end section! - Not in correct scope.");
    section->setLineEnd(line);
    _scope = section->getParent();
}

SimulatorLogScope* SimulatorLogUtil::makeScope(const std::string& name, const char* file,
                                               int line) {
...
void SimulatorLogUtil::addValues(const std::string& description, const std::vector<double>& values,
