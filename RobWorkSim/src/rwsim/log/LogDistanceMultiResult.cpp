...
bool LogDistanceMultiResult::ResultInfo::operator==(const ResultInfo& b) const {
    ...
    if(result.p1 != b.result.p1) return false;
    if(result.p2 != b.result.p2) return false;
    if(result.distance != b.result.distance) return false;
    if(result.p1s != b.result.p1s) return false;
    if(result.p2s != b.result.p2s) return false;
    if(result.geoIdxA != b.result.geoIdxA) return false;
    if(result.geoIdxB != b.result.geoIdxB) return false;
    if(result.p1prims != b.result.p1prims) return false;
    if(result.p2prims != b.result.p2prims) return false;
    if(result.distances != b.result.distances) return false;
...
void LogDistanceMultiResult::addResults(const std::vector<DistanceMultiStrategy::Result>& results) {
    for(const DistanceMultiStrategy::Result& result : results) { addResult(result); }
}
