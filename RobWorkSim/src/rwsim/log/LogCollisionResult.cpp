...
bool LogCollisionResult::ResultInfo::operator==(const ResultInfo& b) const {
    ...
    if(geoNamesA != b.geoNamesA) return false;
    if(geoNamesB != b.geoNamesB) return false;

    if(result._aTb != b.result._aTb) return false;
    if(result._collisionPairs.size() != b.result._collisionPairs.size()) return false;
    for(std::size_t i = 0; i < result._collisionPairs.size(); i++) {
        if(result._collisionPairs[i].geoIdxA != b.result._collisionPairs[i].geoIdxA) return false;
        if(result._collisionPairs[i].geoIdxB != b.result._collisionPairs[i].geoIdxB) return false;
        if(result._collisionPairs[i].startIdx != b.result._collisionPairs[i].startIdx) return false;
        if(result._collisionPairs[i].size != b.result._collisionPairs[i].size) return false;
    }
    if(result._geomPrimIds != b.result._geomPrimIds) return false;
    if(result._nrBVTests != b.result._nrBVTests) return false;
    if(result._nrPrimTests != b.result._nrPrimTests) return false;
...
std::list<SimulatorLogEntry::Ptr> LogCollisionResult::getLinkedEntries() const {
    if(_positions == NULL) return std::list<SimulatorLogEntry::Ptr>();
    else return std::list<SimulatorLogEntry::Ptr>(1, _positions);
}
...
void LogCollisionResult::addResult(const CollisionStrategy::Result& result) {
    ...
    if(a != NULL) {
        ...
        if(frame != NULL) info.frameA = frame->getName();
        info.geoNamesA = a->getGeometryIDs();
        info.result.a  = NULL;
    }
    if(b != NULL) {
        const Frame* const frame = b->getFrame();
        if(frame != NULL) info.frameB = frame->getName();
...
void LogCollisionResult::addResults(const std::vector<CollisionStrategy::Result>& results) {
    for(const CollisionStrategy::Result& result : results) { addResult(result); }
}
