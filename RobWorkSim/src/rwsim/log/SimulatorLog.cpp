...
void SimulatorLog::read(class InputArchive& iarchive, const std::string& id) {
    ...
    for(unsigned int i = 0; i < lines; i++) { _filename.append(iarchive.readString("Filename")); }
...
void SimulatorLog::write(class OutputArchive& oarchive, const std::string& id) const {
    ...
    if(endLine) oarchive.write(str.substr(fullLines * LINE_WIDTH, std::string::npos), "Filename");
...
bool SimulatorLog::operator==(const SimulatorLog& b) const {
    if(_filename != b._filename) return false;
    if(_description != b._description) return false;
    return true;
}
