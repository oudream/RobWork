...
bool SimulatorLogEntry::operator==(const SimulatorLog& b) const {
    if(const SimulatorLogEntry* const entry = dynamic_cast<const SimulatorLogEntry*>(&b)) {
        if(_line != entry->_line) return false;
    }
...
bool SimulatorLogEntry::Factory::hasEntryType(const std::string& entryType) {
    if(entryType == LogCollisionResult::getTypeID()) return true;
    else if(entryType == LogConstraintForceTorque::getTypeID()) return true;
    else if(entryType == LogConstraints::getTypeID()) return true;
    else if(entryType == LogContactForceTorque::getTypeID()) return true;
    else if(entryType == LogContactSet::getTypeID()) return true;
    else if(entryType == LogContactVelocities::getTypeID()) return true;
    else if(entryType == LogDistanceResult::getTypeID()) return true;
    else if(entryType == LogDistanceMultiResult::getTypeID()) return true;
    else if(entryType == LogEquationSystem::getTypeID()) return true;
    else if(entryType == LogPositions::getTypeID()) return true;
    else if(entryType == LogVelocities::getTypeID()) return true;
    else if(entryType == LogMessage::getTypeID()) return true;
    else if(entryType == LogValues::getTypeID()) return true;
    SimulatorLogEntry::Factory factory;
    std::vector<Extension::Descriptor> exts = factory.getExtensionDescriptors();
    for(Extension::Descriptor& ext : exts) {
        if(ext.getProperties().get("entryType", ext.name) == entryType) return true;
    }
    return false;
}
...
SimulatorLogEntry::Ptr SimulatorLogEntry::Factory::makeEntry(const std::string& entryType,
    ...
    else if(entryType == LogMessage::getTypeID()) return rw::core::ownedPtr(new LogMessage(parent));
    else if(entryType == LogValues::getTypeID()) return rw::core::ownedPtr(new LogValues(parent));
