...
void Contact::setFrameB(rw::core::Ptr<const Frame> frame) {
    if(frame == NULL) _nameB = UNKNOWN_NAME;
    else _nameB = frame->getName();
    _frameB = frame;
}
...
bool Contact::operator==(const Contact& b) const {
    if(!(b.getModelA() == getModelA())) return false;
    if(!(b.getModelB() == getModelB())) return false;
    if(b.getFrameA() != getFrameA()) return false;
    if(b.getFrameB() != getFrameB()) return false;
    if(!(b.aTb() == aTb())) return false;
    if(!(b.getPointA() == getPointA())) return false;
    if(!(b.getPointB() == getPointB())) return false;
    if(!(b.getNormal() == getNormal())) return false;
    if(b.getDepth() != getDepth()) return false;
    return true;
}
