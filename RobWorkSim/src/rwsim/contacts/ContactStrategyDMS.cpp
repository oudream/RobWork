...
template<class T> class ContactStrategyDMS<T>::DMSData : public ContactStrategyData::SpecificData
{
  public:
  ...
  public:
    ...
    bool find(std::size_t a, std::size_t b, std::vector<TrackInfo>& res) const {
        ...
        for(std::size_t i = 0; i < info.size(); i++) {
            if(info[i].modelIDa == a && info[i].modelIDb == b) { res.push_back(info[i]); }
        }
        return res.size() > 0;
    }
...
void ContactStrategyDMS<T>::findContact(std::vector<Contact>& contacts, const Model& a,
    ...
    if(distCheck) threshold = getThreshold();
    else threshold = 3 * getThreshold();
...
ContactStrategyDMS<T>::findContacts(ProximityModel::Ptr a, const Transform3D<>& wTa,
    ...
    RW_ASSERT(mA != NULL);
    RW_ASSERT(mB != NULL);
    if(!data.isInitialized()) data.setSpecificData(new DMSData());
    DMSData* const pqpData = dynamic_cast<DMSData*>(data.getSpecificData());
    RW_ASSERT(pqpData);
    if(!tracking.isInitialized()) tracking.setStrategyData(new DMSTracking());
    ...
    for(std::size_t i = 0; i < mA->models.size(); i++) {
        for(std::size_t j = 0; j < mB->models.size(); j++) {
            ...
            for(Contact& c : tmpContacts) { c.setDepth(getThreshold() + c.getDepth()); }
            ...
            tmpContacts.clear();
            for(DMSTracking::TrackInfo& info : newInfo) { info.total = curId; }
        }
    }
    pqpTracking->aTb  = inverse(wTa) * wTb;
    pqpTracking->info = newInfo;
    if(log != NULL) { stratLog->setLineEnd(__LINE__); }
    return contacts;
}
...
ContactStrategyDMS<T>::updateContacts(ProximityModel::Ptr a, const Transform3D<>& wTa,
    ...
    for(it = pqpTracking->info.begin(); it != pqpTracking->info.end(); it++) {
        ...
        if(_filtering == MANIFOLD) { tmpContacts = manifoldFilter(tmpContacts); }
        ...
        for(std::size_t i = 0; i < curId; i++) { newInfo[newInfo.size() - 1 - i].total = curId; }
    }
    pqpTracking->aTb  = inverse(wTa) * wTb;
    pqpTracking->info = newInfo;
    return contacts;
...
template<class T> void ContactStrategyDMS<T>::destroyModel(ProximityModel* model) {
    ...
    RW_ASSERT(bmodel);
    for(const Model& model : bmodel->models) { removeGeometry(bmodel, model.geoId); }
    bmodel->models.clear();
}
...
ContactStrategyDMS<T>::getGeometries(rw::proximity::ProximityModel* model) {
    ...
    for(typename std::vector<Model>::iterator it = bmodel->models.begin();
        it < bmodel->models.end();
        it++) {
        std::vector<rw::core::Ptr<rw::geometry::Geometry>> tmp = (*it).pmodel->getGeometries();
...
std::vector<Contact> ContactStrategyDMS<T>::manifoldFilter(const std::vector<Contact>& contacts) {
    ...
    for(int i = 0; i < contactIdx; i++) {
        ...
        if((0.9 > rwnlength) || (rwnlength > 1.1)) { continue; }
...
template<class T> double ContactStrategyDMS<T>::getThreshold() const {
    ...
    if(threshold == -0.1) threshold = _propertyMap.get<double>("MaxSepDistance", 0.0005);
    return threshold;
}
