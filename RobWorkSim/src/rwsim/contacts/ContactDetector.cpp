...
struct ContactDetector::OrderFramePairs
{
    ...
    bool operator()(const FramePair& a, const FramePair& b) const {
        if(_orderBefore[*a.first][*b.first]) return true;
  ...
  private:
    static FrameMap<FrameMap<bool>> buildMap(const WorkCell::Ptr wc) {
        ...
        for(std::size_t i = 0; i < frames.size(); i++) {
            for(std::size_t j = 0; j < frames.size(); j++) { map[*frames[i]][*frames[j]] = i < j; }
        }
        return map;
    }
...
ContactDetector::getContactStrategies(const std::string& frameA, const std::string& frameB) const {
    ...
    for(it = _strategies.begin(); (it != _strategies.end()); it++) {
        ...
        for(ProximitySetupRule& rule : rules) {
            if(rule.match(frameA, frameB)) match = true;
        }
        if(match) { res.push_back(stratMatch); }
    }
    return res;
}
...
ContactDetector::StrategyTable ContactDetector::getContactStrategies(
    ...
    for(it = table.begin(); it != table.end(); it++) {
        if(it->strategy->match(geometryA, geometryB)) { res.push_back(*it); }
    }
    return res;
}
...
void ContactDetector::addContactStrategy(ProximitySetup rules, ContactStrategy::Ptr strategy,
    ...
    if(pri > _strategies.size()) pri = _strategies.size();
    for(std::size_t i = 0; i < pri; i++) it++;
    ...
    for(it++; it != _strategies.end(); it++) (*it).priority++;
}

void ContactDetector::addContactStrategy(StrategyTableRow& strategy, std::size_t pri) {
    std::list<StrategyTableRow>::iterator it = _strategies.begin();
    if(pri > _strategies.size() - 1) pri = _strategies.size() - 1;
    for(std::size_t i = 0; i < pri; i++) it++;
...
void ContactDetector::removeContactStrategy(std::size_t pri) {
    if(pri > _strategies.size() - 1) return;
    std::list<StrategyTableRow>::iterator it = _strategies.begin();
    for(std::size_t i = 0; i < pri; i++) { it++; }
    ...
    for(; it != _strategies.end(); it++) (*it).priority--;
}

void ContactDetector::clearStrategies() {
    while(_strategies.size() > 0) { removeContactStrategy(0); }
}
...
std::vector<Contact> ContactDetector::findContacts(const State& state, ContactDetectorData& data) {
    ...
    for(std::list<FramePair>::const_iterator pairIt = framePairs.begin();
        ...
        for(it = _strategies.begin(); (it != _strategies.end()); it++) {
            ...
            bool match                            = false;
            for(ProximitySetupRule& rule : rules) {
                if(rule.match(pair.first->getName(), pair.second->getName())) match = true;
            }
            if(match) {
            ...
            if(match) {
                ...
                for(pairIt = geoPairs.begin(); pairIt < geoPairs.end(); pairIt++) {
                    ...
                    if(stratMatch.strategy->match(geoA->getGeometryData(),
                        ...
                        if(stratLog != NULL) { stratLog->setLineEnd(__LINE__); }
                    }
                }
            }
        }
...
std::vector<Contact> ContactDetector::updateContacts(const State& state, ContactDetectorData& data,
                                                     ContactDetectorTracking& tracking,
                                                     SimulatorLogScope* log) {
    std::vector<Contact> res;
...
void ContactDetector::printStrategyTable() const {
    if(_strategies.size() == 0) std::cout << "No strategies registered." << std::endl;
...
void ContactDetector::printStrategyTable(std::ostream& out) const {
    if(_strategies.size() == 0) out << "No strategies registered." << std::endl;
    std::vector<std::vector<ContactDetector::Cell>> table;
    constructTable(table);
        ...
        for(Frame* frame : frames) {
            ...
            if(models.size() > 0) {
                ...
                for(modelIt = models.begin(); modelIt != models.end(); modelIt++) {
                    if(first) row[4].strings.push_back(frame->getName());
                    else row[4].strings.push_back("");
...
void ContactDetector::printTable(const std::vector<std::vector<Cell>>& table, std::ostream& out,
                                 bool header) {
    if(table.size() == 0) return;
    if(table[0].size() == 0) return;
    std::vector<std::size_t> width;
    for(Cell col : table[0]) { width.push_back(0); }
    for(const std::vector<Cell>& row : table) {
        std::size_t colI = 0;
        for(Cell col : row) {
            for(std::string str : col.strings) {
                if(width[colI] < str.length()) width[colI] = str.length();
            }
            colI++;
        }
    }
    std::size_t totalwidth = 0;
    for(std::size_t w : width) { totalwidth += w; }
    totalwidth += width.size() + 1;
    for(std::size_t i = 0; i < totalwidth; i++) out << "-";
    out << std::endl;
    for(const std::vector<Cell>& row : table) {
        ...
        while(more) {
            ...
            std::size_t colI = 0;
            for(Cell col : row) {
                if(stringI + 1 < col.strings.size()) more = true;
                if(col.alignment == Cell::RIGHT) out << std::right;
                else out << std::left;
