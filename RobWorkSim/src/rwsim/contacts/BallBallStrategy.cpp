...
std::vector<Contact> BallBallStrategy::findContacts(ProximityModel::Ptr a, const Transform3D<>& wTa,
    ...
    RW_ASSERT(mA != NULL);
    RW_ASSERT(mB != NULL);
    if(!tracking.isInitialized()) tracking.setStrategyData(new BallTracking());
...
bool BallBallStrategy::addGeometry(ProximityModel* model, const Geometry& geom) {
    ...
    if(geomData->getType() != GeometryData::SpherePrim) return false;
