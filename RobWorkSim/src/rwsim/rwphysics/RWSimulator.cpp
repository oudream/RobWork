...
void saveState(const RWBodyList& bodies, double dt, State& state) {
    ...
    for(RWBody* body : bodies) { body->saveState(dt, state); }
}
...
void RWSimulator::step(double dt, State& state) {
    ...
    for(RWBody* body : _bodies) { totalEnergyBefore += body->calcEnergy(_dwc->getGravity()); }
    ...
    for(RWBody* body : _bodies) { totalEnergyAfter += body->calcEnergy(_dwc->getGravity()); }
...
double RWSimulator::internalStep(double dt, rw::kinematics::State& state) {
    ...
    for(BodyController* manipulator : _manipulators) { manipulator->addForces(state, _time); }
...
void RWSimulator::rollBack(State& state) {
    ...
    // roll back bodies
    for(RWBody* body : _bodies) { body->rollBack(state); }
}
...
//    std::cout << "* reset all forces and impulses" << std::endl;
    ...
    for(; body != _bodies.end(); ++body) { (**body).reset(); }
...
//        std::cout << "* Broad phase" << std::endl;
    ...
    for(body = _bodies.begin(); body != _bodies.end(); ++body) {
        if((*body)->getNodeType() != ConstraintNode::Rigid) continue;
    ...
    _dt = dt;*/
    if(_dt < 0.001) { _dt = 0.001; }
    else { _dt += 0.5 * _dt; }
    if(_dt > 0.03) _dt = 0.03;
#ifdef RWSIMULATOR_DEBUG
