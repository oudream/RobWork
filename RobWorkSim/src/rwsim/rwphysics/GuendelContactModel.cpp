...
void preImpulseCalcFixed(Body& body, ContactPoint& contact) {
    ...
    if(relVelT < 0.000001) { contact.t = Vector3D<>(0, 0, 0); }
...
void preImpulseCalcImpl(Body& bodyA, Body& bodyB, ContactPoint& contact) {
    ...
    // make sure to handle situations where tangential velocity is very small
    if(relVelT < epsilon) { contact.t = Vector3D<>(0, 0, 0); }
    else { contact.t /= relVelT; }
...
void GuendelContactModel::preImpulseCalc(Contact& contact, ContactPoint& point) {
    ...
    if(bodyA->getNodeType() == ConstraintNode::Fixed) { preImpulseCalcFixed(*bodyB, point); }
    else if(bodyB->getNodeType() == ConstraintNode::Fixed) { preImpulseCalcFixed(*bodyA, point); }
    else {
