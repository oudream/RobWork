...
void ContactGraph::broadPhase(rw::kinematics::State& state, bool shortCircuit) {
    ...
    for(ConstraintEdge* ePtr : _pool->getEdges()) {
        ...
        if(edge.getType() != ConstraintEdge::Physical) { continue; }

        if(edge.wasTouching()) { ePtr->setState(ConstraintEdge::VanishingTouch); }
        else { ePtr->setState(ConstraintEdge::VanishingProximity); }
    ...
    for(ConstraintEdge* ePtr : _pool->getEdges()) {
        ...
        if(nodeA.isPassive() || nodeB.isPassive()) { continue; }
            ...
            if(edge.isTouching()) { ePtr->setState(ConstraintEdge::PersistentTouch); }
            if(edge.isPenetrating() && shortCircuit) { assert(0); }
            continue;
        }
        // add e to fEdges
...
bool ContactGraph::narrowPhase(rw::kinematics::State& state, bool shortCircuit) {
    ...
    for(; edgeiter != _fEdges.end(); ++edgeiter) {
        ...
        if(edge.getState() == ConstraintEdge::Static) continue;

        _factory.narrowPhaseCalc(edge, state);

        penetrating |= edge.isPenetrating();

        if(penetrating && shortCircuit) return true;

        if(edge.wasSeperating()) {
            if(!edge.isSeperating()) { edge.setState(ConstraintEdge::NewTouch); }
        }
        else {
            if(edge.isSeperating()) { edge.setState(ConstraintEdge::VanishingTouch); }
            else { edge.setState(ConstraintEdge::PersistentTouch); }
        }
    }
    return penetrating;
}
...
void ContactGraph::updateContacts(rw::kinematics::State& state) {
    ...
    for(; edgeiter != _fEdges.end(); ++edgeiter) {
        ...
        if(nodeA->isPhysical() && nodeB->isPhysical()) {
            if(!edge.isSeperating()) { _factory.DetermineContact(edge, state); }
        }
    }
}
...
std::vector<std::vector<ConstraintEdge*>> ContactGraph::computeGroups() {
    ...
    for(; edge != _fEdges.end(); ++edge) { (*edge)->setColor(WHITE); }
...
void ContactGraph::traverseGroup(ConstraintEdge& edge, std::vector<ConstraintEdge*>& group) {
    ...
    if(!(nodeA->getNodeType() == ConstraintNode::Fixed ||
        ...
        for(; edgeiter != nodeA->getEdges().end(); ++edgeiter) {
            if((*edgeiter)->getColor() == WHITE) traverseGroup(**edgeiter, group);
        }
    }
    ConstraintNode* nodeB = edge.getNodes().second;
...
std::vector<ConstraintNode*> ContactGraph::getConnectedNodes(ConstraintNode* n,
    ...
    while(!nstack.empty()) {
        ...
        for(; edgeiter != edges.end(); ++edgeiter) {
            ...
            if(edge->getColor() != BLACK) continue;
            edge->setColor(BLUE);
            ConstraintNode* cnode;
            if(edge->getNodes().first == node) cnode = edge->getNodes().second;
            else cnode = edge->getNodes().first;
    ...
    for(; edgeiter != colored.end(); ++edgeiter) { (*edgeiter)->setColor(BLACK); }
    return result;
}
...
void printNodeName(std::ofstream& out, ConstraintNode& node) {
    ...
    if(node.getFrame() != NULL) out << node.getFrame()->getName() << " ";
...
void printEdge(std::ofstream& out, ConstraintEdge& edge) {
    ...
    switch(edge.getState()) {
        ...
        case(ConstraintEdge::PersistentProximity): out << "label=\"PerProx\" ]" << std::endl; break;
    ...
    ConstraintNode* nodeA = edge.getNodes().first;
    if(nodeA == NULL) std::cout << "Node is NULL" << std::endl;
...
void ContactGraph::writeToFile(std::string filename) {
    ...
    for(ConstraintEdge* edge : _pool->getEdges()) { printEdge(f, *edge); }
