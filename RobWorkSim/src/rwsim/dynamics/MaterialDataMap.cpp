...
bool MaterialDataMap::hasFrictionData(int matAID, int matBID, int dataType) const {
    ...
    if(res == _frictionMap.end()) return false;
    for(const FrictionData& data : (*res).second) {
        if(data.type == dataType) return true;
    }
    return false;
}
...
const std::vector<FrictionData> MaterialDataMap::getFrictionDatas(int matAID, int matBID) const {
    ...
    if(res != _frictionMap.end()) return res->second;
...
const std::vector<FrictionData> MaterialDataMap::getFrictionDatas(const std::string& matAID,
...
void MaterialDataMap::addFrictionData(const std::string& materialA, const std::string& materialB,
    ...
    if(matAID != matBID) _frictionMap[pairB].push_back(data);
}
