...
bool OBRManifold::addPoint(ContactPoint& p) {
    ...
    if(_nrOfContacts == 5) {
        ...
        for(int i = 0; i < _nrOfContacts; i++) {
            if(MetricUtil::dist2(p.p, _points[i].p) < 0.000001) return true;
        }
        // if the point is inside the obb then we only need to check if its
        // deeper than the deepest point and if it s replace it
        if(isInsideOBB(p.p)) {
            if(_points[_deepestIdx].penetration < p.penetration) _points[_deepestIdx] = p;
            return true;
        }
        ...
        int minPenIdx         = -1;
        for(int i = 0; i < 5; i++) {
            if(i < 4 && _onBorderMap[i]) continue;
        ...
        if(_points[minPenIdx].penetration < p.penetration) {
            _points[minPenIdx] = p;
            if(_points[_deepestIdx].penetration < p.penetration) _deepestIdx = minPenIdx;
        }
        return true;
    }
        ...
        if(_points[_deepestIdx].penetration < p.penetration) _deepestIdx = _nrOfContacts;
        _nrOfContacts++;
    }
    else if(_nrOfContacts == 1) {
        // only add a point if its on the contact manifold plane
...
void OBRManifold::fit(ContactPoint& p) {
    ...
    for(int i = 0; i < 4; i++) _onBorderMap[i] = false;
