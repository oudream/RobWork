...
Eigen::MatrixXd getBlockDiagInertia(Device& dev, std::vector<FixedLink*>& parents,
    ...
    for(size_t i = 0; i < parents.size() + 1; i++) {
        ...
        if(i == parents.size()) { Ar = current->getInertia().e(); }
        else { Ar = parents[i]->getInertia().e(); }
    }
    return A;
}
...
FixedLink::FixedLink(const BodyInfo& info, rw::core::Ptr<rw::kinematics::Frame> base,
    ...
    Body(info, frames),
    _jointFrame(j), _dev(dev), _parents(parents), _jac(dev.baseJframe(&j, state)), _jacRB(_jac),
    _base(base), _acc(0.0), _vel(0.0), _pos(*(j.getQ(state))), _wTbase(dev.worldTbase(state)),
    _baseTw(inverse(_wTbase)), _impulseIterations(0)
...
void FixedLink::addQd(double vel) {
    ...
    if(_targetVel < 0 && _vel < _targetVel) { _vel = _targetVel; }
    else if(_targetVel > 0 && _vel > _targetVel) { _vel = _targetVel; }
}
...
rw::math::Vector3D<> FixedLink::getPointVelW(const rw::math::Vector3D<>& wPp) {
    ...
    for(i = 0; i < _parents.size(); i++) vels(i) = _parents[i]->getQd();

    vels(i) = _vel;
    for(i++; i < _dev.getDOF(); i++) vels(i) = 0;
...
void FixedLink::updateImpulse() {
    ...
    for(size_t i = 0; i < _parents.size(); i++) { _parents[i]->addQd(jointVel(i)); }
...
void FixedLink::updateVelocity(double h, State& state) {
    ...
    for(size_t i = 0; i < _parents.size(); i++) { _parents[i]->addQd(jointTorque(i)); }
    ...
    double MAX_VEL = 1;
    // clamp the velocity
    if(_vel < -MAX_VEL) { _vel = -MAX_VEL; }
    else if(_vel > MAX_VEL) { _vel = MAX_VEL; }
}
