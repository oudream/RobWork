...
double getInvMassImpl(double mass) {
    if(mass < MIN_MASS_SIZE) return 0.0;
    return 1.0 / mass;
}
...
//_rstate(this)
{
    ...
    if(_mframe == NULL) { RW_THROW("Base frame of Object in a RigidBody must be a MovableFrame!"); }
...
RigidBody::calcInertiaTensorInv(const rw::kinematics::State& state) const {
    Transform3D<> pTb = Kinematics::frameTframe(getParent(state), _mframe, state);
