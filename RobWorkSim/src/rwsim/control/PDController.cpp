...
PDController::PDController(const std::string& name, DynamicDevice::Ptr rdev, ControlMode cmode,
    ...
    _currentVel(rw::math::Q::zero(_target.size())), _targetVel(rw::math::Q::zero(_target.size())),
    _pdparams(pdparams), _mode(cmode), _stime(dt), _accTime(0), _enabled(true) {
...
void PDController::update(const rwlibs::simulation::Simulator::UpdateInfo& info,
    ...
    if(!info.rollback) {
        ...
        if(info.dt_prev > 0.0) { _currentVel = (q - _currentQ) / info.dt_prev; }
        else { _currentVel = Q::zero(q.size()); }
    }
    ...
    for(size_t i = 0; i < _pdparams.size(); i++) {
        ...
        if(info.dt_prev > 0.0) nvel[i] += ((_currentError[i] - _lastError[i]) / info.dt_prev) * D;
    }
...
void PDController::setControlMode(ControlMode mode) {
    if(mode != POSITION || mode != VELOCITY) RW_THROW("Unsupported control mode!");
    _mode = mode;
}
