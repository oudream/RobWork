...
void BodyController::updateKinematicBody(KinematicBody* body, TargetData& tdata,
    ...
    if(tdata._type == TargetData::ForceController) return;
...
void BodyController::disableBodyControl(Body::Ptr body) {
    ...
    if(_bodyMap.find(body.get()) == _bodyMap.end()) return;
...
void BodyController::disableBodyControl() {
    boost::mutex::scoped_lock lock(_mutex);
    std::map<Body*, TargetData*>::iterator it;
    for(it = _bodyMap.begin(); it != _bodyMap.end(); it++) { delete it->second; }
    _bodyMap.clear();
}
...
void BodyController::setTarget(Body::Ptr body, const Transform3D<>& target, const State& state,
    ...
    if(MetricUtil::dist2(from.P(), target.P()) < 0.00001) { return; }
...
rw::math::Transform3D<> BodyController::getTarget(Body::Ptr body) {
    boost::mutex::scoped_lock lock(_mutex);
    if(_bodyMap.find(body.get()) != _bodyMap.end()) { return _bodyMap[body.get()]->_target; }
