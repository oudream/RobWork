...
RecursiveNewtonEuler::getBodyMotion(const State& state, const Q& dq, const Q& ddq) const {
    ...
    for(std::size_t k = 1; k <= _jdev->getDOF(); k++) {
        ...
        pCur         = baseTlink.P();
        if(k == 1) baseTlink = baseTlink * joint->getTransform(state);
        else
            baseTlink =
        ...
        e  = 0;
        if(dynamic_cast<PrismaticJoint*>(joint)) e = 0;
        else if(dynamic_cast<RevoluteJoint*>(joint)) e = 1;
        tmp  = w;
        tmp2 = dw;
        w += e * dq[k - 1] * z;
    ...
    // Find tool velocities and accelerations
    {
        pCur = baseTlink.P();
        baseTlink =
            baseTlink * Kinematics::frameTframe(_jdev->getJoints().back(), _jdev->getEnd(), state);
        dr = baseTlink.P() - pCur;
        v += cross(w, dr);
...
RecursiveNewtonEuler::getBodyNetForces(const std::vector<RecursiveNewtonEuler::Motion>& motions,
    ...
    for(std::size_t i = 0; i < _jdev->getDOF(); i++) {
        ...
        if(i == 0) baseTlink = baseTlink * joint->getTransform(state);
        else
            baseTlink =
...
std::vector<Wrench6D<>> RecursiveNewtonEuler::getJointForces(const std::vector<Wrench6D<>>& forces,
                                                             const State& state) const {
    std::vector<Wrench6D<>> res(_jdev->getDOF() + 1);
    // Handle tool
    Transform3D<> baseTlink = Kinematics::frameTframe(_jdev->getBase(), _jdev->getEnd(), state);
    {
        ...
        Vector3D<> torque =
            forces[k].torque() - cross(baseTlink.R() * _payloadCOM, force) + _environment.torque();
...
std::vector<double> RecursiveNewtonEuler::solveMotorTorques(const State& state, const Q& dq,
    ...
    for(std::size_t k = 0; k < res.size() - 1; k++) {
        ...
        if(k == 0) baseTlink = baseTlink * joint->getTransform(state);
        else
            baseTlink =
...
bool RecursiveNewtonEuler::validate() const {
    bool valid = false;
    if(_jdev.cast<SerialDevice>()) valid = true;
    if(!valid) return false;
    for(std::size_t i = 0; i < _jdev->getDOF(); i++) {
        valid        = false;
        Joint* joint = _jdev->getJoints()[i];
        if(dynamic_cast<PrismaticJoint*>(joint)) valid = true;
        if(dynamic_cast<RevoluteJoint*>(joint)) valid = true;
        if(!valid) return false;
    }
    return true;
}
