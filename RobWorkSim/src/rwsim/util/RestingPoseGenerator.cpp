...
RestingPoseGenerator::RestingPoseGenerator(DynamicSimulator::Ptr sim,
    ...
    _sampler(ownedPtr(new FiniteStateSampler(initState, 30000))), _restConstraint(restConstraint),
    _running(false), _stopRunning(true), _nrOfTries(0), _nrOfTests(0), _maxNrOfTests(0),
    _recordStatePath(false), _wasInRestingState(false), _updatePeriod(100),
    _timeEnteringRestingState(0), _minSimTime(0.5), _maxSimTime(30.0), _simStartTime(0),
    _minTimeInRest(0.5), _thread(NULL), _initState(initState) {}
...
RestingPoseGenerator::RestingPoseGenerator(DynamicSimulator::Ptr sim,
    ...
    _wasInRestingState(false), _updatePeriod(100), _timeEnteringRestingState(0), _minSimTime(0.5),
    _maxSimTime(30.0), _simStartTime(0), _minTimeInRest(0.5), _thread(NULL), _initState(initState) {
}
...
void RestingPoseGenerator::start(int nrOfTests) {
    ...
    if(_thread != NULL) RW_THROW("The thread is already started!");
    _running     = true;
    _stopRunning = false;
...
void RestingPoseGenerator::stepperLoop() {
    ...
    while(running) {
        ...
        else { nextTime = time + _updatePeriod; }
        ...
        if(_recordStatePath) _statePath.push_back(Timed<State>(simtime + _simStartTime, state));
        ...
        if(isInRestingState && !_wasInRestingState) _timeEnteringRestingState = simtime;
