...
bool CircleModel::isClose(const CircleModel& circ, double epsilon) const {
    if(fabs(_r - circ._r) > epsilon) return false;
    if(MetricUtil::dist2(_center, circ._center) > epsilon) return false;
    if(MetricUtil::dist2(_n, circ._n) > epsilon) return false;
    return true;
}
