...
std::pair<double, Vector3D<>> getWeightAverage(size_t i, size_t j,
    ...
    if(i_upp >= (size_t) tMatrix.rows()) i_upp = tMatrix.rows() - 1;

    size_t j_low = j - 1, j_upp = j + 1;
    if(j_upp >= (size_t) tMatrix.cols()) j_upp = tMatrix.cols() - 1;
...
boost::array<array_type::index, 2> getShape(const TactileArraySensor::ValueMatrix& tMatrix, int x,
                                            int y) {
    boost::array<array_type::index, 2> shape = {
        {tMatrix.rows() + x, tMatrix.cols() + y}
    };
    return shape;
}
...
TactileArraySensor::getTactileArraySensor(rwlibs::simulation::Simulator::Ptr sim) {
    ...
    else { sensor = sim->getSensorHandle(this).cast<TactileArray>(); }
    return sensor;
}
        ...
        if(mesh) {
            ...
            for(int i = startIdx; i < startIdx + size; i++) {
                ...
                */
                ...
                if(ndist < (&_distDefMatrix(0, 0))[pids.first]) continue;
...
TactileArraySensor::ClassState::ClassState(TactileArraySensor* tsensor, size_t dim_x,
    ...
    _accForces(Eigen::MatrixXf::Zero(dim_x, dim_y)), _pressure(Eigen::MatrixXf::Zero(dim_x, dim_y)),
    _accTime(0), _stime(0.005){
...
TactileArraySensor::getClassState(rw::kinematics::State& state) const {
    if(_sdata.getStateCache<ClassState>(state) == NULL) return NULL;
    return _sdata.getStateCache<ClassState>(state);
}

TactileArraySensor::ClassState::Ptr
TactileArraySensor::getClassState(rw::kinematics::State& state) {
...
double getValueOfTexel(double tx, double ty,    // texel position data
                       ...
                       double cx, double cy, Eigen::MatrixXf& distM, double mwidth,
                       double mheight) {
    ...
    for(int i = xStartIdx; i < xEndIdx; i++) {
        for(int j = yStartIdx; j < yEndIdx; j++) { texelVal += distM(i, j); }
    }
    return texelVal;
}
        ...
        if(data._collisionPairs.size() > 0) {
            ...
            if(mesh) {
                ...
                for(int i = startIdx; i < startIdx + size; i++) {
                    ...
                    // std::cout << "Intersect: " << point << std::endl;
                    // std::cout << "NDist: " << ndist << std::endl;
                    if(ndist > (&_distMatrix(0, 0))[pids.first]) continue;
    ...
    if(hasCollision) {
        for(Eigen::DenseIndex y = 0; y < _distMatrix.cols(); y++) {
            for(Eigen::DenseIndex x = 0; x < _distMatrix.rows(); x++) {
                ...
                if(_distMatrix(x, y) < closest) closest = _distMatrix(x, y);
            }
        }
        // std::cout << "\n\n" << _distMatrix << "\n\n"<< std::endl;
    }
    ...
    if(hasCollision) {
        ...
        for(int i = 0; i < 20; i++) {
            ...
            if(area > 0) {
                ...
                if(fabs(score) > bestScore) continue;
        ...
        for(Eigen::DenseIndex x = 0; x < _accForces.rows(); x++) {
            for(Eigen::DenseIndex y = 0; y < _accForces.cols(); y++) {
                ...
                //    N/m^2 " << std::endl;
                if(_accForces(x, y) <= 0) { continue; }

                pressure(x, y) = (float) std::min(_accForces(x, y) / (_tsensor->_texelArea * 1000),
...
void TactileArraySensor::update(double dt, rw::kinematics::State& state) {
    ...
    for(size_t x = 0; x < _pressure.size1(); x++) {
        for(size_t y = 0; y < _pressure.size2(); y++) { _pressure(x, y) = lastpres(x, y) * 0.8; }
    }
    ...
    for(; iter != _forces.end(); ++iter) {
        ...
        for(const Contact3D& c : (*iter).second) {
            // std::cout << "N: " << c.n << "\n";
            // std::cout << "F: " << c.f << "\n";
            if(MetricUtil::norm2(c.f) < 0.0000001) continue;
    ...
    if(body != NULL && totalForce > 0.0000001) {
        ...
        for(DistPoint& dp : res) {
            ...
            if(contacts.size1() <= xIdx || contacts.size2() <= yIdx) continue;
...
void TactileArraySensor::addForce(const Vector3D<>& point, const Vector3D<>& force,
    ...
    if(scaleSum > 1.01) std::cout << "SCALE SUM: " << scaleSum << std::endl;
}
