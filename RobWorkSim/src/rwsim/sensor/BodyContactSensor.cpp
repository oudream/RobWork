...
BodyContactSensor::BodyContactSensor(const std::string& name,
...
void BodyContactSensor::update(const rwlibs::simulation::Simulator::UpdateInfo& info,
    ...
    if(!isRegistered()) RW_WARN("BOSY NOT REGISTERED IN STATE YET...");
    ...
    if(cstate == NULL) RW_WARN("CSTATE IS NULL");
