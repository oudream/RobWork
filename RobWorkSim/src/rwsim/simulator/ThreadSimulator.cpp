...
ThreadSimulator::ThreadSimulator(rwlibs::simulation::Simulator::Ptr simulator,
    ...
    _thread(NULL),
    //_period(-1),
    _dt(0.001), _timescale(0.0), _tmpState(state), _running(false), _stepcb(NULL), _inError(false),
    _postStop(false) {}

ThreadSimulator::ThreadSimulator(rwlibs::simulation::Simulator::Ptr simulator) :
    _simulator(simulator), _thread(NULL),
    //_period(-1),
    _dt(0.001), _timescale(0.0), _tmpState(simulator->getState()), _running(false), _stepcb(NULL),
...
void ThreadSimulator::stepperLoop() {
    ...
    if(_stepcb != NULL) _stepcb(this, _simulator->getState());
