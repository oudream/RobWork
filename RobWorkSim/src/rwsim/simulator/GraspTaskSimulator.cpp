...
double getMaxObjectDistance(std::vector<RigidBody::Ptr> objects, const State& s1, const State& s2) {
    ...
    for(RigidBody::Ptr& object : objects) {
        ...
        if(MetricUtil::dist2(t1.P(), t2.P()) > max) max = MetricUtil::dist2(t1.P(), t2.P());
    }
    return max;
}
}    // namespace
...
GraspTaskSimulator::GraspTaskSimulator(rwsim::dynamics::DynamicWorkCell::Ptr dwc, int nrThreads) :
    ...
    _gripperDim(0), _failed(0), _success(0), _slipped(0), _collision(0), _timeout(0), _simfailed(0),
    _skipped(0), _nrOfExperiments(0), _lastSaveTaskIndex(0), _totalNrOfExperiments(0), _mbase(NULL),
    _tcp(NULL), _currentTask(NULL), _currentTarget(NULL), _wallTimeLimit(30.0), _simTimeLimit(30.0),
    _storeTimedStatePaths(false), _forceSimulateAll(false) {
    if(nrThreads > 0 && nrThreads < 8) _nrOfThreads = nrThreads;
}
...
void GraspTaskSimulator::init(rwsim::dynamics::DynamicWorkCell::Ptr dwc,
    ...
    for(int i = 0; i < _nrOfThreads; i++) {
        ...
        if(engine == NULL) RW_THROW("No physics engine loaded!");
...
void GraspTaskSimulator::load(GraspTask::Ptr graspTasks) {
    ...
    if(_dhand == NULL) RW_THROW("No such gripper in dynamic workcell: " << handName);
    ...
    if(_hbase == NULL) RW_THROW("The gripper base must be a KinematicBody: " << handName);
    _mbase = _hbase->getMovableFrame();

    std::string controllerName =
        _gtask
            ->getGraspControllerID();    // _roottask->getPropertyMap().get<std::string>("ControllerName",
                                         // "GraspController");
...
void GraspTaskSimulator::startSimulation(const rw::kinematics::State& initState) {
    _nrOfExperiments = 0;
    if(!_initialized) init(_dwc, initState);
    ...
    for(size_t i = 0; i < _simulators.size(); i++) { _simulators[i]->start(); }
    Log::debugLog() << "Simulators started..\n";
}

void GraspTaskSimulator::pauseSimulation() {
    _requestSimulationStop = true;
    for(ThreadSimulator::Ptr tsim : _simulators) { tsim->stop(); }
}

void GraspTaskSimulator::resumeSimulation() {
    _requestSimulationStop = false;
    for(ThreadSimulator::Ptr tsim : _simulators) { tsim->start(); }
}
...
ThreadSimulator::Ptr GraspTaskSimulator::getSimulator() {
    if(_simulators.size() > 0) { return _simulators[0]; }
    return NULL;
}
...
void GraspTaskSimulator::stepCB(ThreadSimulator* sim, const rw::kinematics::State& state) {
    ...
    if(delay != 0) TimerUtil::sleepMs(delay);
    if(_requestSimulationStop) { return; }
    ...
    if(sstate._currentState == GRASPING) {
        ...
        if(sim->getTime() > sstate._approachedTime + 0.5) {
            ...
            if(scup != NULL) { isResting = scup->isClosed(state); }
            ...
            if(sstate._restCount == 3) { sstate._restingTime = sim->getTime(); }
        }
        else { sstate._restingTime = sim->getTime(); }
    }
    ...
    if(sstate._currentState == LIFTING) {
        ...
        SuctionCup::Ptr scup = _dhand.cast<SuctionCup>();
        if(scup != NULL) { isLifted = scup->isClosed(state); }

        if(isLifted) sstate._restCount++;
        ...
        if(isLifted && sstate._restCount > 5) {
            ...
            if((gobj.object == NULL && scup == NULL) || (scup != NULL && !scup->isClosed(state))) {
                ...
                for(Contact3D& c : sstate._target->getResult()->contactsLift) { contactAvg += c.p; }
                contactAvg =
                    contactAvg / ((double) sstate._target->getResult()->contactsLift.size());

                Transform3D<> tcpTo_before =
                    Kinematics::frameTframe(_tcp, object->getBodyFrame(), sstate._postLiftObjState);
                ...
                Transform3D<> oTg_after =
                    inverse(oTc) * Kinematics::frameTframe(
                                       object->getBodyFrame(), gripperBody->getBodyFrame(), state);
                ...
                double liftResult;
                if(slippage <= 0.02) liftResult = (0.02 - slippage) * 50;
                else liftResult = 0.0;
    ...
    if(sstate._currentState == NEW_GRASP) {
        ...
        do {
            ...
            if(_rhand) {
                ...
                if(forceLim.size() > 0) _rhand->setMotorForceLimits(forceLim);
            }
...
void GraspTaskSimulator::simulationFinished(SimState& sstate) {
    ...
    Log::infoLog() << "-- target nr: " << std::setw(5) << _currentTargetIndex
                   ...
                   << " collisions:" << std::setw(5) << _collision << " timeouts:" << std::setw(5)
                   << _timeout << " skipped:" << std::setw(5) << _skipped
...
GraspTaskSimulator::getObjectContacts(const rw::kinematics::State& state, SimState& sstate) {
    ...
    for(size_t i = 0; i < _objects.size(); i++) {
        ...
        if(obj.contacts.size() > 0) result.push_back(obj);
    }
    if(result.size() == 0) return GraspedObject();
    int bestIdx = 0;
    for(size_t i = 1; i < result.size(); i++) {
        if(result[i].contacts.size() > result[bestIdx].contacts.size()) bestIdx = (int) i;
    }
...
rw::math::Q GraspTaskSimulator::calcGraspQuality(const State& state, SimState& sstate) {
    ...
    if(gobj.object == NULL || gobj.contacts.size() == 0) return qualities;
