    ...
    try {
        ...
        for(CI p = root.begin(); p != root.end(); ++p) {
            if(p->first == "Sample") {
                ...
                // std::cout << "pos=" << posq << " rot=" << rpyq << " graspW=" << graspW << std::endl;
...
void SurfaceSample::saveToXML(const std::string& filename,
    ...
    for(const SurfaceSample& sample : samples) {
        ...
        node.put(
            ...
            XMLHelpers::QToString(
                Q(3, sample.transform.P()[0], sample.transform.P()[1], sample.transform.P()[2])));
