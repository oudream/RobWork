...
class TaskGenerator
{
  public:
     ...
     * .
     * Needs a collision detector which typically is generated automatically for a given workcell by
     * RobWorkStudio and is aware of given workcell. In addition to grasp targets, this method also
     * generates the allSamples vector, which consists of targets that only passed the parallel
     * normals check, and not the collision check. This vector can be accessed by getAllSamples()
     * method, and is used for calculating the coverage.
     *
     * A vector of pre-sampled surface samples can be provided, so the same sample set can be used
     * to test different grippers. If the nSamples is provided (>0), the target generation continues
     * until nSamples of samples were used.
     *
     ...
     *
     * For each task, removes other tasks in its neighbourhood which is defined as a 6D box
     * (including both position and orientation). The number of remaining grasps may be then used
     * for coverage calculation. Filtering is only applied to tasks with Success status, and removed
     * grasps have status changed to Timeout. The original task is returned.
     *
    ...
    static rwlibs::task::GraspTask::Ptr filterTasks(const rwlibs::task::GraspTask::Ptr tasks,
                                                    rw::math::Q diff = Q(7, 0.01, 0.01, 0.01, 0.1,
                                                                         0.1, 0.1, 15 * Deg2Rad));
     ...
     */
    static rwlibs::task::GraspTask::Ptr addPerturbations(rwlibs::task::GraspTask::Ptr tasks,
  ...
  protected:
    ...
    static void moveFrameW(const rw::math::Transform3D<>& wTtcp,
                           rw::core::Ptr<rw::kinematics::Frame> tcp,
                           rw::core::Ptr<rw::kinematics::MovableFrame> base,
