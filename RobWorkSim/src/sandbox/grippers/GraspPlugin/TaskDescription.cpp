...
TaskDescription::TaskDescription(rwsim::dynamics::DynamicWorkCell::Ptr dwc) :
    _isOk(false), _dwc(dwc), _wc(dwc->getWorkcell().get()), _initState(_wc->getDefaultState()),
    _interferenceLimit(0.0), _wrenchLimit(0.0), _stressLimit(10.0),    // arbitrary
    _alignmentModel(STABLEPOSE0D), _iterations(100), _minInliers(10), _dataThreshold(0.5),
    _modelThreshold(0.5), _targetObject(NULL) {
    if(_wc == NULL || _dwc == NULL) { RW_THROW("NULL WC or DWC!"); }
}
...
TaskDescription::Ptr
TaskDescriptionLoader::readTaskDescription(PTree& tree, rwsim::dynamics::DynamicWorkCell::Ptr dwc) {
...
void TaskDescriptionLoader::readGripper(PTree& tree, TaskDescription::Ptr task) {
    ...
    if(!task->_gripperDynamicDevice) { RW_THROW("Gripper dynamic device not found!"); }
...
TaskDescription::Ptr TaskDescriptionLoader::load(const std::string& filename,
...
void TaskDescriptionLoader::save(const TaskDescription::Ptr td, const std::string& filename) {
    ...
    /*for(Alignment a : td->_alignments) {
            ...
            node.put("Pos", XMLHelpers::QToString(Q(3, a.pose.P()[0], a.pose.P()[1],
    a.pose.P()[2]))); RPY<> rpy(a.pose.R()); node.put("RPY", XMLHelpers::QToString(Q(3, rpy[0],
    rpy[1], rpy[2]))); node.put("Dist", XMLHelpers::QToString(a.dist));

            tree.add_child("TaskDescription.Alignments.Alignment", node);
    }*/

    // save alignment ransac parameters
    tree.put("TaskDescription.Alignment",
             XMLHelpers::QToString(
                 Q(4, td->_iterations, td->_minInliers, td->_dataThreshold, td->_modelThreshold)));
