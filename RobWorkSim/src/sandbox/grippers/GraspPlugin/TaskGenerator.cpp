...
TaskGenerator::TaskGenerator(TaskDescription::Ptr td) : _td(td), _tasks(NULL), _samples(NULL) {
...
SurfaceSample TaskGenerator::sample(TriMeshSurfaceSampler& sampler, ProximityModel::Ptr object,
                                    ProximityModel::Ptr ray, CollisionStrategy::Ptr cstrategy) {
    Transform3D<> wTobj =
        Kinematics::worldTframe(_td->getTargetObject()->getBase(), _td->getInitState());
    ...
    do {
        ...
        Transform3D<> rayTrans(
            pos - faceNormal * 0.001,
            RPY<>(Math::ran(-0.1, 0.1), Math::ran(-0.1, 0.1), Math::ran(-0.1, 0.1)).toRotation3D() *
                rot);

        // now we want to find any triangles that collide with the ray and which are parallel with
        // the sampled this should look for all the collisions, so should detect a proper grasp
        // regardless of presence of folds in the model
        ...
        for(PrimID pid : data.getCollisionData()._geomPrimIds) {
            ...
            bool closeAngle =
                angle(-faceNormal, normal) < 50 * Deg2Rad;    // are the normals aprox. parallel?
            ...
            if(closeAngle) {
                ...
                if(_td->hasHints()) { targetFound = false; }
                ...
                for(Transform3D<> hint : _td->getHints()) {
                    ...
                    /*bool angleOk = std::fabs(angle(targetEAA.axis(), hintEAA.axis())) <=
                    teachDist[3] && std::fabs(targetEAA.angle() - hintEAA.angle()) <=
                    teachDist[4];*/

                    /* END OF EAA */
                    // find such rotation that z axes are aligned in both target and hint
                    /* THIS IS OKAYISH: */ /*
                    Transform3D<> aligned(RPY<>(0, angle(targetW.R()*Vector3D<>::z(),
                    hint.R()*Vector3D<>::z()), 0).toRotation3D() * hint.R());

                    bool angleOk = std::fabs(angle(targetW.R()*Vector3D<>::z(),
                    hint.R()*Vector3D<>::z())) <= _td->getTeachDistance()[3] &&
                            std::fabs(angle(targetW.R()*Vector3D<>::x(),
                    aligned.R()*Vector3D<>::x())) <= _td->getTeachDistance()[4];
                            //std::fabs(angle(target.R()*Vector3D<>::x(), hint.R()*Vector3D<>::x()))
                    <= _td->getTeachDistance()[4];
                    /* END OF OKAYISH */
                    ...
                    bool angleOk =
                        (std::fabs(angle(targetZ, hintZ)) <= _td->getTeachDistance()[3]) &&
                        (std::fabs(angle(targetY, hintY)) <= _td->getTeachDistance()[4]);
                    /* END OF CRAZY STUFF */

                    /* DEBUG OUTPUTS FROM NOW ON */
                    // std::cout << angleOk << " Z= " <<
                    // Rad2Deg*std::fabs(angle(target.R()*Vector3D<>::z(),hint.R()*Vector3D<>::z()))
                    // << 	" X= " << Rad2Deg*std::fabs(angle(target.R()*Vector3D<>::x(),
                    //aligned.R()*Vector3D<>::x())) << std::endl;

                    /*if (angleOk) {
                            std::cout << "hint: " << hintEAA.axis() << " / " << hintEAA.angle() <<
                    std::endl; std::cout << "target: " << targetEAA.axis() << " / " <<
                    targetEAA.angle() << std::endl; std::cout << Rad2Deg*angle(targetEAA.axis(),
                    hintEAA.axis()) << std::endl;
                    }*/
        ...
        tries++;
        if(tries > 25000) { RW_THROW("Cannot find target that's OK! Tries: " << tries); }
    } while(!targetFound);

    return SurfaceSample(target, graspW);
}

rwlibs::task::GraspTask::Ptr TaskGenerator::filterTasks(const rwlibs::task::GraspTask::Ptr tasks,
                                                        rw::math::Q diff) {
    if(!tasks) {
        // RW_WARN("tasks = NULL");
        return NULL;
        ...
        if(node.value->testStatus != GraspTask::Filtered) {
            ...
            for(const NNSearch::KDNode* n : result) {
                ...
                const_cast<NNSearch::KDNode*>(n)->value->testStatus =
                    GraspTask::Filtered;    // this is a hack
            }
            nRemoved += removed;
        }
    }
...
int TaskGenerator::countTasks(const rwlibs::task::GraspTask::Ptr tasks,
                              const rwlibs::task::GraspTask::Status status) {
    int n = 0;
...
rwlibs::task::GraspTask::Ptr TaskGenerator::addPerturbations(rwlibs::task::GraspTask::Ptr tasks,
                                                             double sigma_p, double sigma_a,
                                                             int perturbations) {
    ...
    for(GraspSubTask& stask : tasks->getSubTasks()) {
        ...
        for(GraspTarget& target : stask.getTargets()) {
            for(int i = 0; i < perturbationsPerTarget; i++) {
                Vector3D<> pos(Math::ranNormalDist(0, sigma_p),
                               Math::ranNormalDist(0, sigma_p),
                               Math::ranNormalDist(0, sigma_p));

                // we can do this only for small sigmas (approximation)
                EAA<> rot(Math::ranNormalDist(0, sigma_a),
                          Math::ranNormalDist(0, sigma_a),
                          Math::ranNormalDist(0, sigma_a));
...
rwlibs::task::GraspTask::Ptr TaskGenerator::generateTask(int nTargets, rw::kinematics::State state,
                                                         std::vector<SurfaceSample>* ssamples,
                                                         int nSamples) {
    ...
    CollisionDetector::Ptr cdetect = new CollisionDetector(
        _td->getWorkCell(), ProximityStrategyFactory::makeDefaultCollisionStrategy());
    for(Object::Ptr obj : _td->getInterferenceObjects()) {
        cdetect->addRule(
            ProximitySetupRule::makeExclude("gripper.Base", obj->getBase()->getName()));
        cdetect->addRule(
            ProximitySetupRule::makeExclude("gripper.LeftFinger", obj->getBase()->getName()));
        cdetect->addRule(
            ProximitySetupRule::makeExclude("gripper.RightFinger", obj->getBase()->getName()));
    }
    ...
    while((nTargets > 0 && successes < nTargets) || (nSamples > 0 && samples < nSamples)) {
        ...
        else { ssample = sample(sampler, object, ray, cstrategy); }
        ...
        if(!cdetect->inCollision(state, &result, true)) {
            ...
            if(failures_in_row > 10000) {
                ...
                RW_THROW("Something is rotten in the state of RobWork: " << successes << "/"
                                                                         << failures_in_row);
                break;
            }
