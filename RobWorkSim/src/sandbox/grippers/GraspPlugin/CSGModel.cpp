...
CSGModel& CSGModel::makeWedge(double angle) {
    return makePlane(Vector3D<>(), Vector3D<>(-sin(angle / 2), -cos(angle / 2), 0)) *
...
//----------------------------------------------------------------------
CSGModel& CSGModel::operator/=(const CSGModel& csgmodel) {
    _model = csgjs_difference(csgjs_union(_model, csgmodel._model),
                              csgjs_intersection(_model, csgmodel._model));
...
ostream& rw::csg::operator<<(ostream& stream, const CSGModel& csgmodel) {
    ...
    // output all vertices
    unsigned n = 0;
    for(std::vector<csgjs_vertex>::const_iterator i = model.vertices.begin();
        i != model.vertices.end();
        ++i) {
        stream << "v" << n++ << ": (" << i->pos.x << ", " << i->pos.y << ", " << i->pos.z << ")"
               << std::endl;
    }
    ...
    for(unsigned i = 0; i < (model.indices.size() / 3); ++i) {
        stream << "t" << n++ << ": (" << model.indices[3 * i + 0] << ", "
               << model.indices[3 * i + 1] << ", " << model.indices[3 * i + 2] << ")" << std::endl;
    }
