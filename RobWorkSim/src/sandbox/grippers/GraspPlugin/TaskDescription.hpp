...
class TaskDescription
{
  public:
    ...
    typedef struct
    {
        double coverage, success, wrench, topwrench, robustness, stress, volume, alignment;
    } Qualities;
    ...
    // setWorkCell(dwc->getWorkcell().get()); }
     ...
     */
    rw::math::Q getRANSACParameters() const {
        return rw::math::Q(4, _iterations, _minInliers, _dataThreshold, _modelThreshold);
    }
  ...
  protected:
    ...
    rw::math::Q _prefilteringDistance;    // distance used to filter samples at the start, so they
                                          // are more uniform
