...
void addPertubations(GraspTask::Ptr grasptask, double sigma_p, double sigma_a,
                     int pertubationsPerTarget);
...
int main(int argc, char** argv) {
    ...
    desc.add_options()("help", "produce help message")(
        ...
        "oformat,b",
        value<string>()->default_value("RWTASK"),
        "The output format, RWTASK, UIBK, Text.")(
        ...
        "Add small random pertubations to all targets.")(
        "perturbations",
        value<int>()->default_value(1),
        "Number of pertubations to perform on each target.")(
        "sigma_a", value<double>()->default_value(8), "Standard deviation in of angle in Degree.")(
        "sigma_p",
        value<double>()->default_value(0.003),
    ...
    store(command_line_parser(argc, argv).options(desc).positional(optionDesc).run(), vm);
    notify(vm);
    ...
    if(vm.count("include")) {
        ...
        // include all
        for(int i = 0; i < GraspTask::SizeOfStatusArray; i++) includeMap[i] = true;
    }
    ...
    for(std::string input : inputs) {
        path ip(input);
        if(is_directory(ip)) { infiles = IOUtil::getFilesInFolder(ip.string(), false, true); }
        else { infiles.push_back(ip.string()); }
    }
    ...
    if(outformat == "RWTASK") { iformat = 0; }
        ...
        RW_THROW("Unknown format:" << outformat << ". Please choose one of: RWTASK, UIBK, Text\n");
    }
    ...
    for(std::string ifile : infiles) {
        ...
        if(perturbe) {
            sstr << outputfile.string() << "_" << sigma_p << "_" << sigma_a << "_"
                 << pertubationsPerTarget;
        }
        else { sstr << outputfile.string(); }
        ...
        for(GraspSubTask& stask : grasptask->getSubTasks()) {
            ...
            for(GraspTarget& target : stask.getTargets()) {
                if(target.result != NULL) {
                    if(useAlignedGrasp) {
                        if(target.result->testStatus == GraspTask::Success ||
                           target.result->testStatus == GraspTask::ObjectSlipped) {
        ...
        else { tasks.push_back(grasptask); }

        // for(std::size_t i=0;i<tasks.size();i++){
        std::stringstream outputfile;
        if(iformat == 0) { outputfile << sstr.str() + ".task.xml"; }
        else if(iformat == 1) { outputfile << sstr.str() + ".uibk.xml"; }
        else if(iformat == 2) { outputfile << sstr.str() + ".txt"; }
        ...
        if(iformat == 0) { GraspTask::saveRWTask(grasptask, sstr.str() + ".task.xml"); }
        else if(iformat == 1) { GraspTask::saveUIBK(grasptask, sstr.str() + ".uibk.xml"); }
        else if(iformat == 2) { GraspTask::saveText(grasptask, sstr.str() + ".txt"); }
    }
    //}
