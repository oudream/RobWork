...
namespace rw { namespace models {
    ...
    struct GripperQuality
    {
        ...
        GripperQuality() :
            ...
            // shape(0.0),
            coverage(0.0), success(0.0), wrench(0.0), topwrench(0.0), robustness(0.0),
            maxstress(0.0), alignment(0.0), quality(0.0) {}
    ...
    /**
     * @class Gripper
     * @brief Gripper device (parallel jaw gripper) with parametrized geometry and kinematic and
     * dynamic parameters.
     *
     * Gripper is described by its geometric and kinematic & dynamic parameters. It contains
     * geometries for jaws (shared between left & right jaw) and gripper base. These geometries can
     * either be initialized from mesh (e.g. from STL file) or procedurally generated using
     * JawPrimitive and Box primitive for fingers and base respectively. Kinematic & dynamic
     * parameters include:
     * - min. and max. opening
     * - max. force
     * - TCP position
    ...
    class Gripper    // : public TreeDevice
    {
      public:
        ...
        rw::math::Q getJawParameters() const {
            if(_isJawParametrized) { return _jawParameters; }
            else { return rw::math::Q(); }
        }
        ...
        rw::geometry::Geometry::Ptr getJawGeometry() {
            ...
            if(_isJawParametrized) {
                ...
                _leftGeometry = ownedPtr(
                    new Geometry(new JawPrimitive(_jawParameters), std::string("LeftFingerGeo")));
                //_rightGeometry = ownedPtr(new Geometry(new JawPrimitive(_jawParameters),
                //std::string("RightFingerGeo")));
            }
        ...
        rw::math::Q getBaseParameters() const {
            if(_isBaseParametrized) { return _baseParameters; }
            else { return rw::math::Q(); }
        }
        ...
        rw::geometry::Geometry::Ptr getBaseGeometry() {
            ...
            if(_isBaseParametrized) {
                _baseGeometry =
                    ownedPtr(new Geometry(new Box(_baseParameters), std::string("BaseGeo")));
            }
         ...
         * Basically, workcell contains a skeleton of gripper device and dynamic gripper device.
         * What this class and method does, is to dress this skeleton up. Geometries are removed and
         * replaced in:
         * - workcell
         ...
         *
         * This was done as the easiest way to try different gripper designs in the same workcell
         * without having it reloaded, which requires restarting RobWorkStudio. Another approach
         * would be to add gripper to the workcell and then remove it, but methods neccessary for
        ...
        /**
         * @brief Calculates parametrized gripper's crossection breadth at the point along its
         * length.
         *
         ...
         *
         * It's calculated by assuming max. grippers force acting on the tip (which gives the
         * largest moment).
         *
         * @return Stress.
         */
