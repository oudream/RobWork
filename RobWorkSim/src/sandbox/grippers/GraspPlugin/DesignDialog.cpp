...
void DesignDialog::guiEvent() {
    ...
    else if(obj == ui.applyButton) { _updateGripper(); }
...
void DesignDialog::_updateGripper() {
    if(_gripper) {
        ...
        //_gripper->setTCP(Transform3D<>(Vector3D<>(0, 0, ui.lengthEdit->text().toDouble() -
        //ui.tcpEdit->text().toDouble())));
...
void DesignDialog::_updateGUI() {
    if(_gripper) {
        ...
        if(jawParams.size() == 10 || jawParams.size() == 11) {
            ...
            else { ui.cutTiltEdit->setText(QString::number(0.0)); }
        }
        else { QMessageBox::warning(NULL, "DesignDialog", "Gripper uses jaw geometry from STL!"); }
        ...
        else { QMessageBox::warning(NULL, "DesignDialog", "Gripper uses base geometry from STL!"); }
