...
void readResult(PTree& tree, Gripper::Ptr gripper, const std::string& path) {
    ...
    if(robustnessNode) { result.robustness = XMLHelpers::readDouble(robustnessNode.get()); }
    else { result.robustness = 0.0; }
    ...
    if(stressNode) { result.maxstress = XMLHelpers::readDouble(stressNode.get()); }
    else { result.maxstress = 0.0; }

    // volume is optional
    boost::optional<PTree&> volumeNode = tree.get_child_optional("Volume");
    if(volumeNode) { result.volume = XMLHelpers::readDouble(volumeNode.get()); }
    else { result.volume = 0.0; }
...
void GripperXMLLoader::save(rw::models::Gripper::Ptr gripper, const std::string& filename) {
    ...
    // tree.put("Gripper.Parameters.Offset",
    // gripper->getJawParameters()[1]-gripper->getTCP().P()[2]);
