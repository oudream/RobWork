...
GraspPlugin::GraspPlugin() :
    RobWorkStudioPlugin("GraspPlugin", QIcon(":/pa_icon.png")), _wc(NULL), _dwc(NULL),
    _graspSim(NULL), _slowMotion(false), _showTasks(true), _showSamples(false),
    _showSuccesses(false), _silentMode(false), _nOfTargetsToGen(10), _tasks(NULL), _td(NULL),
    _wd(""), _interferenceLimit(0.1), _wrenchLimit(0.0) {
    ui.setupUi(this);
    setupGUI();
...
void GraspPlugin::initialize() {
    ...
    getRobWorkStudio()->keyEvent().add(boost::bind(&GraspPlugin::keyEventListener, this, _1, _2),
                                       this);
}
...
void GraspPlugin::guiEvent() {
    ...
    if(obj == ui.startButton) { startSimulation(); }
        ...
        QString taskfile =
            QFileDialog::getOpenFileName(this, "Open file", "", tr("Task files (*.xml)"));
        ...
        QString taskfile =
            QFileDialog::getSaveFileName(this, "Save file", "", tr("Task files (*.xml)"));
    ...
    getRobWorkStudio()->getState()); log().info() << "Target: " << _wTtarget << std::endl;
        ...
        QStringList files = QFileDialog::getOpenFileNames(
            this, "Open file", QString::fromStdString(_wd), tr("Gripper files (*.grp.xml)"));
        ...
        QString filename = QFileDialog::getSaveFileName(
            this, "Save file", QString::fromStdString(_wd), tr("Gripper files (*.grp.xml)"));
    ...
    else if(obj == ui.testButton) { test(); }
}
...
void GraspPlugin::designEvent() {
    ...
    ui.progressBar->setValue(0);
    if(_tasks) { ui.progressBar->setMaximum(_tasks->getAllTargets().size()); }
...
GraspTask::Ptr GraspPlugin::generateTasks(int nTasks) {
    ...
    for(int i = 0; i < nTasks; ++i) { subtask.targets.push_back(GraspTarget(_wTtarget)); }
