...
namespace rw { namespace geometry {
    ...
    class JawPrimitive : public Primitive
    {
      public:
        ...
        //! @brief Cutout types
        enum CutoutType { Prismatic, Cylindrical };
        ...
        JawPrimitive(double length = 0.1, double width = 0.025, double depth = 0.02) :
            _type(Prismatic), _length(length), _width(width), _depth(depth), _chamferDepth(0.0),
            _chamferAngle(0.0), _cutPosition(_length / 2), _cutDepth(0.0),
            _cutAngle(90.0 * rw::math::Deg2Rad), _cutRadius(0.0), _cutTilt(0.0) {}

        /**
         * @brief constructor
         * @param initQ [in] vector with (length, width, depth, chamfer depth, chamfer angle, cut
         * position, cut depth, cut angle, cut radius, cut tilt)
         */
