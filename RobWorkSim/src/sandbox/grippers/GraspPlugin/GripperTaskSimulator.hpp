...
namespace rwsim { namespace simulator {
     ...
     *
     * Inherits from GraspTaskSimulator. Added behaviour is implemented by using callback after the
     * grasp is finished. After each grasp is completed, if it fails interference or wrench limit
     * check, grasp status is changed from Success to Interference or ObjectSlipped appriopriately.
     * Interference for a grasp is a sum of interferences of objects specified in TaskDescription.
     * For each of the objects, interference is a difference in position and angle from the initial
     * pose is completed, and evaluated using weighted metric.
     */
    class GripperTaskSimulator : public GraspTaskSimulator
    {
      ...
      private:
         ...
         * For all the poses matching the model, an angle between pose before grasping, and a pose
         * after grasping is calculated. The alignment index is a width of a histogram of pose angle
         * changes. It is calculated as a standard variation of the angle values.
         */
