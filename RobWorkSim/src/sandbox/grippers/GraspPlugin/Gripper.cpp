...
Gripper::Gripper(const std::string& name) :
    _name(name), _baseGeometry(NULL), _leftGeometry(NULL), _rightGeometry(NULL),
    _tcp(Transform3D<>(Vector3D<>(0, 0, 0.075))), _jawdist(0), _opening(0.05), _force(50) {
...
void Gripper::updateGripper(rw::models::WorkCell::Ptr wc, rwsim::dynamics::DynamicWorkCell::Ptr dwc,
    ...
    Transform3D<> baseT;
    if(_isBaseParametrized) { baseT = Transform3D<>(-0.5 * _baseParameters(2) * Vector3D<>::z()); }
    ...
    rightGeometry->setTransform(
        Transform3D<>(Vector3D<>(), Rotation3D<>(1, 0, 0, 0, 1, 0, 0, 0, -1)));
...
double Gripper::getCrossHeight(double x) const {
    if(_isJawParametrized) {    // jaw is parametrized -- easy
        ...
        if(x > d) { lwidth = width - (x - d) * 1.0 / tan(chfAngle); }
