...
JawPrimitive::JawPrimitive(const rw::math::Q& initQ) {
    ...
    if(initQ.size() == 11) { _cutTilt = initQ(i++); }
}
...
TriMesh::Ptr JawPrimitive::createMesh(int resolution) const {
    // use CSG
    CSGModel base =
        CSGModel::makeBox(_length, _depth, _width).translate(_length / 2, 0, _width / 2);
    ...
    if(_type == Cylindrical) {
        ...
        base -= cutout.rotate(-90 * Deg2Rad, 90 * Deg2Rad, 0)
                    .rotate(_cutTilt, 0, 0)
                    .translate(_cutPosition, 0, _cutDepth - _cutRadius);
    }
    else {
        CSGModel cutout = CSGModel::makeWedge(_cutAngle);
        base -= cutout.rotate(-90 * Deg2Rad, 90 * Deg2Rad, 0)
                    .rotate(_cutTilt, 0, 0)
                    .translate(_cutPosition, 0, _cutDepth);
    }
...
std::string JawPrimitive::toString() const {
    ...
    str << _type << " " << _length << " " << _width << " " << _depth << " " << _chamferDepth << " "
        << Rad2Deg * _chamferAngle;
    str << " " << _cutPosition << " " << _cutDepth << " ";
    if(_type == Cylindrical) { str << _cutRadius; }
    else { str << Rad2Deg * _cutAngle; }
