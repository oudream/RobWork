...
In lines 22-24, the WorkCell is loaded. A SerialDevice is retrieved from the WorkCell in lines 25-27.
Always remember to check for null pointers.

In lines 29-30 the default state is retrieved from the WorkCell, and the closed form solver is constructed.

The desired transformation is defined in lines 32-33 and in line 34 the solutions are calculated by the solver.
The result is a vector of configurations, Q.

The result is printed in lines 36 to 43, along with some relevant information.
...
In lines 6-8, the WorkCell is loaded. A SerialDevice is retrieved from the WorkCell in lines 9-11.
Always remember to check for null pointers.

In lines 13-14 the default state is retrieved from the WorkCell, and the closed form solver is constructed.

The desired transformation is defined in line 16 and in line 17 the solutions are calculated by the solver.
The result is a vector of configurations, Q.

The result is printed in lines 19 to 25, along with some relevant information.
...
.. literalinclude:: ../../../RobWork/example/java/src/InvKin.java
   :language: java
   :linenos:

In lines 13-15, the WorkCell is loaded. A SerialDevice is retrieved from the WorkCell in lines 16-18.
Always remember to check for null pointers.

In lines 20-22 the default state is retrieved from the WorkCell, and the closed form solver is constructed.

The desired transformation is defined in lines 24-25 and in line 26 the solutions are calculated by the solver.
The result is a vector of configurations, Q.

The result is printed in lines 28 to 35, along with some relevant information.
...
The rw module is imported in line 1.
Normally, all classes and functions must be called with the module as a prefix, such as rw.WorkCellLoaderFactory.
To import all the names into the global namespace, the code in lines 3 to 13 can be used. Otherwise, you will have to use the "rw." prefix.

In lines 17-20, the WorkCell is loaded. A SerialDevice is retrieved from the WorkCell in lines 21-24.
Always remember to check for null pointers.

In lines 26-27 the default state is retrieved from the WorkCell, and the closed form solver is constructed.

The desired transformation is defined in line 29 and in line 30 the solutions are calculated by the solver.
The result is a vector of configurations, Q.

The result is printed in lines 32 to 40, along with some relevant information.
